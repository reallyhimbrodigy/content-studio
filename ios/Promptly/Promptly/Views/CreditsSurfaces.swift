...
/// The tier comparison, gated on the credits flag.
///
/// N ROWS FROM STOREKIT, not a hardcoded table. `CreditAllowance.tiers(from:)`
/// builds the list from the product ids the offering actually returned, so Max
/// appears the moment it is configured in the dashboard and nothing appears
/// twice if two products map to the same allowance. Free is prepended because
/// it is not a StoreKit product and never will be — it is the absence of one.
///
/// It draws NOTHING when the meter is dark. A comparison of credit allowances
/// on a screen where credits do not exist would be describing a product the
/// user cannot buy.
struct CreditTierComparison: View {
    @ObservedObject private var onboarding = OnboardingState.shared
    @ObservedObject private var subscription = SubscriptionService.shared

    private var tiers: [(label: String, monthly: Int)] {
        let ids = (subscription.offerings?.current?.availablePackages ?? [])
            .map(\.storeProduct.productIdentifier)
        return CreditAllowance.tiers(from: ids)
    }

    var body: some View {
        if onboarding.creditsEnabled, tiers.count > 1 {
            VStack(spacing: 8) {
                ForEach(tiers, id: \.label) { t in
                    HStack {
                        ...
                        Spacer()
                        // The VIDEO count leads, with credits as the smaller
                        // unit beside it. A user thinks in videos; credits are
                        // our accounting, and leading with them would make the
                        // comparison a maths problem.
                        Text("\(t.monthly / CreditsService.perVideo) videos")
                            .cType(14, .semibold)
                            .foregroundColor(.white)
                        Text(t.monthly, format: .number)
                            .cType(12)
