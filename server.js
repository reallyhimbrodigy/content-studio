...
` : '';
  const brandBrainAddendum = opts.brandBrainDirective
    ? [
        ...
        'Use curiosity gaps, pattern interrupts, open loops, and specificity (numbers/timeframes/outcomes).',
        ...
        'No holiday-themed music suggestions; if audio exists, format as \"Song Title - Artist\" only.',
        'Prevent duplication: Hook cannot appear verbatim in Reel Script hook; CTA cannot repeat Hook phrasing; Story Prompt must be a prompt, not the full post text; Distribution Plan must not restate the body.',
        'FIELD RULES (use exact field names):',
        'HOOK (hook): 1–2 lines; must be one of contrarian claim, problem agitation, surprising stat, \"you\\'re doing X wrong\", \"stop doing X\", or \"I did X for 7 days\"; imply payoff and create a gap; no questions unless a direct accusation.',
        'BODY (caption): pain sentence -> 3 bullets (mistake -> consequence -> fix) -> quick proof cue -> transition into CTA; include 1 objection crusher; include 1 save-worthy checklist/framework; start with a caption hook different from the on-screen hook; use scannable line breaks.',
        'CTA (cta): only one action; keyword CTA format: Comment \"KEYWORD\" for ____ or DM \"KEYWORD\" for ____; KEYWORD must be niche-specific; immediate benefit required.',
        'EXECUTION NOTES (format or execution notes field): include pacing plan with timestamps (0–1s, 1–5s, 5–12s), plus visual reset cadence every 2–3 seconds.',
        'DESIGN NOTES (designNotes): specify first 1s visual (pattern interrupt), overlay text beats (3–5 words), and 2–3 concrete shot ideas with motions/props.',
        'ENGAGEMENT LOOP (engagementScripts and pinned comment if present): include pinned comment prompt (forced choice or \"which one are you?\"), a 1-sentence DM follow-up script, and a follow-up content idea (part 2/case study/results).',
        'REEL SCRIPT (reelScript): must be a script with sections: Hook (spoken), On-screen text, Body (spoken) beats, CTA (spoken). Include one open-loop line and one credibility line (\"Here\\'s what I\\'ve seen work repeatedly…\").',
        'HASHTAGS (hashtags): 5–8 max; 2 broad, 3 niche, 1 intent (howto/tips/mistakes style); no stuffing.',
        'STORY PROMPT (storyPrompt): a qualification/self-disclosure question tied to buyer intent; not a full post dump.',
        'DISTRIBUTION PLAN (distributionPlan or distribution field): return exactly the following format and nothing else:',
        'Objective: (watch time / shares / DMs / conversions)',
        'Target viewer: (1 sentence)',
        'Retention beats: (3 bullets, each includes visual reset + line)',
        ...
        '- Trend-based: niche-specific twist + value + CTA.',
        'SELF-CHECK: Brand Brain enabled means every card must show explicit sales + algorithm elements; no generic versions.',
      ].join('\\n')
    : '';
