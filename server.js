...
const BRAND_BRAIN_DIFFERENTIATION_RULES_BLOCK = [
  ...
  '- ReelScript.body must include all four components in order: mistaken belief (1 sentence), hidden constraint (1–2 sentences, concrete and niche-derived), consequence if ignored (1 sentence, specific), reframe/what to do instead (1 sentence, specific). If any component is missing or generic, rewrite.',
  '- Hidden constraint must be specific to the niche inputs and operational; generic constraints are invalid.',
  '- CTA must be a single deliverable next step (DM/Comment/Download/Checklist/Call). Must not repeat the Hook. No location/industry terms unless present in inputs.',
  'HOOK VARIETY RULE:',
  '- Do not reuse the same hook phrasing across posts.',
