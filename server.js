...
` : '';
  const brandBrainAddendum = opts.brandBrainDirective
    ? [
        `Apply the following Brand Brain rules only when enabled.`,
        `Treat the user as the seller for this niche. Every card must sell their service, not generic education.`,
        `Include one concrete offer per card (consult, audit, assessment, listing/buyer consult, treatment plan, membership, strategy call, teardown).`,
        `Include one differentiator per card without unverifiable claims (local expertise, process, specialization, credentials if provided, or commonly observed outcomes).`,
        `No emojis. No fabricated statistics; no numeric proof unless user-provided. Avoid guarantees and medical promises.`,
        `Use one target persona, one target desire, and one target objection per post. Always include pain/friction, mechanism/why, specific next step, and a retention device.`,
        `Use concrete nouns, local cues, and intent language. Avoid generic influencer filler.`,
        `If nicheStyle contains a location, mention it in Title or Hashtags or CTA.`,
        `Do not include headings or labels; output must read like a finished post.`,
        `For the title field: 4–7 words, include a niche keyword and outcome angle, align with the hook claim, avoid generic titles.`,
        `For the hook field: 1–2 lines, pattern interrupt tied to niche stakes and curiosity gap, include a niche-specific noun, point toward the business action, avoid soft questions.`,
        `For the caption field: tight sentences only; weave in pain/goal, mechanism, mistake, consequence, 3-step fix, objection defuser, soft bridge to CTA, and a save/share trigger. Do not expose internal labels.`,
        `For the cta field: one clear conversion action aligned to niche and offer; include deliverable and friction reducer (keyword + what they get + time expectation). Must match engagementScripts and reelScript CTA.`,
        `For the storyPrompt field: one direct question that qualifies buyer intent (timeline, budget range, area, concern). Do not duplicate the hook.`,
        `For execution notes/format: keep existing format; if Reel, add one retention instruction and one trust/authority instruction using niche-safe visuals.`,
        `For the designNotes field: first 1s visual must be niche-specific; include 2–3 concrete visual beats with niche props/scenes; include one on-screen text line that mirrors the hook claim without quoting it verbatim.`,
        `For the engagementScripts field: include a pinned comment that qualifies leads, a comment reply template, a DM opener that moves toward booking/lead magnet, and a next-post idea that advances the funnel.`,
        `For the reelScript field: Hook (same claim, tightened), then three short beats, then CTA (same CTA). Include one niche-specific objection defuser and one non-numeric authority line. No internal labels.`,
        `For the script field: keep structure aligned with reelScript (hook/body/cta) and match CTA verbatim.`,
        `For the distributionPlan field: keep it operational and niche-specific; include two retention beats with timestamps tied to visuals, a caption path (insight + mechanism + CTA deliverable), and one repurpose idea. Avoid meta-analysis templates.`,
        `For the hashtags field: always present as an array of strings (8–12 tags). Include 2–3 location tags if location exists, 2–3 niche service tags, and 2 intent tags. No irrelevant or holiday tags.`,
