...
const REGULAR_CALENDAR_CEILING_CONTRACT_BLOCK = [
  ...
  '- Every post must include all required fields exactly as schema expects, including storyPrompt (non-empty string) for every post.',
  '- If any required field is missing/empty/wrong-type, the output is invalid.',
  '- storyPrompt is REQUIRED for every post; must be a single, niche-specific interactive prompt (question or A/B choice) aligned to the post’s title/topicAngle; never blank/omitted/null/object.',
  '- Before returning the final JSON, silently verify each post includes storyPrompt and it is not blank.',
  '',
...
const BRAND_BRAIN_UNFAIR_ADVANTAGE_CONTRACT_BLOCK = [
  ...
  '- Return ONLY valid JSON matching schema.',
  '- Every post must include all required base fields exactly as schema expects, including storyPrompt (non-empty string).',
