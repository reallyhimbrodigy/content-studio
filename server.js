...
function buildBrandBrainDirective(settings = {}) {
  ...
  const lines = [
    ...
    '- Lifestyle: reframe expectations about a lifestyle decision (not describe experiences). Hooks contrast perception vs reality and lead with a belief most people get wrong; avoid neutral discovery framing. Body focuses on 1-3 compounding realities or overlooked tradeoffs that, if underestimated, lead to regret/stress/poor decisions. Tie insight to readiness/preparedness; implicitly distinguish informed movers from unprepared ones; sound revealing, not promotional or advisory. Prefer SAVE/SHARE/GUIDE REQUEST; CTA should protect a future decision, not consume tips. Engagement loop qualifies seriousness (timeline/intent/readiness), avoids casual chatter, and progresses toward DM-worthy signals. Bias toward saves/shares; Distribution Plan must justify why it is worth revisiting later. Self-check: shifts how viewers evaluate the lifestyle choice, contrasts expectation vs reality, and keeps subtle sales alignment without explicit selling.',
