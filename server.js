...
function buildPrompt(nicheStyle, brandContext, opts = {}) {
  ...
  const audioRules = `Audio rules:\n1) Research real, currently trending TikTok and Instagram Reels tracks used by creators in this niche within the last 7 days; do not invent mood descriptors.\n2) Assign exactly two audio strings per post (one TikTok, one Instagram Reels) and keep them unique across the ${days}-day calendar.\n3) Each string must start with "TikTok:" or "Instagram Reels:" and include the exact audio title/creator plus a realistic search phrase that matches what creators would type to find that audio.`;
