...
const SHORT_FORM_CONTENT_CONTRACT_BLOCK = [
  ...
  '- Hook must stop scroll in the first 1-2 seconds with immediate stakes (time/money/tradeoff/mistake/opportunity) tied to the viewer.',
  ...
  'Category/Pillar: Education (process steps / checks / what I would do / common mistake), Social Proof (before/after / mistake->fix / turning point / receipts), Promotion (subtle, entertainment-first; no hard sell).',
  ...
  'Each reply must be 1–2 complete sentences, <= 180 characters, and include a concrete comment trigger or A/B choice tied to the mechanism.',
...
const REGULAR_CALENDAR_CEILING_CONTRACT_BLOCK = [
  ...
  '- Forbidden operations (do not do these anywhere in hook/script/caption/engagement):',
  '  1) belief change / “myth” / “misconception” / “you think” / “think again”',
  '  2) “common mistake” / “biggest mistake” / “stop doing”',
  '  3) “hidden cost/hidden constraint you didn’t consider”',
  '  4) decision sequencing (“first do X, then Y because…”) that reframes how to decide',
  '  5) second-order consequence framing (“what happens later if…”)',
  '  6) psychological lever naming or manipulation framing',
  ...
  '- Do NOT use these frames/phrases (case-insensitive): "think again", "common mistake", "biggest mistake", "myth", "misconception", "you think", "you\'re missing", "hidden costs you didn\'t consider", "don\'t let", "derail", "undervaluing", "overlooking", "most people fail", "are you making", "stop doing", "the truth is".',
  '- Do NOT use "Before/Pivot" structure.',
  '- Do NOT include: hidden constraint reveals, decision-order reframes, second-order consequence persuasion, objection pre-removal, "buyer psychology" language.',
  '- Avoid tourism/brochure tone. Every post must be an ad-script engineered for retention and action.',
  '',
  'VIRAL AD EXECUTION REQUIREMENTS (REGULAR)',
  'For each post:',
  '1) HOOK (first 1-2 seconds)',
  '- Must create immediate stakes for the viewer (time/money/tradeoff/mistake/opportunity) using specificity from the niche context.',
  '- Must be 1 sentence with stakes + niche-specific detail.',
  '- Must NOT be a generic question. Must not be contradiction framing.',
  '',
  '2) REEL SCRIPT / BODY (retention pacing)',
  '- Write as 4-6 short beats. Each beat = 1-2 short sentences max.',
  '- Beats must shift focus quickly (example shift types: scenario -> detail -> action -> payoff -> engagement).',
  '- Include exactly ONE concrete mechanism (a specific factor/process step that changes outcomes in this niche).',
  '- Include exactly ONE concrete action step the viewer can take immediately.',
  '- Include exactly ONE concrete proof detail tied to the mechanism (number/example/constraint).',
  '- Include at least ONE concrete example detail derived from the niche inputs (location/tool/process constraint/common scenario). Never hard-code.',
  '',
  '3) ENGAGEMENT + CTA',
  '- Engagement prompt must be tied to the mechanism/action (A/B choice OR comment keyword). No generic "comment below".',
  '- CTA must be low-friction and ad-appropriate (comment keyword/DM prompt/guide request), matching the schema fields.',
  '- CTA must be a single comment keyword OR an A/B question tied to the post topic.',
  '',
  'PILLAR RULES (REGULAR)',
  '- Education: teach one concrete mechanism + one action step. No myth-busting.',
...
const BRAND_BRAIN_UNFAIR_ADVANTAGE_CONTRACT_BLOCK = [
  ...
  'You are generating publish-ready TikTok/Instagram ad content for the provided niche and pillar. Output must be on-topic, niche-specific, and formatted exactly to the required JSON schema. No extra text.',
  '',
  'GENERAL RULES (BRAND BRAIN)',
  '- This tier must feel like a different capability: decision-architecture + persuasion mechanics embedded inside the actual script/caption.',
  '- Do NOT rely on repetitive contradiction trope hooks (forbidden patterns: "think again", "you think...", "myth", "misconception").',
  '- Do NOT use: "think again", "you think", "myth", "misconception", "you\'re missing this", "stop doing", "common mistake", "you\'re missing" as the hook formula.',
  '- Do NOT default to generic "3 tips + stat + CTA" content. If you include steps, they MUST be framed as a reordered decision sequence tied to the real blocker.',
  '',
  ...
  '- Brand Brain differentiation must come from explicit structure inside the content, not slogans.',
  '',
  ...
  '',
  'MANDATORY STRATEGIC FUNCTIONS (MUST APPEAR NATURALLY IN SCRIPT/CAPTION)',
  'Every Brand Brain post MUST include ALL of the following, expressed naturally (do not label them with these names; do not use templated phrases):',
  '1) The audience\'s incorrect optimization (the assumption they\'re optimizing for that fails).',
  '2) The real blocker (the unseen factor actually determining success in this niche).',
  '3) Decision order: explicitly state what must be decided/done FIRST vs later (sequence change).',
  '4) Second-order consequence: what compounds later if they keep the old approach (beyond the obvious outcome).',
  '5) Mechanism-based proof: causal explanation tied to the blocker + sequence (not a floating stat).',
  '6) Conversion push: a psychologically grounded next step that feels rational/inevitable (without naming "levers").',
  '- Inside the Reel Script and/or Caption, the content MUST explicitly communicate all six items above in natural language (not implied, not metadata).',
  '',
  'VIRAL AD EXECUTION REQUIREMENTS (BRAND BRAIN)',
  'For each post:',
  '1) HOOK (first 1-2 seconds)',
  ...
  '- Must embed the six strategic functions above inside the beats (explicitly in script/caption).',
  '- Include the second-order consequence as a distinct beat.',
  '- Include the microProof as a distinct beat with mechanism + example.',
  '- Include at least ONE concrete example detail derived from niche inputs (never hard-code).',
  '- No "3 tips + stat" template.',
  '',
  '3) ENGAGEMENT + CTA',
  '- Engagement prompt must commit the viewer to a position (A/B, "comment keyword if you want X", or a choice that reveals intent).',
  '- The 3 comment replies must validate + qualify + deliver the next asset.',
  '- CTA must be aligned with the strategic frame (e.g., request guide/checklist/DM) and match schema.',
  '- CTA must logically follow from the decision architecture (not a generic "comment below").',
  '',
...
const CALENDAR_HARD_SEPARATION_BLOCK = [
  'CRITICAL:',
  '- Regular Calendar posts must NEVER include belief collapse, hidden levers, sequencing, or algorithm rewatch engineering.',
  '- Brand Brain posts must ALWAYS include belief collapse, hidden levers, sequencing, and algorithm rewatch engineering.',
