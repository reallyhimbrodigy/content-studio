...
const REGULAR_CALENDAR_CEILING_CONTRACT_BLOCK = [
  ...
  '- This is a Neutral/Informational calendar. Do NOT use persuasion teardown, belief attacks, or manipulative framing.',
  '- Forbidden anywhere in text fields: "Belief:", "Feels-true because:", "Hidden constraint:", "Concrete consequence:", "Reframe:", "Tiny action:", "myth", "misconception", "you\'ve been lied to".',
  '- Pillar integrity (must be true before output):',
  '  Education = definition + 2-4 factors/steps + practical takeaway. No testimonials. No event/service promotion.',
  '  Lifestyle = routine/moment + practical tip. No listicle tourism ("top X places") framing. No promotion.',
  '  Social Proof = specific outcome story (challenge → support → result). Not a tutorial.',
  '  Promotion = explicit offer/event/service (what it is → who it’s for → what to do next). Not a tutorial.',
  '- CTA gate applies to ALL CTA surfaces (CTA, Engagement Comment/DM, Follow-up, Distribution):',
  '  If pillar is Education or Lifestyle: DM/guide/report/brochure/"I can send you" is forbidden. Allowed actions: save, try, observe, share.',
  '  If pillar is Social Proof or Promotion: DM is allowed.',
  '- Anti-redundancy: Title, Hook, Caption, Reel Script must not share any identical 5+ word phrase.',
  '- Self-audit ONCE: If any rule above fails, rewrite the entire JSON once internally, then output final JSON only. No explanation.',
...
const BRAND_BRAIN_UNFAIR_ADVANTAGE_CONTRACT_BLOCK = [
  ...
  '- This is Belief Teardown content. Every post MUST include the teardown chain as labeled segments inside the Reel Script BODY text (use plain text labels; do NOT create new JSON keys):',
  '  Belief:',
  '  Feels-true because:',
  '  Hidden constraint:',
  ...
  '  Reframe:',
  '  Tiny action:',
  '- The six labels above must appear exactly as written, in that order, in the Reel Script body for every post.',
  '- Hidden constraint must be a limiting condition/variable that explains when/why the belief fails (not "people forget/overlook").',
  '- Concrete consequence must be a specific second-order cost (time/money/risk/opportunity), not generic stress/fear.',
  '- Tiny action must be non-salesy and must NOT require contacting/DM/clicking/booking.',
  '- Key contract (schema stability): topic_signature and angle MUST be present and non-empty strings (exact spelling, case-sensitive). Do NOT output alias keys for these fields.',
  '- Anti-redundancy: Title, Hook, Caption, Reel Script must not share any identical 5+ word phrase.',
  '- Self-audit ONCE: If teardown labels are missing OR topic_signature/angle missing/empty OR any rule fails, rewrite the entire JSON once internally, then output final JSON only. No explanation.',
