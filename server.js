...
function buildBrandBrainDirective(settings = {}) {
  ...
  const lines = [
    ...
    '  - Hook (1-2s): tension-first; do not lead with results or numbers. Use one intent (common seller/brand mistake, myth about why listings/ads do not convert, direct callout that they are doing X right but still losing, why most attempts stall after launch). Address the viewer as the business owner or seller.',
    '  - Problem framing: name a specific failure mode the viewer is likely experiencing and why common fixes fail or backfire.',
    '  - Mechanism + validation: introduce a real result only as validation, not the headline. Explain the causal chain (what variable changed, in what sequence, why that sequence mattered psychologically or algorithmically). Separate what most people do vs what worked.',
    '  - Copyable structure (mandatory): provide a reusable framework (listing structure, ad creative sequence, or messaging checklist) the viewer can apply immediately for their next ad or listing.',
    '  - CTA (decision protection): protect against loss, waste, or delay; request a contextual keyword tied to the deliverable; do not reuse the same keyword across posts.',
    '  - Execution / Design Notes: include rewatch mechanics (open loop early, on-screen text progression, pattern interrupts). Avoid generic visuals; no default signs, storefronts, or lifestyle filler.',
    '  - Distribution Plan: state why the target audience would save or share; package for each platform with platform-aware framing and no filler.',
    '  - Global rules (for Social Proof): no generic tactics, no motivational filler, no obvious ads, no vague phrases like "targeted marketing", "competitive pricing", or "boost sales". Enforce perception shift with informed vs uninformed separation, maintain pillar label integrity, ensure time correctness, and keep Social Proof posts unique.',
