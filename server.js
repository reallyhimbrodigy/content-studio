...
const SHORT_FORM_CONTENT_CONTRACT_BLOCK = [
  ...
  '- Style: reel-first, native social, concise, conversational.',
  '- Keep each field short; avoid long paragraphs or lectures.',
  '- Use short lines and tight sentences; avoid dense blocks.',
  ...
  'UNIQUENESS:',
  'Do not reuse phrasing across posts. Avoid generic filler like "Absolutely!" and avoid repeating identical distributionPlan/engagementScripts structures beyond the required format.',
  'Each post must use topic-specific wording and details.',
  'Title: specific to topic and niche; avoid brochure phrases ("Dream Home Awaits", "Exclusive", "Happy Client", "Personalized Service").',
...
const REGULAR_CALENDAR_CEILING_CONTRACT_BLOCK = [
  ...
  '- Output must feel like native social content, not a lecture or ad copy.',
  '- Reel/video-first in every field. Keep each field concise and specific to the niche.',
  '- Hooks: 1-2 lines max, conversational, specific to the title/topic.',
...
const BRAND_BRAIN_UNFAIR_ADVANTAGE_CONTRACT_BLOCK = [
  ...
  '- Brand Brain must NOT sound like Regular. Make it sharper, more sales-psych forward.',
  '- Use contrarian / myth / "stop doing X" angles that create tension without cringe.',
  '- Hooks should create discomfort/curiosity and stay niche-specific.',
  '- Captions: concise POV (not a step-by-step explainer).',
...
const CALENDAR_HARD_SEPARATION_BLOCK = [
  'CRITICAL:',
  '- Regular Calendar posts must feel native, simple, reel-first, and concise.',
  '- Brand Brain posts must feel sharper and more sales-psych forward (contrarian POV + DM keyword follow-up), not like a better-written Regular post.',
  '- The difference must be obvious without labeling.',
].join('\n');

const REGULAR_CONTENT_QUALITY_RULES_BLOCK = [
  'REGULAR CALENDAR MODE (NEUTRAL)',
  'REGULAR MODE RULES (NEUTRAL EXECUTION):',
  '- Produce execution-ready, simple, neutral Reel-based posts.',
  '- Be broadly usable by any creator in the niche (Miami real estate agent) without strong positioning.',
  '- Optimize for clear video premise + clean hook + short reel script + concrete design notes.',
  '- Use a calm, helpful, non-salesy voice.',
  '- Prefer concrete specifics (neighborhoods, scenarios, buyer/seller situations) without fear/pressure framing.',
  'DO NOT:',
  '- DO NOT use any of these phrases or patterns: "Stop believing", "Think again", "Most people decide", "Most people think", "myth", "hidden truth", "you\'re overpaying", "you\'re making a mistake", "fail", "failure", "risk losing", "opportunity cost", "secret", "guru", "contrarian", "wake-up call".',
  '- DO NOT use authority/superiority framing (no "savvy buyers know", no "what others won\'t tell you", no "the truth is").',
  '- DO NOT moralize or pressure (no fear, shame, urgency manipulation).',
  '- DO NOT write like an ad. Avoid "Act now", "Don\'t miss out", "game changer", "revolution", "must-see" unless it is factual for an open house.',
  '- DO NOT include "decide X first" reframes.',
  'MUST:',
  '- Every post must include a watch-past-3s reason in the hook (specific promise: "3 things...", "here\'s what happens when...", "tour of...", "breakdown of...").',
  '- Reel Script must be 4 parts: Hook (1 line), Body (3-6 short lines), CTA (1 line), On-screen text cues (2-4 short cues). Put the cues at the end of reelScript.body so the JSON schema stays the same.',
  '- Design Notes must be concrete scene beats with timestamps (0-2s, 2-5s, 5-12s, 12-18s, 18-22s) and must match the script.',
  '- Captions must be concise (2-4 short sentences max) and NOT repeat the entire Reel Script.',
  '- Keep tone informational/relatable, not persuasive.',
  'PILLAR INTENT (REGULAR):',
  '- Promotion: showcase an offer/listing/event neutrally, focus on what to show on video.',
  '- Social Proof: real-world story structure, human details, no grand claims.',
  '- Education: explain one concept simply with a concrete example.',
  '- Lifestyle: show lived experience/tradeoffs/routines, not tourism hype.',
  'QUALITY SELF-CHECK (internal):',
  '- If this post could be mistaken for Brand Brain mode, rewrite it to be neutral and practical.',
].join('\n');

const BRAND_BRAIN_DIFFERENTIATION_RULES_BLOCK = [
  'BRAND BRAIN MODE (STRATEGIC + DIFFERENTIATED)',
  'BRAND BRAIN RULES:',
  '- Be opinionated and differentiated.',
  '- Challenge a common assumption or default behavior relevant to the niche.',
  '- Include a non-obvious insight AND a consequence (risk/opportunity cost) tied to Miami real estate realities.',
  '- Sound like one specific agent\'s worldview (confident, direct), but still professional and credible.',
  'MUST INCLUDE:',
  '- A challenged belief (one sentence).',
  '- The agent\'s reframing thesis (one sentence).',
  '- One concrete Miami-specific example (neighborhood, HOA, insurance, taxes, appraisal gap, inventory reality, etc.).',
  '- One actionable step/checklist item the viewer can do today.',
  '- CTA must be either "DM ____" or "Comment ____" with a specific keyword.',
  'STYLE CONSTRAINTS:',
  '- Do NOT reuse the exact same opening pattern across posts.',
  '- Limit the following phrases to at most 1 occurrence per calendar batch: "Think again", "Stop believing", "Most people decide".',
  '- Prefer varied openings (questions, assertions, mini-story, data-point lead, contradiction).',
  '- Avoid generic guru tone; make claims grounded with specifics, not vague warnings.',
  'PILLAR INTENT (BRAND BRAIN):',
  '- Promotion: position the offer as a strategic advantage; include the "why people miss this" angle.',
  '- Social Proof: story proves a thesis; include a "what they did differently" insight.',
  '- Education: myth-busting or reframing with a specific consequence.',
  '- Lifestyle: tradeoffs/costs/opportunity-cost framing tied to decisions.',
  'QUALITY SELF-CHECK (internal):',
  '- If this post could pass as Regular mode, rewrite it to be sharper, more opinionated, and more specific.',
].join('\n');
