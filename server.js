...
function buildVoiceLockInstructionBlock({ mode, presetKey, sample }) {
  ...
  const lines = [
    ...
    'Do NOT change: Distribution Plan, Suggested Audio.',
...
const SHORT_FORM_CONTENT_CONTRACT_BLOCK = [
  ...
  '- Each run is isolated to the provided inputs; do not reuse tokens from examples or prior runs.',
  '- Before finalizing, scan output for any proper noun, location term, or industry noun not present in inputs; if found, remove and rewrite that sentence.',
  '- Do not fall back to default industries, default locations, default professions, lifestyle tropes, or market metaphors unless present in inputs.',
  ...
  '- Reel Script must stay on the same specific topic; do not introduce unrelated pivots.',
  '- Concrete specificity: include at least one niche-relevant detail derived from the provided niche context (tool, step, constraint, example scenario).',
...
const TITLE_ANCHOR_ECHO_BLOCK = [
  ...
  '- Title is binding. Hook, caption, and reelScript must echo the title\'s key noun phrase(s) without copying the full title.',
  '- If title implies testimonial/success story/case study, include that exact phrase in hook, caption, and reelScript hook without repeating the full title.',
].join('\n');
...
const QUALITY_ALIGNMENT_BLOCK = [
  ...
  '- No vague language. No filler adjectives.',
  '- Ban generic hype adjectives and vague claims; use concrete, observable descriptors.',
  '- Ban vibe language not tied to a concrete point.',
  '- Ban generic filler phrases (e.g., "something for everyone", "vibrant", "unique blend", "dynamic market", "hidden gems", "goldmine", "exclusive opportunity"). If present, rewrite.',
...
const REGULAR_CALENDAR_CEILING_CONTRACT_BLOCK = [
  'REGULAR CALENDAR (FREE):',
  'REGULAR MODE RULES (NON-NEGOTIABLE)',
  'REGULAR MODE CONTRACT (BINDING)',
  '- This is a Neutral / Informational calendar. Do NOT use belief-attacks or persuasion teardown structures.',
  '- Forbidden frames/words anywhere in content fields: "belief", "feels-true because", "hidden constraint", "second-order consequence", "reframe", "misconception", "myth", "you\'ve been lied to".',
  '- Pillar-type integrity check (must pass):',
  '  - Education: teaches a concept or process (definition + factors/steps + practical takeaway). No testimonials. No promotional pitch.',
  '  - Lifestyle: a moment/routine + practical tip. No "top places" listicles. No promotional pitch.',
  '  - Social Proof: must contain a client/story outcome. Not a tutorial.',
  '  - Promotion: must directly describe an offer/service. Not a tutorial.',
  '- CTA gate applies across ALL CTA surfaces (CTA, Engagement, Follow-up, Distribution):',
  '  - If pillar is Education or Lifestyle: DM / "I can send you" / guides/reports/brochures are forbidden. Allowed CTAs: save, share, try.',
  '  - If pillar is Promotion or Social Proof: DM allowed.',
  '- Anti-redundancy: title, hook, caption, reelScript must not share any 5+ word phrase.',
  '- Self-audit before output: if any rule fails, rewrite the entire JSON once internally, then output final JSON only.',
  'GLOBAL RULES (BINDING):',
  '- CTA surfaces = cta, script.cta, reelScript.cta, engagementScripts (commentReply/dmReply), distributionPlan.',
  '- If pillar is Education or Lifestyle: DM CTAs are forbidden across all CTA surfaces; comment-to-receive and "I will send you" offers are forbidden; guide/report/brochure/analysis offers are forbidden; allowed CTAs only: save, share, try, follow.',
  '- If pillar is Promotion or Social Proof: DM CTA is allowed.',
  '- Anti-redundancy: no 5+ word phrase may appear in more than one of title/hook/caption/reelScript.',
  '- Hook cannot restate the title’s main clause; caption cannot restate the hook’s opening sentence.',
  '- Pillar scaffolds are mandatory; if any step is missing, rewrite:',
  '  - Education: definition -> 3 concrete factors/steps -> 1 specific (generic) example -> 1 quick self-check.',
  '  - Lifestyle: specific moment/routine -> observation -> small practical tip; forbid listicles and brochure tone.',
  '  - Social Proof: specific situation -> obstacle -> what changed -> outcome; forbid generic praise.',
  '  - Promotion: feature -> who it helps -> when it does NOT help/limitation -> CTA.',
  '- One-pass self-audit before output: CTA gate across all CTA surfaces, anti-redundancy, pillar scaffold, required keys present/non-empty, no unknown keys. If any check fails, rewrite the entire JSON once internally, then output final JSON only.',
  'STRICT OUTPUT + ONE-PASS SELF-AUDIT (REGULAR):',
  '- Output must be a single JSON object matching the existing schema exactly; no markdown; no commentary.',
  '- Before output, run one self-audit: required keys present/non-empty, no extra keys, no 5+ word phrase repeated across title/hook/caption/reelScript, pillar contract satisfied, CTA rules satisfied.',
  '- If any check fails, rewrite the entire JSON once internally, then output final JSON only.',
  '- Do not output lecture-style or ad-copy language.',
  '- Each field must be specific to the niche.',
  '- Hooks: 1-2 lines max; specific to the title/topic.',
  '- Captions: 2-4 sentences max. No long paragraphs.',
  '- CTA: one single-step line aligned to the pillar contract; no urgency or pressure.',
  ...
  '- Each post object MUST include ALL required keys (non-empty): post_key, day, slotIndex, title, topicCapsule, pillar, format, hook, caption, cta, hashtags, script{hook,body,cta}, reelScript{hook,body,cta}, designNotes, engagementScripts{commentReply,dmReply}, distributionPlan, topic_signature, angle.',
  '- Do not reuse the same hook pattern across posts.',
].join('\n');

const BRAND_BRAIN_UNFAIR_ADVANTAGE_CONTRACT_BLOCK = [
  'BRAND BRAIN CALENDAR (PRO):',
  'BRAND BRAIN RULES (NON-NEGOTIABLE)',
  'BRAND BRAIN IDENTITY CONTRACT (BINDING)',
  '- This is a Belief Teardown / Persuasion calendar.',
  '- Every post MUST include the teardown chain in reelScript.body using these exact labels:',
  '  Belief:',
  '  Feels-true because:',
  '  Hidden constraint:',
  ...
  '  Reframe:',
  '  Tiny action:',
  '- The labeled chain must appear verbatim in reelScript.body for every post.',
  '- Key contract (schema stability):',
  '  - Required keys must be present and non-empty strings: topic_signature, angle (exact spelling, case-sensitive).',
  '  - Do not use alias keys for these fields (e.g., topicSignature, angleText).',
  '  - topicCapsule is required by the schema; do not treat it as an alias for topic_signature.',
  '- Idea eligibility gate: testimonials, pure offers, or generic lifestyle inspiration are invalid unless reframed into the teardown chain above.',
  '- If an idea is not teardown-compatible, rewrite the idea internally into a teardown-compatible angle before writing the post.',
  '- Hidden constraint must be a limiting condition/variable that explains when/why the belief fails.',
  '- Consequence must be causal and specific (not generic "stress/miss out/mistakes").',
  '- Tiny action must be non-promotional and must not require contacting/DM/clicking/booking.',
  '- Anti-redundancy: title, hook, caption, reelScript must not share any 5+ word phrase.',
  '- Self-audit before output: if the chain labels are missing OR topic_signature/angle missing/empty OR any rule fails, rewrite the entire JSON once internally, then output final JSON only.',
  'DEPTH + SAFETY RULES (BINDING):',
  '- Output JSON only; match schema keys exactly; no unknown keys.',
  '- Required keys topic_signature and angle must always be present and non-empty (exact spelling).',
  '- Do not use alias keys for required fields; map to correct schema keys instead.',
  '- Belief must be plausible and decision-relevant; avoid trivial or strawman beliefs.',
  '- Hidden constraint must be a genuine limiting condition (specific variable/condition) that explains failure; do not use vague overlook phrasing without a concrete condition.',
  '- Consequence must be specific and causal, directly tied to the hidden constraint; avoid vague outcomes unless made concrete.',
  '- Tiny action must be non-promotional and must not require contacting/DM/click/booking; it is a small observational or decision step.',
  '- One-pass self-audit before output: required keys present/non-empty, no forbidden/unknown keys, teardown chain present, anti-redundancy across title/hook/caption/reelScript, belief/constraint/consequence/tiny-action gates satisfied. If any check fails, rewrite the entire JSON once internally, then output final JSON only.',
  'KEY CONTRACT (BRAND BRAIN):',
  '- Output must be a single JSON object matching the existing schema exactly; no markdown; no commentary.',
  '- Required keys must always be present and non-empty: topic_signature, angle (exact spelling, case-sensitive).',
  '- Do not use alias keys for required fields; use schema keys only (e.g., topicSignature/angleText are forbidden).',
  '- topicCapsule is required by the schema; keep it as topicCapsule and do not rename or flatten it.',
  '- If you would produce an alias key, rewrite to the correct schema key name instead.',
  '- No unknown keys.',
  '- Brand Brain must NOT sound like Regular.',
  'ONE-PASS SELF-AUDIT (BRAND BRAIN):',
  '- Before output, verify required keys present/non-empty, no forbidden keys, teardown chain present, and no 5+ word phrase repeated across title/hook/caption/reelScript.',
  '- If any check fails, rewrite the entire JSON once internally, then output final JSON only.',
  '- engagementScripts must follow engagement loop rules; no sales or pressure prompts.',
  '- All required fields must be present and non-empty, including cta, script.cta, reelScript.cta, engagementScripts.commentReply, engagementScripts.dmReply.',
  '- Format must be "Reel" for every post.',
  '',
  ...
  '- Do not leave any required field empty.',
  '- Each post object MUST include ALL required keys (non-empty): post_key, day, slotIndex, title, topicCapsule, pillar, format, hook, caption, cta, hashtags, script{hook,body,cta}, reelScript{hook,body,cta}, designNotes, engagementScripts{commentReply,dmReply}, distributionPlan, topic_signature, angle.',
  '- topic_signature and angle must be top-level keys spelled exactly as "topic_signature" and "angle".',
  '- Do not use alternate keys such as topicSignature, angleText, or angle_summary.',
  '- topicCapsule must remain a single top-level object; do not lift topicCapsule fields into top-level keys.',
  '- If you cannot produce topic_signature and angle, rewrite once internally before output; never omit or rename them.',
...
const REGULAR_CONTENT_QUALITY_RULES_BLOCK = [
  'REGULAR CALENDAR MODE (NEUTRAL)',
  'REGULAR CALENDAR PURPOSE (VERBATIM):',
  'The Regular calendar exists to inform, explain, and build familiarity without attempting to change beliefs.',
  'It must never position the creator as superior, contrarian, or opinionated.',
  ...
  '- No belief attacks. No contrarian framing.',
  '- Explicitly ban belief-attack terms: myth, misconception, stop believing, truth is, dangerous, flawed.',
  '- Forbid teardown vocabulary: reframe, hidden constraint, second-order consequence, you have been lied to, most people believe.',
  '- No urgency, fear, or pressure. No persuasion framing.',
  '- BANNED RHETORIC (REGULAR ONLY): belief framing, consequence/threat framing, urgency/scarcity, exclusivity/insider framing, superiority claims, aspirational lifestyle selling, outcome/benefit promises, evaluative/judgmental adjectives, adversarial/shaming tone, conversion intent.',
  '- Disallowed Regular tokens (non-exhaustive): "don\'t miss", "act now", "before it\'s gone", "limited", "exclusive", "unlock", "dream", "perfect", "ideal", "game changer", "life-changing", "transform", "upgrade", "luxury", "must-see", "prime", "upscale", "stunning", "breathtaking", "unparalleled", "best", "amazing", "reach out", "contact", "book", "buy", "sell", "schedule", "sign up", "visit", "check out", "discover", "explore", "learn more", "click", "tap", "watch", "follow", "share", "go live".',
  '- No belief-attack framing (e.g., "stop believing", "myth", "misconception", "truth is").',
  '- No sales or persuasion CTAs; any CTA must be a single micro-action aligned to the pillar contract.',
  '- Questions are allowed only in engagementScripts.commentReply; all other sections must be declarative.',
  ...
  '- PERSUASION KILL SWITCH: If any sentence contains persuasion or marketing intent, rewrite it into neutral descriptive language or delete it.',
  '- CREATOR EGO SUPPRESSION: No first-person authority framing (e.g., "I help", "my clients", "why choose me", "trust me").',
  'HOOK CONSTRAINT (REQUIRED STRUCTURE):',
  '- All hooks must be neutral topic statements, not questions or pitches.',
  '- Avoid urgency or fear.',
  '- Regular posts must be neutral and informational. No contrarian tone. No urgency/pressure.',
  '- Tone should read like documentation, a neutral guide, an encyclopedia entry, or a factual catalog description.',
  'CAPTION RULE (ANTI-REDUNDANCY):',
  '- The caption must NOT restate the reel script.',
  'CTA LIMITATION:',
  '- CTA must be a single micro-action aligned to the pillar contract. No urgency, no pressure, no implied loss.',
  '- CTA must remain informational and must not reference the creator.',
  '- CTA must not imply contact, messaging, scheduling, buying, or exclusivity except where explicitly allowed by the pillar contract.',
  '- Hashtags must be descriptive only; no CTA/action tokens.',
  '- Engagement loop must be neutral and informational; avoid customer-support tone.',
  '- Distribution plan must be neutral posting mechanics only; no sales actions or hype.',
  '- REGULAR REWRITE RULE: If any sentence could plausibly appear in Brand Brain mode because it contains persuasion, belief framing, consequence framing, urgency, or adversarial language, rewrite it into neutral descriptive language or remove it.',
  '- Hook: 1 sentence, specific to the niche inputs; cannot reference any location unless provided.',
  '- Caption: must add 2–3 distinct points not stated in Hook.',
  '- ReelScript.body must add a scenario example and one objection + answer not stated in Hook/Caption.',
  '- CTA: single step only; keep it informational and non-transactional.',
  '- Anti-redundancy: no 5+ word phrase may appear in more than one of title/hook/caption/reelScript.',
  '- Hook cannot restate the title’s main clause; caption cannot restate the hook’s opening sentence.',
  '- Each section (except engagementScripts.commentReply) must end with a neutral declarative statement.',
  '- If the model cannot produce concrete details from inputs, it must remain abstract and avoid invented specifics.',
  '- If Hook/Caption/Body repeat the same idea, rewrite until each adds new info.',
  'Each post must answer one specific, niche-relevant decision question and avoid generic, touristy, or filler framing; the hook, caption, script, and CTA must stay tightly aligned to the post title and the niche-specific angle.',
  'REGULAR SOCIAL PROOF (NEUTRAL CASE):',
  '- Must be: before → constraint → intervention → outcome, stated plainly without praise or evaluation.',
  '- Outcome must be specific without fabricated numbers.',
  '- Include one quoted-sounding line as paraphrased voice (no quotation marks).',
  '- Must not include testimonial language, endorsements, or praise wording.',
  '- Disallowed in Regular Social Proof: "satisfied", "thrilled", "excited", "raving", "trust", "why they chose", "success story", "inspiration", "made all the difference".',
  'PROMOTION (REGULAR, INFORMATIONAL ONLY):',
  '- Must include one differentiator + mechanism (how it works) and one specific offer.',
  '- Must include one micro-action CTA (single step).',
  'PILLAR INTENT (REGULAR):',
  '- Education: definition -> 3 concrete factors/steps -> 1 specific example (generic) -> 1 quick self-check.',
  '- Lifestyle: specific moment/routine -> observation -> small practical tip; forbid sales framing.',
  '- Social Proof: specific situation -> obstacle -> what changed -> outcome; forbid generic praise.',
  '- Promotion: feature -> who it helps -> when it does NOT help/limitation -> CTA.',
  '- Education: include a reusable decision tool (checklist/rubric/framework) with 3–5 steps and one if/then rule.',
  '- Lifestyle: include identity + tradeoff + choice heuristic; include one "not this / but this" contrast; no travel-guide framing.',
  '- Pillar content must match its pillar label; no cross-pillar drift.',
  'CTA GATE BY PILLAR (REGULAR):',
  '- Education or Lifestyle: CTA must be save/share/try only. DM CTAs and comment-to-receive CTAs are forbidden. Guide/report/"I will send you" offers are forbidden.',
  '- Promotion or Social Proof: DM CTA is allowed.',
  '- Engagement loop: forced-choice comment prompt (A/B or 1/2) plus "why" prompt.',
  '- Engagement loop: specify exactly one follow-up asset type and the question it answers.',
  '- DM prompt is allowed only in Promotion and Social Proof; it is banned in Education and Lifestyle.',
  'OUTPUT ACCEPTANCE GATE (MANDATORY):',
  '- Draft internally, then run the checklist below on the completed post.',
  '- Before output, self-audit against all rules in this prompt.',
  '- If any item fails, rewrite the entire JSON once internally, then output final JSON only.',
  '- Do not mention this gate or the rewrite in the output.',
  'REGULAR CHECKLIST (ALL MUST PASS):',
  '- Tone is neutral, reference-style, and non-persuasive.',
  '- CTA is a single micro-action aligned to the pillar; no sales, urgency, or exclusivity framing.',
  '- No urgency/scarcity/exclusivity framing.',
  '- No aspirational/hype/value-judgment adjectives.',
  '- Hook supports the Title without repeating the Title wording.',
  '- No repeated ideas or paraphrases across sections.',
  '- No 5+ word phrase appears in more than one of title/hook/caption/reelScript.',
  '- Questions appear only in engagementScripts.commentReply.',
  '- Do not add any niche facts not present in inputs.',
  '- If any check fails, rewrite once internally; never output a best-effort draft with violations.',
].join('\n');

const BRAND_BRAIN_DIFFERENTIATION_RULES_BLOCK = [
  'BRAND BRAIN MODE (STRATEGIC + DIFFERENTIATED)',
  'BRAND BRAIN PURPOSE (VERBATIM):',
  'REQUIRED 6-PART STRUCTURE (MANDATORY):',
  '- Explicit false belief (directly stated).',
  '- Why it feels true (surface evidence or assumption).',
  '- Hidden constraint or ignored variable that makes it fail.',
  '- Concrete consequence of holding it (what goes wrong).',
  '- Reframed truth (a worldview shift, not a generic tip).',
  '- Tiny action (single step tied to the reframe, not a sales pitch).',
  '- If any element is missing, vague, or implied instead of explicit, the post is invalid.',
  'Each post must explicitly include all six steps above; keep all parts aligned to the post title and niche.',
  '- Belief must be plausible and non-trivial; avoid strawman beliefs.',
  '- Hidden constraint must be a genuine limiting condition, not a restatement of the belief.',
  '- Consequence must be specific, causal, and non-generic; avoid vague "stress/miss out/mistakes" phrasing.',
  '- Tiny action must be non-promotional and must not require DM/click/contact; it is a small observational or decision step.',
  '- Exactly one belief per post; do not blend multiple beliefs.',
  '- The post must read like a belief teardown with a causal chain; avoid generic edgy marketing or brochure tone.',
  '- BANNED BRAND BRAIN FRAMING: empty edginess without causal reasoning, generic motivational slogans, vague mechanism-free claims, hype framing, exclusivity or scarcity framing.',
  '- Disallowed Brand Brain tokens (non-exhaustive): "discover", "explore", "unlock", "act now", "don\'t miss", "dream", "perfect", "transform", "game changer", "life-changing", "upgrade", "vibrant", "thriving", "unique", "opportunity", "opportunities", "dynamic", "insights".',
  '- Advisory verbs are allowed only in the tiny action line; avoid instruction framing elsewhere.',
  '- Belief reversal and myth framing are required and allowed.',
  ...
  '- Consequence must be specific and tied to time, money, risk, stress, or opportunity cost.',
  '- Contrarian framing and cognitive reframing are required.',
  '- ReelScript.body must include six labeled sentences in this exact order:',
  '  Belief: ...',
  '  Feels-true because: ...',
  '  Hidden constraint: ...',
  '  Concrete consequence: ...',
  '  Reframe: ...',
  '  Tiny action: ...',
  '- The labeled sentences must appear in the written content (hook/caption/script); ReelScript.body must include the full labeled chain.',
  '- The six labeled sentences must be explicit; if any is missing, rewrite and ensure all six are present before output.',
  '- Integrate the scenario example and objection + answer within these labeled sentences without breaking the order.',
  '- Hidden constraint must be specific to the niche inputs and operational; generic constraints are invalid.',
  '- Reframed truth must be a descriptive model-of-reality, not advice or action.',
  '- CTA fields are required by schema; cta, script.cta, reelScript.cta must be non-empty and mirror the tiny action in a single step. Must not repeat the Hook.',
  '- Hashtags must be descriptive only; no CTA/action tokens.',
  '- CREATOR EGO SUPPRESSION: No first-person authority framing (e.g., "I help", "my clients", "why choose me", "trust me").',
  '- Attack the belief, not the reader; no insults, no shaming.',
  '- Engagement loop: forced-choice comment prompt (A/B or 1/2) plus "why" prompt.',
  '- Engagement loop: specify exactly one follow-up asset type and the question it answers.',
  '- DM prompt is allowed only in Promotion and Social Proof; it is banned in Education and Lifestyle.',
  'OUTPUT ACCEPTANCE GATE (MANDATORY):',
  ...
  '- If any item fails, rewrite the entire JSON once internally, then output final JSON only.',
  '- Do not mention this gate or the rewrite in the output.',
  'BRAND BRAIN CHECKLIST (ALL MUST PASS):',
  '- Contains explicit causal chain: belief -> why it feels true -> hidden constraint -> consequence -> reframed truth -> tiny action.',
  '- Exactly one belief is addressed; no blending.',
  '- Reframed truth is analytical and model-based, not advice or promotion.',
  '- CTA is a single tiny action; no sales, urgency, or exclusivity framing.',
  '- ReelScript.body contains labeled sentences: Belief:, Feels-true because:, Hidden constraint:, Concrete consequence:, Reframe:, Tiny action:.',
  '- topic_signature and angle are present at top level and spelled exactly; do not use alternate spellings.',
  '- Required fields are present and non-empty, including cta, script.cta, reelScript.cta, engagementScripts.commentReply, engagementScripts.dmReply.',
  '- No repeated ideas or paraphrases across sections.',
  '- No 5+ word phrase appears in more than one of title/hook/caption/reelScript.',
  '- Hook supports the Title without repeating the Title wording.',
  '- Do not add any niche facts not present in inputs.',
  ...
  '- Focus on tradeoffs, constraints, or second-order effects.',
  '- Avoid tourism-style or vibe-only descriptions.',
  '- Tie lifestyle directly to decision quality or long-term impact.',
  'PILLAR INTENT (BRAND BRAIN):',
  '- Education: include a reusable decision tool (checklist/rubric/framework) with 3–5 steps and one if/then rule.',
  '- Promotion: include one differentiator + mechanism (how it works) and one specific offer; include one micro-action CTA.',
  '- Pillar content must match its pillar label; no cross-pillar drift.',
  '- Social Proof must be: before → constraint → intervention → outcome, stated plainly without praise; outcome must be specific without fabricated numbers; include one quoted-sounding line as paraphrased voice (no quotation marks).',
  '- Lifestyle: include identity + tradeoff + choice heuristic; include one "not this / but this" contrast; no travel-guide framing.',
  'MODE SELF-CHECK (internal):',
  '- Before finalizing output, verify: Brand Brain content contains a clear challenged belief and consequence. If not, revise until compliant.',
].join('\n');
