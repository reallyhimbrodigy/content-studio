...
function buildBrandBrainDirective(settings = {}) {
  ...
  const lines = [
    ...
    '  - Hook (1-2s): tension-first; do not lead with results or numbers. Use one intent (common ad mistake, myth blocking profitable clothing ads, direct callout of a specific step done wrong, why ads stall after early traction, what most clothing brands misinterpret about conversions). Address the viewer as the brand owner.',
    '  - Frame the problem: name a specific failure mode in clothing brand advertising and why common fixes fail.',
    '  - Mechanism + proof: introduce a real result only as validation, not the headline. Explain the causal chain (what variable changed, in what order, why it unlocked conversions). Separate what most brands do vs what worked.',
    '  - Copyable takeaway (mandatory): provide a concrete, reusable structure (ad sequence, messaging checklist, or creative framework) the viewer can apply immediately for their next ad.',
    '  - CTA (decision protection): frame around avoiding waste or protecting spend; request a contextual keyword tied to the deliverable; do not reuse the same keyword across posts.',
    '  - Execution Notes / Design Notes: include rewatch mechanics (open loop early, on-screen text sequencing, visual contrast or pattern interrupt). Avoid generic visuals; no vague storefront or lifestyle shots.',
    '  - Distribution Plan: state why a clothing brand owner would save or share; package for TikTok vs IG with platform-specific framing and no filler.',
    '  - Global rules (for Social Proof): no generic inspiration, no motivational filler, no obvious ads, no vague growth/engagement hype, enforce perception shift with informed vs uninformed separation, maintain pillar label integrity, ensure time accuracy, and keep Social Proof posts unique. If the niche is clothing brand owner, scripts must advertise the viewer\'s brand and never narrate another brand\'s story as inspiration.',
