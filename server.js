...
const REGULAR_CALENDAR_CEILING_CONTRACT_BLOCK = [
  ...
  '- This is Neutral/Informational content. Do NOT use persuasion teardown or belief-attack framing.',
  ...
  '- Pillar eligibility (must be satisfied BEFORE writing the post):',
  '  Education: teaches a concept/process using (definition → 2-4 factors/steps → practical takeaway). Not testimonials. Not offers/events/listings/services.',
  '  Lifestyle: routine/moment + practical tip. Not listicles ("top X places/neighborhoods") framing. Not offers/events/listings/services.',
  '  Social Proof: specific outcome story (challenge → support → result). Not a tutorial.',
  '  Promotion: explicit offer/event/service/listing (what it is → who it’s for → next step). Not a tutorial.',
  '- If the provided Idea does not match the declared pillar, rewrite the Idea internally to match the declared pillar. If it still cannot match, reassign the pillar internally and proceed. Do NOT fail or omit the post.',
  '- CTA gate applies to ALL CTA surfaces (CTA field, Engagement Comment, Engagement DM, Follow-up Idea, Distribution Plan):',
  ...
  '- Self-audit ONCE: If any rule above is violated, rewrite the entire JSON once internally, then output final JSON only. No explanation.',
...
const BRAND_BRAIN_UNFAIR_ADVANTAGE_CONTRACT_BLOCK = [
  ...
  '- This is Belief Teardown content. Every post MUST include a teardown chain inside the Reel Script BODY text using these exact labels in order (plain text labels, not JSON keys):',
  '  Belief:',
  '  Feels-true because:',
  '  Hidden constraint:',
  ...
  '  Reframe:',
  '  Tiny action:',
  '- The six labels must appear exactly as written, in that order, in every post’s Reel Script body. No exceptions.',
  '- Idea eligibility gate (pre-check): Testimonials, listings, generic lifestyle inspiration, or listicles are invalid unless rewritten into a belief teardown. If the Idea is invalid, rewrite the Idea internally into a teardown-compatible belief before writing the post. Do NOT omit the post.',
