...
const BRAND_BRAIN_UNFAIR_ADVANTAGE_CONTRACT_BLOCK = [
  ...
  '- If you cannot produce topic_signature and angle, rewrite until you can; never omit or rename them.',
...
const REGULAR_CONTENT_QUALITY_RULES_BLOCK = [
  ...
  '- Before output, self-audit against all rules in this prompt; rewrite until fully compliant.',
  '- If any item fails, discard the entire post (keep title/pillar/format) and regenerate from scratch, then rescan.',
  '- Repeat until all checks pass. Do not mention this gate or the rewrite in the output.',
  ...
  '- If any check fails, rewrite until clean; never output a best-effort draft with violations.',
].join('\n');
...
const BRAND_BRAIN_DIFFERENTIATION_RULES_BLOCK = [
  ...
  '- The six labeled sentences must be explicit; if any is missing, rewrite until all six are present.',
