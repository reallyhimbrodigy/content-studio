...
` : '';
  const brandBrainAddendum = opts.brandBrainDirective
    ? [
        ...
        `BUSINESS ANCHORS (REQUIRED): Treat the user as the seller for this niche. Every card must sell THEIR service, not generic education. Include one concrete Offer Anchor per card (consultation, audit, strategy call, assessment, listing consult, buyer consult, treatment plan, membership, challenge signup, teardown). Include one Differentiator Anchor without unverifiable claims (local expertise, process, specialization, credentials if provided, or "commonly seen" outcomes).`,
        `GLOBAL: No emojis. No fabricated statistics; no numeric proof unless user-provided. Use one target persona, one target desire, and one target objection per post. Always include pain/friction, mechanism/why, specific next step, and a retention device. Keep language compliant (no medical guarantees; avoid "cure" or "permanent"). Use concrete nouns, local cues, and intent language. No generic influencer filler. Return JSON only; do not rename or omit fields. Use plain text only, no markdown.`,
        `NICHE + LOCATION BINDING: Every card must include at least two niche identifiers across Title/Hook/Body/CTA. If nicheStyle contains a location, mention the location in Title or Hashtags or CTA.`,
        `FIELD-BY-FIELD REQUIREMENTS (Brand Brain On):`,
        `TITLE (title): 4–7 words. Must include niche-specific keyword and outcome angle. Aligns with hook claim. Avoid generic titles.`,
        `HOOK (hook): 1–2 lines. Pattern interrupt tied to the niche + stakes + curiosity gap. Must include a niche-specific noun and point toward the business action. Avoid generic "Most people..." without specifics.`,
        `BODY (caption): exact persuasion structure in compact lines: Pain/goal (niche-specific) -> Mechanism/insight -> Mistake -> Consequence -> Fix (3 steps, niche-specific) -> Objection line -> Soft CTA bridge (why next step with this creator) -> Save/share trigger line. No invented numbers.`,
        `CTA (cta): one clear action aligned to the niche and the offer. Include deliverable + friction reducer (keyword + what they get + time expectation). Must match engagementScripts + reelScript CTA.`,
        `EXECUTION NOTES / FORMAT: keep existing format; if Reel, include one retention instruction and one trust/authority instruction (e.g., show map, blurred MLS, calendar, process board).`,
        `STORY PROMPT (storyPrompt): single direct question that collects buyer intent or sales qualification (timeline, budget range, area, concern). No generic prompts. Must not duplicate hook.`,
        `DESIGN NOTES (designNotes): first 1s visual must be niche-specific. Provide 2–3 concrete visual beats with niche props/scenes. Include one on-screen text line that mirrors the hook claim without quoting verbatim.`,
        `ENGAGEMENT LOOP (engagementScripts): include a pinned comment prompt that qualifies leads, a comment reply template, a DM opener that moves toward booking/lead magnet, and a next-post idea that advances the funnel.`,
        `REEL SCRIPT (reelScript): Hook (same hook, tightened) -> 3 short beats -> CTA (same CTA). Include one niche-specific objection-handling line and one non-numeric authority/credibility line.`,
        `SCRIPT (script): keep structure aligned with reelScript (hook/body/cta) and match CTA verbatim.`,
        `DISTRIBUTION PLAN (distributionPlan if present): operational and niche-specific. Include: 2 retention beats (0–1s, 3–5s) tied to visuals; caption path (insight sentence + proof mechanism sentence + CTA deliverable sentence); one repurpose idea (reel -> carousel/story). Avoid meta-analysis templates.`,
        `HASHTAGS (hashtags): ALWAYS present as an array of strings (8–12 tags). Include 2–3 location tags if location exists in nicheStyle, 2–3 niche service tags, and 2 intent tags. No irrelevant/holiday tags.`,
        `SUGGESTED AUDIO (if present in schema): must be "Song Title - Artist" only, non-holiday, no platform prefixes.`,
        `SCHEMA COMPLIANCE CHECK: return valid JSON only. Every post object MUST include all required keys: day, title, hook, caption, cta, hashtags, script, reelScript, designNotes, storyPrompt, storyPromptPlus, engagementScripts. No empty strings. No nulls. No extra keys.`,
      ].join('\\n')
    : '';
