...
function buildCalendarPostSchema(minDay = 1, maxDay = 30) {
  ...
  return {
    ...
    required: ['post_key', 'day', 'slotIndex', 'title', 'topicCapsule', 'hook', 'caption', 'pillar', 'cta', 'hashtags', 'script', 'reelScript', 'designNotes', 'engagementScripts', 'distributionPlan', 'topic_signature', 'angle'],
    properties: {
...
function buildBrandBrainDirective(settings = {}) {
  ...
  const lines = [
    ...
    'GLOBAL BRAND BRAIN REQUIREMENTS (apply to ALL categories):',
    '- All categories must produce ad-ready persuasion assets, not informational posts.',
    '- Hooks must create tension via mistake, myth, or contradiction; never announce results as the hook.',
    '- Must include a perception shift that separates informed vs uninformed viewers.',
    '- Must explain a causal mechanism or sequence; no generic lists.',
    '- Must resolve to a copyable framework, script, or structure.',
    '- CTAs must be decision-protective (avoid loss, waste, delay, regret).',
    '- Enforce rewatch mechanics (open loop + progressive reveal).',
    '- Avoid generic tactics, lifestyle praise, feature dumps, or motivational filler.',
    '- Maintain pillar label integrity, time correctness, and uniqueness.',
    ...
    '- SOCIAL PROOF — Ad Pattern Validation (Brand Brain structure required; use ONLY Instant Hooks, Tips/Tools/Hacks, or Beginners/How-To archetypes):',
    '  - Hook (1-2s): tension-first; do not lead with results or numbers. Use one intent (common mistake the viewer is making, myth about why results stall, direct callout that they are doing X right but still failing, why most attempts die after launch). Address the viewer as the business owner or seller.',
    '  - Problem framing: identify a specific failure mode the viewer recognizes (stale listings, low-intent leads, discount pressure, wasted ad spend) and why common advice fails or backfires.',
    '  - Mechanism + validation: frame the result as validation, not the headline. Explain the causal chain (what variable changed, in what order, why that sequence worked psychologically or algorithmically). Explicitly contrast what most do vs what worked.',
    '  - Structure focus: focus on the structure that worked, not the people.',
    '  - Copyable framework (mandatory): must end with a copy-this framework (listing sequence, ad creative framework, or messaging checklist) the viewer can apply immediately for their next ad or listing.',
    '  - CTA (decision protection): frame around avoiding loss, waste, or delay; request a contextual keyword tied to the deliverable; do not reuse the same keyword across posts.',
    '  - Execution / Design Notes: enforce rewatch mechanics (open loop early, on-screen text progression, pattern interrupts). Avoid generic visuals or filler shots.',
    '  - Distribution Plan: state why the target audience would save or share; package for each platform with platform-aware framing and no generic inspiration language.',
    '  - Global rules (for Social Proof): no generic tactics, no motivational filler, no obvious ads, no vague phrases like "targeted marketing", "right price", or "boost sales". Enforce perception shift with informed vs uninformed separation, maintain pillar label integrity, ensure time correctness, and keep Social Proof posts unique.',
    '- EDUCATION — Belief Correction',
    '  1. Hook (first 1–2 seconds)',
    '  - Must challenge a false belief the viewer currently holds.',
    '  - Must NOT be framed as “tips,” “mistakes,” or “things to know.”',
    '  - Use one of these hook intents (intent only):',
    '    - “You think X matters most — it doesn’t”',
    '    - “This is why people get stuck even when they do everything ‘right’”',
    '    - “The advice everyone follows that causes problems later”',
    '  2. Problem reframing',
    '  - Explain why the common advice fails or backfires.',
    '  - Tie the belief error to a concrete negative outcome',
    '    (overpaying, delays, regret, wasted spend, bad timing).',
    '  3. Mechanism (mandatory)',
    '  - Replace lists with a causal model; teach decision order or control logic:',
    '    - what variable actually matters',
    '    - in what order decisions should be made',
    '    - why this sequence changes outcomes psychologically or financially',
    '  4. Copyable framework',
    '  - Provide a simple, reusable structure:',
    '    - decision order',
    '    - evaluation checklist',
    '    - question set to ask before committing',
    '  - Must feel like “use this before you decide.”',
    '  5. CTA (decision protection)',
    '  - Frame CTA around avoiding loss, regret, or wasted effort.',
    '  - Position expertise as preventing downstream loss.',
    '  - Request a contextual keyword tied to the deliverable.',
    '  - Do not reuse the same keyword across posts.',
    '  6. Execution / Design Notes',
    '  - Enforce rewatch mechanics:',
    '    - early contradiction',
    '    - progressive reveal',
    '    - on-screen text sequencing',
    '  - Avoid generic filler visuals or talking-head lists.',
    '  7. Distribution Plan',
    '  - Specify why this should be SAVED before a decision or SHARED with someone about to decide.',
    '  - Platform-aware without generic filler.',
    '  GLOBAL BRAND BRAIN CONSTRAINTS:',
    '  - No generic lists.',
    '  - No motivational filler.',
    '  - No obvious ads.',
    '  - Must create perception shift.',
    '  - Must separate informed vs uninformed viewers.',
    '  - Maintain pillar label integrity.',
    '  - Ensure time correctness.',
    '  - Ensure uniqueness across Education posts.',
    '- PROMOTION — Buying Trigger',
    '  - Do not describe features.',
    '  - Frame why this offer resolves a specific buyer tension now.',
    '  - Use scarcity, timing, or mismatch framing (who it is for / not for).',
    '  - Must feel like a reason to act, not browse.',
    ...
    '- LIFESTYLE — Decision Context',
    '  - Lifestyle content must increase sales readiness.',
    '  1. Hook (first 1–2 seconds)',
    '  - Must challenge a common assumption the viewer believes.',
    '  - Must create risk awareness, not vibes.',
    '  - Use one of these hook intents (intent only):',
    '    - “You think X means Y — it doesn’t”',
    '    - “What people don’t factor in before deciding”',
    '    - “This is why people regret X later”',
    '    - “Everyone focuses on X, ignores Y”',
    '  2. Decision context',
    '  - Explain how the overlooked factor leads to bad decisions, not just inconvenience.',
    '  - Tie lifestyle details to financial, timing, or commitment consequences.',
    '  3. Sales mechanism',
    '  - Show how misunderstanding this factor causes people to:',
    '    - overpay',
    '    - choose the wrong option',
    '    - rush decisions',
    '    - delay action',
    '  - Position professional guidance as the filter that prevents regret.',
    '  4. Copyable insight',
    '  - Provide a simple framework or checklist the viewer can use:',
    '    - what to evaluate',
    '    - what to compare',
    '    - what to ask before committing',
    '  - Must feel like “use this before you decide.”',
    '  5. CTA (decision protection)',
    '  - Frame CTA around avoiding regret, wasted money, or bad timing.',
    '  - Request a contextual keyword tied to the deliverable.',
    '  - Do not reuse the same keyword across posts.',
    '  6. Execution / Design Notes',
    '  - Enforce rewatch mechanics:',
    ...
    '    - progressive reveal',
    '    - on-screen text sequencing',
    '  - Avoid generic skyline or lifestyle filler visuals.',
    '  7. Distribution Plan',
    '  - Specify why this should be SAVED before a decision or SHARED with someone considering it.',
    '  - Platform-aware without generic filler.',
    '  GLOBAL BRAND BRAIN CONSTRAINTS:',
    '  - No generic lifestyle praise.',
    '  - No tourism framing.',
    '  - No motivational filler.',
    '  - Must create perception shift.',
    '  - Must separate informed vs uninformed viewers.',
    '  - Maintain pillar label integrity.',
    '  - Ensure time correctness.',
    '  - Ensure uniqueness across Lifestyle posts.',
    'CTA RULES (ALL CATEGORIES):',
    '- CTA must protect against a specific risk.',
    '- CTA keyword must be contextual and unique per post.',
    '- No generic "comment for info" framing.',
    'CTA + Engagement Loop: CTA must match the primary intent with specific, decision-protective wording and a contextual keyword. Engagement loop replies must ladder: easy answer -> qualifying question -> offer resource, all tied to the post angle.',
...
const SHORT_FORM_CONTENT_CONTRACT_BLOCK = [
  ...
  '- Every post must read like a ready-to-run TikTok/IG ad, not a blog.',
  '- Hook must stop scroll in the first 1-2 seconds with immediate stakes (time/money/tradeoff/opportunity) tied to the viewer.',
  '- Retention pacing: 4-6 short beats with visible shifts every ~2-3 seconds (pattern interrupt, quick contrast, rapid examples).',
  '- Use short lines and tight sentences; avoid dense paragraphs.',
  ...
  'Category/Pillar: Education (process steps / checks / what I would do / surface how-to), Social Proof (before/after / turning point / receipts), Promotion (subtle, entertainment-first; no hard sell).',
  ...
  'Caption: short lines (5-8 lines). Use 4-6 tight beats with one concrete example and one proof detail; no fixed before/pivot skeleton.',
  'Use the same single proof element in Caption and Reel Script.',
  'CTA: exactly ONE CTA, comment-first and intent-revealing; can include comment keyword, DM prompt, booking/guide prompt; must say what they get.',
  'ExecutionNotes: 20-30s, 6-10 cuts, avg shot length 1-2s, on-screen captions each beat; structure: Hook (0-2s) → Beat shifts (2-14s) → CTA (14-22s).',
  ...
  'engagementScripts format (STRICT):',
  'Exactly 3 lines. Each line starts with "Reply 1:", "Reply 2:", "Reply 3:".',
  'Each reply must be 1–2 complete sentences, <= 180 characters, and include a concrete comment trigger or A/B choice tied to the post\'s core point.',
  'Must be niche-specific to the post topic. Never blank.',
  'Output: engagementScripts.commentReply = Reply 1 + Reply 2 separated by "\\n"; engagementScripts.dmReply = Reply 3.',
  'ReelScript: 4-6 beat lines total; Hook line must match Hook exactly; then 2-4 short body beats + CTA line. Each beat is 1-2 short sentences with at least one pattern interrupt or specificity jump mid-script. For testimonials/social proof, require one specific moment; ban "happy client" / "personalized service".',
...
const LIFESTYLE_REDEFINITION_BLOCK = [
  ...
  '- Ban neighborhood rankings, tourism reels, "vibrant culture", "outdoor fun", and vibes-only lifestyle content.',
...
const QUALITY_ALIGNMENT_BLOCK = [
  ...
  '- All content must be short-form video first (visuals + narration).',
  '- ReelScript hook/body/cta must match Hook/Caption/CTA exactly; no off-topic drift.',
].join('\n');
...
const REGULAR_CALENDAR_CEILING_CONTRACT_BLOCK = [
  ...
  '',
  'OUTPUT CONTRACT (REGULAR)',
  '- Return ONLY valid JSON matching schema.',
  '- Every post must include all required fields exactly as schema expects.',
  '- If any required field is missing/empty/wrong-type, the output is invalid.',
  '- Before returning the final JSON, silently verify each post includes all required fields and none are blank.',
  '',
  'PURPOSE (REGULAR)',
  '- You are creating high-quality short-form video ads.',
  '- Do NOT explain strategy, psychology, optimization, or leverage.',
  '- Do NOT explain why something works.',
  '- Show outcomes, existence, or desirability only.',
  '- Leave the viewer informed and interested, but not taught a system.',
  '',
  'HARD RULES (REGULAR MODE = PRESENTATION ONLY)',
  '- Your job is to create post-ready short-form ad scripts optimized for TikTok/IG retention, but you MUST NOT use persuasion frameworks or decision architecture.',
  '- Forbidden concepts: belief teardown, hidden constraint, decision order, second-order consequence, micro-proof, conversion lever (and close paraphrases).',
  '- Forbidden: explanations of why something works, optimization logic, leverage framing, or second-order outcomes.',
  '- Forbidden phrases/frames (content-level): "think again", "myth", "mistake", "you\'re missing out", "most people", "hidden", "overlook", "don\'t get left behind", "common misconception", "the truth is".',
  '- No “Before/Pivot” frameworks.',
  '',
  'GENERAL RULES (REGULAR)',
  '- Baseline tier: strong execution without strategy mechanics.',
  '- Focus on awareness, surface education, lifestyle context, and social proof results only.',
  '- Avoid tourism/brochure tone. Every post must be an ad-script engineered for retention and action.',
  '',
  'REGULAR CONTENT ALGORITHM (NON-STRATEGIC)',
  '- Hook: 1 line, declarative, concrete, niche-specific; no rhetorical question.',
  '- Body: 4-6 short beats (1-2 sentences each), fast pacing.',
  '  * Include exactly ONE observable action step without explaining why it works.',
  '  * Include exactly ONE specific proof detail tied to the result (not causality).',
  '- CTA: a single comment keyword OR an A/B question tied to the post topic.',
  '- No persuasion frameworks. No belief reframes. No decision sequencing. No second-order consequence framing.',
  '',
  'SLOT DETERMINISM',
  '- Anchor each post to title + pillar + topic + topicAngle + day + slotIndex.',
  '- Write as if this post is the only correct output for its post_key/day/slot; do not drift to other days/topics.',
  '',
  'PILLAR RULES (REGULAR)',
  '- Education: surface-level how-to or observation + one action step; no mechanism/causality.',
  '- Promotion: present offer/value/access plainly. No hype urgency as psychology.',
  '- Social Proof: show outcome + 1-2 operational steps taken (what happened), not why it "inevitably" worked.',
  '- Lifestyle: connect lifestyle -> practical decision implication (cost/commute/fees/rules/resale/etc.) + one action step. No identity/priority reframes.',
  '',
  ...
  '- Each post object MUST include ALL required keys (non-empty): title, hook, caption, cta, topic_signature, angle, designNotes, distributionPlan, day, hashtags, script{hook,body,cta}, reelScript{hook,body,cta}, engagementScripts{commentReply,dmReply}.',
  '- If any required key would be missing, you MUST still output it with a non-empty value. Missing required keys is a failure.',
  'POST OBJECT SHAPE (keys only, placeholders):',
  '{"post_key":"<string>","day":<number>,"slotIndex":<number>,"title":"<string>","hook":"<string>","caption":"<string>","pillar":"<string>","topic_signature":"<string>","angle":"<string>","cta":"<string>","hashtags":["<string>"],"script":{"hook":"<string>","body":"<string>","cta":"<string>"},"reelScript":{"hook":"<string>","body":"<string>","cta":"<string>"},"designNotes":"<string>","distributionPlan":"<string>","engagementScripts":{"commentReply":"<string>","dmReply":"<string>"}}',
...
const BRAND_BRAIN_UNFAIR_ADVANTAGE_CONTRACT_BLOCK = [
  ...
  'You are generating publish-ready TikTok/Instagram ad content for the provided niche and pillar. Output must be on-topic, niche-specific, and formatted exactly to the required JSON schema. No extra text.',
  '',
  'OUTPUT CONTRACT (BRAND BRAIN)',
  '- Return ONLY valid JSON matching schema.',
  '- Every post must include all required base fields exactly as schema expects.',
  '- Must include Brand Brain locked fields when present in schema: beliefTeardown, hiddenConstraint, decisionOrder, secondOrderConsequence, microProof, conversionLever (non-empty strings).',
  '- If any required field is missing/empty/wrong-type, the output is invalid.',
  '- Before returning the final JSON, silently verify each post includes all required fields and none are blank.',
  '',
  'BRAND BRAIN MODE = DECISION ARCHITECTURE ENGINE',
  '- Each post must actively reframe how the viewer thinks.',
  '- If you are not tearing down a belief and replacing it with a better decision model, the post is wrong.',
  '- The difference between Regular and Brand Brain must be obvious without labeling.',
  '- You MUST apply a strategic decision-architecture sequence before writing any content, but DO NOT output the sequence itself.',
  '- No cheap contrarian trope hooks: forbid “you think”, “think again”, “myth”, “misconception”, “common mistake”, “stop doing”, “you’re missing”, “the truth is”.',
  '',
  'HARD PROHIBITIONS (BRAND BRAIN)',
  '- Must NOT sound like a better-written Regular post.',
  '- Must NOT be motivational without being instructional.',
  '- Must NOT be descriptive without restructuring how the viewer thinks.',
  '',
  'INTERNAL (NON-OUTPUT) SEQUENCE (must be followed silently)',
  '1) incorrectOptimization: what the audience is optimizing for incorrectly',
  '2) hiddenConstraint: the unseen factor blocking results',
  '3) decisionOrder: the correct order of decisions/actions (explicit sequence)',
  '4) secondOrderConsequence: what compounds later if they keep the old approach',
  '5) microProof: causal mechanism + one concrete example detail',
  '6) conversionLever: the psychological lever used to move action (not named as a buzzword; expressed as content)',
  '',
  'EXECUTION REQUIREMENT (VISIBLE CONTENT)',
  '- The hook/body/reelScript/caption MUST explicitly include all six elements above as natural sentences (not labels).',
  '- beliefTeardown must include a false belief sentence + a replacement belief sentence.',
  '- hiddenConstraint must reveal the unseen blocker.',
  '- decisionOrder must state the correct order (do X before Y).',
  '- secondOrderConsequence must describe what compounds later if they keep the old approach.',
  '- microProof must explain how/why the mechanism works with one concrete detail.',
  '- conversionLever must apply a specific lever (diagnostic, scarcity of attention, commitment, contrast, risk reversal, specificity) tied to the CTA.',
  '',
  'BRAND BRAIN SCRIPT SKELETON (NO FIXED WORDING)',
  '- Hook: contrast false belief vs true belief.',
  '- Body beat 1: hiddenConstraint.',
  '- Body beat 2: decisionOrder (first/then/after).',
  '- Body beat 3: secondOrderConsequence.',
  '- Proof beat: microProof (mechanism + example).',
  '- Close: conversionLever -> CTA.',
  '',
  'AD EXECUTION REQUIREMENTS (BRAND BRAIN)',
  'For each post:',
  '1) HOOK (first 1-2 seconds)',
  '- Must be one of: hyper-specific scenario, counterintuitive observation, contrast with stakes, or a precise question implying a costly tradeoff.',
  '- Must not use contradiction trope phrasing.',
  '',
  '2) REEL SCRIPT / BODY (retention pacing)',
  '- 5-7 short beats, quick shifts, with clear sequencing (first/then/after).',
  '- Include the secondOrderConsequence and microProof as distinct beats.',
  '- Include at least ONE concrete example detail derived from niche inputs (never hard-code).',
  '- No "3 tips + stat" template.',
  '',
  '3) ENGAGEMENT + CTA',
  '- Engagement prompt must commit the viewer to a position (A/B, comment keyword, or intent-revealing choice).',
  '- The 3 comment replies must validate + qualify + deliver the next asset.',
  '- CTA must logically follow from the decision architecture and match schema.',
  '',
  'PILLAR RULES (BRAND BRAIN)',
  '- Education: assumption -> blocker -> decision order -> second-order consequence -> mechanism proof -> CTA.',
  '- Promotion: blocker about access/timing/selection -> decision order -> second-order consequence -> mechanism proof -> CTA.',
  '- Social Proof: outcome explained by the blocker + decision order + what would have happened otherwise -> CTA.',
  '- Lifestyle: lifestyle choice framed as a decision-order problem with downstream consequence + proof -> CTA.',
  '',
  'OUTPUT CONSTRAINTS (BRAND BRAIN)',
  '- Return ONLY valid JSON matching schema. No markdown. No prose outside JSON.',
  '- Do not leave any required field empty.',
  '- Keep language natural and varied across posts; no repeated hook trope.',
  '- If the existing schema/validator requires specific marker strings/lines, KEEP THEM EXACTLY AS CURRENTLY REQUIRED (do not remove/rename). All other content should remain natural and non-templated.',
].join('\n');

const CALENDAR_HARD_SEPARATION_BLOCK = [
  'CRITICAL:',
  '- Regular Calendar posts must NEVER explain why something works or present decision architecture.',
  '- Brand Brain posts must ALWAYS include belief teardown, hidden constraint, decision order, second-order consequence, micro-proof, and conversion lever in the content.',
  '- The difference must be obvious without labeling; Regular should feel incomplete vs Brand Brain.',
].join('\n');
...
` : '';
  const brandBrainAddendum = opts.brandBrainDirective
    ? [
      ...
      '- DesignNotes: follow the designNotes format (STRICT) with 5 timecoded lines.',
...
`;
  ...
  const requiredKeysBlock = [
    ...
    'post_key, day, slotIndex, title, topicCapsule{summary,mustUse[],mustAvoid[],audienceAngle,keyEntities[]}, hook, caption, pillar, topic_signature, angle, cta, hashtags[], script{hook,body,cta}, reelScript{hook,body,cta}, designNotes, distributionPlan, engagementScripts{commentReply,dmReply}.',
  ...
  const finalCheckBlock = [
    ...
    '5) designNotes has 5 timecoded lines; engagementScripts use 3 Reply lines; distributionPlan has TikTok/Instagram lines.',
...
function buildCalendarSchemaBlock(expectedCount) {
  return `Calendar schema: ${expectedCount} posts with post_key, day, slotIndex, title, topicCapsule{summary,mustUse[],mustAvoid[],audienceAngle,keyEntities[]}, hook, caption, pillar, cta, hashtags[], script{hook,body,cta}, reelScript{hook,body,cta}, designNotes, distributionPlan, engagementScripts{commentReply,dmReply}. Each field must be non-empty and JSON must be valid.`;
}
