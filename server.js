...
function buildBrandBrainDirective(settings = {}) {
  ...
  const lines = [
    ...
    'Choose ONE persuasion lever per post (loss aversion, curiosity gap, authority, social proof, identity/status, risk reversal) and make it explicit in the writing.',
    ...
    'Optimize for ONE primary algorithm signal per post (comments, saves, shares, watch time) and state it in DistributionPlan.',
...
` : '';
  const brandBrainAddendum = opts.brandBrainDirective
    ? [
        ...
        `Explicitly select ONE persuasion lever per post (loss aversion, curiosity gap, authority, social proof, identity/status, risk reversal).`,
        ...
        `Hook: 1–2 lines; 0–3s retention hook; pattern interrupt/contrarian/mistake framing; explicit niche pain/goal; curiosity gap; no soft questions unless contrarian.`,
        `Caption (body): short beats for retention; include pain, mechanism, mistake, consequence, fix steps (3), proof type (market stat/case outcome/what I see with clients), and objection handling; keep it tight and readable; under-resolve to invite engagement.`,
        `CTA: two-step — primary growth CTA is a COMMENT keyword; secondary conversion CTA is DM for personalized help. Example: "Comment 'SELL' and I'll DM the checklist."`,
        `StoryPrompt: decision-based and intent-revealing; force a choice (timeline, budget, area, concern). No open-ended "what do you think".`,
        `DesignNotes: 3–5 concrete visual beats + on-screen text beats + pattern changes every 2–3s + a pause-and-read overlay moment, all niche-specific.`,
        `EngagementScripts: include one comment reply and one DM opener that qualify the lead; include a forced-choice prompt and a follow-up post idea in the comment reply or DM script.`,
        `ReelScript: include Hook line, Body beats, mid re-hook line (~5–7s), CTA line; loopable ending that mirrors or re-opens the hook.`,
        `DistributionPlan: actionable and specific (best posting window, pinned comment, caption sequence). Must name the primary algorithm signal (comments/saves/shares/watch time) and include a follow-up trigger (e.g., "If people comment X, post part 2 on Y").`,
