...
const SHORT_FORM_CONTENT_CONTRACT_BLOCK = [
  ...
  '- Use ONLY the user-provided niche inputs for this request (niche, audience, pillars, brand settings, any user-entered details). Do NOT invent or assume location, industry, profession, or any named entities. If a term is not present in inputs, it must not appear in output.',
  '- Do not reuse example niche tokens from prior conversations or training examples.',
  '- Forbidden unless present in inputs: any city/state/country names; any industry labels; any platform-specific local references; any prior example terms.',
  '- Forbidden unless present in inputs: Miami, real estate, Brickell, Wynwood, Coral Gables, Key Biscayne, condo.',
...
const REGULAR_CONTENT_QUALITY_RULES_BLOCK = [
  ...
  '- Regular posts must be neutral and informational.',
  ...
  '- CTA: action-only, 4–8 words. Do not use "learn more", "visit website", or "today".',
  '- CTA must not introduce any term not present in inputs.',
...
const BRAND_BRAIN_DIFFERENTIATION_RULES_BLOCK = [
  ...
  '- ReelScript.body must include all four components in order: mistaken belief (1 sentence), hidden constraint (1–2 sentences, concrete and non-obvious; no generic phrases like "market dynamics", "unique challenges", "underlying issues"), consequence if ignored (1 sentence, specific), reframe/what to do instead (1 sentence, specific). If any component is missing or generic, rewrite.',
  '- Hidden constraint must be specific to the niche inputs and operational; generic constraints are invalid.',
  '- CTA must be a single next step (DM, Comment, Download, Checklist, or Call). Do not use "learn more", "discover today", or "explore now". Must not repeat the Hook.',
  'HOOK VARIETY RULE:',
  '- Do not reuse the same hook phrasing across posts.',
