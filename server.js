...
` : '';
  ...
  const brandBrainAddendum = opts.brandBrainDirective
    ? [
        `NO TEMPLATE RULES: do not start any field with "Day X:" (except if a field explicitly requires on-screen text, and even then only once). Avoid repetitive coaching filler. Do not repeat the exact niche phrase more than once across the entire card.`,
        ...
        `Forbidden outputs: "placeholder", "quick hook", "explain the idea", "explain how", "ask for feedback", "neutral background", "let me know what you think", "talk briefly", "screenshot this so you remember", "office hours", "lorem", "tbd", "insert", "Loss aversion:", "call out what gets lost", "anchor the fix", "the payoff is", "open with the local cue", "proof detail", "cleaner next steps", "without chasing", "pattern interrupt in the first second".`,
        `Never output meta-instructions or templates. No headings, labels (Hook:/CTA:/Caption:/Story Prompt:), or instructional verbs (Explain/List/Outline). Output must read like finished content.`,
        ...
        `Each post must embed exactly one sales psychology lever (loss aversion, social proof, scarcity/urgency, commitment/consistency, authority, contrast framing, reciprocity, uncertainty reduction) and one social algorithm tactic (3-second hook pattern, retention loop, pattern interrupt, caption skim structure, comment trigger, save/share trigger, series framing, open-loop teaser, visual pacing). Execute them concretely in the copy; do not name the tactic.`,
        ...
        `Hook: <= 10 words; curiosity gap + local specificity; no "Day X" prefix; no soft questions unless contrarian.`,
        `Caption: 3 short lines max for retention. Line 1: local scenario + tension. Line 2: step/comparison with a save trigger. Line 3: micro-proof and a comment trigger. Line 4: CTA keyword.`,
        `CTA: single clear conversion action with keyword (DM or comment) tied to the exact asset described in the body.`,
        ...
        `StoryPrompt format: one sentence question that is either constraint-based or binary.`,
        `DesignNotes: include one retention device, one pacing rule, and one on-screen text line. No "Day X" unless chosen format demands it.`,
        `EngagementScripts: include one pinned-comment text and one DM opener that qualify the lead (timeline + budget range + neighborhood focus or selling timeframe + property type).`,
        ...
        `For each post, silently choose ONE category and follow its rules (do not label the category):`,
        `Educational: include a Miami-specific scenario (e.g., Brickell condo buyer, Kendall upgrader, out-of-state investor) and implement ONE psychology lever through the example; include a save trigger + a comment trigger; CTA matches the asset described.`,
        `Lifestyle: hook is identity-based in the niche; body shows behind-the-scenes that builds trust; CTA is soft conversion (DM for checklist, tour list, appointment slot); StoryPrompt is short and reactive to the moment.`,
        `Promotional/Offer: hook frames the offer as pain relief; body explains what/for who/why now + one objection; CTA is direct booking/DM keyword with believable scarcity.`,
...
function normalizePost(post, idx = 0, startDay = 1, forcedDay, nicheStyle = '', options = {}) {
  ...
  const repurposeFallback = allowFallbacks ? ['Reel -> Remix with new hook', 'Reel -> Clip as teaser'] : [];
  const analyticsFallback = allowFallbacks ? ['Reach', 'Saves'] : [];
  const repurpose = ensureStringArray(post.repurpose || [], repurposeFallback, allowFallbacks ? 2 : 0);
  const analytics = ensureStringArray(post.analytics || [], analyticsFallback, allowFallbacks ? 2 : 0);
  ...
  const ideaValue = toPlainString(post.idea || post.title || (allowFallbacks ? 'Engaging post idea' : ''));
  const normalized = {
...
async function callOpenAI(nicheStyle, brandContext, opts = {}) {
  ...
  console.log('[Calendar][Schema] calendar_batch', {
    requestId: loggingContext?.requestId || 'unknown',
