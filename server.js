...
` : '';
  const brandBrainAddendum = opts.brandBrainDirective
    ? [
        ...
        'Apply curiosity gaps, pattern interrupts, open loops, and concrete specificity (numbers/timeframes/outcomes).',
        ...
        'No emojis. No holiday-themed music suggestions; if audio exists, format as "Song Title - Artist" only.',
        'FIELD CONTRACT (use exact field names):',
        `HOOK (hook): 1–2 lines, <= 18 words; must be one of contrarian claim, problem agitation, surprising stat, "stop doing X", "you're doing X wrong", or "I did X for 7 days"; imply payoff + open loop; not a generic question; reference one concrete pain or cost.`,
        'BODY (caption): 3–5 bullet beats, each beat = "Mistake -> Consequence -> Fix"; include credibility marker ("I see this constantly with [audience]" or "In practice, the fastest fix is..."); include one micro-proof (example/before-after/scenario); include one objection crusher; include one save-worthy checklist/framework; no fluff or padding.',
        'CTA (cta): exactly one action; keyword CTA format: Comment "KEYWORD" for ____ OR DM "KEYWORD" for ____; KEYWORD must be niche-specific; immediate benefit required; no "Ready for..." questions.',
        'STORY PROMPT (storyPrompt): single tight narrative prompt for story asset; opening frame visual + 3 beats + CTA frame; must not repeat full body; max 450 chars.',
        'DESIGN NOTES (designNotes): first 1s visual (pattern interrupt), cut cadence (fast/medium/slow), 2 pattern interrupts, 2 overlay texts (exact text), 1 prop or on-screen action, and a save moment cue ("pause here" or "screenshot this").',
        'ENGAGEMENT LOOP (engagementScripts): include pinned comment (exact copy), one reply template for comments, one DM opener template, and one follow-up content suggestion (carousel/testimonial/demo).',
        `REEL SCRIPT (reelScript): 7–12 seconds for hook, total 25–40 seconds; short lines; include a pattern interrupt line ("Pause—this is why it’s not working."), a specific instruction line, and a credibility line ("Here’s what I’ve seen work repeatedly…").`,
        'EXECUTION NOTES (if a separate execution/format field exists; otherwise include in designNotes): include timestamps 0–1s, 1–5s, 5–12s with visual resets every 2–3 seconds.',
        'HASHTAGS (hashtags): 5–8 max; 2 broad, 3 niche, 1 intent; no stuffing.',
        'DISTRIBUTION PLAN (distributionPlan if present): return exactly:',
        'Objective: (watch time / shares / DMs / conversions)',
        'Target viewer: (1 sentence)',
        'Retention beats: (3 bullets with visual reset + line)',
        'Caption: (3 sentences: insight, proof/detail, CTA)',
        'Post timing: (one specific suggestion)',
        'Repurpose: (2 variants: carousel + story)',
        'Ban meta-instructions like "Explain how..." or "without quoting."',
        'CATEGORY PLAYBOOK (apply based on type/pillar/category):',
        '- Educational: Mistake -> Consequence -> Fix, authority, save/share trigger.',
        '- Promotional: Problem -> Mechanism -> Offer -> Proof -> CTA; include "who it’s for".',
        '- Testimonial: Before -> After -> Process -> Proof -> CTA; numbers only if plausible.',
        '- Behind-the-scenes: "what people think vs reality", process transparency, credibility.',
        '- Trend: trend hook + niche twist + value; no generic dance prompts.',
        '- Community: identity + debate prompt + pinned comment structure.',
        'SELF-CHECK: Brand Brain enabled means every card must include explicit sales + algorithm elements; no generic versions.',
      ].join('\n')
    : '';
