...
function buildBrandBrainDirective(settings = {}) {
  ...
  const lines = [
    ...
    '- Lifestyle: reframe assumptions about a lifestyle decision (not enumerate facts). Hook challenges a common belief and leads with contrast/misalignment; avoid discovery framing. Body is not list-style; focus on 1 primary overlooked mechanism or compounding factor, explain why it is underestimated and how it affects decisions. Any data supports the insight, not replaces it. Tie insight to preparedness/timing/decision quality; implicitly separate informed vs uninformed movers; sound revealing, not advisory. Prefer SAVE/SHARE/GUIDE REQUEST; CTA protects a future decision, not consuming tips, and avoid checklist-first framing unless readiness is established. Engagement loop qualifies seriousness (timeline/intent/readiness), avoids casual acknowledgements, and progresses toward DM-worthy signals. Bias toward saves/shares; Distribution Plan justifies why it is worth revisiting later. Self-check: changes how viewers evaluate the lifestyle, creates perception vs reality tension, and keeps subtle sales alignment without explicit selling.',
