...
function buildBrandBrainDirective(settings = {}) {
  ...
  const lines = [
    ...
    '- Education/How-To: 3-5 step method OR common mistake + fix with concrete checks.',
    ...
    '- Lifestyle: choose ONE framing (myth-vs-reality, tradeoff, decision filter, or mindset shift). Ban purely descriptive or aspirational language without tension. Hook must introduce contrast, contradiction, or a decision. Body is 3-4 points only; each point is a decision-relevant factor (daily-life impact). Facts must support a decision outcome, not standalone benefits. Choose ONE intent (SAVE or COMMENT); CTA must reference the decision framework and avoid mixed intent. Engagement loop continues decision logic (priorities/constraints/timeline) without generic prompts. Distribution Plan must state the save/share reason tied to future decision-making. Self-check: one archetype, one intent, tension present, decision utility clear, no generic lifestyle filler.',
