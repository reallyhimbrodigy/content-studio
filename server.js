...
` : '';
  const brandBrainAddendum = opts.brandBrainDirective
    ? [
        `Brand Brain Output Playbook (apply to every card and field):`,
        `Audience intent: convert attention -> lead -> appointment/purchase using ethical persuasion.`,
        `Writing must be concrete, niche-specific, and outcome-oriented.`,
        `Use one primary offer angle per card and keep it consistent across CTA + engagementScripts + reelScript.`,
        `If medical/aesthetic niche: avoid medical guarantees; use "may/can/often" and encourage consult.`,
        `Never include emojis unless base rules already do.`,
        `No fabricated statistics; no numeric proof unless user-provided.`,
        `Do not write generic platitudes. Every line must reference the niche and a specific customer desire or pain.`,
        `If proof requires numbers you do not have, use a proof type phrased without metrics.`,
        `Return JSON only; do not rename or omit fields.`,
        `FIELD-BY-FIELD SPEC (use exact field names):`,
        `HOOK (hook): 1–2 lines max. Must be one of: contrarian claim; "you're doing X wrong"; "stop doing X"; "most people miss this"; "if you have X, do Y"; "I did X for 7 days" (only if it fits niche). Must create a curiosity gap + implied payoff. Avoid questions unless it's a direct challenge.`,
        `BODY (caption): compact bullets or tight sentences in this order: Problem (1 line) -> Mechanism/why (1 line) -> Mistake (1 line) -> Consequence/hidden cost (1 line) -> Fix (1–3 steps) -> Objection crusher (1 line) -> Save/share trigger (1 line) -> Soft proof (1 line) using allowed proof types (no numbers unless provided).`,
        `CTA (cta): single action only. Value-exchange CTA with DM or comment keyword where relevant. Must match engagementScripts + reelScript CTA.`,
        `STORY PROMPT (storyPrompt): one-line IG Story interaction prompt (poll/slider/quiz). Must drive segmentation or buying intent. Do not repeat the hook.`,
        `DESIGN NOTES (designNotes): specify first 1s visual + on-screen text + 2 pattern interrupts + one retention device. Use specific shots, not generic.`,
        `ENGAGEMENT LOOP (engagementScripts): include comment reply template (1–2 sentences), DM follow-up opener (1–2 sentences) that asks for one qualifying detail, and a follow-up post idea (testimonial/FAQ/myth-bust/case-study framework).`,
        `REEL SCRIPT (reelScript): structured as Hook (same as hook field) -> Body (3 beats max) -> CTA (same as CTA field). Include a mid-video pattern-interrupt line.`,
        `DISTRIBUTION PLAN (distributionPlan if present): include: 1-line why this hook retains (no quoting hook), two concrete visual beats, caption path (insight sentence + support sentence + CTA sentence matching CTA), three SEO keyword phrases (not hashtags), and a timing heuristic ("test two windows").`,
        `HASHTAGS (hashtags): ALWAYS present as an array of strings. Use 8–12 tags mixing niche, service, local intent, problem, and format. No holiday tags unless explicitly requested.`,
      ].join('\\n')
    : '';
