...
const REGULAR_CALENDAR_CEILING_CONTRACT_BLOCK = [
  'REGULAR CALENDAR OUTPUT RULES:',
  '- Do NOT write inspiration, praise, "magic", "transformations", "dream", "vibrant", "unmatched", or tourism-style lifestyle content.',
  '- Every post must directly support the niche\'s business outcome (leads, inquiries, bookings, sales).',
  '- Write as ad-adjacent content: practical, concrete, scenario-based.',
  '- Scripts should be usable as simple ads, but without unfair-advantage mechanics.',
  '- Provide 3 practical actions/checkpoints plus 1 concrete example; include exactly ONE concrete mechanism and exactly ONE action step.',
  '- Never use empty steps like "trust your agent", "stay open", "define must-haves".',
  '- CTA must be lead-intent oriented (comment keyword for checklist, DM prompt, or "reply with X"), NOT generic ("comment your thoughts", "share your story").',
  '- Engagement prompt must be tied to the mechanism.',
  '- Hook must be observational/curiosity-based; no contradiction framing.',
  '- Body must be practical and helpful; avoid framing the audience as wrong.',
  '- Close with a clear engagement trigger and low-friction CTA.',
  '- Each post must use a distinct, concrete scenario/example/data point tied to its title/pillar; do not reuse the same core angle across the calendar.',
  '- No myth-busting or misconception teardown.',
  '- Do NOT use "you think X / think again" style contradiction hooks or misconception teardowns.',
  '- Do NOT use before/pivot/proof skeletons or pre-handle objections.',
  '- Do NOT reorder decisions or emphasize downstream consequences as persuasion.',
  '- Do NOT use buyer-psychology or objection pre-removal language.',
  '- Avoid inevitability framing; stay surface-level persuasive.',
  '- Explicitly AVOID revealing:',
  '  - sequencing ("do this first, then...")',
  '  - hidden levers',
  '  - conversion psychology',
  '  - algorithm exploitation mechanics',
  '',
  'REGULAR PILLAR SHAPING:',
  '- REGULAR — EDUCATION: teach a concrete process/mechanic; avoid myth-busting.',
  '- REGULAR — PROMOTION: highlight offer/value/access straightforwardly; avoid urgency-as-psychology.',
  '- REGULAR — SOCIAL PROOF: specific outcome story; avoid inevitability framing or belief correction.',
  '- REGULAR — LIFESTYLE: connect lifestyle to practical considerations; avoid identity/priority reframes.',
  '',
  'REGULAR CEILING ENFORCEMENT:',
  '- No belief collapse.',
  '- No hidden levers.',
  '- No sequencing.',
...
const BRAND_BRAIN_UNFAIR_ADVANTAGE_CONTRACT_BLOCK = [
  'BRAND BRAIN OUTPUT RULES (MANDATORY, FAIL IF MISSING ANY):',
  'Every post MUST include ALL 6 elements below. If any element is missing, regenerate internally and output only a compliant post.',
  '- Must always feel like an unfair advantage and outcome-forward, never inspirational.',
  '- Keep required marker lines concise (1-2 sentences), concrete, niche-specific, causal; avoid generic filler.',
  '- Required marker lines must read like natural sentences, not obvious headers, while keeping the exact marker prefixes.',
  '- After the marker lines, continue the reel script as a viral TikTok/IG ad with strong pacing, engagement trigger, and CTA.',
  '- The script must change how the audience thinks before asking them to act.',
  '- Do NOT rely on a single "you think X" contradiction pattern or generic urgency as the core device.',
  '- Do NOT reuse the Regular 3-tips + proof skeleton.',
  '- Must include in script/caption (naturally, no labels): incorrect assumption, real blocker, reordered sequence, downstream consequence, mechanism-based proof, conversion lever.',
  '- Hook must be one of: hyper-specific scenario, counterintuitive observation, contrast with stakes, or a precise tradeoff question. Avoid "think again" / "you think" trope hooks.',
  '- If steps appear, they must be framed as decision order tied to the blocker and mechanism (not generic tips).',
  '',
  '1) BELIEF COLLAPSE HOOK (1-2 seconds)',
  '- Start by attacking a specific false belief the target customer holds.',
  '- Must imply risk, loss, or "you\'re doing it wrong."',
  '- Ban openings like: "Unlock the secrets", "Curious about", "Witness the magic", "This city is full of...".',
  '',
  '2) HIDDEN LEVER (non-obvious mechanism)',
  '- Introduce one lever most people ignore that changes outcomes.',
  '- Must be something Regular never says (no generic tips).',
  '- Must NOT be a listicle or obvious advice.',
  '',
  '3) SEQUENCING (order beats effort)',
  '- Explicitly state that order/timing/framing is the advantage.',
  '- Must include a "wrong order fails" contrast.',
  '- Do NOT give a generic 1-2-3 list of obvious steps.',
  '',
  '4) ALGORITHM STRUCTURE (rewatch + save engineering)',
  '- Include a delayed reveal (e.g., a late contradiction or callback).',
  '- Must include at least one rewatch trigger (contradiction resolved late, pattern break, or callback).',
  '',
  '5) AD-READY POSITIONING (user as operator)',
  '- Must position the niche user (agent/brand) as the operator of the lever.',
  '- Must imply "this is why clients choose us / why our method wins" without sounding like an ad.',
  '- Must be directly runnable as a paid ad script.',
  '',
  '6) PROTECTIVE CTA (decision protection)',
  '- CTA must feel like mistake prevention or decision protection.',
  '- Must filter for intent (timeline/budget/goal) and push toward DM or a qualified comment keyword.',
  '- Ban generic CTAs: "comment your thoughts", "share your story".',
  '',
  'PILLAR SHAPING (BRAND BRAIN):',
  '- EDUCATION: assumption disruption + mechanism + decision sequence + second-order consequence (ad-paced).',
  '- PROMOTION: sequence change + hidden constraint about timing/access + lever; avoid hype.',
  '- SOCIAL PROOF: outcome is causally inevitable (what changed + what would have happened otherwise) + sequence + second-order.',
  '- LIFESTYLE: priority/identity reframe expressed naturally + consequence + mechanism.',
  '',
  'BRAND BRAIN BANS:',
  '- No tourism/lifestyle vibes.',
  '- No listicles like "Top 5 neighborhoods".',
  '- No generic "market trends are changing" blog talk.',
  '- No empty "Before/Pivot/Steps" unless each is specific, measurable, and ties to the hidden lever + sequencing.',
  '- No platitudes ("trust your agent", "stay open", "define must-haves").',
].join('\n');
