...
` : '';
  ...
  const brandBrainAddendum = opts.brandBrainDirective
    ? [
        ...
        '',
        'Apply exactly one primary persuasion lever per card (choose one): loss aversion, social proof, authority, scarcity, urgency, reciprocity, commitment/consistency, identity signaling, status/aspiration, contrast/anchoring, objection preemption, risk reversal, narrative transportation, curiosity.',
        'Apply exactly one primary algorithm lever per card (choose one): 1-3s retention hook, completion rate, rewatch trigger, saves utility, shares, comment depth/velocity, profile taps/follows, pattern interrupt, novelty, pacing/cuts/captions, comment-to-DM loop, community signaling, topic consistency.',
        'Embed these levers in the copy; do not name them explicitly.',
        '',
        'Section guidance (use existing fields only):',
        '- Hook: pattern interrupt + curiosity or stakes in one punchy line.',
        '- Caption/Body: concrete local detail, specific proof or objection handling, short paragraphs.',
        '- CTA: one clear action tied to a keyword (DM "..." or comment "...") aligned to the chosen algorithm lever.',
        '- Story Prompt: designed for easy, specific replies without sounding bait-y.',
        '- Engagement Loop: 2-3 realistic reply templates that move toward DM using reciprocity/proof/authority.',
        '- Reel Script: tight beats with fast pacing; include a rewatch trigger (list/reveal/before-after/checklist) without meta wording.',
        '- Design Notes: visuals that create a pattern interrupt and clarify the promise (on-screen text, cuts, B-roll).',
        '- Distribution Plan: platform-native actions (share to Stories, pin comment, remix/duet prompt if relevant) and timing; no vague "share on social media."',
        '- Hashtags: relevant, non-stuffed; avoid repeating the niche phrase in every tag.',
        '',
        'Quality bar: specific, non-generic, no niche stuffing, no instructional/meta language, no "Day X" framing. Keep it skimmable and creator-native.',
      ].join('\n')
    : '';
