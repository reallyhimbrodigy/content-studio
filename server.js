...
const SHORT_FORM_CONTENT_CONTRACT_BLOCK = [
  ...
  '- You MUST use only the user-provided inputs for this request (niche, audience, offers, pillars, brand settings, any user-entered details).',
  '- Do NOT invent any location (city/region/country), neighborhoods, landmarks, local hotspots, weather, skyline, or place-specific culture.',
  '- Do NOT invent any industry or profession terms or scenario nouns unless they appear verbatim in the current request inputs.',
  '- If the input does not contain a location, output must be location-agnostic.',
  '- Never reuse tokens from examples, prior runs, or training. Each run is isolated to the provided inputs.',
  '- Any proper noun or place name is forbidden unless it appears verbatim in the inputs. If you are about to output one, remove it and rewrite.',
...
const QUALITY_ALIGNMENT_BLOCK = [
  ...
  '- No section may repeat another section’s phrasing or premise. The Hook text must not be restated in Caption/Body/CTA.',
  '- Hook, caption, reelScript, CTA, engagementScripts, and distributionPlan must not restate each other; each must add new information.',
  ...
  '- reelScript.body must add new reasoning not already in the Caption.',
  '- CTA: action-only; do not repeat Hook/Caption phrasing.',
  '- engagementScripts.commentReply: one specific follow-up question tied to the post angle.',
  '- engagementScripts.dmReply: clear next step; must not restate the CTA verbatim.',
  '- distributionPlan: generic platform placement guidance only; no Hook/CTA language.',
].join('\n');
...
const REGULAR_CONTENT_QUALITY_RULES_BLOCK = [
  ...
  '- Do not use belief reversal, myth framing, hidden-constraint language, consequence framing, contrarian framing, urgency, fear, or pressure.',
  '- Do not use urgency/pressure phrasing, including: "don\'t miss out", "act now", "before anyone else", "exclusive access", or competitive framing.',
  ...
  '- Regular posts must be neutral and informational. No contrarian "Stop believing..." tone. No urgency/pressure.',
  '- Ban filler openers in Hook/CTA: "Discover", "Explore", "Let\'s dive", "Understanding X is crucial", "Understanding X is essential", "Learn more", "Visit our website", "today", "now".',
  'CAPTION RULE (ANTI-REDUNDANCY):',
  '- The caption must NOT restate the reel script.',
  'CTA LIMITATION:',
  '- CTA must be a single neutral action. No urgency, no pressure, no implied loss.',
  '- Hook: 1 sentence, specific to the niche inputs; cannot reference any location unless provided.',
  '- Caption: 2–4 sentences; must add 2 concrete details not stated in Hook.',
  '- ReelScript.body must add 2 additional concrete details not stated in Hook/Caption; no paraphrasing.',
  '- CTA: action-only, 4–8 words. Must be a single concrete next step with a deliverable; no banned filler words. No location/industry terms unless present in inputs.',
  '- If the model cannot produce concrete details from inputs, it must generalize without adding invented specifics (no place names, no niche defaults), but remain actionable (frameworks/steps/questions).',
...
const BRAND_BRAIN_DIFFERENTIATION_RULES_BLOCK = [
  ...
  'Each post must explicitly include: (1) a false belief the audience holds, (2) one concrete hidden constraint in the niche, (3) an explicit decision reversal, and (4) a specific consequence of ignoring the constraint; keep all parts aligned to the post title and niche.',
  ...
  '- ReelScript.body must include all four components in order: mistaken belief (1 sentence), hidden constraint (1–2 sentences, concrete and non-obvious; generic phrases like "market dynamics", "unique challenges", "hidden truth", "underlying issues", "diverse range", "cost of living", "competition" are invalid), consequence if ignored (1 sentence, specific), reframe/what to do instead (1 sentence, specific). If any component is missing or generic, rewrite.',
  '- Hidden constraint must be specific to the niche inputs and operational; generic constraints are invalid.',
  '- CTA must be a single next step (DM, Comment, Download, Checklist, or Call). Do not use "learn more", "discover today", or "explore now". Must not repeat the Hook. No location/industry terms unless present in inputs.',
  'HOOK VARIETY RULE:',
  '- Do not reuse the same hook phrasing across posts.',
