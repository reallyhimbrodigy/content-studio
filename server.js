...
const REGULAR_CONTENT_QUALITY_RULES_BLOCK = [
  'REGULAR HARD RULES:',
  '1) Hook: exactly 1 sentence. Must NOT use generic claims like "easier than you think", "don\'t miss out", "booming", "full of opportunities", "amazing homes". Must include either contrast or a specific Miami anchor.',
  '2) Miami Anchor (required): every post must include EXACTLY ONE of:',
  '   - one named Miami neighborhood (e.g., Brickell, Edgewater, Coral Gables, Pinecrest, Wynwood, Downtown), OR',
  '   - one property-type constraint (e.g., "condo HOA/rental rules", "condo reserves/special assessments", "flood zone + insurance").',
  '   Do NOT include multiple neighborhoods in the same post.',
  '3) Constraint (required): include EXACTLY ONE friction/tradeoff: HOA rules, condo reserves/special assessments, insurance/flood, inspection leverage, appraisal gap, property taxes, rental restrictions, parking/fees.',
  '4) Pillar requirements:',
  '   - Promotion: include (i) one differentiator you provide (screening checklist / off-market access / negotiation plan / underwriting screen), (ii) the single constraint, (iii) CTA must be a DM or comment keyword only.',
  '   - Social Proof: include one observable "proof object" event (inspection credit negotiated, appraisal gap handled, reserve study issue avoided, rental cap caught before offer, multiple-offer win). No vague "winning strategy".',
  '   - Education: pick ONE trend only, state why it matters, then give ONE "do this first" heuristic tied to the anchor + constraint. Ban filler like "stay informed".',
  '   - Lifestyle: include "who it\'s for" + tradeoff in one sentence; do not add tourism-only vibes.',
  '5) No repetition: do not repeat the hook verbatim in caption or script body.',
].join('\n');

const BRAND_BRAIN_DIFFERENTIATION_RULES_BLOCK = [
  'BRAND BRAIN HARD RULES:',
  '1) Hook diversity across the 4 pillars in a batch:',
  '   - At most ONE hook may start with "Stop".',
  '   - At most ONE hook may start with "Most people...".',
  '   - At most ONE hook may use "You\'re missing out...".',
  '   - At most ONE hook may use "Think X? Think again".',
  '   Generate four distinct hook patterns.',
  '2) Body must include ALL five items below, written naturally:',
  '   (i) incorrect belief -> replacement belief',
  '   (ii) one non-obvious factor (choose ONE: HOA rental cap, condo reserves/special assessments, insurance/flood cost, appraisal gap risk, inspection leverage, property tax/HOA fees)',
  '   (iii) decision-order correction using this EXACT grammatical template:',
  '        "Most people decide [WRONG FIRST]; decide [RIGHT FIRST] first."',
  '        Ensure [WRONG FIRST] and [RIGHT FIRST] are short, concrete nouns/verbs (e.g., "the listing photos" vs "the HOA/rental rules"; "the price" vs "the long-term carrying costs").',
  '   (iv) practical second-order consequence (specific cost/risk), no melodrama',
  '   (v) choose EXACTLY ONE conversion lever per post and obey its language constraints:',
  '       - Certainty lever: use "check/verify/steps/framework"; forbid urgency words ("now", "before", "act fast") and forbid status language ("savvy/insiders").',
  '       - Urgency lever: use "window/this week/before"; forbid "verify/steps/framework" and forbid fear-heavy "avoid/don\'t lose".',
  '       - Loss aversion lever: use "avoid/costs you/don\'t lose"; forbid urgency and forbid "framework/steps".',
  '       - Status lever: use "savvy/insiders/people who win"; forbid urgency and forbid fear-heavy "avoid/costs you".',
  '   Do NOT mix levers.',
  '3) Brand Brain should be mechanism-centric (buyer/seller advantage), not agent-ego. Reduce "me vs other agents" framing; make it about the client\'s decision edge.',
].join('\n');
