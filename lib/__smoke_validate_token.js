...
// ── AND THE PROCESS REFUSES TO BOOT WITHOUT IT ─────────────────────────────
// A guard that disables itself when misconfigured is the silent-inert pattern:
// with the secret unset the mint returns null, the field leaves the snapshot,
// and /validate goes anonymous with nothing red anywhere. The boot gate makes
// that a loud outage instead.
ok(/for \(const k of \['MODAL_CALLBACK_SECRET', 'MODAL_RUN_SECRET', 'MODAL_VALIDATE_SECRET'\]\)/.test(srv),
   'MODAL_VALIDATE_SECRET is not in the fail-closed boot array — the server '
   + 'would start with the mint dark and /validate anonymous, silently');
ok(/modal_validate_secret:\s*!!process\.env\.MODAL_VALIDATE_SECRET/.test(srv),
   '/api/health must expose the secret as a presence boolean — it is the only '
   + 'way to check from OUTSIDE that arming the boot gate will not crash-loop');
