...
/** The jsonb containment operand, as a STRING. This is not a style choice.
 *
 *  MEASURED IN PRODUCTION: passing the ARRAY form — .contains('messages',
 *  [{jobId}]) — makes supabase-js serialize it as a POSTGRES ARRAY LITERAL, and
 *  the objects inside it stringify to the literal text `[object Object]`:
 *
 *      array  ->  messages=cs.{[object+Object]}    ERROR: invalid input syntax
 *                                                         for type json
 *      string ->  messages=cs.[{"jobId":"..."}]    correct
 *
 *  The first deploy of this file used the array form. Every lookup threw, the
 *  throw was caught as `lookup_failed`, and the feature ran for its first pass
 *  attaching NOTHING while looking healthy. The unit smoke could not catch it
 *  because a MOCK implements .contains() the way its author imagined the client
 *  behaves — so the regression check for this lives in
 *  __smoke_chat_attach.js as an assertion on the URL the REAL client builds. */
function jobIdContainsFilter(jobId) {
...
async function findChatHoldingJob(supabaseAdmin, userId, jobId) {
  const { data, error } = await supabaseAdmin
    ...
    .eq('user_id', userId)
    .contains('messages', jobIdContainsFilter(jobId))
    .limit(1);
  if (error) throw new Error(`chat lookup failed: ${error.message}`);
  return (Array.isArray(data) && data[0]) || null;
}

/** Read one chat by its id, whatever it holds. `findChatHoldingJob` cannot be
 *  used for this: it searches for a chat CONTAINING the job, so a chat that
 *  exists but has lost the render message is invisible to it — which is
 *  precisely the row we need to look at after a key collision. */
async function findChatById(supabaseAdmin, chatId) {
  const { data, error } = await supabaseAdmin
    .from('chats')
    .select('id, user_id, messages, updated_at')
    .eq('id', chatId)
...
/**
 * Make an EXISTING chat hold this render, under the same CAS the heal path uses.
 *
 * WHY THIS EXISTS (2026-09-10). The reconstruct path treated a duplicate-key
 * collision as success: `23505` → `{ attached: true, reason:
 * 'raced_already_created' }`, on the reasoning that someone else derived the
 * same id first. But the row existing does NOT mean the row holds the render.
 * The client PATCHes this column blind, so a chat this code created moments
 * earlier can already have been overwritten — one such chat was found holding
 * ZERO messages, another holding twenty with the render not among them. Four of
 * 66 stranded renders were counted fixed and were not.
 *
 * So a collision now falls through to here: re-read the row, append the render
 * message if it is absent, heal it if it is present but stale, and take the
 * same CAS so a concurrent client write loses rather than being clobbered.
 */
async function ensureRenderInChat(supabaseAdmin, chatId, job, log) {
  for (let attempt = 1; attempt <= CAS_ATTEMPTS; attempt += 1) {
    // eslint-disable-next-line no-await-in-loop
    const chat = await findChatById(supabaseAdmin, chatId);
    if (!chat) return { attached: false, reason: 'raced_then_vanished', chat_id: chatId };

    const messages = Array.isArray(chat.messages) ? chat.messages : [];
    const holds = messages.some((m) => m && typeof m === 'object' && m.jobId === job.id);
    let next;
    if (holds) {
      const healed = healMessages(messages, job);
      if (!healed.changed) {
        return { attached: true, reason: 'raced_already_current', chat_id: chatId };
      }
      next = healed.next;
    } else {
      // The row is here and the render is not. Append rather than replace —
      // whatever else the chat holds is the user's and is not ours to drop.
      next = messages.concat([buildRenderMessage(job)]);
    }
    ...
    if (updErr) return { attached: false, reason: `raced_append_failed:${updErr.message}`, chat_id: chatId };
    if (Array.isArray(won) && won.length > 0) {
      return {
        attached: true,
        reason: holds ? 'raced_healed_in_place' : 'raced_appended',
        chat_id: chatId,
      };
    }
    log.log(`[chat-attach] job=${String(job.id).slice(0, 8)} CAS miss on collision `
      + `attempt ${attempt} — the chat changed under the read; re-reading`);
  }
  // Declining is the safe end state: the sweep retries within minutes, and
  // reporting attached:false keeps this render in the stranded count instead of
  // silently leaving it out of one.
  return { attached: false, reason: 'raced_cas_exhausted', chat_id: chatId };
}
...
async function attachRenderToChat(supabaseAdmin, job, { log = console } = {}) {
  ...
  if (insErr) {
    ...
    if (insErr.code === '23505' || /duplicate key/i.test(insErr.message || '')) {
      // THE ROW EXISTING IS NOT THE RENDER BEING IN IT. This used to return
      // attached:true here and stop; four of 66 stranded renders were counted
      // fixed on the strength of a row that did not hold them.
      return ensureRenderInChat(supabaseAdmin, chatId, src, log);
    }
