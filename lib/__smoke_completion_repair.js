...
function db({ rows = [{ id: 'j' }], error = null, capture = {} } = {}) {
  // The guard was captured and asserted; the id filter was not, so the repair
  // could name ANOTHER job's row and stay green (verified by mutation,
  // 2026-09-10). Capture the eq filters too, and assert the write is scoped.
  return { from: () => ({ update(p) { capture.payload = p; return this; },
    eq(col, val) { (capture.filters = capture.filters || {})[col] = val; return this; },
...
(async () => {
  ...
  assert.strictEqual(cap.filters && cap.filters.id, 'j',
    'the repair must write to THE job it was asked about — an unscoped or '
    + 'mis-scoped write completes someone else\'s row with this render\'s URL');
