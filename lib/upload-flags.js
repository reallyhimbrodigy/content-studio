...
async function resolve(flag, userId, supabaseAdmin) {
  ...
  // ── "NO ROW" AND "DATABASE DOWN" ARE DIFFERENT, AND resolve() COLLAPSED
  //    THEM. Both fall through to the env path, so a caller reading only
  //    `from: 'env'` cannot tell a flag nobody has configured from a flag
  //    store it could not reach. For a canary that is harmless. For a SPEND
  //    path it is the whole question: a route that costs credits must fail
  //    CLOSED when we cannot read the switch that governs it, and a caller
  //    that cannot see the difference cannot fail closed on it.
  //
  //    _dbRows already knows — it returns null for unreadable and an array
  //    (possibly empty) for readable. It just never said so.
  const dbState = rows === null ? 'unreadable' : 'ok';
  const row = rows && rows.find((r) => r.flag === flag);
  if (row) {
    if (row.enabled_all) return { on: true, source: 'all', from: 'db', dbState };
    if (Array.isArray(row.allowlist) && row.allowlist.includes(userId)) {
      return { on: true, source: 'allowlist', from: 'db', dbState };
    }
    const p = Number(row.percent || 0);
    if (p > 0 && _pct(flag, userId) < p) return { on: true, source: 'percent', from: 'db', dbState };
    return { on: false, source: 'off', from: 'db', dbState };
  }
  ...
  if (all === 'true' || all === '1') return { on: true, source: 'all', from: 'env', dbState };
  const allow = _envList(spec.envAllow);
  const list = allow.length ? allow : spec.defaultAllowlist;
  if (list.includes(userId)) return { on: true, source: 'allowlist', from: 'env', dbState };
  const p = Number(process.env[spec.envPct] || 0);
  if (p > 0 && _pct(flag, userId) < p) return { on: true, source: 'percent', from: 'env', dbState };
  return { on: false, source: 'off', from: 'env', dbState };
}
