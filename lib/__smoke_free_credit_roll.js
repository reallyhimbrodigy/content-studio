...
ok(/const creditsAreTheLimiter = CREDITS_DEBIT_ENABLED && _debitApplies && _credits\.isConfigured\(\) && _credits\.shouldDebit\(\{ mode: 'full' \}\)/.test(flat),
  'the debit is not gated on the build floor — arming it would charge builds '
  + 'that cannot receive credits, 402ing every user below the floor');

// ── EXACTLY ONE LIMITER PER REQUEST ────────────────────────────────────────
...
// naming the wrong limit and pointing at an upgrade they may not need. NEITHER
// live and the render is uncapped. One predicate governs both, which is what
// makes those two states unreachable.
