...
//
// IT MUST NOT WIDEN TO pro_until. `comp_pro` is the only entitlement column with
// a database guard (trg_guard_comp_pro: service role only). `tier`, `pro_until`
// and `rc_app_user_id` have UPDATE granted to `authenticated` at table level and
// an RLS policy of `auth.uid() = id` with NO column restriction — a signed-in
// user can write their own pro_until. A bypass keyed on that is self-serve free
// renders, and it is the intuitive way to phrase this, which is exactly why it
