...
// IDENTIFIED BY ARGUMENTS, NOT BY A NEARBY COMMENT. These two read
//   /LAZY ROLL \(write side\)[\s\S]{0,400}?await ensureFreePeriodGrant/
//   /LAZY ROLL \(read side\)[\s\S]{0,200}?await ensureFreePeriodGrant/
// — a byte window anchored on PROSE. On 2026-09-26 the read-side one broke when a
// comment was added above that call explaining why mayCreateCustomer is
// deliberately omitted there. The behaviour was untouched; the documentation
// moved the code out of the window.
//
// That was the fifth and sixth window of the session to fire on correct code, and
// these two were the worst of them: anchoring a BEHAVIOUR check on COMMENT TEXT
// means writing down why the code is right is what makes the check say it is
// wrong. Widening 200 to 600 buys one paragraph and re-arms the trap.
//
// A call site is identified by what it PASSES, which is behaviour: the debit site
// is the one that resolves isPaid from the entitlement it just read, and the
// balance read is the one that derives it from the tier it resolved.
ok(/await ensureFreePeriodGrant\(authUser\.id,\s*\{[^}]*isPaid: entitlement\.isPro/.test(SRC),
  'the debit-site roll is missing — a user whose month just turned over would '
  + 'be 402d holding an allowance they are owed');
ok(/await ensureFreePeriodGrant\(authUser\.id,\s*\{[^}]*isPaid: tier !== 'free'/.test(SRC),
  'the balance-read roll is missing — the badge would show a stale number');
