...
/**
 * Is this Supabase user id on the admin user-id allowlist?
 *
 * Same shape as isSubmissionAdmin but matches opaque user UUIDs instead of
 * emails — lets the owner be authorized by stable id (which never changes)
 * rather than whatever email their account happens to use.
 *
 * @param {string} userId     Caller's Supabase user id.
 * @param {string} allowlist  Raw env string (SUBMISSION_ADMIN_USER_IDS),
 *                             comma-separated, case-insensitive + trimmed.
 * @returns {boolean} false if either input is empty/missing.
 */
function isSubmissionAdminUserId(userId, allowlist) {
  if (!userId || !allowlist) return false;
  const normalizedId = String(userId).toLowerCase().trim();
  if (!normalizedId) return false;
  const adminIds = String(allowlist)
    .split(',')
    .map((id) => id.toLowerCase().trim())
    .filter(Boolean);
  return adminIds.includes(normalizedId);
}
