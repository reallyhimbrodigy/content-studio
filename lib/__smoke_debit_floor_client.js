...
// ── 4b. THE CLAMP IS A FREE-USER RULE AND ONLY A FREE-USER RULE ──────────
// CLIENT_CLAIM_MIN_BUILD exists because a FREE balance comes from a device
// claim the client has to make. A paid balance comes from RevenueCat and has
// nothing to do with the binary, so clamping paid users made them render free:
// 106 paid first edits across 12 users on builds 245-246 since metering armed,
// against 70/70 charged on 247+.
{
  const F = require('./free-credits');
  // the leak, closed
  assert.strictEqual(F.debitApplies({ build: 246, minBuild: 245, isPaid: true }), true,
    'a PAID user on 246 must be charged — their balance does not depend on the client claim');
  // the protection, kept
  assert.strictEqual(F.debitApplies({ build: 246, minBuild: 245, isPaid: false }), false,
    'a FREE user on 246 must NOT be charged — 246 has the server half and no caller');
  // the env floor still governs both
  assert.strictEqual(F.debitApplies({ build: 244, minBuild: 245, isPaid: true }), false,
    'the env floor applies to paid users too — 244 is below it');
  // BOTH FAIL-OPEN CASES SURVIVE THE SPLIT. Refusing a paying customer over a
  // missing version header is not recoverable; leaking one render is.
  assert.strictEqual(F.debitApplies({ build: null, minBuild: 245, isPaid: true }), false,
    'an unreadable build must fail open for paid users as well');
  assert.strictEqual(F.debitApplies({ build: 999, minBuild: NaN, isPaid: true }), false,
    'an unset floor still ships dark for paid users');
  // and the floors themselves differ by exactly the clamp
  assert.strictEqual(F.effectiveDebitFloor(245, { isPaid: true }), 245);
  assert.strictEqual(F.effectiveDebitFloor(245, { isPaid: false }), F.CLIENT_CLAIM_MIN_BUILD);
}
...
// variable, and a clamp nobody can see is a clamp nobody maintains.
{
  ...
  // THE PROPERTY IS THAT IT READS THE EFFECTIVE FLOOR, NOT THE ARITY. Pinning
  // the exact argument list makes every additive change look like a
  // regression: adding the {isPaid} option failed a check about a value that
  // is still being read the same way.
  assert.ok(/effectiveDebitFloor\(_debitFloor\b/.test(srv),
