...
(async () => {
  ...
  // ── EDITOR_TAB_OPEN IS KNOWN, TERMINAL, AND NOT A REFUSAL ──────────────
  //
  // Three separate properties, and each one wrong costs something different:
  // unknown -> log noise on a demo day; designed -> the customer is charged for
  // OUR open tab; absent -> a retry that cannot possibly succeed.
  //
  // Driven through the real collectAgenticResult, not asserted against the
  // exported sets — the sets are the implementation and the classification is the
  // behaviour.
  _obj = JSON.stringify({ state: 'FAILED', error_code: 'EDITOR_TAB_OPEN',
    error: 'an editor tab is open on this project; retrying without closing it '
      + 'will fail in exactly the same way' });
  {
    const r = await C.collectAgenticResult('j', { log: quiet });
    assert.strictEqual(r.state, 'FAILED');
    assert.strictEqual(r.code, 'EDITOR_TAB_OPEN', 'the code must survive verbatim');
    // NOT a designed refusal. Somebody left a tab open: that is OUR failure, not
    // the product refusing the customer's request. Marking it designed would tell
    // the refund leg the charge was legitimate and the customer would pay for it.
    assert.strictEqual(r.designedRefusal, false,
      'EDITOR_TAB_OPEN must NOT be a designed refusal — the customer must be '
      + 'refunded for our open tab, and this flag is what decides that');
    assert.ok(C.KNOWN_CODES.has('EDITOR_TAB_OPEN'),
      'it must be allowlisted, or every occurrence logs as an unknown code');
    console.log('  ok  EDITOR_TAB_OPEN is known, kept verbatim, and refundable');
  }
