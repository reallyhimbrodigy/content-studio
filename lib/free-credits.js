 ...
 *
 * ONE ENV VAR GOVERNS BOTH SIDES on purpose: the same FREE_CREDITS_MIN_BUILD
 * decides who can receive credits and who can be charged them, so the two can
 * never drift into a window where a build is charged but ungrantable.
 *
...
function debitApplies({ build, minBuild }) {
  if (!Number.isInteger(minBuild)) return false;
