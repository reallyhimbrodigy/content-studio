...
const {
  readGateReceipt, writeGateReceipt, clearGateReceipt, watermarkArmingWarning, readBuildMarker,
...
// ── BUILD MARKER: the discriminator for a null receipt. It only earns its keep
// if the postinstall hook is actually wired and the reader is strict.
const mk = fs.mkdtempSync(path.join(os.tmpdir(), 'build-marker-'));
assert.strictEqual(readBuildMarker(mk), null, 'absent marker must read null');
fs.writeFileSync(path.join(mk, '.build_marker.json'),
  JSON.stringify({ npm_install_ran: true, at: '2026-08-11T00:00:00Z', node: 'v22.0.0' }));
assert.deepStrictEqual(readBuildMarker(mk), { at: '2026-08-11T00:00:00Z', node: 'v22.0.0' });
fs.writeFileSync(path.join(mk, '.build_marker.json'), '{not json');
assert.strictEqual(readBuildMarker(mk), null, 'malformed marker must read null, never throw');
fs.writeFileSync(path.join(mk, '.build_marker.json'), JSON.stringify({ npm_install_ran: false }));
assert.strictEqual(readBuildMarker(mk), null, 'npm_install_ran:false must read null — a marker that lies present is worse than none');

// the hook must exist AND be wired into npm's lifecycle, or the marker is never written
const pkg = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'package.json'), 'utf8'));
assert.ok(/build-marker/.test(String(pkg.scripts && pkg.scripts.postinstall || '')),
  'package.json postinstall must run scripts/build-marker.js — an unwired hook writes nothing');
assert.ok(fs.existsSync(path.join(__dirname, '..', 'scripts', 'build-marker.js')),
  'scripts/build-marker.js must exist');
assert.ok(/build:\s*BOOT_BUILD_MARKER/.test(fs.readFileSync(path.join(__dirname, '..', 'server.js'), 'utf8')),
  'server.js must expose the marker on /api/health as `build`');
...
console.log('gate-receipt smoke: PASS (absent-null, valid, alt-keys, malformed-null, shape-null, '
  ...
  + 'build-marker strict+wired, arming alarm x4)');
process.exit(0);
