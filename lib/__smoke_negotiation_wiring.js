...
// 2026-09-26: the classifier's await now follows the flag — ARMED awaits (the live
// refusal must gate), DARK starts it and records on resolve, because an awaited
// Haiku round trip was ~95% of a 1.6-3.5s "insert" phase on every job. THE
// PROPERTY HERE is which ENTRY POINT the server uses: the adjudicated one, never
// the regex-only classify(). That is unaffected by where the await sits, so the
// check names the call and stops naming the await. __smoke_negotiate_off_critical_
// path.js owns the await placement, including the ARMED half, which is the safety
// floor and is asserted there rather than weakened here.
check('the server calls classifyWithSafety, not the bare regex classify',
  /negotiation\.classifyWithSafety\(/.test(src)
  && !/=\s*negotiation\.classify\(/.test(src),
  'the sync classify() is regex-only and is blind to 15.2% of briefs by construction');
// The recorder was extracted to one copy and takes the decision as `decision`, so
// the binding name moved. What must hold is that the shadow row carries the safety
// STATE and its AUTHOR — a refusal whose author is unrecorded cannot be audited.
// Accepts either binding so the check is about the recorded fields, not the
// variable that happened to hold them.
check('the recorded decision carries the safety verdict and WHO made it',
  /safety: (negotiationDecision|decision)\.safety/.test(src)
  && /by: (negotiationDecision|decision)\.safety\.by/.test(src),
...
check('the outage alert fires on a degraded read',
  /(negotiationDecision|decision)\.alert[\s\S]{0,300}sendOwnerAlert\(/.test(src),
