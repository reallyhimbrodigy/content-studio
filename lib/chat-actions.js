...
function buildToolSystemPrompt(ctx = {}) {
  ...
  return [
    ...
    // reply_language — same helper /api/chat uses, so the assistant does not
    // answer the same user in two languages depending on the surface. Empty
    // for English, so today's prompt is unchanged for most traffic.
    require('./reply-language').replyLanguageInstruction(ctx.replyLanguage),
  ].join('\n');
}
...
async function decideChatAction(message, ctx = {}) {
  ...
  const payload = {
    ...
    // TEMPERATURE 0, BECAUSE THIS IS A VERDICT AND NOT A SENTENCE.
    // This call decides between converse / status / act_render / act_reedit —
    // the code BRANCHES on the result. At 0.2 the same message from the same
    // user could take a different branch on two runs, and that failure is
    // invisible: the user gets a plausible reply either way, so there is no red
    // leg to re-run, only a support message saying "it ignored me".
    // (Found on Builder 2's classifiers, which ran at the 1.0 default and
    // returned unsafe true,true,false,false,true across five runs on one brief.
    // Every rate quoted off such a path is one sample of a distribution.)
    // composeConfirmation below deliberately stays at 0.6 — it writes the
    // user-facing sentence, nothing branches on it, and variety is wanted there.
    generationConfig: { maxOutputTokens: DECIDE_MAX_TOKENS, temperature: 0 },
  };
