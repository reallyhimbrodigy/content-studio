...
function decideRoute(status, { env = process.env, reserve = null,
  ...
  // B1's session detector returns LIVE / DEAD / UNREACHABLE (2026-09-25). LIVE
  // joins OK as a routing state; the other two map onto the reasons that
  // already exist, because "the session is dead" is a fact about the account
  // and "I could not ask" is an outage on our side, and only one of those is
  // worth paging on.
  //
  // STILL AN ALLOWLIST. A state nobody thought to list — a typo, or a fourth
  // name added later — routes AWAY rather than falling through to the path
  // that spends. B1 cannot break this by adding a state; he can only make it
  // conservative.
