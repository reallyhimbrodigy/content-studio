...
// A REAL job id is a uuid, and that is the case worth asserting: it must
// collapse to the :id ROUTE, not onto the list endpoint. This leg used to check
// 'abc-123' and expect '/api/video-jobs' — true, still true, and it tested the
// wrong half: the status poll and the SSE stream were both being booked against
// the list endpoint, which is how 118 stream 429s were reported against a route
// that has no rate limit and cannot 429.
assert.strictEqual(L.normalizeRoute('/api/video-jobs/eade2320-324d-4059-b480-ef35cccbeba8'),
  '/api/video-jobs/:id',
  'a uuid path collapses onto the :id ROUTE — one job must not make its own '
  + 'bucket, AND the status poll must not be filed as the list endpoint');
assert.strictEqual(L.normalizeRoute('/api/video-jobs/eade2320-324d-4059-b480-ef35cccbeba8/stream'),
  '/api/video-jobs/:id/stream',
  'the SSE stream is its own door with its own rate limit (12/60s on jobId); '
  + 'filing its failures under the list endpoint is what hid them');
assert.strictEqual(L.normalizeRoute('/api/video-jobs/abc-123'), '/api/video-jobs',
  'a NON-uuid segment is left alone — collapsing anything id-shaped ate four '
  + 'real route words (upload-multipart-abort/-complete/-init, '
  + 'internal/lifecycle-push-proof) on this rule\'s first writing');
...
// ── EVERY MOUNTED ROUTE MUST RESOLVE TO ITSELF ──────────────────────────────
//
// A route missing from KNOWN_PREFIXES is not uncounted — it is MISCOUNTED.
// normalizeRoute takes the longest matching prefix, so an unlisted sub-route's
// outcomes are booked against its parent and an unlisted top-level route
// vanishes into /__unrouted.
//
// `/api/chat/actions` was missing, and it 404s BY DESIGN while its feature flag
// is dark. 2,039 of those deliberate 404s across 1,620 users landed on
// `/api/chat`, and the primary chat endpoint read as broken for a quarter of
// the user base for six days. It was never broken. The ledger was the only
// thing saying it was.
{
  const fs = require('fs');
  const path = require('path');
  const server = fs.readFileSync(path.join(__dirname, '..', 'server.js'), 'utf8');
  // ── THE POPULATION EXCLUDED THE ROUTES THAT WERE MISFILED ────────────
  //
  // This scanned only `parsed.pathname === '/api/...'` — the LITERAL mounts.
  // The per-job sub-routes are mounted by REGEX on a path with a uuid in the
  // middle (/api/video-jobs/<uuid>/stream), so they were never in the set, and
  // the leg that exists to catch misfiled routes could not see the four that
  // were misfiled. It passed, correctly and uselessly, for as long as they
  // existed.
  //
  // THE COST, MEASURED 2026-09-25: the ledger reported "worst GET
  // /api/video-jobs 429 n=118/1u" for four minutes. The list endpoint has no
  // rate limit and cannot 429 — every one was the SSE stream hitting its own
  // guard, booked against the parent. A standing red on the wrong door.
  //
  // Regex mounts are extracted as their :id form so they can be compared with
  // KNOWN_PREFIXES on equal terms.
  const literal = [...server.matchAll(/parsed\.pathname === '(\/api\/[^']+)'/g)]
    .map((m) => m[1]);
  // ANY capture group is an id, not just ([^/]+). /api/jobs/:id/export is
  // mounted as ([0-9a-f-]{8,64}), and a rule written for one spelling found
  // five of six routes and missed that one — which is the same shape as the
  // defect this leg is being widened to catch.
  const byRegex = [];
  for (const m of server.matchAll(/\.match\(\/\^(.+?)\$\//g)) {
    if (!m[1].startsWith('\\/api')) continue;
    byRegex.push(m[1].replace(/\([^)]*\)/g, ':id').replace(/\\\//g, '/'));
  }
  const mounted = [...new Set([...literal, ...byRegex])].sort();

  assert.ok(mounted.length > 20,
    `positive control: only ${mounted.length} mounted /api routes found — the `
    + 'extraction broke and this check would pass vacuously');
  // A SECOND POSITIVE CONTROL, for the half that was blind. The literal scan
  // stayed green through the whole defect, so "the extraction works" has to be
  // asserted about the REGEX half specifically or the widening is decorative.
  assert.ok(byRegex.length >= 4,
    `positive control: only ${byRegex.length} regex-mounted /api routes found — `
    + 'the sub-route extraction is what this leg was widened for, and an empty '
    + 'one restores exactly the blind spot it was written to close');

  const wrong = mounted
    .map((p) => [p, L.normalizeRoute(p)])
    .filter(([p, got]) => got !== p);

  assert.deepStrictEqual(wrong, [],
    'these mounted routes do not resolve to themselves, so their outcomes are '
    + 'booked against another route:\n'
    + wrong.map(([p, got]) => `    ${p}  ->  ${got}`).join('\n')
    + '\n  Add each to KNOWN_PREFIXES. A 404 that is correct for ONE route reads '
    + 'as an outage on ANOTHER.');

  // And the reverse: an entry that matches no mounted route is stale, and a
  // stale prefix can shadow a real one added later.
  const known = [...new Set(
    [...fs.readFileSync(path.join(__dirname, 'api-outcome-ledger.js'), 'utf8')
      .match(/const KNOWN_PREFIXES = \[[\s\S]*?\];/)[0]
      .matchAll(/'(\/api\/[^']+)'/g)].map((m) => m[1]),
  )];
  const orphans = known.filter((k) => !mounted.some((p) => p === k || p.startsWith(`${k}/`)));
  assert.deepStrictEqual(orphans, [],
    `KNOWN_PREFIXES entries matching no mounted route: ${orphans.join(', ')} — `
    + 'a stale prefix shadows a real route added under it later');
}
