 ...
 */
/**
 * Has the CLIENT already told us this upload died?
 *
 * DEAD-UPLOAD FAST-FAIL (2026-08-04). 76% of orphaned jobs never completed
 * their upload (37 of 49, vs 6% of successful jobs — a 12x enrichment, control
 * verified). Those users wait the full 600s source budget and only then learn
 * the upload never arrived. The client already KNOWS: it emits upload_failed
 * with a mechanism (background_orphan/cancelled, timeout, http_403) the moment
 * its URLSession dies on suspension.
 *
 * So stop waiting for bytes the client has told us are not coming. This helps
 * the largest live class WITHOUT an app release — the 225 background-URLSession
 * fix is the real cure; this is the honest wait in the meantime.
 *
 * FAIL-OPEN AND NEVER FAIL-CLOSED: a client may report a failure for one
 * attempt and then succeed on a retry, so this is consulted ONLY while the
 * object is still absent, and any error answers "no signal" rather than
 * aborting a legitimate slow upload. Returns null when it cannot tell.
 */
async function clientReportedUploadFailure(jobId, sinceIso, { db = supabaseAdmin } = {}) {
  if (!jobId || !sinceIso || !db) return null;
  try {
    const { data, error } = await db
      .from('analytics_events')
      .select('created_at, props')
      .eq('event', 'upload_failed')
    ...
    const hit = data.find((e) => {
      const p = (e && e.props) || {};
      return String(p.job_id || p.jobId || '') === String(jobId);
    });
    if (!hit) return null;
    const p = hit.props || {};
    return {
      mechanism: String(p.mechanism || p.path || 'unknown').slice(0, 40),
      at: hit.created_at,
    };
  } catch (_) {
    return null;                    // unmeasurable -> keep waiting, never abort
  }
}

async function waitForSource(key, {
  budgetMs = SOURCE_WAIT_MS, log = () => {}, jobId = null, since = null,
} = {}) {
  ...
  for (;;) {
    ...
    // The object is ABSENT. Only now is a client failure report meaningful —
    // checking it earlier could abort an upload that was merely slow.
    if (jobId && since && (attempts === 2 || attempts % 10 === 0)) {
      const dead = await clientReportedUploadFailure(jobId, since);
      if (dead) {
        log(`client reported upload_failed (${dead.mechanism}) — not waiting out the `
          + `${Math.round(budgetMs / 1000)}s budget`);
        return {
          present: false, attempts, reason: 'client_reported_failure',
          mechanism: dead.mechanism,
          waitedMs: budgetMs - (deadline - Date.now()),
        };
      }
    }
