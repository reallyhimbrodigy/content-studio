...
function record(rec) {
  try {
    const r = {
      ...
      // ── DID THE DECISION TOUCH THE NETWORK, AND HOW OLD WAS THE NUMBER ────
      //
      // From 2026-09-26 the guard reads a cached account_status and never calls
      // out on the job path. Without these two fields a working cache and a fast
      // endpoint are indistinguishable in the log, and the day the refresher dies
      // the symptom is "routing got slow and then went to handler" with nothing
      // saying the cache was the cause.
      //
      // `=== null` means the guard was never asked (the ramp said no first) —
      // which is different from false, and the log must not merge them.
      statusCached: (rec && typeof rec.statusCached === 'boolean')
        ? rec.statusCached : null,
      statusAgeMs: (rec && typeof rec.statusAgeMs === 'number')
        ? rec.statusAgeMs : null,
    };
    _ring.push(r);
    ...
    console.log(`[chatcut-route] job=${r.job} user=${r.user} pipeline=${r.pipeline} `
      ...
      + `balance_path=${r.balancePath} cached=${r.statusCached} `
      + `status_age_ms=${r.statusAgeMs}`);
    return r;
  } catch (_) { return null; }
}
