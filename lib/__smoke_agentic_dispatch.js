...
function mockDb(rows) {
  ...
  return { calls, store, from() {
    const f = {}; const nots = []; const gtes = []; const ins = []; let upd = null;
    const b = {
      select: (...a) => {
        if (!upd) return b;
      ...
      // ── .in IS THE SWEEP'S STATUS FILTER NOW, AND IT MUST FILTER ────────
      //
      // The sweep moved from .not('status','in',...) to .in('status',[...]) so
      // the query can use idx_video_jobs_agentic_inflight (39 shared buffers ->
      // 2). This stub had no `.in`, so the first run failed with
      // "...eq(...).in is not a function" — which is the stub doing its job:
      // it drives the REAL chain, so a method the code starts calling has to
      // exist here or the smoke cannot run at all.
      //
      // IT APPLIES THE FILTER RATHER THAN RECORDING IT. A no-op `.in` would
      // hand back rows the real query excludes — the same defect
      // __smoke_mock_filters_gate caught on this file's `gte`, where a fixture
      // that agrees with the code proves nothing about either.
      in: (col, vals) => { ins.push({ col, vals: [].concat(vals) }); return b; },
      ...
      limit: () => Promise.resolve({
        data: store.filter((r) => Object.entries(f).every(([k, v]) => r[k] === v)
          ...
          && ins.every((n) => n.vals.map(String).includes(String(r[n.col])))
...
(async () => {
  ...
  const live = () => mockDb([
    ...
    //
    // IT CARRIES updated_at NOW BECAUSE THE SWEEP BOUNDS ON updated_at. When the
    // bound moved off created_at these rows had no updated_at at all, the mock's
    // gte treats an absent column as PASSING, and the decoy silently stopped
    // being a decoy — scanned went 1 -> 2 and this smoke caught it. Note the
    // mock and Postgres disagree on absence (Postgres would EXCLUDE a NULL), so
    // the fixture carries the column rather than relying on either behaviour.
    // Verified on production: 0 of 12,671 rows have a null updated_at, and none
    // has updated_at < created_at, so the window is a strict superset.
  ...
  // ── THE CALLER SECRET RIDES ON /run_agentic (2026-09-26) ─────────────────
  //
  // IT DID NOT, AND IT COST THE FIRST REAL ROUTED JOB. 5af26805 routed correctly at
  // 07:43:57Z — route=chatcut, balance_path=account.balance — and died two seconds
  // later on
  //   run_agentic 401 {"spawned":false,"state":"REFUSED","auth":"REQUIRED",
  //     "why":"this container carries a caller secret and the request sent none"}
  // "sent none". This call and /account_status go to the SAME container; the guard
  // resolved the secret and sent it, this one sent Content-Type and nothing else.
  //
  // INVISIBLE BY CONSTRUCTION UNTIL THE GUARD WAS FIXED: the guard runs first and
  // fails closed, so while it was broken every job fell back before reaching this
  // line and the missing header could not produce a symptom. That is the shape to
  // watch for — a second defect hidden behind a first on the same path, where
  // fixing the first is what makes the second reachable.
  {
    const seen = [];
    const fakeFetch = async (url, opts) => {
      seen.push({ url, headers: (opts && opts.headers) || {} });
      return { ok: true, status: 200, text: async () => '{"spawned":true}',
               json: async () => ({ spawned: true }) };
    };
    await D.dispatchAgentic({
      baseUrl: 'https://x.test', jobId: 'j-secret', sourceKey: 'k',
      srcUrl: 'https://s', outUrl: 'https://o', outKey: 'ok', brief: 'b',
      fetchImpl: fakeFetch, log: quiet,
      env: { MODAL_CALLBACK_SECRET: 'a-test-value' },
    }).catch(() => {});
    const sent = seen.find((d) => /run_agentic$/.test(d.url));
    assert.ok(sent, 'no POST reached /run_agentic at all');
    const hdr = Object.keys(sent.headers).find((h) => h.toLowerCase() === 'x-promptly-secret');
    assert.ok(hdr,
      'the /run_agentic POST carries NO x-promptly-secret header — B1\'s gate '
      + 'answers 401 "the request sent none" and every routed job falls back');
    assert.strictEqual(sent.headers['Content-Type'], 'application/json',
      'the content type must survive the header merge');
    // The VALUE is never asserted against a real secret — only that the header is
    // present and carries what was injected. A check that pins a live secret's
    // value writes it into the repo.
    assert.strictEqual(sent.headers[hdr], 'a-test-value');
    console.log('  ok  /run_agentic carries the caller secret alongside Content-Type');
  }

  // ── ONE RESOLVER, NOT TWO COPIES ────────────────────────────────────────
  {
    const sec = require('./chatcut-secret');
    // ABSENT, NOT EMPTY, when nothing is configured. An empty header exists and
    // matches nothing: B1's gate says "sent none" for an absent header and reports
    // a MISMATCH for an empty one, and those point at different people.
    assert.deepStrictEqual(sec.authHeaders({}), {},
      'with no secret configured the header must be ABSENT, not empty — an empty '
      + 'value reads as a wrong secret rather than an unconfigured server');
    assert.deepStrictEqual(sec.authHeaders({ MODAL_CALLBACK_SECRET: '   ' }), {},
      'whitespace is not a secret');
    assert.strictEqual(sec.authHeaders({ MODAL_CALLBACK_SECRET: 'a' })['x-promptly-secret'],
      'a', 'the fallback must ride');
    assert.strictEqual(sec.authHeaders({ MODAL_CALLBACK_SECRET: 'a',
      CHATCUT_ACCOUNT_STATUS_SECRET: 'b' })['x-promptly-secret'], 'b',
      'the dedicated name must WIN when set, so it can be rotated independently');

    // AND BOTH CALLERS MUST USE IT. Two copies of the precedence is how one gets
    // updated and the other does not — exactly what produced the 401.
    const _fs = require('fs'); const _p = require('path');
    const { stripComments: _strip } = require('./__gate_strip');
    for (const f of ['chatcut-routing.js', 'agentic-dispatch.js']) {
      // COMMENTS STRIPPED, and this leg is why. It read the raw file for the string
      // `chatcut-secret`, and removing the require from chatcut-routing.js left it
      // GREEN — because the comment above the require says "THE RESOLUTION MOVED TO
      // lib/chatcut-secret.js". My own prose satisfied the check. RED-proved NOT
      // RED, which is the only reason I know.
      //
      // That is the _match_is_prose class, in a check written an hour after fixing
      // the same class twice. Through the shared stripper, which a gate requires
      // precisely because ad-hoc ones cannot tell a string from code.
      const code = _strip(_fs.readFileSync(_p.join(__dirname, f), 'utf8'));
      assert.ok(/require\(['"]\.\/chatcut-secret['"]\)/.test(code),
        `${f} does not REQUIRE the shared caller-secret resolver — a second copy of `
        + 'the env precedence is how /account_status and /run_agentic came to '
        + 'disagree, and a mention in a comment is not a usage');
    }
    console.log('  ok  one caller-secret resolver, used by the guard AND the dispatch');
  }
