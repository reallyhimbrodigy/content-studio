...
/**
 * Count a retry OUTCOME. Absorbed or exhausted — both are the answer to "did the
 * fix work", and neither is answerable from the failure rows alone.
 *
 * WHY THIS EXISTS. I shipped the transient-429 retry logging `[chat-retry]
 * ABSORBED` to the console and persisting NOTHING. Minutes later the 502s
 * continued and I could not tell whether the retry had run and lost, or never
 * fired at all — because an absorbed retry writes no failure row and a console
 * line is not a queryable artefact. That is the exact defect class the whole
 * night was spent closing, reproduced inside the fix for it.
 *
 * ABSORPTION IS THE DENOMINATOR THE FIX IS JUDGED ON. Without it, "429s are still
 * happening" is compatible with the retry working perfectly (absorbing most and
 * logging only the losses) AND with it never running.
 */
async function recordRetryOutcome(supabaseAdmin, {
  route, model, absorbed, waitMs, userId = null, log = console,
}) {
  log.log(`[chat-retry] ${route} ${absorbed ? 'ABSORBED' : 'EXHAUSTED'} `
    + `model=${model} wait=${waitMs}ms`);
  try {
    if (!supabaseAdmin) return;
    await supabaseAdmin.from('analytics_events').insert({
      event: 'chat_retry_outcome',
      user_id: userId,
      props: { route, model, absorbed: !!absorbed, wait_ms: waitMs },
    });
  } catch (e) {
    log.error(`[chat-retry] outcome persist failed (${e && e.message})`);
  }
}

module.exports = { parseQuotaFailure, recordQuotaFailure, recordRetryOutcome };
