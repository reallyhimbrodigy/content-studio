...
function rcReasons(event) {
  return {
    ...
    // IDENTITY CHAIN (2026-08-28). 32% of purchase webhooks arrive under an
    // anonymous `$RCAnonymousID:…`, because the first-launch paywall sells
    // BEFORE sign-in — 99% of that surface's wall views are pre-auth. Those
    // rows cannot join to profiles.id, so revenue-by-surface would silently
    // under-report the one surface the conversion work is built around.
    //
    // RevenueCat already tells us how to resolve them and we were throwing it
    // away: on a merge, `original_app_user_id` carries the ANONYMOUS id while
    // `app_user_id` carries the identified one, and any further ids arrive in
    // `aliases`. Storing them turns an unjoinable row into a joinable one with
    // no API call and no key. (The v2 REST path is NOT a substitute — it is
    // reported to omit original_app_user_id and to return incomplete aliases.
    // The webhook payload is the sound source.)
    original_app_user_id: event?.original_app_user_id || null,
    aliases: Array.isArray(event?.aliases) ? event.aliases.slice(0, 12) : null,
  };
}

/**
 * Every id this event proves belongs to ONE customer. Feed the alias map that
 * resolves an anonymous purchase to the auth UUID it later became.
 */
function identityChain(event) {
...
module.exports = { rcReasons, churnCause, isRecoverable, identityChain };
