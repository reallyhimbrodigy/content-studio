...
let row = null;
// THE ROW MUST BE EARNED BY THE FILTER (2026-09-10). This mock returned `row`
// from maybeSingle() regardless of the query, so the durable poller could read
// ANY job's row — verified by mutation: pointing `.eq('id', jobId)` at
// 'some-other-job' left "poller settle/skip" green, meaning a job could be
// settled from another job's outcome and this file would not notice.
// `rowFor` scopes the fixture to one job id; a read scoped elsewhere gets null.
let rowFor = null;
      ...
      from() {
        const b = {
          ...
          maybeSingle: () => Promise.resolve({
            data: (rowFor === null || b._f.id === rowFor) ? row : null, error: null,
          }),
...
(async () => {
  ...
  rowFor = 'job-4'; row = { status: 'needs_input', result: {}, rendered_video_url: null, result_url: null };
  let settled4 = false;
  ...
  rowFor = 'job-6'; row = { status: 'failed', result: {}, rendered_video_url: null, result_url: null, error_message: null };
  let settled6 = false;
