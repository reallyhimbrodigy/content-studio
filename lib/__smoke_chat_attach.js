...
const {
  ...
  attachRenderToChat, sweepChatAttach, deliveryFields, jobIdContainsFilter,
} = require('./chat-attach');

// ── THE ASSERTION A MOCK CANNOT MAKE ────────────────────────────────────────
// The first deploy of chat-attach passed every mock-based assertion below and
// attached NOTHING in production: .contains('messages', [{jobId}]) makes
// supabase-js emit a POSTGRES ARRAY LITERAL whose objects stringify to the text
// `[object Object]`, so every lookup threw `invalid input syntax for type json`,
// was caught as lookup_failed, and the sweep ran clean and empty.
//
// A mock cannot catch that — it implements .contains() the way its author
// imagined the client works, which is precisely the assumption that was wrong.
// So this drives the REAL client and asserts the URL it builds. No network: the
// query builder composes its URL before anything is awaited.
function assertRealClientSerialization() {
  const { createClient } = require('@supabase/supabase-js');
  const db = createClient('https://example.supabase.co', 'test-key', { auth: { persistSession: false } });
  const q = db.from('chats').select('id').contains('messages', jobIdContainsFilter('JOB-1'));
  const search = decodeURIComponent(q.url.search);
  assert.ok(search.includes('messages=cs.[{"jobId":"JOB-1"}]'),
    `the containment filter must serialize as JSON, got: ${search}`);
  assert.ok(!/\[object Object\]/.test(search),
    'THE PRODUCTION BUG: an array operand stringifies to [object Object] inside a '
    + 'Postgres array literal and every lookup throws invalid-input-syntax');
}
  ...
  return {
    ...
    from(table) {
      ...
      const b = {
        ...
        limit: () => {
          calls.reads += 1;
          // THE OPERAND IS A JSON STRING, as `jobIdContainsFilter` builds it
          // and as PostgREST receives it (`messages=cs.[{"jobId":"…"}]`). This
          // read `st.contains[0].jobId` — indexing a CHARACTER of that string —
          // so `want` was always undefined and THE CONTAINMENT FILTER NEVER
          // FILTERED: every read matched on user_id alone. A chat that had lost
          // the render was handed back as one that held it, which is the exact
          // production shape this module exists to fix. Parse it, as Postgres does.
          const operand = st.contains === null || st.contains === undefined
            ? null
            : (typeof st.contains === 'string' ? JSON.parse(st.contains) : st.contains);
          const want = Array.isArray(operand) && operand[0] ? operand[0].jobId : null;
          const rows = store.chats.filter((c) => (
            (!st.filters.id || c.id === st.filters.id)
...
(async () => {
  // ── 0. the real client serializes the filter as JSON (the production bug) ─
  assertRealClientSerialization();
  ...
  // ── 8. A DUPLICATE KEY IS NOT, BY ITSELF, SUCCESS (2026-09-10) ───────────
  //
  // This used to assert `raced_already_created` — that a 23505 meant someone
  // else had derived the same id, so the render must be attached. The row
  // existing does not mean the row HOLDS the render: the client PATCHes this
  // column blind, so a chat this code created moments earlier can already have
  // been overwritten. One such chat was found holding ZERO messages, another
  // holding twenty with the render not among them. FOUR of 66 stranded renders
  // were counted fixed on the strength of that assumption, and this assertion
  // is what made it survive review.
  const derivedId = deterministicChatId(JOB.id);

  // 8a. The row exists and does NOT hold the render — the four-of-66 case.
  //     It must APPEND, not declare victory.
  const dbRaced = mockDb({
    chats: [{ id: derivedId, user_id: JOB.user_id, updated_at: 'v1', messages: [] }],
    insertError: { code: '23505', message: 'duplicate key value' },
  });
  const raced = await attachRenderToChat(dbRaced, JOB, { log: { log() {}, error() {} } });
  assert.strictEqual(raced.attached, true, 'a collision onto an empty chat must attach');
  assert.strictEqual(raced.reason, 'raced_appended');
  const appended = dbRaced.store.chats[0].messages.filter((m) => m && m.jobId === JOB.id);
  assert.strictEqual(appended.length, 1, 'the render message is actually in the row now');
  assert.strictEqual(appended[0].renderedVideoUrl, 'https://cdn/v.mp4');
  assert.ok(dbRaced.calls.casGuards.length > 0,
    'the collision append takes the same CAS the heal path takes');

  // 8b. The row exists, holds the render, and is already current → no write.
  const held = chatWithProcessingBubble()[0];
  held.id = derivedId;
  held.messages = healMessages(held.messages, JOB).next;
  assert.strictEqual(healMessages(held.messages, JOB).changed, false,
    'fixture guard: this chat must genuinely be current, or 8b tests nothing');
  const dbCur = mockDb({ chats: [held], insertError: { code: '23505', message: 'duplicate key' } });
  const curRes = await attachRenderToChat(dbCur, JOB, { log: { log() {}, error() {} } });
  assert.strictEqual(curRes.attached, true);
  assert.strictEqual(curRes.reason, 'already_current',
    'a chat that already holds a current render is found by the normal path');
  assert.strictEqual(dbCur.calls.updates.length, 0, 'an already-correct chat performs ZERO writes');

  // 8c. The row collided and then genuinely vanished — that is NOT attached,
  //     and it must say so rather than inflate a recovery count.
  const dbGone = mockDb({ chats: [], insertError: { code: '23505', message: 'duplicate key' } });
  const gone = await attachRenderToChat(dbGone, JOB, { log: { log() {}, error() {} } });
  assert.strictEqual(gone.attached, false, 'a collision with no row is not a success');
  assert.strictEqual(gone.reason, 'raced_then_vanished');

  // 8d. The collision append under REAL contention. The client's own PATCH
  //     lands between our read and our write. The append must lose that CAS,
  //     re-read, and re-merge onto what is actually there — so the render
  //     arrives AND the client's message survives. Appending the stale array
  //     would attach the render by erasing a user's typing, which is the
  //     lost-update defect wearing the fix's clothes.
  const dbRace2 = mockDb({
    chats: [{ id: derivedId, user_id: JOB.user_id, updated_at: 'v1', messages: [] }],
    insertError: { code: '23505', message: 'duplicate key' },
    concurrentWrites: 1,
  });
  const race2 = await attachRenderToChat(dbRace2, JOB, { log: { log() {}, error() {} } });
  assert.strictEqual(race2.attached, true, 'a lost CAS must be retried, not surrendered');
  assert.strictEqual(race2.reason, 'raced_appended');
  const finalMsgs = dbRace2.store.chats[0].messages;
  assert.strictEqual(finalMsgs.filter((m) => m && m.jobId === JOB.id).length, 1,
    'exactly one render message — the retry must not double-append');
  assert.ok(finalMsgs.some((m) => m && m.id === 'client-added'),
    "THE LOST UPDATE: the client's concurrent message must survive the append");
  assert.ok(dbRace2.calls.updates.length >= 2, 'it actually re-issued after the lost CAS');
