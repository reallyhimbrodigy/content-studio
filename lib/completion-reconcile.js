...
async function findUnprojectedCompletions(supabaseAdmin, { lookbackHours = DEFAULT_LOOKBACK_HOURS } = {}) {
  ...
  const { data, error } = await supabaseAdmin
    .from('video_jobs')
    // THE FIVE KEYS THIS PATH READS, NOT THE WHOLE BLOB. Measured 2026-09-21:
    // 1,740 shared buffer blocks and 94.7ms per call across 36,132 calls, almost
    // all of it detoasting a `result` jsonb to read five scalars out of it.
    //
    // IT IS FIVE AND NOT THREE, and the difference would have been silent. The
    // FILTER below needs video_url/public_url/rendered_video_url, so three looks
    // sufficient — but projectCompletion() also reads hls_manifest_url and
    // thumbnail_url, behind `if (!row.x && res.x)` guards. Ship three and those
    // guards simply never fire: no crash, no error, HLS and thumbnails quietly
    // stop being projected. The first version of this change did exactly that.
    .select('id, user_id, created_at, status, rendered_video_url, result_url, '
            + 'hls_manifest_url, thumbnail_url, completed_at, '
            + 'r_video_url:result->>video_url, r_public_url:result->>public_url, '
            + 'r_rendered_video_url:result->>rendered_video_url, '
            + 'r_hls:result->>hls_manifest_url, r_thumb:result->>thumbnail_url')
    .eq('status', 'completed')
    .is('rendered_video_url', null)
    // updated_at, not created_at: a completion that landed late on an old row
    // is exactly the unprojected case this hunts, and a created_at window hides it.
  ...
  // REBUILD THE `result` SHAPE DOWNSTREAM EXPECTS. projectCompletion takes a row
  // and reads row.result.*; it is also called from other paths with a full row,
  // so its contract must not change just because this reader got cheaper. The
  // projection is reassembled here and nowhere else.
  return (data || [])
    .map((r) => ({
      ...r,
      result: {
        video_url: r.r_video_url || undefined,
        public_url: r.r_public_url || undefined,
        rendered_video_url: r.r_rendered_video_url || undefined,
        hls_manifest_url: r.r_hls || undefined,
        thumbnail_url: r.r_thumb || undefined,
      },
    }))
    .filter((r) => Boolean(
      r.result.video_url || r.result.public_url || r.result.rendered_video_url,
    ));
}
