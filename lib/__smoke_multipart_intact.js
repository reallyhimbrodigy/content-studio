...
// THE PROPERTY IS THE NUMBER, NOT THE PUNCTUATION AFTER IT. The pattern used
// to require a closing paren immediately after 604800, so adding a THIRD
// argument — the per-user acceleration option — failed a check about a TTL
// that had not moved. A pattern tight enough to reject a correct
// implementation is not a check; it sends someone to "fix" working code to
// match the checker's spelling.
assert.ok(/createPresignedPutUrl\(key,\s*604800\s*[,)]/.test(server),
...
// Same loosening as the single-PUT pin above: capture the TTL wherever the
// call ends, so an added option argument cannot fail a check about a number
// that has not moved.
const initCall = server.match(/initMultipartUpload\(key,\s*partCount,\s*(\d+)\s*[,)]/);
