...
async function sweepOrphanRedispatch(supabaseAdmin, { pushProgressToSSE, dispatchJobToModal, sourceExists } = {}) {
  ...
  const { data: rows, error } = await supabaseAdmin
    ...
    .select('id, user_id, status, video_url, vibe_input, edit_recipe, reedit_mode, '
      ...
      // Without it job.updated_at is undefined, the CAS degrades to
      // `.is('updated_at', null)`, and since updated_at is never null in
      // practice the claim would match ZERO rows — silently disabling the
      // redispatch entirely rather than guarding it.
      + 'trend_snapshot, created_at, updated_at, modal_call_id, result')
  ...
  for (let i = 0; i < orphans.length; i += 1) {
    ...
    // CAS-AUDITED 2026-08-14 [Law 2]. This is the SECOND unguarded
    // read-merge-write on the `result` jsonb, found by auditing every writer
    // after the lifecycle-push one. `job.result` came from the sweep's earlier
    // read, so a worker result landing in between would be erased by this merge
    // — the identical lost-update shape.
    //
    // It has never fired (0 rows carry result._redispatch since Aug 1), which is
    // precisely why it is worth guarding NOW: the lifecycle-push race also sat
    // harmless for 12 days at 0.0% and only woke when write ordering changed on
    // Aug 11-12. A dormant race is not a safe race, it is an unexercised one.
    //
    // Note the existing filters do NOT close this: `.in('status', [queued,
    // processing])` still admits the whole window in which the worker writes its
    // envelope, since the worker writes `result` while the row is 'processing'.
    const claimResult = {
      ...(job.result || {}),
    ...
    let claimQ = supabaseAdmin
      ...
      .is('modal_call_id', null)
      .in('status', ['queued', 'processing']);
    claimQ = (job.updated_at === null || job.updated_at === undefined)
      ? claimQ.is('updated_at', null)
      : claimQ.eq('updated_at', job.updated_at);
