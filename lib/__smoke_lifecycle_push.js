...
function mockClaimDb(initialResult, opts = {}) {
  const state = { result: initialResult, reads: 0, updates: 0, isFilters: [], eqFilters: [] };
  function builder() {
    const b = {
      // PER-BUILDER filter capture. state.isFilters is cumulative across every
      // call on this mock, so scanning it would match a guard from an EARLIER
      // claim and judge this update by the wrong key.
      _mode: null, _payload: null, _isPath: null, _isF: [], _eqF: [],
      select() { return b; },
      eq(col, val) { b._eqF.push([col, val]); state.eqFilters.push([col, val]); return b; },
      maybeSingle() {
        state.reads++;
        const seen = ('staleRead' in opts) ? opts.staleRead : state.result;
        // updated_at rides the read because the claim CASes on it (2026-08-14).
        // Omitting it here would let the fake pass a claim that had silently
        // dropped its concurrency guard.
        return Promise.resolve({ data: { result: seen, updated_at: state.updated_at || null }, error: null });
      },
      update(payload) { b._mode = 'update'; b._payload = payload; return b; },
      is(col, val) { b._isPath = col; b._isF.push([col, val]); state.isFilters.push([col, val]); return b; },
      insert() { return Promise.resolve({ data: null, error: null }); },
      then(resolve) {
        // terminal await of update(...).select('id') — enforce the jsonb guard
        state.updates++;
        // BOTH guards must be present, and they answer different questions:
        //   the result-> jsonb IS NULL filter = EXACTLY-ONCE (one push per job+kind)
        //   the updated_at CAS               = NO LOST UPDATE (2026-08-14)
        // Scanning every filter rather than only the last one, because appending
        // the CAS made "the last .is() call" the wrong place to look — and a smoke
        // that reads one guard cannot notice the other going missing.
        const jsonbGuard = b._isF
          .map(([col]) => /^result->([a-z0-9_]+)->([a-z0-9_]+)$/i.exec(col || ''))
          .find(Boolean);
        assert.ok(jsonbGuard,
          `claim update must be guarded by a result-> jsonb IS NULL filter (saw: ${JSON.stringify(b._isF)})`);
        const hasCas = b._eqF.some(([col]) => col === 'updated_at')
          || b._isF.some(([col]) => col === 'updated_at');
        assert.ok(hasCas,
          'claim update must ALSO compare-and-swap on updated_at — without it a worker '
          + 'result landing between the read and the write is erased, which is the '
          + '180/466 envelope-loss class');
        const m = jsonbGuard;
...
(async () => {
  ...
  // 3. claim: winner then loser (the ACTIVE path, both outcomes)
  {
    ...
    // The reason may be either now (2026-08-14). A 0-row update no longer ends
    // the attempt: since the claim CASes on updated_at, 0 rows can mean EITHER
    // "another racer holds the claim" (lost_claim_race) OR "the row moved under
    // my read" (retry, and after 4 tries cas_exhausted). This fake models a
    // PERMANENTLY stale reader, which no real replica is, so it lands on
    // cas_exhausted. Both are non-wins and both decline to write, which is the
    // property this test actually protects.
    assert.ok(['lost_claim_race', 'cas_exhausted'].includes(l.reason),
      `guard loss must surface as a non-win reason, got ${l.reason}`);
    assert.ok(s2.updates >= 1, 'guard path actually attempted the update');
    // THE SAFETY PROPERTY, now asserted directly rather than implied by the
    // reason string: the loser must not have overwritten the winner's row.
    assert.deepStrictEqual(s2.result, marked,
      'a losing racer must leave the row exactly as the winner wrote it');
  ...
  // 5. (retired) owner-title [Promptly] law — normalizeOwnerTitle was removed and
  // titles are no longer runtime-tagged, so this smoke no longer asserts it (it had
  // been red since 2026-07-26, crashing on the missing function and teaching agents
  // to ignore the gate). Re-add if the tagging invariant is ever reintroduced.
