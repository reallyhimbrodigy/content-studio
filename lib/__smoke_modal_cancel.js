...
assert.strictEqual(callIdFromRow({ result: { modal_call_id: 'fc-abc123' } }), 'fc-abc123');
assert.strictEqual(callIdFromRow({ result: { modal_call_id: '  fc-xyz  ' } }), 'fc-xyz');
// Anything that is NOT ours must be ignored, or we would POST garbage as a call id.
for (const bad of [
  { result: {} }, { result: null }, { result: 'str' }, { result: { modal_call_id: '' } },
  { result: { modal_call_id: 42 } }, { progress_step: 'modal_call:x' }, {}, null, undefined,
]) {
  assert.strictEqual(callIdFromRow(bad), null, `must not read a call id from ${JSON.stringify(bad)}`);
}
// progress_step must NOT be used — select('*') sends it to the client.
assert.strictEqual(callIdFromRow({ progress_step: 'modal_call:fc-1' }), null,
  'the handle must not be read from a client-visible progress field');
...
(async () => {
  ...
  const ROW = { id: 'j1', result: { modal_call_id: 'fc-abc123' } };
  ...
  r = await cancelModalCall({ id: 'j2', result: {} }, { log: quiet });
