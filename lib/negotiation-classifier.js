...
async function classifyWithSafety(text, opts = {}) {
  ...
  if (adj.unsafe === true) {
    ...
    return { verdict: 'REFUSE', reason: 'unsafe (model)', oos: [], constraints: [],
             ...
             safety: { state: 'MEASURED', unsafe: true, why: 'model', by: 'model', typed_in: adj.typed_in } };
  }
  ...
  if (!negotiable.length) {
    return { verdict: 'PASS', reason: adj.constraints.length
               ...
               : 'no out-of-scope ask',
             oos: [], constraints: adj.constraints, sentence: null, degraded: false, alert: null,
             safety: { state: 'MEASURED', unsafe: false, by: 'model', typed_in: adj.typed_in } };
  }
  return {
    verdict: 'NEGOTIATE', reason: `out-of-scope: ${negotiable.join(', ')}`,
    oos: negotiable, constraints: adj.constraints,
