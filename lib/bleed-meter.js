...
// ONE WALL EVENT: `upgrade_wall_viewed` (2026-09-23).
//
// `paywall_view` is DEAD on every shipped build and has been since the paywall
// rebuild. Measured over 120 days: 161 distinct users on builds <=250 and
// ZERO on builds >=251, against 10,374 and 1,669 for `upgrade_wall_viewed` over
// the same cohorts. This line kept it in the commerce set, so the meter's
// "paywalls N" has been reporting 0 for every current install — a denominator
// that is always zero reads as "nobody saw a paywall", which is the shape of a
// catastrophic outage and was in fact a retired event name.
const COMMERCE_EVENTS = [
  'upgrade_wall_viewed', 'offerings_loaded', 'offerings_load_failed',
...
async function _commerceLine(supabaseAdmin, { sinceISO, untilISO }) {
  try {
    ...
    // SANDBOX. Client events (upgrade_wall_viewed/purchase_attempt) have no tag
    // and pass.
    for (const r of data) {
      if (r.props && r.props.environment === 'SANDBOX') continue;
      c[r.event] = (c[r.event] || 0) + 1;
    }
    const pv = c.upgrade_wall_viewed || 0;
...
module.exports = {
  ...
  _countEvents,
  _isNoDelivery,
  // EXPORTED SO THE EVENT NAMES CAN BE TESTED. The smoke said out loud that
  // nothing depended on COMMERCE_EVENTS naming real events — "mutating it to a
  // nonexistent event stays green" — because the commerce string was passed
  // INTO formatDigest rather than derived. Retiring paywall_view relied on
  // exactly that gap, so it closes here.
  _commerceLine,
  COMMERCE_EVENTS,
  RATE_PER_SEC,
};
