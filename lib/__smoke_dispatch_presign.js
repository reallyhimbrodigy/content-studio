...
// ── 5. DETECTION READS THE GRANT, NOT THE STORED URL ──────────────────────
// detectBurnedText downloads over HTTP, so if detection keeps the raw stored
// url it 403s the moment sources/ is restricted — and it is FAIL-OPEN, so every
// render still succeeds while burned-text detection silently stops working.
// Captions then get placed over existing burned-in text with nothing reporting
// it. A silent quality regression is worse than a break.
check('burned-text detection reads the presigned url, not the stored one', () => {
  assert.ok(/const detectionUrl = dispatchProxyUrl \|\| dispatchVideoUrl;/.test(src),
    'detectionUrl still resolves to the raw stored url');
});

check('the presign runs BEFORE detection is kicked off', () => {
  // ANCHOR ON CODE, NOT A COMMENT. The first cut looked for the string
  // "DISPATCH-PRESIGN", which lives in a comment — and this file strips comments
  // before asserting, so the anchor could never be found and the check failed
  // against correct code. Same class as the assertions it sits beside.
  const pres = src.indexOf('SOURCE_GRANT_S');
  const det = src.indexOf('const detectionUrl =');
  assert.ok(pres > 0, 'presign block not found');
  assert.ok(det > pres,
    'detection is kicked off before the grant exists — it would be handed an '
    + 'undefined url');
});

check('detection still CACHES against the stable url (3rd arg), not the grant', () => {
  assert.ok(/enrichAnalysisWithDetection\(cachedAnalysis, detectionUrl, videoUrl, jobId\)/.test(src),
    'the cache-key argument moved off the stable videoUrl — a signed url as a '
    + 'cache key makes every lookup a permanent miss');
});
