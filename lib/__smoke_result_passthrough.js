...
let row = null;
// THE ROW MUST BE EARNED BY THE FILTER, NOT HANDED OVER (2026-09-10).
// This mock answered `maybeSingle()` with `row` no matter what was asked, so
// `.eq('id', jobId)` could name ANY job — or be deleted — and every assertion
// below still passed. Verified by mutation: pointing the read at
// 'some-other-job' left this file green. A stub that answers a question it was
// not asked cannot test the question. Same class as the chat-attach containment
// filter that read a character of a JSON string.
// `rowFor` is the id the fixture belongs to; a read scoped anywhere else gets
// null, exactly as Postgres would answer.
let rowFor = null;
      ...
      from() {
        ...
        const b = {
          ...
          maybeSingle: () => Promise.resolve({
            data: (rowFor === null || f.id === rowFor) ? row : null,
            error: null,
          }),
...
(async () => {
  ...
  rowFor = 'j2'; row = { status: 'processing', result: { public_url: 'https://cdn/only-public.mp4' } };
  ...
  rowFor = 'j4'; row = { status: 'processing', result: null };
