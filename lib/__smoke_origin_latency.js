...
// L8 THE SECOND CONDITION CATCHES WHAT THE FIRST CANNOT: a database that
// answers FAST AND WRONG. A pool that refuses instantly produces a flood of
// quick 5xx and a p95 that looks HEALTHIER than normal — one bar would call
// that recovery. Two consecutive minutes over 20%.
leg('L8 a_fast_5xx_flood_pages_even_though_the_p95_looks_great', () => {
  g._reset();
  const pages = [];
  g.configure({ sink: (t, b) => pages.push([t, b]) });
  let t = 1000;
  for (let m = 0; m < 2; m += 1) {
    // 40 calls a minute, ALL FAST (5ms) and HALF of them 500s
    for (let i = 0; i < 40; i += 1) {
      g.observe(5, t + i, { status: i % 2 === 0 ? 500 : 200 });
    }
    t += MIN + 1;
    g.observe(5, t, { status: 500 });
  }
  assert.strictEqual(pages.length, 1, 'two minutes over the 5xx bar must page');
  assert.ok(/5xx/.test(pages[0][0]), `the page must name the 5xx cause, got ${pages[0][0]}`);
  assert.strictEqual(g._state().breaches, 0, 'the LATENCY streak must be zero — it was fast');
});

// L9 AND A CALLER THAT REPORTS NO STATUS IS NOT COUNTED AS A SUCCESS.
// Counting an unreported call as a 2xx would dilute the rate with calls
// nobody classified, which is absence rendered as a value in the denominator.
leg('L9 an_unreported_status_is_not_a_success', () => {
  g._reset();
  for (let i = 0; i < 10; i += 1) g.observe(5, 1000 + i);
  assert.strictEqual(g._state().errN, 0, 'unreported calls must not enter the rate');
  g.observe(5, 1020, { status: 500 });
  assert.strictEqual(g._state().errN, 1);
  assert.strictEqual(g._state().errBad, 1);
});
