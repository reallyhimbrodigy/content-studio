...
(async () => {
  ...
  // THAT GAP IS NOW CLOSED (2026-09-23). The block below derives the commerce
  // line FROM the db, so the event names in COMMERCE_EVENTS are covered: point
  // the set at an event nothing emits and this goes red. It was closed because
  // retiring `paywall_view` — dead on every shipped build, 0 distinct users on
  // builds >=251 — changed exactly the names nothing was checking.
  //
  // Still not covered, and still said out loud rather than left to look
  // covered: Same for the three
  ...
  // --- commerce line DERIVED from the db, so the event names are covered ---
  const dbLive = mockDb({ internalIds: [], jobs: [], commerce: [
    { event: 'upgrade_wall_viewed', created_at: t0 },
    { event: 'upgrade_wall_viewed', created_at: t0 },
    { event: 'upgrade_wall_viewed', created_at: t0 },
    { event: 'trial_start', created_at: t0 },
    // The retired name must NOT be counted: it is dead on every shipped build,
    // and counting it would resurrect a denominator that is always zero.
    { event: 'paywall_view', created_at: t0 },
  ] });
  const liveLine = await bm._commerceLine(dbLive, { sinceISO, untilISO });
  assert.ok(liveLine && liveLine.includes('paywalls 3'),
    `commerce line derived from db: expected 3 walls, got ${liveLine}`);
  assert.ok(liveLine.includes('trials 1'), 'commerce line counts trials');
  assert.ok(!bm.COMMERCE_EVENTS.includes('paywall_view'),
    'paywall_view is retired — dead on every shipped build, 0 users on >=251');
  assert.ok(bm.COMMERCE_EVENTS.includes('upgrade_wall_viewed'),
    'upgrade_wall_viewed is the wall event');
