...
// TRAILING comments too, not just full-line ones. The first stripper handled
// `^\s*//` only, so the word "below" in a trailing `// ...` comment tripped the
// ratio check — the THIRD time this gate has failed on its own prose rather
// than on the code. Stripping both forms is the fix; narrowing the needles
// again would have been the wrong one.
const block = sb.slice(i, j)
  .replace(/^\s*\/\/.*$/gm, '')
  .replace(/\s\/\/[^\n]*/g, '');
