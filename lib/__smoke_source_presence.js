...
(async () => {
  ...
  // THE ROW IS SCOPED BY THE FILTERS, as Postgres scopes it (2026-09-10).
  // Three chained `eq: () => ...` used to discard every argument, so this stub
  // handed back the dead-upload row for ANY user, ANY url, ANY status. Verified
  // by mutation: deleting `.eq('video_url', videoUrl)`, `.eq('user_id', userId)`
  // or `.eq('status','failed')` from findDeadSourceJob each left this file
  // GREEN — while the assertion below claims the match is "on this exact URL".
  // The user_id one is the dangerous mutation: it would let one user's dead
  // upload block a DIFFERENT user's render, and nothing here would object.
  const rows = (code, { forUser = 'u', forUrl = 'url' } = {}) => ({
    from: () => {
      const f = {};
      const b = {
        select: () => b,
        ...
        limit: () => Promise.resolve({
          data: (code
            ...
            && f.video_url === forUrl
            && f.status === 'failed')
            ? [{ id: 'j1', created_at: 'x', result: { error_code: code } }]
            : [],
        }),
      };
      return b;
  ...
  // NEGATIVE CONTROLS the old stub structurally could not express.
  assert.strictEqual(await sp.findDeadSourceJob(rows('UPLOAD_STALLED'), 'OTHER-USER', 'url'), null,
    "another user's dead upload must NEVER block this user's render");
  assert.strictEqual(await sp.findDeadSourceJob(rows('UPLOAD_STALLED'), 'u', 'a-different-url'), null,
    'a dead upload of a DIFFERENT clip must not block this one — the match is per-URL');
