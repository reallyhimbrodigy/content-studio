...
const MARKER_NAME = '.build_marker.json';
...
/**
 * Read the npm `postinstall` build marker (scripts/build-marker.js).
 * Same null-never-throws contract as the receipt. Paired with `gate` on
 * /api/health it discriminates the two meanings of a null receipt:
 *   gate null + marker null    → build-time writes do not reach runtime
 *   gate null + marker present → npm install ran and validate_deploy.js did
 *                                not: the Render blueprint never synced
 */
function readBuildMarker(dir) {
  try {
    const r = JSON.parse(fs.readFileSync(path.join(dir, MARKER_NAME), 'utf8'));
    if (!r || typeof r !== 'object' || Array.isArray(r)) return null;
    if (r.npm_install_ran !== true) return null;
    return { at: typeof r.at === 'string' ? r.at : null, node: typeof r.node === 'string' ? r.node : null };
  } catch (_) {
    return null;
  }
}
...
module.exports = {
  readGateReceipt, writeGateReceipt, clearGateReceipt, readBuildMarker,
  watermarkArmingWarning, RECEIPT_NAME, MARKER_NAME,
};
