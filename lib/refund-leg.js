...
//
// SINGLE-WRITER LAW: the WORKER marks eligibility on the job row's `result`
// envelope — `result.error_code === 'INTEGRITY_TRIP'` (the worker's actual key;
// `result.error` accepted as legacy belt) or `result.designed_rejection === true`.
// The app NEVER re-classifies a failure; it only refunds what the worker marked.
// Designed rejections are free: users pay for renders, not boundaries.
//
...
/** Worker-marked eligibility. Strict: no app-side re-classification. */
...
async function sweepRefundLeg(supabaseAdmin) {
  ...
  const eligible = (Array.isArray(rows) ? rows : []).filter(
    (r) => r.parent_job_id == null && r.reedit_mode == null && isRefundEligible(r.result)
  );
  const outcomes = [];
