...
async function sweepRefundLeg(supabaseAdmin) {
  ...
  const SELECT_COLS = 'id, user_id, created_at, result, status, parent_job_id, '
    + 'reedit_mode, refunded_at, credits_debited, credits_refunded_at, demo, refund_attempts';

  // ── TWO REFUNDS, TWO CLAIMS, SO TWO REASONS TO BE SELECTED ────────────────
  // This was ONE query filtered `.is('refunded_at', null)`, and that quietly
  // made the CREDIT refund un-retryable. Per job the sweep runs
  // refundJobCharge FIRST, which claims refunded_at (NULL -> now);
  // refundJobCredits then runs and, on an RC failure, deliberately UNCLAIMS
  // credits_refunded_at so a later pass can retry. There was no later pass —
  // the row no longer matched `refunded_at IS NULL`, so it left this set
  // forever: CHARGED AND NEVER REFUNDED, from one transient RC error.
  //
  // The two claims are independent by design ("a charge refund that errors must
  // not prevent the credits coming back"), so the SELECTOR has to be
  // independent too, or the first claim to land silently ends the second's
  // retries.
  //
  // Two plain queries rather than a PostgREST `.or(...)`: every filter here
  // stays a primitive the mocks already apply by value. A mock that
  // half-implements an `or` expression is the presence-for-shape failure this
  ...
  const unclaimed = await base().is('refunded_at', null);
  const creditsOutstanding = await base()
    .not('credits_debited', 'is', null)
    .is('credits_refunded_at', null);

  const error = unclaimed.error || creditsOutstanding.error;
  const byId = new Map();
  for (const r of [...(unclaimed.data || []), ...(creditsOutstanding.data || [])]) {
    if (r && r.id) byId.set(r.id, r);           // de-duped: a row can match both
  }
