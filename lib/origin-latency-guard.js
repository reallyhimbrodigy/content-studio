...
function observe(ms, now = Date.now()) {
  try {
...
function _page(p95, n, now) {
  ...
  _lastPagedAt = now;
  const title = '🐢 [Promptly] database slowing down';
  const body = `origin p95 ${Math.round(p95)}ms for ${BREACH_MINUTES} min (${n}/min). `
