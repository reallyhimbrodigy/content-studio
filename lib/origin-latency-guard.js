...
function _page(p95, n, now, why = 'latency') {
  ...
  // THE FACTS TRAVEL WITH THE PAGE, as a third argument. A sink that has to
  // re-derive p95 from _state() reads the bucket AFTER the roll that produced
  // this page — a different minute — and would record a number that is not the
  // one that fired. Third arg, so any two-argument sink is unaffected.
