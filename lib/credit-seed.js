...
async function runSeed({ userId, profileRow, deps, log = console } = {}) {
  ...
  try {
    ...
    // THE CUSTOMER MUST EXIST BEFORE THE TRANSACTION. Measured 2026-09-26: a
    // virtual-currency POST for an unknown customer returns 404 "Customer could
    // not be found" — RevenueCat does NOT auto-create one. So an absent balance
    // read is not just a zero, it is a STATE that has to be repaired before the
    // grant can land, and the previous version of this line credited straight
    // into that 404 forever.
    //
    // ONLY WHEN THE READ SAID ABSENT. Calling it unconditionally would POST a
    // customer create on every seed of every already-working account.
    if (bal && bal.customerAbsent === true && typeof deps.ensureCustomer === 'function') {
      const _made = await deps.ensureCustomer(userId);
      log.log('  [credit-seed] customer %s for %s', _made, String(userId).slice(0, 8));
    }
    ...
    // and the behavioural legs drive a throwing credit to prove the row stays
    // provider_ok=false and the retry then lands exactly once.
    ...
    // AND WHICH CALL. A 404 on the balance GET is an absent customer the grant
    // can fix; a 404 on the transaction POST means the grant itself cannot run
    // against a customer that does not exist. Opposite diagnoses, and the first
    // version of this line forced me to infer which.
    const _call = e && e.rcMethod ? ` call=${e.rcMethod} ${e.rcPath}` : '';
    log.error('[credit-seed] seed failed (non-fatal): %s%s%s%s',
      (e && (e.code || e.message)) || 'unknown', _st, _msg, _call);
