...
async function runSeed({ userId, profileRow, deps, log = console } = {}) {
  ...
  try {
    ...
    log.log('  [credit-seed] %s tier=%s allowance=%s granted=%s balance_before=%s balance_after=%s found_before=%s',
      seedKey(String(userId).slice(0, 8)), tier, allowance, delta,
      balance, balance + delta, !!(bal && bal.found));
    return { granted: delta, reason: decision.reason, seeded: true,
             tier, allowance, balance_before: balance,
             balance_after: balance + delta, found_before: !!(bal && bal.found) };
  } catch (e) {
    log.error('[credit-seed] seed failed (non-fatal):', e && (e.code || e.message));
    return skip(`exception:${(e && (e.code || e.message)) || 'unknown'}`.slice(0, 60));
  }
}
