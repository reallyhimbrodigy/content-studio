...
// The first LIVE run failed on "column free_credit_grants.id does not exist"
// while this smoke was green: it asserted the SHAPE of the update and a
// structural check cannot know whether a column exists. free_credit_grants is
// keyed by device_id and has no id. Pin the columns this module names so the
// next rename fails here rather than on a user's deletion request.
check('grants are selected by device_id, the table\'s actual key',
  /free_credit_grants[\s\S]{0,160}select\('device_id'\)/.test(src),
  'the table has device_id, user_id, claimed_at, app_build — no id column');
check('profiles is keyed by id, not user_id',
  /del\('profiles', 'id', userId\)/.test(src));
