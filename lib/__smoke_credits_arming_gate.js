'use strict';
// Gate for the CREDITS arming coupling and the RevenueCat health block
// (2026-09-02).
//
// FORGED FROM A 23-DAY FAILURE. On 2026-08-10 /sync was diagnosed as 100%
// NO_RC_CUSTOMER from a mismatched REVENUECAT_PROJECT_ID / REVENUECAT_SECRET_KEY
// pair, and a `CONFIG SUSPECT` log line was added to make it visible. It fired
// continuously for 23 days. A log line is only visible to someone already
// looking — the same disease as the event-allowlist smoke that ran inside
// Render's build, where the app-* branches it scans do not exist, and therefore
// passed by scanning nothing.
//
// The specific trap this closes: `credits.isConfigured()` is a PRESENCE test,
// not a validity test — it returns true for a mismatched pair. So `CREDITS=1`
// against a 404ing project would have served a meter that 503s for 100% of
// users, with nothing saying so first.
//
// WHAT IS ASSERTED, and why each is separable:
//   1. `credits` on /api/health is CONJOINED with the RC reachability probe.
//      Deleting that conjunction is what makes the failure possible again.
//   2. The probe is REFRESHED, not cached for the process lifetime. The older
//      _rcProjectProbe is a lifetime cache: once it says http_404 it repeats
//      that verdict from memory forever, so a FIXED credential still reads
//      broken until a restart. A stale 'ok' would be worse.
//   3. The health block leaks NOTHING. /api/health is UNAUTHENTICATED.
//
// THIS IS A SOURCE ASSERTION, and its limits are stated rather than implied: it
// proves the coupling is written, not that it executes. The executing proof is
// the boot test in the commit that added it (CREDITS=1 + a 404ing project ->
// credits served 'off'). A cert reasons about text; text is not behaviour.
const fs = require('fs');
const path = require('path');

const SRC = fs.readFileSync(path.join(__dirname, '..', 'server.js'), 'utf8');
const fail = [];
const ok = (cond, msg) => { if (!cond) fail.push(msg); };

// ── 1. credits is gated on the probe, not on the env var alone ──────────────
// ANCHORED ON THE RESOLVER, NOT ON AN INLINE TERNARY. The arming logic moved
// into _resolveCreditsSwitch when each 'off' gained a named cause; this gate
// pinned the whole inline expression and failed a refactor that changed no
// property at all. Fourth pinned literal to do that in a day. The PROPERTY is
// what matters and it is asserted below, wherever the logic lives.
const arming = SRC.replace(/\s+/g, ' ');
const resolver = (SRC.match(/function _resolveCreditsSwitch[\s\S]*?\n\}/) || [''])[0];
ok(resolver.length > 0,
  'positive control: _resolveCreditsSwitch not found — without it every '
  + 'assertion below passes vacuously against an empty string');

// THE GATE ITSELF: a non-ok probe must force 'off'. CREDITS=1 against an
// unreachable RevenueCat project arms a meter that 503s for every user.
ok(/probe !== 'ok'/.test(resolver) && /reason: `probe_failed/.test(resolver),
  "credits is NOT gated on the RC probe — CREDITS=1 could arm a meter against "
  + 'an unreachable RevenueCat project, which 503s for every user. This '
  + 'conjunction IS the gate.');
ok(/probe === null \|\| probe === undefined/.test(resolver),
  'a probe that has not fired yet must also force off — pending is not ok');

// Fail closed: 'on' is reachable ONLY after every guard has passed, and it is
// the LAST statement. An inverted resolver would arm exactly when RC is broken.
const onIdx = resolver.indexOf("value: 'on'");
ok(onIdx > 0, "the resolver never returns 'on'");
ok(onIdx > resolver.indexOf("probe !== 'ok'"),
  "the 'on' return must come AFTER the probe guards — an early 'on' arms the "
  + 'meter exactly when RevenueCat is unreachable');
ok(/credits: _creditsDisplay\.value/.test(arming),
  'health must publish the resolver\'s value rather than recomputing it inline, '
  + 'or the gate above guards a branch nothing uses');

// ── 2. the probe REFRESHES; it is not a process-lifetime verdict ────────────
ok(/_rcHealthProbe = \{ at: 0, value: null \}/.test(arming),
  '_rcHealthProbe must start unmeasured (value: null) — "not probed" must never '
  + 'read as "ok"');
ok(/Date\.now\(\) - _rcHealthProbe\.at > \d+/.test(arming),
  '_rcHealthProbe has no time-based refresh — a lifetime cache repeats a stale '
  + 'verdict forever, so a FIXED credential keeps reading broken until a restart '
  + '(and a stale "ok" would be worse). This is the flaw in _rcProjectProbe.');

// ── 3. nothing sensitive on an UNAUTHENTICATED endpoint ────────────────────
const block = (SRC.match(/revenuecat: \(\(\) => \{[\s\S]*?\n {6}\}\)\(\),/) || [''])[0];
ok(block.length > 0, 'the revenuecat health block was not found at all');

// Check the RETURNED KEYS, not mentions of the identifiers. `projectId` and
// `secret` are READ inside the block (`Boolean(projectId && secret)`) and that
// is fine — reading them is the point. What must never happen is one of them
// becoming a key of the returned object. An earlier version of this smoke
// searched for the identifier anywhere after `return {` and reported a leak
// that did not exist: a false RED is as much a broken check as a false green.
const ret = (block.match(/return \{([\s\S]*?)\n {8}\};/) || [, ''])[1];
const keys = [...ret.matchAll(/^ {10}(\w+):/gm)].map((m) => m[1]);
ok(keys.length > 0, 'could not parse the returned keys of the revenuecat block');
for (const banned of ['projectId', 'secret', 'secretKey', 'key']) {
  ok(!keys.includes(banned),
    `the revenuecat health block returns \`${banned}\` — /api/health is UNAUTHENTICATED`);
}
for (const required of ['configured', 'projectIdShape', 'secretShape', 'probe']) {
  ok(keys.includes(required),
    `the revenuecat health block no longer returns \`${required}\``);
}
ok(/probe: _rcHealthProbe\.value/.test(block),
  'the block must expose the probe result — it is the field that distinguishes '
  + '"configured" (presence) from "actually reachable"');

// ── 4. the probe hits an endpoint that actually EXISTS ─────────────────────
// The first version of this probe fetched the BARE project (`/projects/{id}`),
// which RC's v2 API does not serve: it 404s under a perfectly valid key. Since
// `credits` is gated on the probe, that made arming impossible — a false RED
// that shipped. It was caught only because `entitlementTiers` (which requires a
// 200 from /entitlements) and `probe` disagreed inside the same response.
//
// RE-AIMED 2026-09-26. The leg read
//   /\/projects\/\$\{encodeURIComponent\(projectId\)\}\/entitlements`, \{\n\s*headers: \{ Authorization/
// which pins the FORMATTING of the call — URL and `{ headers` on adjacent lines
// with the brace on the comma's line. Hoisting the probe out of the /api/health
// handler into probeRevenueCat (so the credit sweep can await a real answer
// instead of reading a cache nothing had filled) wrapped the arguments and broke
// it, with the property fully intact. Third check in one session whose anchor was
// a layout rather than a fact.
//
// The property has two halves and both are asserted separately, because a single
// pattern satisfied by either one proves neither: the probe must REACH
// /entitlements, and it must NOT read the bare project.
{
  const probeFn = SRC.slice(SRC.indexOf('async function probeRevenueCat('),
                            SRC.indexOf('async function probeRevenueCat(') + 1600);
  ok(/async function probeRevenueCat\(/.test(SRC),
    'probeRevenueCat is gone — the health probe has no named producer, which is '
    + 'the defect that made the sweep read a null and report rc_unreachable while '
    + 'RevenueCat was fine');
  ok(/\/projects\/\$\{encodeURIComponent\(projectId\)\}\/entitlements`/.test(probeFn),
  'the health probe must call /projects/{id}/entitlements — a bare /projects/{id} '
    + 'read 404s under a VALID key, which makes probe permanently non-ok and '
    + 'silently prevents CREDITS=1 from ever arming');
  // THE NEGATIVE HALF. A probe that fetches BOTH would satisfy the positive leg
  // while still carrying the 404 that made arming impossible.
  ok(!/\/projects\/\$\{encodeURIComponent\(projectId\)\}`/.test(probeFn),
    'the probe reads the BARE project — RC v2 does not serve that path and it '
    + '404s under a valid key, which is the false RED that shipped once already');
}

if (fail.length) {
  console.error('credits arming gate: FAIL');
  for (const f of fail) console.error(`   ✗ ${f}`);
  process.exit(1);
}
console.log('credits arming gate: PASS (credits conjoined with a REFRESHING RC probe; '
  + 'health block exposes shapes + probe and leaks neither id nor secret)');
