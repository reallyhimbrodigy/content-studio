...
//   rc_environment — sandbox tag on profiles (add-rc-environment-to-profiles,
//   authored 2026-07-27, PENDING apply). server.js writes it as an OBJECT KEY in
//   the RC-webhook update payload (applyTo), which the gate deliberately does NOT
//   scan (writes ≠ filters/selects) — the write is instead guarded by the
//   rcOrderingColumnMissing() fallback + marked `migration-guarded[rc_environment]`
//   there. Listed here to document the pending state; REMOVE once applied.
const PENDING = ['rc_environment'];
