...
//   preview — §5 progressive marker (add-preview-to-video-jobs). server.js does NOT
//   reference it today (the capability flows via the Modal dispatch payload, not a
//   column), so the gate passes vacuously — but it's ARMED: any future server.js
//   read/write of `preview` before the migration is confirmed will fail the gate
//   until it's guarded. Remove once Zac confirms the column is live.
