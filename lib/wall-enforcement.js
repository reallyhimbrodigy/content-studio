...
/**
 * The tier a gate should ACT on. Under enforcement, the real tier. Knob OFF must
 * be byte-for-byte today's behavior, where the gate keyed off `isPro`:
 *   - an active entitlement (trial OR paid) was `isPro` → unlimited → treat as `paid`.
 *   - no entitlement was the 3/day free tier → treat as `trial` (today's caps).
 * So a straggler trial isn't silently limited before the flip, and a free user is
 * unchanged. Under enforcement, trial becomes limited and `none` becomes the wall.
 */
function effectiveTier(tier, enforce) {
  if (enforce) return tier;
