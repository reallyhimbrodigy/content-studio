...
const LEASE_MS = {
  queued: 10 * 60 * 1000,
  // 3-minute-clip lockstep (2026-07-23): 20 → 30 min. Longer clips (CLIP_TOO_LONG
  // 120s → 180s) mean longer legitimate renders, so the heartbeat lease grows with
  // the worker's execution ceiling. Raised HERE FIRST, before the worker's ceiling
...
// Was 1200s = 900s ceiling + 300s slack. 3-minute-clip lockstep (2026-07-23):
// raised to 2100s to stay ahead of the worker's new 1800s (30-min) ceiling —
// 2100s = 1800s ceiling + 300s (Modal cold-start + reaper-interval) slack.
// LOAD-BEARING ORDER: this wall rises FIRST (ships tonight); the backend raises
// the worker's ceiling + CLIP_TOO_LONG (120s → 180s) AFTER it is live, so
// reaper_wall >= worker_ceiling holds at every instant — no 3-min render is
// ever false-reaped.
const EXEC_WALL_MS = 35 * 60 * 1000;
