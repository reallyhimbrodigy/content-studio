...
// 1800s (30min) BACKSTOP (Zac 2026-08-04). queued_stall is no longer the primary
// never-dispatch handler — the orphan re-dispatch cron (lib/orphan-redispatch.js) is:
// it RECOVERS the ~24% whose upload completed (re-dispatch) and honestly terminalizes
// the rest, acting at ~11min and giving up by ~22min. queued_stall must therefore fire
// ABOVE that window, purely as a safety net for when the cron is down — otherwise it
// would reap (as JOB_NEVER_STARTED) an orphan the cron would have recovered, and
// mis-label the upload-absent ones. Was 8min (when queued_stall WAS the handler).
// Env-overridable (QUEUED_STALL_MS_OVERRIDE, in ms) — one config value, no redeploy.
const QUEUED_STALL_MS = Number(process.env.QUEUED_STALL_MS_OVERRIDE) || 30 * 60 * 1000;

// ── RENDER-STALL WALL (the third wall) ──────────────────────────────────────
// The worker writes `progress_at` on EVERY frame advance and writes NOTHING when
// frames freeze (_start_render_frame_watcher: `if _tot > _state["frames"]`). So a
// stale progress_at is an unambiguous zero-movement signal — and until now
// NOTHING READ IT. The column was written every 4s and consumed by no one: a
// producer with no consumer, which looks exactly like instrumentation and is
// dead weight.
//
...
// job only dies at the 55-min execution wall — regardless of when frames actually
// stopped. This wall catches it in minutes instead.
//
// DERIVED, NOT ROUND. The worker's own pre-frame guard is the floor: bundle +
// openBrowser + selectComposition emit no frames for a legitimate ~10-30s, and
// the watcher polls every 4s. So the shortest honest gap between two frame
// writes on a HEALTHY render is the 30s startup ceiling plus one poll interval.
// The wall is 4x that, which tolerates a slow chunk boundary and a cold browser
// while still firing ~50 minutes before the execution wall.
//   30s startup ceiling + 4s poll = 34s   ->   x4 = 136s
const RENDER_STARTUP_CEILING_MS = 30 * 1000;   // worker: bundle/openBrowser/selectComposition
const RENDER_POLL_MS = 4 * 1000;               // worker: _start_render_frame_watcher interval
const RENDER_STALL_MS = Number(process.env.RENDER_STALL_MS_OVERRIDE)
  || (RENDER_STARTUP_CEILING_MS + RENDER_POLL_MS) * 4;
...
/**
 * Pure selector — is this a RENDER whose frames have stopped moving?
 *
 * Only applies while current_step/phase is 'render': progress_at is written by
 * the render frame watcher and by nothing else, so outside the render stage a
 * stale value means "not rendering", not "dead". Requires progress_at to be
 * present — a row that never rendered has none, and absence must never reap.
 *
 * This is the ONLY signal that distinguishes a frozen render from a slow one.
 * updated_at cannot: the worker heartbeats it through _async_job_status right up
 * to the kill, which is exactly why a frozen job survived to the 55-min wall.
 */
function isRenderStalled(row, nowMs = Date.now()) {
  if (row.status !== 'processing') return false;
  const stage = String(row.current_step || row.phase || '');
  if (stage !== 'render') return false;
  if (!row.progress_at) return false;          // never rendered — absence is not death
  const at = new Date(row.progress_at).getTime();
  if (!Number.isFinite(at)) return false;      // unparseable is not death either
  return nowMs - at > RENDER_STALL_MS;
}
...
function reapReason(row, nowMs = Date.now()) {
  ...
  // render_stall outranks timeout and stall: when frames have stopped we know
  // WHY the job is dead and roughly when, ~50 minutes before either other wall
  // would notice. A truer cause and an earlier one.
  if (isRenderStalled(row, nowMs)) return 'render_stall';
...
async function sweepJobReaper(supabaseAdmin, { pushProgressToSSE, sendAlert = sendOwnerAlert } = {}) {
  ...
  const { data: rows, error } = await supabaseAdmin
    .from('video_jobs')
    .select('id, user_id, status, progress, created_at, updated_at, started_at, current_step, phase, progress_at')
  ...
  for (const { job, reason } of due) {
    ...
    const copy = reason === 'timeout'
      ...
      : reason === 'queued_stall'
        ? STALLED_COPY.queued
        // render_stall uses the RENDER stage copy explicitly rather than falling
        // through to it. Same words today, but the fallthrough would silently
        // change meaning if STALLED_COPY.render were ever re-keyed, and this is
        // the branch a user actually sees when a render freezes.
        : reason === 'render_stall'
          ? (STALLED_COPY.render || STALLED_COPY.processing)
...
module.exports = {
  ...
  isRenderStalled,
  LEASE_MS, EXEC_WALL_MS, RENDER_STALL_MS, STALLED_COPY, _qsAlert,
};
