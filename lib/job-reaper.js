...
async function sweepJobReaper(supabaseAdmin, { pushProgressToSSE } = {}) {
  ...
  // KILL-SWITCH (Zac 2026-08-03, URGENT): queued_stall reaping is OFF unless
  // QUEUED_STALL_ENABLED=1. Disabled by default after it fired more often than the
  // 1-per-6h background stall rate during the surge; re-enable ONLY once the full
  // reaped population is confirmed NULL-modal_call_id (genuinely dead, never
  // dispatched) so it can never terminalise a live render. One config value, no
  // logic redeploy. While OFF the ordinary 50-min processing lease still catches
  // genuinely-dead queued jobs (the pre-2026-08-03 behaviour).
  if (process.env.QUEUED_STALL_ENABLED !== '1') {
