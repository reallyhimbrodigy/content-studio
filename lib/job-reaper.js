...
//                        died with it.
//   processing  50 min — matches the worker's CURRENT 3000s function timeout
//                        (raised 900->1800->3000 for 5-minute support); the worker
...
// server-side death the worker could not self-report (SIGKILL at the 3000s
...
// Execution wall (W4 #1 — the platform SIGKILL class). Anchored on started_at
// (the execution-start instant the dispatch now stamps), NOT on updated_at.
// Modal's @app.cls(timeout=3000) SIGKILLs a run mid-execution — the finally +
...
// the SIGKILL ~5min after death. SAFE: the worker CANNOT run past its 3000s
// ceiling (EXEC_WALL_MS=3300s is deliberately above it), so no legitimate job
// is still 'processing' this far past start —
...
async function sweepJobReaper(supabaseAdmin, { pushProgressToSSE } = {}) {
  ...
  for (const { job, reason } of due) {
    ...
    // A 'timeout' reap is the platform execution-wall kill (preemption/3000s
    // SIGKILL) — the canonical PLATFORM_TIMEOUT class the worker also uses; a
    // 'stall' is a server-side dispatch/heartbeat stall.
    const reapCode = reason === 'timeout' ? 'PLATFORM_TIMEOUT' : 'JOB_STALLED';

    // CANCEL THE CONTAINER BEFORE RECORDING ITS DEATH (Zac 2026-08-03). Measured:
    // stalled rows live a median 3050s against Modal's 3000s function timeout —
    // the container burned its full 50 minutes and we only wrote down that it
    // died. ~$1.17 a stall, ~4/day, ~$140/month. Best-effort and awaited BEFORE
    // the write so the row's own timestamp bounds the spend; a failed or
    // unconfigured cancel changes nothing about the terminalize below.
    const _cancel = await cancelModalCall(job);
    if (_cancel.attempted && !_cancel.cancelled) {
      console.warn(`[reaper] ${job.id} cancel attempted but FAILED (${_cancel.reason}) — container may bill to its 3000s timeout`);
    } else if (!_cancel.attempted && _cancel.reason !== 'no_call_id') {
      console.warn(`[reaper] ${job.id} cancel NOT attempted (${_cancel.reason}) — container will bill to its 3000s timeout`);
    }
    ...
    // Operator [ALERT] — a reaped job is a real finish-line death (a 3000s
