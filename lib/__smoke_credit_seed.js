...
(async () => {
  ...
  // 20. AN ABSENT CUSTOMER IS SEEDED, AND THE GRANT IS WHAT CREATES IT.
  {
    ...
    assert.deepStrictEqual(h.calls.credit, [credits.TIER_ALLOWANCE.pro],
      'an absent customer must be granted the full allowance — the grant is the '
      + 'only operation that can create the customer, and RC auto-creates on a '
      + 'transaction');
    ...
    // ZAC'S RULE, satisfied by construction: the customer comes into existence at
    // a POSITIVE balance, created by a grant, never by a debit.
    assert.ok(h.balance > 0,
      'the customer must be created at a positive balance — never by a debit');
    legs += 1; console.log('  ok  an absent customer is seeded, created by the grant at a positive balance');
  }

  console.log(`__smoke_credit_seed: ${legs}/20 legs green`);
  assert.strictEqual(legs, 20, `expected 20 legs, ran ${legs}`);
})().catch((e) => {
