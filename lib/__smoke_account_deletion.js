...
if (i > 0) {
  ...
  // ── EVERY URL COLUMN IS CAPTURED, OR ITS OBJECTS ARE ORPHANED ────────────
  // The keys live in video_jobs and the capture runs BEFORE the row delete, so
  // a column missing here is a bucket object nothing can ever attribute again.
  // Five exist; the capture read four until 2026-09-21.
  for (const col of ['video_url', 'rendered_video_url', 'thumbnail_url',
    'hls_manifest_url', 'result_url']) {
    ok(new RegExp(`job\\.${col}\\b`).test(body),
       `the S3 key capture does not read job.${col} — those objects stay in the `
       + 'bucket attached to nobody');
  }

  // ── THE FREE-CREDIT TABLES GO WITH THE ACCOUNT ───────────────────────────
  // Neither has a foreign key to auth.users, so the auth delete does not reach
  // them. Measured 2026-09-21: 12 rows in EACH still named an already-deleted
  // user. A deletion that leaves the person named in two tables is not one.
  const authUserDel = body.indexOf('auth.admin.deleteUser');
  // SCRUB vs DELETE IS THE POINT, NOT AN IMPLEMENTATION DETAIL. grants keeps the
  // row (device_id is the anti-abuse primary key) with a NULL user_id; periods
  // is deleted outright. A leg that accepted either for both would pass the
  // exact mistake this reverses — deleting grants frees the device to re-claim.
  const GONE = { free_credit_grants: `from('free_credit_grants').update({ user_id: null })`,
    free_credit_periods: `from('free_credit_periods').delete()` };
  ok(!/from\('free_credit_grants'\)\.delete\(\)/.test(body),
     'free_credit_grants is DELETED — that frees the device to claim its 30 free '
     + 'credits again. It must be scrubbed to a NULL user_id, not removed');
  for (const t of ['free_credit_grants', 'free_credit_periods']) {
    const at = body.indexOf(GONE[t]);
    ok(at >= 0,
       `${t} is not handled as ${GONE[t]} — it has no FK to auth.users, so the `
       + 'row survives the account and keeps naming the user');
    // PRESENCE FIRST, THEN ORDER. indexOf returns -1 when absent and -1 < authDel
    // is true, so an ordering check alone passes when the delete is removed
    // entirely — the same trap the auth check above already documents.
    ok(at >= 0 && authUserDel >= 0 && at < authUserDel,
       `${t} must be handled BEFORE the auth user`);
  }
}
