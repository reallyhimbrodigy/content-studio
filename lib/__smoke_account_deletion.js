...
if (i > 0) {
  ...
  for (const t of ['free_credit_grants', 'free_credit_periods']) {
    const at = body.indexOf(`from('${t}').delete()`);
    ok(at >= 0,
       `${t} is never deleted — it has no FK to auth.users, so the row survives `
       + 'the account and keeps naming the user');
    ...
    ok(at >= 0 && authUserDel >= 0 && at < authUserDel,
       `${t} must be deleted BEFORE the auth user`);
  }
}
