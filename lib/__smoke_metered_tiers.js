...
// MOVED 100 -> 200 WITH THE GRANT, 2026-09-22, AND THE JUSTIFICATION CHANGED
// WITH IT RATHER THAN BEING LEFT TO ROT. The old text said the grant must equal
// the listing's "100 videos a month". The grant is now 200 and THE LISTING
// STILL SAYS 100, so the two no longer agree and this check can no longer claim
// they do. What it asserts now is narrower and still worth asserting: the grant
// equals the number we have DECIDED to deliver. The listing is the open half.
//
// WE ARE OVER-DELIVERING AGAINST PUBLISHED COPY — 200 videos where the App
// Store says 100. That is the harmless direction for a user and a real one for
// cost, and it is the outstanding item, not a resolved one.
assert.strictEqual(TIER_ALLOWANCE.max / COST_PER_RENDER, 200,
  'Max grants 2000 credits = 200 videos a month as of the 2026-09-22 flip. '
  + 'promptly_max_monthly and _yearly still read "100 videos a month", so the '
  + 'LISTING is the half that is now wrong in public. If this stops equalling '
  + '200 the grant has drifted from the decision.');
assert.strictEqual(TIER_ALLOWANCE.free / COST_PER_RENDER, 3);
assert.strictEqual(TIER_ALLOWANCE.pro / COST_PER_RENDER, 50,
