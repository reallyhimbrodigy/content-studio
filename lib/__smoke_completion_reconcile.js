...
// PostgREST PROJECTS `alias:result->>key` INTO A TOP-LEVEL `alias`. The mock
// used to ignore the select string entirely and hand back each fixture row
// verbatim — so a reader that asked for projected aliases got `undefined` here
// and the real values in production, which is the mock-that-half-implements-a-
// feature failure this repo has paid for before. It applies the projection now,
// which lets the fixtures stay in their natural `result: {...}` shape AND makes
// the alias mapping something this smoke actually exercises.
function applyProjection(sel, row) {
  if (typeof sel !== 'string' || !sel.includes(':')) return row;
  const out = { ...row };
  for (const tok of sel.split(',').map((t) => t.trim())) {
    const m = /^([A-Za-z0-9_]+):([A-Za-z0-9_]+)->>?([A-Za-z0-9_]+)$/.exec(tok);
    if (!m) continue;
    const [, alias, col, key] = m;
    const src = row[col];
    out[alias] = (src && typeof src === 'object') ? src[key] : undefined;
  }
  return out;
}
...
function mockDb(rows, { failWrite = false, raceLost = false } = {}) {
  ...
  return {
    ...
    from() {
      const st = { filters: {} };
      const b = {
        select: (sel) => { st.sel = sel; return b; },
        eq: (k, v) => { st.filters[k] = v; return b; },
        is: (k, v) => { calls.guards.push([k, v]); st.filters[`is:${k}`] = v; return b; },
        gte: () => b,
        order: () => b,
        limit: () => Promise.resolve({ data: rows.map((r) => applyProjection(st.sel, r)), error: null }),
