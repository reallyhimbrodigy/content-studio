...
//
// A sweep of the other 72 smokes found the same class in four more files, each
// confirmed by mutation against the real code:
//
...
const KNOWN = {
  '__smoke_bleed_meter.js':
    'VERIFIED EXPLOITABLE (3/3). Moving the window\'s lower bound to 2020, pointing '
    + 'the commerce read at an event that does not exist, and counting renders from '
    + 'FAILED rows instead of completed all stay GREEN. This is a SPEND reader: it '
    + 'can report a number from the wrong period, the wrong events, or the wrong '
    + 'rows and nothing objects. Needs the filters applied, per file.',
  ...
  '__smoke_completion_repair.js':
    'VERIFIED, PARTIAL (1/3). The anti-clobber guard is genuinely tested — narrowing '
    + 'it and deleting it are both caught. But .eq(\'id\', jobId) is not: the repair '
    + 'can write a completion onto ANOTHER job\'s row and stay GREEN.',
  ...
  '__smoke_install_seen.js':
    'VERIFIED EXPLOITABLE (3/3). Pointing either id filter at a different device, or '
    + 'deleting both, stays GREEN — so ANY row anywhere reads as "this device is known '
    + 'to the server". That is the second layer of the first-install rule: it would '
    + 'tell every genuine first install it had been here before and the funnel would '
    + 'never fire. Highest-consequence of the four.',
  ...
  '__smoke_terminal_invariant.js':
    'VERIFIED EXPLOITABLE (4/4). Wrong job id on all three sites, terminalList '
    + 'narrowed, the sweep reading completed instead of failed, and the sweep dropping '
    + 'its has-a-deliverable filter ALL stay GREEN. The file tests the violation '
    + 'PREDICATE exhaustively (5 deliverable shapes, no false positives) and tests '
    + 'nothing at all about the queries that feed it.',
};
