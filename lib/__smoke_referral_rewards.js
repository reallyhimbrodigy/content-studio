...
assert.strictEqual(daysEarned(0, 1), 1,
  'one qualified referral must earn a day — a first success paying nothing is what the ladder fixes');

// ── 2. the bonus lands exactly once, at three ───────────────────────────────
assert.strictEqual(cumulativeDays(2), 2, 'two referrals: two days, no bonus yet');
assert.strictEqual(cumulativeDays(3), 3 + BONUS_DAYS, `three referrals: three days plus the ${BONUS_DAYS}-day bonus`);
assert.strictEqual(cumulativeDays(4), 4 + BONUS_DAYS, 'the bonus does not repeat on the fourth');
assert.strictEqual(daysEarned(3, 1), 1, 'past the milestone, each referral is worth a day again');
...
assert.strictEqual(daysEarned(0, 3), cumulativeDays(3),
