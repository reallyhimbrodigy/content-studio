...
(async () => {
  ...
  // THE QUERY MUST NAME THE RIGHT USER AND THE RIGHT MONTH (2026-09-10).
  // Two chained `eq: () => ...` discarded both arguments, so this stub answered
  // for any user and any feature_key. Verified by mutation: hardcoding
  // readMonthlyUsage's key to 'lumen_render_2020_01' — every user reading 0 used
  // forever, i.e. unlimited premium renders — left this file GREEN. LAW 4 says
  // "the month rides the key"; asserting monthKey() in isolation does not test
  // that the key reaches the query, which is the half that bills us.
  const db = (res) => ({
    from: () => {
      const f = {};
      const b = {
        ...
        maybeSingle: async () => (
          (f.user_id === 'u' && f.feature_key === A.monthKey(new Date()))
            ? res
            : { data: null, error: { message: `unscoped read: ${JSON.stringify(f)}` } }
        ),
      };
      return b;
    },
