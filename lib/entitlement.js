 ...
 *
 * WHY THAT WIDER FORM IS UNSAFE (measured 2026-09-03). Until that date `tier`,
 * `pro_until` and `rc_app_user_id` were CLIENT-WRITABLE: UPDATE is granted to
 * `authenticated` at table level and the RLS policy is `auth.uid() = id` with
 * no column restriction, so a signed-in user could PATCH their own `pro_until`
 * and any bypass keyed on it would have been self-serve free renders.
 *
 * That hole is now closed by `protect_entitlement_columns_trg` (drafted
 * 2026-06-23, applied 2026-09-03 — it sat unapplied for ten weeks). This
 * function still keys on `comp_pro` alone, deliberately: the narrow form does
 * not depend on that trigger staying applied, and the two accounts the wider
 * form would additionally cover have rendered ZERO times, ever. It buys nothing
 * and re-couples a money decision to a database object that has already been
 * missing once.
 *
 ...
 *  72 hours. */
// WHAT A REVOKE MAY WRITE (2026-09-07). The raise-guard on grants is scoped to
// grants on purpose — 'free' ranks below every paid tier, so applying it to a
// revoke would make subscriptions unrevokable. But that left EXPIRATION writing
// 'free' UNCONDITIONALLY, and there is a real customer shape it destroys:
//
//   A Max subscriber downgrades to Pro. The Pro grant arrives; the raise-guard
//   keeps 'max' over it, correctly, because max outranks pro. Later the Max
//   period ends and its EXPIRATION fires. Nothing in the stored row records
//   that a Pro grant ever arrived, so the expiration writes 'free' and revokes
//   the Pro the customer is paying for.
//
// It is ORDERING-DEPENDENT and today UNOBSERVABLE: there are zero Max rows, so
// it cannot fire and cannot be caught by watching production. It becomes live
// the day someone buys Max, which is why it is fixed against a test rather than
// against a customer.
//
// The fix is to ask RevenueCat what is STILL ACTIVE instead of assuming
// nothing is. This decides what to write from that answer.
//
// FAILS TOWARD REVOKING. A null/failed/unconfigured reconcile returns 'free' —
// exactly today's behaviour. An unreachable RC must never make a cancelled
// subscription permanent, which is the far worse bug and the one the grant
// guard's own comment warns about.
function tierAfterRevoke(reconcile) {
  if (!reconcile || reconcile.ok !== true) return 'free';
  if (reconcile.isPro === false) return 'free';
  const t = String(reconcile.tier || '').toLowerCase().trim();
  if (!t || !(t in TIER_RANK)) return 'free';
  return tierRank(t) > tierRank('free') ? t : 'free';
}
