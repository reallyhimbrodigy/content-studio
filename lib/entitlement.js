...
function revenuecatWebhookAuthMatches(authHeader, expectedSecret) {
  ...
  const got = stripBearer(authHeader);
  // CONSTANT TIME. `===` on strings short-circuits at the first differing byte,
  // so the time it takes to say no leaks how much of the secret was right —
  // and this is a bearer secret on a webhook anyone on the internet can call
  // as often as they like. RevenueCat signs with a header rather than an HMAC
  // of the body, so this string IS the whole authentication; there is no
  // second factor to fall back on.
  //
  // LENGTH IS COMPARED FIRST AND OUTSIDE THE TIMING-SAFE CALL, deliberately:
  // timingSafeEqual THROWS on a length mismatch, and the length of a secret is
  // not the secret. Leaking it costs nothing a determined caller could not get
  // by trying.
