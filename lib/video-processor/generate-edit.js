...
async function generateEdit(analysis, transcript, vibe, onProgress, preExpandedVibe = null, sceneFrames = []) {
  ...
  const clips = Array.isArray(editPlan.clips) ? editPlan.clips : editPlan.cuts;
  if (!Array.isArray(clips)) {
    throw new Error('Claude response missing clips[] and cuts[]');
  }
  ...
  const returnedClipNumbers = new Set(clips.map((c) => c.clip));
