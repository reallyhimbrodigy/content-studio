...
function buildPrompt(analysis, transcript, vibe) {
  ...
  return `You are a professional video editor. You've been handed raw footage and a client brief. Your job is to deliver a finished social media video that holds the viewer's attention from the first frame to the last.
...
Start by reading through all the speech segments in order. That is the content of this video. Understand what the speaker is communicating and how each section connects to the next. Identify how each section of speech builds on the previous one — what is the speaker working toward? You need to understand this before you can know which parts are safe to remove.

Your cuts, transitions, and pacing should all support what the speaker is trying to communicate.

Placing cuts:
The sentence endings and cut points listed below were measured from the audio waveform. They mark the exact silences between words where a cut will be invisible to the viewer. Use these timestamps for your source_start and source_end values — they are your edit points. Picking a timestamp that isn't one of these will almost certainly clip a word.

Pacing:
Social media viewers lose attention when the visual stays static. Break the edit into short clips by cutting at sentence boundaries, even within sections you're keeping. Each clip should cover one complete sentence or one shift in topic. When a talking-head clip runs long without a visual change, the viewer's eye has nothing new to grab onto — splitting it at the next sentence boundary keeps the energy up.

Starting and ending the edit:
Your first clip should start at the beginning of the video — that's the first thing the viewer sees. Your last clip should carry through to the end of the speaker's final thought. The viewer should feel the video reached its natural conclusion.

Checking continuity:
When you remove a section of footage, the last words from your previous clip and the first words of your next clip play back-to-back instantly. Before you lock each cut, read those two pieces of speech together. They need to flow as a natural, complete thought. If the meaning jumps or context is missing, adjust your cuts until the speech connects.

Choosing transitions:
Think about what the viewer sees when two clips meet. A clean_cut is instant — use it when the visual content stays the same between clips. A dissolve blends the outgoing frame into the incoming frame — use it when the visual content between the two clips is distinctly different. When both frames look the same, a dissolve creates a distracting ghost effect. A fade takes the image to or from black. Wipes are stylistic section dividers.

Deciding what to keep:
The footage assessment suggests a target duration. That duration is a guideline — if the footage is all strong content, keep it rather than cutting good material to hit a number. When you do need to shorten, read through the speech text and identify sections that repeat a point already made, or that don't connect to what comes before and after them. The editing_value labels on each shot and the weakest_moments field will also guide you. If you remove a section, verify that speech later in the video doesn't reference something the viewer never heard.
...
Before you write your edit plan, do a final check: for each pair of adjacent cuts, read the last speech from the previous clip and the first speech from the next clip together. Confirm they connect.

Color: Gemini measured corrective color values from the footage. Choose one color_intent word that matches the client brief and the visual character.
...

{
  ...
  "strategy": "<name the specific content you kept and any content you removed, and explain why the speech still flows and makes sense>",
  "color_intent": "<one word from the list>",
  "cuts": [
    {
      "source_start": <use a timestamp from the cut points or sentence endings>,
      "source_end": <use a timestamp from the cut points or sentence endings>,
      "speech_before_cut": "<last few words the viewer hears before this clip ends>",
      "speech_after_next_cut_starts": "<first few words the viewer hears when the next clip begins — write 'end' for the last clip>",
