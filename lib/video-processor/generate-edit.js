...
Transition sounds (use VERY sparingly — most clips should be "none"):
  none — no sound (THIS IS THE DEFAULT. Use this for most transitions, especially dissolves and fades)
  whoosh — fast punchy air swoosh. ONLY use on smoothleft, smoothright, wipeleft, wiperight transitions
  swoosh — softer longer swoosh. ONLY use on smoothup, smoothdown, wipeup, wipedown transitions
  pop — soft bubble pop. Use when a text overlay appears, or on a single hard cut for emphasis
  boom — low bass impact. Use ONCE MAXIMUM per video, only on a dramatic reveal or the hook moment
  ding — clean chime. Use on CTA moments ("link in bio", "try it free") or "look at this" moments
  rise — ascending tone. Use right before a big reveal to build micro-tension

IMPORTANT: Transition sounds should be subtle accents, not the main event. Rules:
- Educational/talking-head content: 0-1 sounds total (usually none)
- High-energy/hype content: 1-3 sounds total
- NEVER put sounds on dissolve or fade transitions — they should be silent
- NEVER use more than 2 of the same sound in one video
- When in doubt, use "none"

Background music:
  none, lofi_chill, upbeat_pop, cinematic_epic, acoustic_warm, electronic_pulse, hip_hop_beat, ambient_calm, corporate_bright, dramatic_tension, funky_groove
...

{
  ...
  "clips": [
    { "clip": 1, "transition_out": "<transition>", "transition_sound": "<sound or none>", "zoom": "<zoom or none>", "speed": <number> },
    { "clip": 2, "transition_out": "<transition>", "transition_sound": "<sound or none>", "zoom": "<zoom or none>", "speed": <number> }
  ]
}`;
}
