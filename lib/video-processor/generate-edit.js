...
Each clip in your recipe has these parameters:
  source_start / source_end — timestamps in the source video (set these directly to define clip boundaries)
...

{
  ...
  const dynamicStartMarker = '=== THIS VIDEO ===';
  const toolsStartMarker = '=== TOOLS ===';
  const dynamicStart = fullPrompt.indexOf(dynamicStartMarker);
  const toolsStart = fullPrompt.indexOf(toolsStartMarker);
  if (dynamicStart === -1 || toolsStart === -1 || toolsStart <= dynamicStart) {
    throw new Error('Prompt split markers not found for Anthropic cache blocks');
  }
  const dynamicSuffix = fullPrompt.slice(dynamicStart, toolsStart).trim();
  const staticPrefix = `${fullPrompt.slice(0, dynamicStart)}${fullPrompt.slice(toolsStart)}`.trim();
