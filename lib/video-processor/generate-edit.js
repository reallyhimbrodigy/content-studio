...
Transition sounds (optional — only use if the vibe calls for sound design or energy):
  none, whoosh, swoosh, pop, click, snap, rise, drop, boom, ding, slide, glitch, tape_stop

Background music (optional — only use if the vibe calls for music or a specific mood):
...
Zoom (optional — only use if the vibe calls for energy, punch, or dynamic movement):
  none, slow_in, slow_out, punch_in, punch_out

Captions (optional — only use if the vibe mentions captions, subtitles, or text):
  none, standard, bold_centered, minimal_bottom, animated_word

Speed (optional — only use if the vibe calls for energy, pacing changes, or dramatic effect):
  1.0 (normal), 0.5 (half speed), 0.75 (slight slow-mo), 1.25 (slightly fast), 1.5 (fast), 2.0 (double speed)

Text overlays (optional — only use if the vibe calls for titles, callouts, or on-screen text):
...
Outro (optional — only use if the vibe calls for a polished ending):
  none, fade_black, fade_white

Audio ducking (optional — automatically lowers music during speech, raises during pauses):
  true, false
  Only relevant when background_music is not "none". Default false.

Aspect ratio (optional — only use if the vibe specifies a platform or aspect ratio):
  original, 9:16, 16:9, 1:1, 4:5

Vignette (optional — only use if the vibe calls for cinematic, moody, or dramatic feel):
...

{
  ...
  "caption_style": "<style from the captions list, or none>",
