...
Transitions are part of your editing toolkit — a video with no transitions at all can feel like raw unedited footage, while a video with a transition on every cut can feel over-produced. Most well-edited short-form content uses a mix: some clean cuts for pace, some transitions for moments that benefit from visual connective tissue like scene changes, topic shifts, or energy changes. Think about what each cut point represents in the content and whether a transition would help the viewer feel the shift.
...
Sound effects:
  Transitions and text overlays can include subtle sound effects. Include an sfx_style field to indicate the character of sound that would fit the moment.
  For transitions: sfx_style options include "whoosh", "impact", "slide", or "none".
  For text overlays: sfx_style options include "pop", "ding", "click", "ching", or "none".
  The "ching" style is a cash register / coin sound — it pairs naturally with moments where the speaker talks about money, revenue, pricing, deals, or free offers.
  Think about sound effects the way a real editor does. They work best as accents on moments that carry visual weight — a transition that marks a topic shift, a text overlay that should grab attention, a dramatic reveal. The relationship between sound and visuals should feel intentional, like each sound was chosen for that specific moment. Consider what this video's energy and content call for, and where silence serves the edit better than sound.
...
Zoom is most effective when it serves the moment. On short-form content, a subtle zoom on the opening clip can draw the viewer into the frame and create an immediate sense of motion that helps stop the scroll. On later clips, zoom can start to feel like a visual gimmick if it doesn't have a reason to be there. Think about whether the zoom adds energy and intention to the clip, or whether it's just adding motion for the sake of it. A clip with both a zoom effect and cut-zoom enabled creates two competing framing techniques that can feel disorienting.
...
Text overlays:
  ...
  If the video already has burned-in captions (check the frame layout analysis), the screen is already carrying text throughout the video. Adding text overlays on top of existing captions creates visual clutter — the viewer's eye has to choose between reading the captions and reading your overlay. This doesn't mean you can't add any text, but weigh whether each overlay genuinely adds something the burned-in captions don't already provide. A video with captions already running has less room for additional text before it starts feeling crowded.
...
Vignette is a creative choice, not a default. Think about whether darkening the frame edges serves this specific footage — it can add focus and cinematic warmth to talking head content with busy backgrounds, but it can also feel heavy on content that's already well-composed or dark. Choose intentionally: "none" when it doesn't add value, and "light"/"medium"/"strong" only when it clearly improves focus and mood.
...

{
  ...
  "text_overlays": [
    { "text": "<text to display>", "position": "<top, center, or bottom>", "appear_at_clip": <clip number>, "style": "<title, callout, or cta>", "sfx_style": "<pop, ding, click, ching, or none>" }
  ],
  "broll": [
    { "keyword": "<visual search term>", "timestamp": <seconds>, "duration": <1-3> }
  ],
  "clips": [
    { "clip": 1, "transition_out": "<transition>", "transition_sound": "<sound or none>", "sfx_style": "<whoosh, impact, slide, or none>", "zoom": "<zoom or none>", "cut_zoom": <true or false>, "speed": <number> },
    { "clip": 2, "transition_out": "<transition>", "transition_sound": "<sound or none>", "sfx_style": "<whoosh, impact, slide, or none>", "zoom": "<zoom or none>", "cut_zoom": <true or false>, "speed": <number> }
  ]
}`;
}
