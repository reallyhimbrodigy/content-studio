...
Each clip in your recipe has these parameters:
  ...
  transition_sound — audio that plays during the transition:
    ...
    whoosh — fast air swoosh, pairs with lateral movement
    swoosh — softer version of whoosh, pairs with vertical movement
    pop — short bubble pop
    boom — low bass impact hit
    ding — bright chime tone
    rise — ascending pitch tone that builds anticipation
    cashier — cash register cha-ching sound

  sfx_style — sound accent on the clip itself:
    none, whoosh, impact, slide
  ...
  speed_segments — optional array of time ranges within the clip, each with its own speed. When present, these override the clip-level speed value. Start and end are in seconds relative to clip start (0 = beginning of clip). Segments should cover the full clip duration.
    Example: [{ "start": 0, "end": 3.5, "speed": 1.25 }, { "start": 3.5, "end": 5.8, "speed": 0.75 }]
...
Transition sounds on short-form: On phone speakers, short percussive sounds (pop, whoosh) cut through clearly. Longer tonal sounds (rise, ding) can get lost under speech. Boom is a low bass impact designed for subwoofers — on phone speakers which have minimal low-frequency response, it sounds thin rather than impactful. Cashier (cha-ching) works as a text overlay sound on money-related moments but as a transition sound between clips it doesn't match any visual movement pattern. Transition sounds are audible during the 0.3-second transition window — if the speaker is talking through the transition, the sound competes with the voice. The sound draws the viewer's ear to that specific transition, so it has more impact when used on one or two transitions per video rather than on every transition.
...

{
  ...
  "clips": [
    { "clip": 1, "transition_out": "<transition>", "transition_sound": "<sound>", "sfx_style": "<sfx>", "zoom": "<zoom>", "cut_zoom": <true|false>, "speed": <n>, "speed_segments": [] }
  ]
}`;
}
