...
function buildPrompt(analysis, transcript, vibe) {
  ...
  if (analysis.audio) {
    const parts = [];
    if (analysis.audio.overall_quality || analysis.audio.audio_quality) {
      parts.push(`Quality: ${analysis.audio.overall_quality || analysis.audio.audio_quality}`);
    }
    if (analysis.audio.music || (analysis.audio.has_music && analysis.audio.music_description)) {
      parts.push(`Music: ${analysis.audio.music || analysis.audio.music_description}`);
    }
  ...
  return `You are a professional social media video editor.

Client direction:
"${vibe}"

You do NOT choose numeric color values. Gemini has already measured baseline corrective color from the footage.
Your color task is ONLY to choose one color_intent word that best matches the client direction.
