...
async function generateEdit(analysis, transcript, vibe, onProgress) {
  ...
  // Pre-build clips from safe cut points
    ...
    preCutsBlock = `\nClips:\n` +
      preCuts.map((cut, i) => {
        ...
        const preview = clipSpeech.length > 80
          ...
          : clipSpeech;
        return `  Clip ${i + 1}: ${cut.source_start.toFixed(2)}s – ${cut.source_end.toFixed(2)}s${preview ? ` — "${preview}"` : ''}`;
      })
      .join('\n');
  }
...
The clips above are your edit structure — each one is a clean cut between natural pauses or scene changes. Your job is to choose a transition for each clip and set the color grade.

The client brief is your creative direction. Every decision you make should deliver what the client described.

If any clip is weak or repetitive, you can remove it. If you do, make sure the speech still flows naturally into the next clip.
...

{
  ...
  "strategy": "<what you kept, what you removed if anything, and your creative rationale>",
  "color_intent": "<one word from the list>",
  "cuts": [
    {
      "source_start": <exact start time of clip>,
      "source_end": <exact end time of clip>,
      "transition_out": "<transition from the list above>"
    }
  ]
}`;
}
