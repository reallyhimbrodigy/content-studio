...
/**
 * Expand vague user language into clear editorial intent.
 * This is not mapping to presets — Claude still makes every creative decision.
 * This just removes ambiguity so Claude understands what the human actually means.
 */
async function expandVibeIntent(vibe) {
  ...
  const response = await client.messages.create({
    ...
    messages: [
      {
        role: 'user',
        content: `You are translating a video editing client's brief into clear language for an editor.

The client said: "${vibe}"

Rewrite this as a clear, specific editorial direction in 1-2 sentences. Expand any vague words into what they actually mean in video editing context. Keep the same intent — just remove ambiguity.

Examples of how vague words translate in video editing:
- "simple" usually means smooth and polished, not minimal or absent
- "clean" usually means professional and refined
- "enhanced" usually means visibly improved, noticeably better
- "cinematic" usually means film-like color grading with intentional mood
- "fun" usually means energetic pacing with vibrant color
- "professional" usually means polished with consistent quality

Respond with ONLY the rewritten direction, nothing else.`
      }
    ]
  });
...
- "fade": a smooth crossfade where one clip blends into the next over 0.3s. Feels polished and intentional.
- "dissolve": a softer, slower blend. Feels dreamier or more reflective than a fade.
- "clean_cut": an instant hard switch between clips. No blending at all. Feels punchy and raw.
- "wipeleft" / "wiperight": one clip slides off screen revealing the next. Feels energetic and dynamic.

Effects:
- "slow_zoom_in": camera slowly pushes in over the clip duration. Adds movement to static shots.
- "slow_zoom_out": camera slowly pulls back. Creates a reveal or breathing feeling.
- "none": no camera movement added.

Color grade (what the viewer sees at different values):
...
- "warm": shifts everything toward golden/amber. "cool": shifts toward blue. "neutral": no shift.
