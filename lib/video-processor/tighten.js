...
function normalizeShotEditingValue(value) {
  return String(value || '').trim().toLowerCase();
}

function getOverlappingShots(shots, start, end) {
  const safeShots = Array.isArray(shots) ? shots : [];
  const s = toNum(start, 0);
  const e = toNum(end, s);
  if (e <= s) return [];
  return safeShots.filter((shot) => {
    const shotStart = toNum(shot?.start, 0);
    const shotEnd = toNum(shot?.end, shotStart);
    return shotEnd > s && shotStart < e;
  });
}

function hasNonFillerVisualValue(shots, start, end) {
  const overlaps = getOverlappingShots(shots, start, end);
  if (!overlaps.length) return false;
  return overlaps.some((shot) => normalizeShotEditingValue(shot?.editing_value) !== 'filler');
}
...
function tightenTranscript(deepgramWords, options = {}) {
  ...
  const words = (Array.isArray(deepgramWords) ? deepgramWords : [])
    ...
    .sort((a, b) => a.start - b.start);
  if (!words.length) {
    const hasKeepVisual = hasNonFillerVisualValue(shots, 0, originalDuration);
    if (originalDuration > 0 && hasKeepVisual) {
      return {
        segments: [{ start: 0, end: round3(originalDuration) }],
        removedSeconds: 0,
        fillerWords: [],
        deadAirCuts: [],
        deadAirRemovedSeconds: 0,
        deadAirTrimmedCount: 0,
        timeline_map: buildTimeMap([{ start: 0, end: round3(originalDuration) }]),
        tightened_duration: round3(originalDuration),
      };
    }
    return {
      segments: [],
  ...
  for (let i = 1; i < keepWords.length; i++) {
    ...
    if (gap > longNoSpeechSeconds && hasNonFillerVisualValue(shots, prevEnd, currStart)) {
      console.log(
        `[tighten] Kept long no-speech gap at ${prevEnd.toFixed(3)}s-${currStart.toFixed(3)}s `
        + `(${gap.toFixed(3)}s) due to non-filler visual content`
      );
      continue;
    }
  ...
  const firstWordStart = Math.max(0, keepWords[0].start - paddingSeconds);
  const lastWordEnd = keepWords[keepWords.length - 1].end + paddingSeconds;
  const sourceEnd = originalDuration > 0 ? originalDuration : lastWordEnd;

  const keepLeading = hasNonFillerVisualValue(shots, 0, firstWordStart);
  const keepTrailing = sourceEnd > lastWordEnd && hasNonFillerVisualValue(shots, lastWordEnd, sourceEnd);
  const first = keepLeading ? 0 : firstWordStart;
  const last = keepTrailing ? sourceEnd : lastWordEnd;
  ...
  const sourceWindowDuration = Math.max(0, last - first);
  const tightenedDuration = merged.reduce((sum, s) => sum + Math.max(0, s.end - s.start), 0);
  const removedSeconds = Math.max(0, round3(sourceWindowDuration - tightenedDuration));
