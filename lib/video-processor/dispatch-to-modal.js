...
async function dispatchJobToModal({
  ...
  (async () => {
    try {
      ...
      // HLS manifest URL — worker generates a 4-variant adaptive bitrate
      // ladder alongside the progressive MP4. AVPlayer prefers HLS for
      // instant first-segment playback. Manifest is served through the
      // same CDN as the MP4 (CloudFront) — unlike the signed MP4 URL,
      // HLS doesn't need re-signing because the master + segments are
      // public assets behind the CDN. Worker returns the public CDN
      // URL directly; we store it as-is.
      ...
      if (hlsManifestUrl) completionUpdate.hls_manifest_url = hlsManifestUrl;
