  ...
  (async () => {
    try {
      ...
      if (result && result.spawned === true && result.call_id) {
        ...
        // so the reaper — a separate sweep over DB rows — could not see it. The
        // stalls worth cancelling are exactly the ones that OUTLIVE this process
        // (a deploy drops the in-process pending map), where the closure is gone.
        //
        // result.modal_call_id, NOT progress_step: /api/video-jobs returns rows
        // with select('*'), so a field named progress_step reaches the client and
        // would surface an internal Modal handle to a user. NOT partial_state
        // either — the worker owns it for ask-back resume. `result` already
        // carries internal detail the client receives and ignores.
        // MERGED and status-guarded so a completion landing first is never
        // clobbered. Best-effort: a failed write must never block the dispatch.
        try {
          const { data: _cur } = await supabaseAdmin
            .from('video_jobs').select('result').eq('id', jobId).maybeSingle();
          const _merged = { ...((_cur && _cur.result) || {}), modal_call_id: callId };
          await supabaseAdmin
            .from('video_jobs')
            .update({ result: _merged })
            .eq('id', jobId)
