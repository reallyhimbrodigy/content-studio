...
const { registerPendingModalJob, settlePendingModalJob, stampDelivery,
        isDeliveredResult } = require('./modal-webhook');
...
const { repairCompletedRender, findRenderInS3 } = require('../completion-repair');
const { attachRenderToChat } = require('../chat-attach');
const { S3Client, ListObjectsV2Command } = require('@aws-sdk/client-s3');

// Lazy so a machine without S3 creds still loads this module (the recovery
// script's constraint, and the reason this cannot be a top-level client).
let _repairS3Client = null;
function repairS3Client() {
  if (!_repairS3Client) _repairS3Client = new S3Client({ region: s3.AWS_REGION });
  return _repairS3Client;
}

/**
 * EVIDENCE BEFORE FAILURE. Call immediately before any fallback terminalises a
 * job as failed. If the render exists in S3, the row merely lost its URL — the
 * user's video is done and calling it a failure is a lie that also refunds a
 * delivered render.
 *
 * [MEASURED 2026-08-11] 9 users / 32 (28%) in the clean cohort were failed this
 * way; S3 held a playable render for 10/10 probed, 7.5-42 MB. Returns true when
 * it repaired, meaning the caller must NOT write its failure and must NOT
 * refund. Never throws.
 */
async function deliveredNotFailed(jobId, userId, pushProgressToSSE) {
  try {
    const r = await repairCompletedRender({
      jobId, supabaseAdmin, s3, s3Client: repairS3Client(), ListObjectsV2Command,
    });
    if (!r.repaired) return false;
    if (typeof pushProgressToSSE === 'function') {
      pushProgressToSSE(jobId, {
        status: 'completed', progress: 100, step: 'complete',
        message: 'Your video is ready!', videoUrl: r.url, thumbnailUrl: null,
        final: true, error: null,
      });
    }
    sendLifecyclePush(supabaseAdmin, {
      jobId, userId, kind: 'completed', errorMessage: null, refunded: false,
    }).catch(() => {});
    return true;
  } catch (e) {
    // A repair that throws must never swallow the caller's terminal write.
    console.error(`[repair] threw for ${jobId} — falling through to failure:`, e && e.message);
    return false;
  }
}
...
// KEY BY SOURCE, NOT BY URL STRING (Zac 2026-08-04). The worker's prewarm cache
// keys on sha1(bucket/key) via _parse_aws_s3_url (which drops the ?query), so a
// hit is the same source by construction. This in-memory hint map used the RAW
// videoUrl string — so a presigned URL registered at intent and a re-signed (or
// raw) URL at dispatch, though the SAME object, missed the lookup because their
// X-Amz-Signature/Expires query params differ. Normalising to the source key
// (sourceKeyFromUrl uses new URL().pathname, which strips the query) makes the
// hint "same-source" instead of "same-string" — strictly more correct, and the
// only consumers of this map are register/await below (internal). Falls back to
// the raw string when the URL can't be parsed, i.e. never worse than today.
function _prewarmKey(videoUrl) {
  return sourceKeyFromUrl(videoUrl) || String(videoUrl || '');
}

function registerPrewarm(videoUrl, promise) {
  const key = _prewarmKey(videoUrl);
  const entry = { promise, firedAt: Date.now() };
  prewarmRegistry.set(key, entry);
  setTimeout(() => {
    if (prewarmRegistry.get(key) === entry) {
      prewarmRegistry.delete(key);
    }
  }, 5 * 60 * 1000);
}
 ...
 */
async function awaitPrewarmHint(videoUrl, outcome) {
  const entry = prewarmRegistry.get(_prewarmKey(videoUrl));
  // NEVER FIRED and TOO SLOW ARE DIFFERENT FAILURES, and this returned null for
  // both. The single log line downstream read `timeout/not-fired`, which is one
  // string covering two states that need OPPOSITE fixes: "never fired" means the
  // client did not call /api/prewarm (or the registry entry was evicted), so the
  // fix is upstream; "too slow" means it fired and did not settle inside the
  // wait, so the fix is the wait or the prewarm. 84 of these a day and nobody
  // could tell which. `outcome` is an optional out-param so both existing
  // callers keep their contract — the return value is unchanged.
  if (!entry) {
    if (outcome) { outcome.reason = 'not_fired'; }
    return null;
  }
  ...
  try {
    ...
    if (outcome) {
      // `firedAgoMs` is how long prewarm had ALREADY been running when dispatch
      // asked; `waitedMs` is what dispatch actually spent blocking. Both are
      // needed: a timeout at firedAgoMs=200 means prewarm is slow, a timeout at
      // firedAgoMs=200000 means it was never going to finish and the wait is
      // pure cost.
      outcome.reason = result === null ? 'timeout' : 'hit';
      outcome.firedAgoMs = elapsedMs;
      outcome.capMs = timeoutMs;
      outcome.waitedMs = Date.now() - waitStart;
    }
    return result;
  } catch (e) {
    if (outcome) { outcome.reason = 'threw'; outcome.error = e && e.message; }
    return null;
  }
}
...
async function markJobFailed(jobId, { errorCode, userMessage, userId, pushProgressToSSE } = {}) {
  ...
  // ── TWO DEFECTS IN THIS FUNCTION, FIXED TOGETHER (2026-08-18) ─────────────
  //
  // (1) NO DELIVERABLE GUARD. This guarded `.not(status,in,TERMINAL)` — it
  //     refused to overwrite a TERMINAL row — but a row sitting at progress=100
  //     with rendered_video_url ALREADY SET and status still 'processing' was
  //     fully writable. MEASURED: 114 jobs / 90 users since Aug 16 failed while
  //     holding a finished video, and were shown "Something went wrong."
  //
  //     The TypeError that triggered it this week is fixed in the worker, but
  //     THE GUARD IS THE REAL FIX: it makes the NEXT unknown post-render
  //     exception harmless instead of costing a user their result.
  //
  // (2) IT DESTROYED THE ENVELOPE. `result:` was assigned a fresh 3-key object,
  //     discarding stage_timings, the funnel, the design system and everything
  //     else the worker had written. terminalizeFailure MERGES instead, so a
  //     failure keeps its telemetry and stays diagnosable.
  //
  // terminalizeFailure re-reads the row inside the same call, so a deliverable
  // that lands between the caller's decision and this write is still caught —
  // that window is exactly where the interleaving lives. When it finds one it
  // COMPLETES the row rather than merely refusing, because refusing would strand
  // it non-terminal, which is the 900s reaper wall again.
  const { terminalizeFailure } = require('../terminal-invariant');
  let _existingResult = null;
  try {
    const { data: _pre } = await supabaseAdmin
      .from('video_jobs').select('result').eq('id', jobId).limit(1);
    _existingResult = (Array.isArray(_pre) && _pre[0]) ? _pre[0].result : null;
  } catch (_) { /* best effort — a read outage must not block the terminal */ }
  const _mergedResult = {
    ...(_existingResult && typeof _existingResult === 'object' ? _existingResult : {}),
    error_code: errorCode, user_message: msg, retryable: false,
  };
  const _outcome = await terminalizeFailure(supabaseAdmin, jobId, {
    status: 'failed',
    error_message: msg,
    result: _mergedResult,
  }, { terminalList: TERMINAL_SQL_LIST });
  if (_outcome && _outcome.outcome === 'healed') {
    // A finished video was about to be reported as a failure. Loud, and the
    // funnel/SSE below must NOT fire — the user's job SUCCEEDED.
    console.error(`[markJobFailed] HEALED job=${jobId} — it carried a deliverable; `
      + `completed instead of failing (would have been ${errorCode})`);
    return;
  }
 ...
 *   replyLanguage  validated BCP-47 code the assistant replies in ('en' omitted)
...
//   WORKER_DIED_AFTER_START — the worker WAS reached: it spawned, stamped
//                          worker_started_at, and then died. Nothing about
//                          dispatch was unreachable.
//
// THE THIRD CODE EXISTS BECAUSE THE OTHER TWO LIED (2026-08-16). A 10-hour
// failure class — ~50 jobs, 40 distinct users — was labelled
// DISPATCH_UNREACHABLE while 49/55 of those rows carried BOTH worker_started_at
// AND modal_call_id, and 48/55 died at step=plan. The worker was reached every
// time. The label named the wrong layer, the user was told "trouble reaching the
// render service", and every investigation it triggered started at the
// dispatcher — the one component that had done its job correctly.
//
// A message that names the wrong layer is not a cosmetic problem: it is a
// permanent misdirection for every future reader of these rows.
const DISPATCH_ERROR_CODES = ['RENDER_UNAVAILABLE', 'DISPATCH_UNREACHABLE',
                              'WORKER_DIED_AFTER_START'];

function dispatchFailureResult({ httpStatus = null, userMessage, detail,
                                workerReached = false }) {
  // workerReached is decided by EVIDENCE ON THE ROW (worker_started_at /
  // modal_call_id), never by the shape of the exception — an exception thrown in
  // the dispatcher looks identical whether or not a worker ever ran.
  const code = workerReached
    ? 'WORKER_DIED_AFTER_START'
    : (httpStatus === 404 ? 'RENDER_UNAVAILABLE' : 'DISPATCH_UNREACHABLE');
  return {
    error: code,
    error_code: code,
    error_class: workerReached ? 'worker' : 'dispatch',
    error_where: workerReached
      ? 'worker/after-start (dispatch succeeded; the worker died later)'
      : 'content-studio/dispatch-to-modal',
...
function respawnDecision(initial, recheck, artifactInS3 = false) {
  // AN ARTIFACT IN S3 DISQUALIFIES A RESPAWN, FULL STOP (2026-08-15).
  //
  // This is the first test because it is the only one that can be RIGHT when
  // every other signal says "dead". A render that produced a playable file
  // SUCCEEDED; whatever went wrong afterwards is a WRITE problem, and the answer
  // to a lost write is to project or repair it — never to render it again.
  //
  // It cost a live incident to learn. The post-upload watchdog killed hung
  // containers on the premise that the evidence-triggered repair would recover
  // the row from S3. Both the repair AND this function watch a non-terminal
  // row, and this one got there first: 294 fires across 49 jobs, up to 9 on one
  // job, each a full re-render. dbNonTerminal below is TRUE for a killed worker,
  // so without this guard the loop is structural rather than unlucky.
  //
  // It also closes the older hazard independently: `_delivered` reading the
  // wrong shape once left a delivered job one failed DB read from a respawn.
  // With this test first, an artifact makes that unreachable too.
  if (artifactInS3) return 'project';
  // rendered-but-write-lost: the worker DID complete, the write just landed
...
/**
 * Is a playable render already in S3 for this job? Never throws; on ANY doubt it
 * returns false, which preserves today's behaviour rather than silently
 * suppressing a legitimate respawn.
 */
async function artifactExistsInS3(jobId) {
  try {
    const picked = await findRenderInS3({
      jobId, s3, s3Client: repairS3Client(), ListObjectsV2Command,
    });
...
// ── Durable-row EARLY poll (lane/delivery 2026-08-10) ───────────────────────
// The worker's durable status layer (write_job_status) lands the terminal row —
// status + full result envelope — BEFORE it POSTs /api/modal-complete. When that
// POST misses (the Aug-3 secret-flip burst; the ~2/day drip), the ONLY recovery
// trigger used to be the 15-minute registerPendingModalJob timer: a job whose
// video existed at 205s settled at 901s, which IS the completed-jobs p99 wall.
// This poller reads the job's own row every POLL_MS while the dispatch awaits
// the callback, and settles the pending promise the moment the row goes
// terminal — the fallback latency drops from ~900s to ≤POLL_MS with no new
// writer: settle-once (the pending-map delete) makes it structurally race-proof
// against the callback/webhook/timer, and the tail that runs is the same tail.
// needs_input is deliberately NOT settled here: the ask-back envelope
// (clarification_question) rides the callback, and the resume rail owns that
// row — the poller must not degrade an ask-back into a failure.
// ── THE WALL ALIGNMENT (2026-08-16, RULE-1) ────────────────────────────────
// EVERY OUTER WALL MUST BE >= THE WORKER'S EXECUTION CEILING, AT EVERY INSTANT.
//
// This await was a 15-minute (900s) timer while run_pipeline_bg carries
// `timeout=1200, retries=0`. The dispatcher therefore declared failure 300
// SECONDS BEFORE MODAL WOULD EVEN KILL THE WORKER, and every terminalization
// clustered at 888-900s with ~4s spread across 8 samples — a TIMER, not work.
//
// ONE DEFECT, BOTH BUCKETS:
//   worker finishes between the old wall and the ceiling -> the artifact lands
//     in S3 while the row already says failed. That is EXACTLY the 34 rows /
//     29 users found stranded with a playable video and the copy "This render
//     hit our time limit".
//   worker does not finish by the ceiling -> Modal kills it, no artifact, and
//     the row reads "dispatch threw: spawned job did not complete".
//
// The reaper already obeys this rule — modal_app.py's own comment states it:
// "the content-studio reaper's EXEC_WALL_MS MUST be >= this at every moment ...
// or a healthy long render gets false-reaped mid-flight" (EXEC_WALL 3300s,
// processing lease 3000s, both well clear). THE DISPATCHER'S OWN WAIT WAS NEVER
// BROUGHT INTO THAT RULE. It is now, and a gate enforces it.
//
// ORDERING LAW, from this repo's own precedent: the OUTER wall rises FIRST and
// the inner ceiling only after, so wall >= ceiling holds at every instant. Any
// raise of run_pipeline_bg's timeout must therefore land HERE first.
const WORKER_EXECUTION_CEILING_S = 1200;   // run_pipeline_bg @app.function(timeout=)
const WALL_SLACK_S = 300;                  // Modal cold start + spawn queue + reaper interval
const SPAWN_AWAIT_MS = (WORKER_EXECUTION_CEILING_S + WALL_SLACK_S) * 1000;  // 1500s

const DURABLE_POLL_MS = 75_000;

// PROACTIVE REPAIR FIRES ON EVIDENCE, NOT ON THE CLOCK (2026-08-14) [Law 2].
//
// The repair path already existed and was already evidence-based — it asks S3
// whether the render is actually there — but it was only ever REACHABLE after a
// 15-minute timer had run out, so a job whose container died right after upload
// sat at a dead progress bar for the full 904s before anyone looked. The
// measured repair cohort settles at p50 904s: a timer, not a diagnosis.
//
// WHY "QUIET" IS A TRUSTWORTHY SIGNAL. The worker heartbeats progress every 4.0s
// (_start_progress_heartbeat, handler.py), and every worker patch sets
// updated_at unconditionally. So a row that has not been touched in
// QUIET_ROW_MS has missed ~30 consecutive heartbeats — that is a container that
// is GONE, not one that is merely slow. This is exactly why the check is on
// updated_at rather than on progress: a stalled-but-live render keeps updated_at
// fresh, and must NOT be repaired out from under itself.
//
// AND QUIET ALONE IS NEVER ENOUGH. Both conditions are required, because either
// one on its own is wrong: quiet without an artifact is a job that genuinely
// failed and must take the failure path, and an artifact without quiet is a live
// worker mid-upload that would be cut off. repairCompletedRender writes only
// when it FINDS a playable render, and its `.not(status,in,TERMINAL)` guard
// means racing the real completion tail is a no-op rather than a double-write.
//
// Detection lands at QUIET_ROW_MS + up to one poll interval (~120-195s, median
// ~157s) against the 904s wall. Not "instant" — stated honestly, because the
// poll cadence is the floor and pretending otherwise would misreport the fix.
const QUIET_ROW_MS = Number(process.env.QUIET_ROW_REPAIR_MS) || 120_000;

function startDurableCompletionPoll({ jobId, callId, intervalMs = DURABLE_POLL_MS,
                                     pushProgressToSSE = null }) {
  let stopped = false;
  let repairTried = false;   // one S3 probe per job, never a poll-rate S3 bill
  const timer = setInterval(async () => {
    if (stopped) return;
    try {
      const { data: row } = await supabaseAdmin
        .from('video_jobs')
        .select('status, result, error_message, rendered_video_url, result_url, updated_at, user_id')
        .eq('id', jobId)
        .maybeSingle();
      const dbStatus = String(row?.status || '').toLowerCase();
      if (!['completed', 'failed', 'canceled'].includes(dbStatus)) {
        // NON-TERMINAL. If it has also gone quiet, ask S3 whether the render
        // already exists — the one question that turns a 15-minute wait into a
        // delivery.
        if (repairTried || !row || !row.updated_at) return;
        const quietMs = Date.now() - new Date(row.updated_at).getTime();
        if (!(quietMs >= QUIET_ROW_MS)) return;
        repairTried = true;
        console.warn(`[repair] ${jobId} quiet ${Math.round(quietMs / 1000)}s at status=${dbStatus} `
          + '— probing S3 for a finished render rather than waiting out the 15-minute timer');
        const repaired = await deliveredNotFailed(jobId, row.user_id, pushProgressToSSE);
        if (!repaired) {
          console.log(`[repair] ${jobId} quiet but NO render in S3 — leaving it to the normal `
            + 'failure path; a quiet row with no artifact is a real failure, not a lost URL');
          return;
        }
        // Settle the dispatcher's pending promise so the tail runs NOW instead
        // of at the timeout. Without this the repair would fix the row and the
        // caller would still sit on the 15-minute wall — the fix would be
        // invisible in exactly the number it exists to move.
        const settled = settlePendingModalJob({
          id: callId,
          status: 'COMPLETED',
          output: {
            ...carryWorkerResult((row && row.result) || {}),
            status: 'success',
            job_id: jobId,
            video_url: null,      // the repair wrote the row's URL directly
            hls_manifest_url: null,
          },
          via: 'repair',
        });
        console.log(`[repair] ${jobId} REPAIRED from S3 evidence after ${Math.round(quietMs / 1000)}s `
          + `quiet (settled=${settled}) — 15-minute timer bypassed`);
        return;
      }
      const r = (row && row.result) || {};
      if (dbStatus === 'completed') {
        const anyVideoUrl = r.video_url || r.public_url || r.rendered_video_url
          || row.rendered_video_url || row.result_url;
        const settled = settlePendingModalJob({
          id: callId,
          status: 'COMPLETED',
          output: {
        ...
        if (settled) {
          console.log(`[dispatch] ${jobId} durable row went terminal (completed) before any callback — settled via durable_poll`);
        }
      } else if (dbStatus === 'failed' && (r.error || r.error_code)) {
        // Worker-coded failure: settle RESOLVED with the envelope — the exact
        // shape the /api/modal-complete callback delivers — so the tail's
        // result.error branch runs (honest copy, refund, failure push) instead
        // of the generic outer catch. Minus ~13 minutes of dead progress bar.
        const settled = settlePendingModalJob({
          id: callId,
          status: 'COMPLETED',
          output: { ...carryWorkerResult(r), job_id: jobId },
          via: 'durable_poll',
        });
        if (settled) {
          console.log(`[dispatch] ${jobId} durable row went terminal (failed, coded ${r.error_code || r.error}) before any callback — settled via durable_poll`);
        }
      } else if (dbStatus === 'canceled') {
        const settled = settlePendingModalJob({
          id: callId,
          status: 'FAILED',
          error: row?.error_message || 'job canceled',
          via: 'durable_poll',
        });
        if (settled) {
          console.log(`[dispatch] ${jobId} durable row went terminal (canceled) before any callback — rejected via durable_poll`);
        }
      }
      // failed WITHOUT a coded envelope (drained/reaper shapes) is left to the
      // 15-min timer's respawnDecision — the poller must not preempt re-spawn.
    } catch (_) { /* poll is best-effort; the timer net is untouched */ }
  }, intervalMs);
  return () => { stopped = true; clearInterval(timer); };
}

// Persist WHICH mechanism delivered the completion — the queryable half of the
// stampDelivery marker. A dedicated column (not a result-jsonb merge) because
// result is a multi-writer read-merge-write surface (worker terminal write,
// lifecycle-push claims) and a merge here could interleave with the push-claim
// merge and clobber it. Best-effort + migration-tolerant like persistModalCallId:
// before 20260810_completion_delivery.sql runs this soft-fails silently.
async function persistCompletionDelivery(jobId, via) {
  if (!jobId || !via) return;
  try {
    // WON vs LOST IS NOW VISIBLE [Law 2]. First-stamp-wins means a losing path
    // wrote nothing and left NO trace, so "callback stamped and lost the race"
    // and "callback never ran at all" were the same observation — and the board
    // read 442 reconciler / 0 callback out of 465 with no way to tell which.
    // .select() makes the attempt countable: a 0-row result is a LOSS, and a
    // path that never appears here at all never ran.
    const { data, error } = await supabaseAdmin
      .from('video_jobs')
      .update({ completion_delivery: String(via) })
      .eq('id', jobId)
      .is('completion_delivery', null)
      .select('id');
    if (!error) {
      const won = Array.isArray(data) && data.length > 0;
      console.log(`[delivery] job=${jobId} via=${via} ${won ? 'WON' : 'LOST'}`);
      // Only the LOSS needs a durable row — a win is already in the column.
      if (!won) {
        supabaseAdmin.from('analytics_events').insert({
          event: 'delivery_stamp_lost', platform: 'server',
          props: { job_id: jobId, via: String(via) },
        }).then(() => {}).catch(() => {});
      }
    }
    if (error && !/completion_delivery/.test(error.message || '')) {
      console.warn(`[dispatch] completion_delivery persist soft-failed job=${jobId}: ${error.message}`);
    }
  } catch (e) {
    console.warn(`[dispatch] completion_delivery persist threw job=${jobId}: ${e && e.message}`);
  }
}
...
async function dispatchJobToModal({
  ...
  // ── PERSIST THE PRODUCER STAMP ──────────────────────────────────────────
  // analyze-video mints {name, version, model, measured_at} on every analysis it
  // parses, "so any consumer can identify which producer/version/model minted
  // this blob rather than inferring it from field-shape". Nothing persisted it:
  // analysis_data is NULL on all 4,728 jobs in 30 days, because that column is
  // only written when the WORKER returns one and the worker never does. The
  // server's own analysis goes out as `cached_analysis` and is then dropped.
  //
  // The cost was immediate: after re-pinning the analyze model off an alias,
  // there was no way to confirm on real traffic which model produced any given
  // analysis, or to tell later which jobs predate the pin.
  //
  // ITS OWN COLUMN, deliberately not analysis_data. A stamp-only object there
  // would be fed back into a re-edit AS the parent's analysis (server.js:7367),
  // which is presence-where-shape-was-needed one more time.
  //
  // Fire-and-forget: provenance must never gate a render.
  //
  // ── THIS CANNOT FIRE TODAY, AND THAT IS THE FINDING ────────────────────────
  // `cachedAnalysis` comes from video_analysis_cache, which has been cold for 61
  // days: 485 rows, newest 2026-07-12, ZERO in the last 30. The only writer is
  // triggerPreAnalysis, reached only from POST /api/upload — the legacy upload
  // route the app abandoned when uploads moved to direct-S3. So the cache is
  // always a MISS, `cachedAnalysis` is always null, and this branch is never
  // taken. Measured: 6 jobs after deploy, 0 stamped.
  //
  // It is left here deliberately rather than deleted. The branch is correct for
  // the moment a server-side analysis exists again, and the CHECK constraint on
  // analysis_producer means a malformed stamp cannot land even then. What it
  // must not do is read as working: a write nothing reaches is the inert half of
  // a pair, and this file now says which half it is.
  //
  // The real provenance question — which model produced the analysis behind a
  // render — is answered in the WORKER, which does the analysis and pins
  // concrete models. Persisting it belongs there, not here.
  if (cachedAnalysis && typeof cachedAnalysis === 'object' && cachedAnalysis.producer) {
    supabaseAdmin.from('video_jobs')
      .update({ analysis_producer: cachedAnalysis.producer })
      .eq('id', jobId)
      .then(({ error }) => {
        if (error) console.error(`[dispatch] analysis_producer write failed for ${jobId}: ${error.message}`);
      }, () => {});
  }
  ...
  // ── DISPATCH-PRESIGN (2026-08-23) ────────────────────────────────────────
  //
  // The worker downloads whatever string lands in payload.video_url. Today that
  // is the PUBLIC CDN url, which is the only reason sources/ has to stay
  // world-readable — 9,306 raw user videos across 577 users, permanently
  // fetchable by anyone holding a link. Minting a time-limited grant here is
  // what makes restricting the prefix possible at all (POSTURE_S3_PREFIXES.md).
  //
  // SAFE IN ALL FOUR STATES, which is why it lands before the key pair id is
  // fixed rather than after:
  //   sources public  + signing broken -> S3 presign, works
  //   sources public  + signing fixed  -> CF signed, unrestricted path ignores
  //                                       the signature, works
  //   sources private + signing broken -> S3 presign, works (S3 presign does
  //                                       not depend on CloudFront at all)
  //   sources private + signing fixed  -> CF signed against the key group, works
  //
  // IDEMPOTENT ON RE-DISPATCH. sourceKeyFromUrl parses via new URL().pathname,
  // which strips the query, so re-signing an already-signed url yields the same
  // key rather than nesting credentials. Respawns and resumes therefore get a
  // FRESH grant instead of inheriting an expiring one.
  //
  // `videoUrl` itself is deliberately NOT reassigned: it is the cache key for
  // video_analysis_cache and the prewarm registry key, and rewriting it would
  // turn every cache hit into a miss.
  //
  // 7 days = the SigV4 maximum, matching the upload presign. A job can sit
  // queued behind a long backlog or be respawned hours later; anything shorter
  // reintroduces the UPLOAD_NEVER_STARTED class on the download side.
  const SOURCE_GRANT_S = 604800;
  let dispatchVideoUrl = videoUrl;
  let dispatchProxyUrl = proxyVideoUrl;
  if (s3.isConfigured()) {
    for (const [label, src, set] of [
      ['source', videoUrl, (v) => { dispatchVideoUrl = v; }],
      ['proxy', proxyVideoUrl, (v) => { dispatchProxyUrl = v; }],
    ]) {
      if (!src) continue;
      const k = sourceKeyFromUrl(src);
      // A non-S3 url (legacy Supabase Storage) yields no usable key — leave it
      // alone rather than minting a grant against a bucket it does not live in.
      if (!k) continue;
      try {
        set(await s3.createPresignedGetUrl(k, SOURCE_GRANT_S));
      } catch (e) {
        // FAIL OPEN, LOUDLY. While sources/ is still public the original url
        // works, so a mint failure must not kill a render. Once the prefix is
        // restricted this line becomes the failure that matters, so it is
        // logged with the key and never swallowed.
        console.error(`[dispatch] ${label} presign FAILED for ${jobId} key=${k}: ${e && e.message}`
          + ' — falling back to the stored url (works only while the prefix is public)');
      }
    }
  }

  // Detection reads the SAME grant the worker gets. It was previously handed
  // the raw stored url, and detectBurnedText downloads over HTTP — so once
  // sources/ is restricted that fetch 403s. It is fail-open, which is the
  // dangerous part: burned-text detection would silently stop working and
  // every render would still succeed, with captions mis-placed over existing
  // burned-in text and nothing anywhere saying why.
  const detectionUrl = dispatchProxyUrl || dispatchVideoUrl;
  ...
  if (useS3) {
    // WRITES-FORWARD (2026-08-24, POSTURE_S3_PREFIXES.md §6). New renders land
    // on a CDN-RESTRICTED prefix and are delivered by grant. The legacy
    // `renders/` prefix stays unrestricted and ages out with the fleet: 6,200
    // permanent links are already in the hands of 5,121 users, and restricting
    // that prefix would 403 all of them at once.
    //
    // SAFE ONLY BECAUSE THE DELIVERY SIDE LANDED FIRST. Every completion write
    // now routes through lib/deliverable-url.js, so rendered_video_url is a
    // grant rather than the bare public url. Moving the prefix before that would
    // have written urls that 403 with nothing reporting it.
    //
    // `publicUrl` below stays a bare url for this key. It is an IDENTIFIER the
    // worker echoes back, never a source anything fetches (checked), and the
    // completion write grants it before it reaches a client.
    //
    // thumbnails/ deliberately unchanged — public by decision, because social
    // crawlers cache og:image and re-fetch later, so a short TTL breaks link
    // previews in iMessage and Slack.
    s3OutputKey = `renders-private/${jobId}/${timestamp}-edited.mp4`;
  ...
  if (resolvedMode === 'full') {
    ...
    const hintOutcome = {};
    const hintResult = prewarmHintResult !== undefined
      ? prewarmHintResult
      : await awaitPrewarmHint(videoUrl, hintOutcome);
    if (hintResult && !hintResult.error) {
      ...
      // ONE LINE PER STATE, each naming where its fix lives.
      if (hintOutcome.reason === 'not_fired') {
        console.log(`[dispatch] prewarm NOT_FIRED for ${jobId} — no registry entry; `
          + 'the client never called /api/prewarm for this source, or the entry was '
          + 'evicted. Nothing was waited for. Fix is upstream of dispatch.');
      } else if (hintOutcome.reason === 'timeout') {
        console.log(`[dispatch] prewarm HINT_TIMEOUT for ${jobId} — fired `
          + `${hintOutcome.firedAgoMs}ms ago, dispatch blocked ${hintOutcome.waitedMs}ms `
          + `of a ${hintOutcome.capMs}ms cap and gave up. This wait is pure added `
          + 'latency: fix is the cap or the prewarm, not the client.');
      } else if (hintOutcome.reason === 'threw') {
        console.log(`[dispatch] prewarm HINT_ERROR for ${jobId} — ${hintOutcome.error}`);
      } else {
        // prewarmHintResult was passed in as null by the caller, so no await
        // happened here at all. Distinct again: no latency was spent.
        console.log(`[dispatch] prewarm HINT_ABSENT for ${jobId} — resolved upstream, `
          + 'no wait spent at dispatch.');
      }
    }
  }
  ...
  const payload = {
    ...
    ...(dispatchProxyUrl ? { proxy_video_url: dispatchProxyUrl } : {}),
    vibe,
    user_id: userId,
    ...
    // reply_language — the language the ASSISTANT answers in. The worker's
    // generate_plan_diff reads it (handler.py _parse_reply_language) and writes
    // human_summary / clarification_question in it. Forwarded ONLY now that the
    // worker has a reader: forwarding a field nothing reads is the inert state
    // this whole change exists to fix.
    //
    // It governs the REPLY, never the CONTENT — captions and the transcript
    // follow the clip's own language, and the worker's prompt says so.
    ...(replyLanguage && replyLanguage !== 'en' ? { reply_language: replyLanguage } : {}),
  ...
  // answer, so an S3 blip can never reject a real upload.
  {
    ...
    const _pres = await waitForSource(_srcKey, {
      ...
      // DEAD-UPLOAD FAST-FAIL: let the wait abort early when the CLIENT has
      // already reported this upload dead, instead of burning the full budget
      // on bytes that are not coming. Consulted only while the object is
      // absent, so a slow-but-working upload is never cut short.
      jobId,
      since: new Date(_t0 - 60_000).toISOString(),
    });
  ...
  (async () => {
    try {
      ...
      if (!res.ok) {
        ...
        // EVIDENCE FIRST: a non-ok dispatch response says nothing about whether
        // the render finished. It routinely HAS (the response is lost, not the
        // work). Ask S3 before calling a delivered video a failure.
        if (await deliveredNotFailed(jobId, userId, pushProgressToSSE)) return;
      ...
      if (result && result.spawned === true && result.call_id) {
        ...
        // Early durable-row poll: settles this await the moment the worker's
        // durable terminal write lands, so a missed callback costs ≤75s, not
        // the 15-min timer (the completed-jobs p99 wall). Settle-once safe.
        const stopEarlyPoll = startDurableCompletionPoll({ jobId, callId, pushProgressToSSE });
        try {
          result = await registerPendingModalJob(callId, {
            timeoutMs: SPAWN_AWAIT_MS,  // >= worker ceiling; see THE WALL ALIGNMENT
        ...
        // per undelivered job.
        // SHAPE-CORRECT as of 2026-08-14 — see isDeliveredResult's contract in
        // modal-webhook.js. This predicate used to test the ENVELOPE shape only,
        // while registerPendingModalJob resolves the OUTPUT shape, so every
        // callback/durable_poll/fallback_timer delivery read as undelivered:
        // it hid three of the five delivery values behind the reconciler and left
        // a delivered job one failed DB read from a double render.
        const _delivered = isDeliveredResult(result);
        if (!_respawned && !_delivered) {
          const recheck = await resolveSpawnedCompletionFallback({ jobId, callId });
          const _artifact = await artifactExistsInS3(jobId);
          if (_artifact) {
            console.warn(`[dispatch] ${jobId} has a playable render in S3 — a respawn is `
              + 'DISQUALIFIED regardless of row state; projecting instead (never render twice)');
          }
          const decision = respawnDecision(result, recheck, _artifact);
          if (decision === 'project') {
            ...
            if (recheck && recheck.output) stampDelivery(recheck.output, 'reconciler');
            result = recheck;
            ...
            try {
              ...
              if (r2 && r2.spawned === true && r2.call_id) {
                ...
                const stopEarlyPoll2 = startDurableCompletionPoll({ jobId, callId: callId2, pushProgressToSSE });
                try {
                  result = await registerPendingModalJob(callId2, {
                    timeoutMs: SPAWN_AWAIT_MS,
      ...
      if (result?.error) {
        ...
        const { data: failWonRows } = await supabaseAdmin
          ...
          .not('status', 'in', TERMINAL_SQL_LIST)
          .select('id');
        persistCompletionDelivery(jobId,
          (result && (result.completion_delivery
            || (result.output && result.output.completion_delivery))) || 'sync');
      ...
      // BELT AND BRACES, and it covers the branch above NOT running. All three
      // rendered_video_url writes below read this one variable, so granting here
      // covers them at a single point instead of three. Idempotent: a url that
      // is already a grant re-derives the same key (the query is stripped) and
      // gets a fresh one, never nested credentials.
      //
      // NOTE the comment above is stale: it asserts public CloudFront urls 403
      // on renders/, but renders/ measured 200 unauthenticated on 2026-08-23.
      // Filed, not chased — this line makes the claim true rather than aspirational.
      videoUrl = await require('../deliverable-url')
        .toDeliverableUrl(videoUrl, { label: 'dispatch-completion' });
      ...
      // WHICH mechanism delivered this completion (stampDelivery). The settled
      // output carries the stamp on the spawn path; the sync inline path has
      // none → 'sync'. Fire-and-forget; first-stamp-wins at the DB.
      persistCompletionDelivery(jobId,
        (result && (result.completion_delivery
          || (result.output && result.output.completion_delivery))) || 'sync');
      ...
      // SERVER-AUTHORITATIVE CHAT MEMBERSHIP (SERVER_CHAT_ATTACH_SPEC §3).
      // 498 completed videos across 441 users sit in NO chat — 140 in the last
      // 7 days — because the message that references a render is written by the
      // CLIENT as a debounced PATCH that a backgrounded session simply drops.
      // The job row knows its own id and owner, so membership does not need the
      // client to participate. Fire-and-forget: a chat write must never sit
      // between a finished render and its delivery. The 10-minute sweep in
      // server.js is the backstop for everything this tail misses — and it
      // misses plenty, which is the entire reason completion-reconcile exists.
      Promise.resolve(
        attachRenderToChat(supabaseAdmin, {
          id: jobId,
          user_id: userId,
          rendered_video_url: videoUrl,
          hls_manifest_url: hlsManifestUrl || null,
          thumbnail_url: thumbnailUrl || null,
          // created_at / vibe_input are NOT in this scope and are needed only to
          // RECONSTRUCT a chat. attachRenderToChat fetches them itself, on that
          // path only — so the common case costs no extra read, and this call
          // site never references an identifier that does not exist here.
        }),
      ).catch((e) => console.warn('[chat-attach] inline attach failed (non-fatal):',
        e && e.message));
      ...
      // DID A WORKER ACTUALLY RUN? Decided by EVIDENCE ON THE ROW, never by the
      // shape of the exception — a dispatcher throw looks identical whether or
      // not a worker ever started, which is exactly how ~50 jobs across 40 users
      // were labelled "unreachable" while 49/55 carried worker_started_at AND
      // modal_call_id and 48/55 died at step=plan.
      let _workerReached = false;
      try {
        const { data: _wr } = await supabaseAdmin
          .from('video_jobs')
          .select('worker_started_at, modal_call_id')
          .eq('id', jobId)
          .maybeSingle();
        _workerReached = Boolean(_wr && (_wr.worker_started_at || _wr.modal_call_id));
      } catch (_) {
        _workerReached = false;   // on doubt, keep today's label rather than invent one
      }
      const dispatchCopy = dispatchErrorMessage(_workerReached);
      // EVIDENCE FIRST — this is the exact path that failed 9 users in one
      // evening. The fetch threw or hit the 960s backstop; that is a statement
      // about the RESPONSE, never about the render. S3 is the arbiter.
      if (await deliveredNotFailed(jobId, userId, pushProgressToSSE)) return;
      const { data: threwWonRows } = await supabaseAdmin
        ...
        .update({
          ...
          result: dispatchFailureResult({
            ...
            detail: `${_workerReached ? 'worker started then died' : 'dispatch threw'}: `
              + String(err && err.message).slice(0, 200),
            workerReached: _workerReached,
          }),
          updated_at: new Date().toISOString(),
        })
        .eq('id', jobId)
...
// The one worker endpoint we do NOT proxy: /validate is called by the app
// itself, so it sent no `_worker_auth` and 11 of 11 real calls arrived missing
// — an unauthenticated GPU endpoint anyone can bill us for. The client now
// carries the field, and this is the value it is handed, on the authenticated
// /api/usage response. Lives here, beside workerAuthField, so the field NAME
// has one definition: the app must spell it exactly as the server does or the
// worker's _require_worker_auth will not find it.
//
// MODAL_VALIDATE_SECRET FIRST, and it should be a DIFFERENT value from
// MODAL_RUN_SECRET. Anything the app can send, someone can extract — that is
// unavoidable for a client-called endpoint, and the point is to stop anonymous
// use, not to make it unforgeable. But sharing run_job's secret means an
// extraction also buys the ability to dispatch arbitrary GPU renders, which is
// a far larger bill than validate. The fallback exists only so the client half
// can ship before the separate secret is provisioned.
//
// Returns null when neither is set, so the field is OMITTED rather than sent
// empty — an empty string is a value the worker would have to special-case.
const WORKER_AUTH_FIELD = '_worker_auth';
function clientValidateAuth() {
  // Trimmed PER CANDIDATE, not after the ||. A whitespace-only
  // MODAL_VALIDATE_SECRET is truthy, so trimming the winner made it beat a
  // perfectly good MODAL_RUN_SECRET and then resolve to null — the endpoint
  // would read as armed and be handed nothing.
  const candidates = [process.env.MODAL_VALIDATE_SECRET, process.env.MODAL_RUN_SECRET];
...
module.exports = {
  dispatchJobToModal, registerPrewarm, workerAuthField, clientValidateAuth, WORKER_AUTH_FIELD,
  ...
  startDurableCompletionPoll, persistCompletionDelivery,
};
