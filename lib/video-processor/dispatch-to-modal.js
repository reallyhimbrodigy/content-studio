...
const { dispatchErrorMessage, clarificationMessage, renderTooShortMessage, rejectionCopy, coreErrorMessage } = require('../failure-copy');
...
// Back-compat wrapper — the render-lifecycle push mirror rides funnelEvent now.
function logPushEvent(userId, type, jobId, r) {
  funnelEvent(userId, 'push_sent', { type, job_id: jobId, sent: (r && r.sent) || 0, skipped: (r && r.skipped) || null });
}
const {
...
async function dispatchJobToModal({
  ...
  const fetchModalWithRetry = async () => {
    ...
    // ceiling. Re-firing the latter just burns ANOTHER ~900s — this is exactly
    // the 15min→31min long-hang compound seen in prod (10 cases, all "trouble
    // reaching the render service" that were really wedged renders). So a slow
    // failure is TERMINAL here: surfaced as a coded, user-retryable
    // RENDER_TIMEOUT (capped at one ceiling) instead of silently re-run.
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      // another ~900s). Tag it so the caller surfaces the honest, user-retryable
      // RENDER_TIMEOUT rather than the transient DISPATCH_UNREACHABLE line.
      if (failedSlow) {
        const err = new Error(`Modal render timed out for ${jobId} after ${Math.round((Date.now() - startedAt) / 1000)}s`);
  ...
  (async () => {
    try {
      ...
      // Two shapes of dispatch failure, two honest messages:
      //  • isRenderTimeout — the render STARTED but wedged past Modal's ceiling
      //    and was killed. NOT an unreachable service. Coded + user-retryable so
      //    the failure screen shows the one-tap retry (a fresh dispatch may land
      //    on a healthy GPU). This is the 15–31min long-hang class, now capped
      //    at a SINGLE attempt (~one ceiling) instead of compounding × retries.
      //  • otherwise — a genuine fast-failing reach error (the 3 quick retries
      //    already burned through in seconds): the transient DISPATCH_UNREACHABLE
      //    line, no coded envelope.
      ...
      // above; the row + SSE get clean copy.
      const renderTimedOut = err?.isRenderTimeout === true;
      const failCopy = renderTimedOut ? rejectionCopy('RENDER_TIMEOUT') : dispatchErrorMessage();
      const rowUpdate = {
        status: 'failed',
        error_message: failCopy,
        updated_at: new Date().toISOString(),
      };
      if (renderTimedOut) {
        // Coded envelope so both delivery paths (SSE + the Supabase reconcile
        // poll, which reads result.error_code) classify this as a retryable
        // failure and the app offers the retry affordance instead of a dead end.
        rowUpdate.result = { error_code: 'RENDER_TIMEOUT', retryable: true };
      }
      await supabaseAdmin
        .from('video_jobs')
        .update(rowUpdate)
        .eq('id', jobId)
        .not('status', 'in', TERMINAL_SQL_LIST);
      await refundJobInline(jobId, renderTimedOut ? 'render-timeout' : 'dispatch-threw');
      if (renderTimedOut && userId) {
        funnelEvent(userId, 'render_failed', { job_id: jobId, error_code: 'RENDER_TIMEOUT' });
      }
      if (typeof pushProgressToSSE === 'function') {
        pushProgressToSSE(jobId, {
          status: 'failed',
          ...
          thumbnailUrl: null,
          final: true,
          error: renderTimedOut ? 'RENDER_TIMEOUT' : failCopy,
          ...(renderTimedOut
            ? { error_code: 'RENDER_TIMEOUT', user_message: failCopy, retryable: true }
            : {}),
        });
      }
      if (userId) {
