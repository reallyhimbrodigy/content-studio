...
// finished and NEITHER side overwrites it. Covers the WORKER's authoritative
// durable vocab (complete/canceled/needs_input, written with result + phase via
// its monotonic write_job_status) AND the app/client vocab (completed/cancelled/
// needs_clarification) + failed. The app must never respell/clobber a terminal
// status the worker set (that silently drops its write-once result/phase and
// breaks monitoring queries) and must never resurrect a user's cancel.
const TERMINAL_STATUSES = [
  'complete', 'completed', 'failed', 'canceled', 'cancelled',
...
// columns (URLs) must not land on it (don't resurrect a cancelled render). A
// worker 'complete'/'completed' is "soft" for owned columns — the URLs still
// need to be written — but hard for the status itself.
const HARD_TERMINAL_SQL_LIST = '(failed,canceled,cancelled)';
  ...
  (async () => {
    try {
      ...
      if (result?.error) {
        ...
        const userMsg = typeof result.user_message === 'string' && result.user_message
          ? result.user_message
          : null;
      ...
      if (result?.analysis_data) ownedUpdate.analysis_data = result.analysis_data;
      ...
      if (userId) {
        ...
        let body = 'Tap to try again.';
        if (raw.includes('timeout') || raw.includes('timed out')) {
          body = 'Render timed out. Tap to try again.';
        } else if (raw.includes('hls') || raw.includes('manifest')) {
          body = 'Couldn\u2019t finish the stream files. Tap to try again.';
        } else if (raw.includes('s3') || raw.includes('upload')) {
          body = 'Upload failed. Check your connection and try again.';
        } else if (raw.includes('gemini') || raw.includes('quota') || raw.includes('rate limit')) {
          body = 'The AI hit a rate limit. Try again in a minute.';
        }
