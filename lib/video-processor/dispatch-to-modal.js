  ...
  (async () => {
    try {
      ...
      if (result && result.spawned === true && result.call_id) {
        ...
        // RE-SPAWN ONCE (Zac 2026-08-02, dispatch-loss mitigation): a spawn that
        // was ACCEPTED but whose worker NEVER STARTED — the fallback timed out
        // with an EMPTY worker envelope (no stage_timings, no coded error, i.e.
        // the worker wrote nothing) — has lost nothing, so a single fresh spawn
        // costs one more dispatch and saves the whole "worker never runs" class
        // (the drain/cold-start race). Fires exactly ONCE (a second empty timeout
        // terminalizes) so a genuinely down platform cannot loop; a WEDGED render
        // (any worker output present) never re-spawns.
        if (!_respawned && workerNeverStarted(result)) {
          _respawned = true;
          console.warn(`[dispatch] ${jobId} spawn ${callId} never started a worker (empty envelope) — RE-SPAWNING once`);
          try {
...
module.exports = {
  ...
  resolveSpawnedCompletionFallback, carryWorkerResult, workerNeverStarted, DISPATCH_TRANSPORT_KEYS,
