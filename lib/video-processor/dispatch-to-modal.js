...
async function markJobFailed(jobId, { errorCode, userMessage, userId, pushProgressToSSE } = {}) {
  ...
  try {
    const { data: _pre } = await supabaseAdmin
      .from('video_jobs').select('id,result,credits_debited,credits_refunded_at')
      .eq('id', jobId).limit(1);
    _jobRow = (Array.isArray(_pre) && _pre[0]) ? _pre[0] : null;
    _existingResult = _jobRow ? _jobRow.result : null;
  ...
  // ── THE REFUND COMMITS WITH THE FAILURE, NOT A MINUTE LATER ────────────
  //
  // refund-leg sweeps failed jobs every 60s and that sweep works — but it
  // cannot close the WINDOW. The row was marked `failed` first and refunded up
  // to a minute later, so for that minute the client reads a failure with the
  // charge standing. A sweep is the right backstop and the wrong primary: the
  // user is looking at the screen during exactly that minute.
  //
  // So: return the credits FIRST, then write status='failed' and
  // credits_refunded_at in ONE UPDATE. A single statement touching both
  // columns is atomic by definition — no stored procedure, and no migration
  // standing between the fix and production.
  //
  // AND IF THE REFUND FAILS, THE ROW IS NOT MARKED. The client cannot see
  // failed-without-refunded because it cannot see failed at all yet; the job
  // stays in flight, which is true, the alert fires, and refund-leg retries.
  // Marking it failed and hoping the sweep catches up IS the silent charge.
  const _fwr = require('../fail-with-refund');
  const _credits = require('../credits');
  const _fw = await _fwr.failWithRefund(_jobRow || { id: jobId }, {
    refund: async (amount) => {
      const r = await _credits.credit(userId, amount);
      if (r && r.ok === false) throw new Error(r.error || 'credit refused');
      return r;
    },
    terminalize: (patch) => terminalizeFailure(supabaseAdmin, jobId, {
      status: 'failed',
      error_message: msg,
    ...
    alert: async (title, body) => {
      try {
        ...
        await fetch(`${base}/api/internal/ops-alert`, {
          method: 'POST',
          headers: { 'content-type': 'application/json',
                     'x-modal-secret': process.env.MODAL_CALLBACK_SECRET || '' },
          body: JSON.stringify({ title, body, thread_id: 'refund' }),
          signal: AbortSignal.timeout(8000),
        });
      } catch (e) { console.error('[fail-with-refund] alert failed:', e && e.message); }
    },
  });
  if (!_fw.marked) {
    // NOT TERMINAL, DELIBERATELY. Returning here skips the funnel event and the
    // SSE push below — telling the client "failed" is the one thing we must not
    // do while the charge stands.
    console.error(`[markJobFailed] HELD job=${jobId} — ${_fw.state}; not marked failed `
      + 'because the credits are not back yet.');
    return;
  }
  const _outcome = _fw.outcome;
