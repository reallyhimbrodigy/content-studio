  ...
  (async () => {
    try {
      ...
      if (result && result.spawned === true && result.call_id) {
        ...
        // was gone entirely.
        //
        // Stored in result.modal_call_id — NOT progress_step (rows go to the
        // client via select('*'), and a field named progress_step could surface
        // an internal handle to a user) and NOT partial_state (the worker owns
        // it for ask-back resume). `result` already carries internal detail the
        // client receives and ignores. MERGED, never overwritten: a completion
        // that lands first must not be clobbered by this write.
        ...
        try {
          ...
          await supabaseAdmin
            ...
            .eq('id', jobId)
            .in('status', ['queued', 'pending', 'processing']);  // never touch a settled job
        } catch (err) {
