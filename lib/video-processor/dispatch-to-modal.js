...
function noteDispatchSuccess() { _dispatchFails.length = 0; }  // any success clears the window
function noteDispatchFailure(status) {
  const now = Date.now();
  _dispatchFails.push(now);
  while (_dispatchFails.length && _dispatchFails[0] < now - _DF_WINDOW_MS) _dispatchFails.shift();
  ...
  console.error(`[ALERT] DISPATCH-LAYER OUTAGE — ${body}`);
  ...
  const fetchModalWithRetry = async () => {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (attempt < maxAttempts) {
        // COLD-START-RACE WIDENING (Zac 2026-07-26): the render worker scales to
        // zero at low traffic; a dispatch to a cold worker gets a FAST router 502
        // during the ~15-30s cold-start race. The old 2s/4s backoff fired all 3
        // attempts within ~6s — before the cold start finished — so a run of
        // cold-race 502s exhausted the retries and failed the job ("trouble
        // reaching the render service"; 10/10 such failures measured on cold
        // workers). Widen so the 3 attempts SPAN the cold-start window
        // (~0s / ~12s / ~34s): the last attempt lands after the container is
        // ready and returns 2xx. This adds NO new attempts and does NOT touch
        // healthy jobs — a 2xx on attempt 1 never reaches this backoff; only a
        // failing attempt waits. (A slow/wedged failure is already terminal via
        // failedSlow above, so widening never prolongs a real render hang.)
        const backoffMs = attempt === 1 ? 12000 : 22000;
  ...
  (async () => {
    try {
      ...
      if (!res.ok) {
        ...
        noteDispatchFailure(res.status);
