...
function startDurableCompletionPoll({ jobId, callId, intervalMs = DURABLE_POLL_MS }) {
  ...
  const timer = setInterval(async () => {
    ...
    try {
      ...
      if (!['completed', 'failed', 'canceled'].includes(dbStatus)) return;
  ...
  (async () => {
    try {
      ...
      if (result && result.spawned === true && result.call_id) {
        ...
        const stopEarlyPoll = startDurableCompletionPoll({ jobId, callId });
        try {
          ...
          if (decision === 'project') {
            ...
            try {
              ...
              if (r2 && r2.spawned === true && r2.call_id) {
                ...
                const stopEarlyPoll2 = startDurableCompletionPoll({ jobId, callId: callId2 });
                try {
