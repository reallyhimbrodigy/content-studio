...
const { detectBurnedText, mergeBandsIntoAnalysis, hasCurrentDetection } = require('./burned-text-detector');
...
async function triggerPreAnalysis(videoUrl) {
  ...
  const promise = analyzeVideo(videoUrl)
    .then(async (analysis) => {
      // Enrich the (real) Gemini analysis with deterministic burned-text detection
      // before caching, so a warmed cache row carries NUMERIC caption bands — not
      // just Gemini's unreliable boolean. Fail-open: on any detector issue we cache
      // the Gemini analysis alone rather than block the warm.
      let toCache = analysis;
      try {
        if (!hasCurrentDetection(analysis)) {
          const r = await detectBurnedText(videoUrl, { source: 'source', duration: Number(analysis?.duration) || undefined });
          if (r && r.detection) {
            toCache = mergeBandsIntoAnalysis(analysis, r.detection, r.meta);
            console.log(`[pre-analyze] detection: has_burned_captions=${r.detection.has_burned_captions} bands=${r.detection.text_bands.length}`);
          }
        }
      } catch (e) {
        console.warn('[pre-analyze] detection enrich failed (caching Gemini-only):', e.message);
      }
      console.log('[pre-analyze] Analysis complete, writing to cache...');
      const { error: upsertError } = await supabaseAdmin
        .from('video_analysis_cache')
        .upsert({ video_url: videoUrl, analysis: toCache }, { onConflict: 'video_url' });
      if (upsertError) {
