...
function getShotstackRenderUrl() {
  const env = String(process.env.SHOTSTACK_ENV || 'production').toLowerCase();
  const isProduction = ['production', 'prod', 'live', 'v1'].includes(env);
  const url = isProduction ? 'https://api.shotstack.io/v1/render' : 'https://api.shotstack.io/stage/render';
  console.log('[Shotstack] Using environment:', isProduction ? 'production' : 'sandbox');
  console.log('[Shotstack] API base URL:', url);
  return url;
}
...
function buildShotstackTimeline(clipAnalysis, editRecipe, videoUrl, sfxAssets = []) {
  ...
  const animations = Array.isArray(editRecipe.animations) ? editRecipe.animations : [];
  ...
  const sourceWidth = Number(clipAnalysis?.dimensions?.width);
  const sourceHeight = Number(clipAnalysis?.dimensions?.height);
  const hasDimensions = Number.isFinite(sourceWidth) && Number.isFinite(sourceHeight) && sourceWidth > 0 && sourceHeight > 0;
  const isPortrait = hasDimensions ? sourceHeight > sourceWidth : false;
  const outputAspectRatio = isPortrait ? '9:16' : '16:9';

  if (cuts.length === 0) {
    throw new Error('buildShotstackTimeline: editRecipe.cuts is empty');
  }
  ...
  const videoClips = cuts.map((cut, index) => {
    ...
    const cutStart = Number.isFinite(Number(cut?.start))
      ? Number(cut.start)
      : Number(cut?.sourceStart) || 0;
    const cutDuration = Math.max(
      0.05,
      Number(cut?.duration) || Math.max(0.05, (Number(cut?.end) || cutStart + 0.05) - cutStart),
    );
    const timelineStart = Number.isFinite(Number(cut.timelineStart))
      ? Number(cut.timelineStart)
      : Number(cumulativeStart.toFixed(3));
    const cutEnd = cutStart + cutDuration;
    const cutAnimations = animations.filter((a) => {
      const ts = Number(a?.timestamp);
      const start = Number(a?.start);
      return (Number.isFinite(ts) && ts >= cutStart && ts <= cutEnd)
        || (Number.isFinite(start) && start >= cutStart && start <= cutEnd);
    });
    const zoomIn = cutAnimations.find((a) => String(a?.type || '').toLowerCase() === 'zoom_in');
    const scale = Number(zoomIn?.scale);
    const safeScale = Number.isFinite(scale) ? Math.max(1.0, Math.min(1.5, scale)) : 1.0;
    ...
    const clip = {
      asset: {
        ...
        src: videoUrl,
        trim: cutStart,
        volume: Number.isFinite(Number(cut.volume)) ? Number(cut.volume) : 1.0,
        crop: {
          top: 0,
          bottom: 0,
          left: 0,
    ...
    cumulativeStart += cutDuration;
    return clip;
  });

  console.log('[buildShotstackTimeline] Video clip conversion preview', videoClips.slice(0, 5).map((clip, index) => ({
    index,
  ...
  console.log('[buildShotstackTimeline] Timeline built', {
    ...
    outputAspectRatio,
    sourceWidth: hasDimensions ? sourceWidth : null,
    sourceHeight: hasDimensions ? sourceHeight : null,
  });
