...
async function submitVideo(videoUrl) {
  ...
  const url = `https://api.twelvelabs.io/v1.3/indexes/${indexId}/videos`;
  const payload = {
    video_url: videoUrl,
    transcription: false
  };
...
async function waitForAnalysis(taskId, onProgress) {
  ...
  while (attempts < maxAttempts) {
    ...
    const indexId = process.env.TWELVE_LABS_INDEX_ID;
    const url = `https://api.twelvelabs.io/v1.3/indexes/${indexId}/videos/${taskId}`;
...
async function getAnalysis(taskId) {
  ...
  // Get video details
  const videoUrl = `https://api.twelvelabs.io/v1.3/indexes/${indexId}/videos/${taskId}`;
