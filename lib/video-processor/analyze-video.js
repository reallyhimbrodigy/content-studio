...
function buildAnalysisPrompt() {
  return `You are watching this video for a professional editor who cannot see it. Give them a precise timing map so they can place every cut, choose transitions, and grade color correctly.

CRITICAL: All timestamps must be in SECONDS (not minutes, not frames). A 39-second video has duration 39, not 0.65. A moment at the 15-second mark is 15.0, not 0.25. Timestamps must be precise to 0.01 seconds. If your timestamps are off by even a tenth of a second, the edit will land mid-word or clip a breath.

Return ONLY valid JSON:

{
  "duration": <total seconds — e.g. 39 for a 39-second video, NOT 0.65>,

  "shots": [
    {
      "start": <seconds from start of video>,
      "end": <seconds from start of video>,
  ...
  "speech": {
    ...
    "segments": [
      {
        "start": <seconds to 0.01s>,
        "end": <seconds to 0.01s>,
        "text": "<exact words>",
    ...
    "sentence_boundaries": [
      {
        "time": <seconds to 0.01s — the exact moment the last word ends>,
...
Provide as many cut points as possible. Prioritize timestamp accuracy on speech above everything else.

REMINDER: All numeric timestamps are in SECONDS. duration is in SECONDS.`;
}
...
function normalizeTimestamps(parsed) {
  ...
  if (Array.isArray(parsed.shots)) {
    for (const shot of parsed.shots) {
      if (typeof shot.start === 'number') allTimestamps.push(shot.start);
      if (typeof shot.end === 'number') allTimestamps.push(shot.end);
    }
  }

  if (Array.isArray(parsed.cut_points)) {
    for (const cp of parsed.cut_points) {
      if (typeof cp.time === 'number') allTimestamps.push(cp.time);
    }
  }
  ...
  const needsConversion = maxTimestamp < 2.0 && allTimestamps.length > 2;

  if (!needsConversion) return parsed;
  ...
  if (typeof fixed.duration === 'number') fixed.duration *= 60;

  if (Array.isArray(fixed.shots)) {
    for (const shot of fixed.shots) {
      if (typeof shot.start === 'number') shot.start *= 60;
      if (typeof shot.end === 'number') shot.end *= 60;
    }
  }

  if (fixed.speech?.segments && Array.isArray(fixed.speech.segments)) {
    for (const seg of fixed.speech.segments) {
      if (typeof seg.start === 'number') seg.start *= 60;
      if (typeof seg.end === 'number') seg.end *= 60;
    }
  }
  ...
  if (fixed.speech?.sentence_boundaries && Array.isArray(fixed.speech.sentence_boundaries)) {
    for (const sb of fixed.speech.sentence_boundaries) {
      if (typeof sb.time === 'number') sb.time *= 60;
      if (typeof sb.end_time === 'number') sb.end_time *= 60;
      if (typeof sb.pause_after === 'number' && sb.pause_after < 0.1) sb.pause_after *= 60;
      if (typeof sb.pause_duration === 'number' && sb.pause_duration < 0.1) sb.pause_duration *= 60;
    }
  }

  if (Array.isArray(fixed.cut_points)) {
    for (const cp of fixed.cut_points) {
      if (typeof cp.time === 'number') cp.time *= 60;
    }
  }
  ...
  if (Array.isArray(fixed.highlights)) {
    for (const h of fixed.highlights) {
      if (typeof h.time === 'number') h.time *= 60;
    }
  }
  ...
  console.log(`[analyze] Converted timestamps: duration ${parsed.duration?.toFixed(3)} → ${fixed.duration?.toFixed(1)}s`);
