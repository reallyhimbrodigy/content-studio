...
// Trim paste-whitespace/newline (Render dashboard) — a trailing \n in the header
// value 401s at Gemini exactly like a wrong key. No-op on a clean value.
const GEMINI_API_KEY = (process.env.GEMINI_API_KEY || '').trim();
const GEMINI_UPLOAD_URL = 'https://generativelanguage.googleapis.com/upload/v1beta/files';
const GEMINI_FILES_URL = 'https://generativelanguage.googleapis.com/v1beta/files';
// PINNED TO A CONCRETE MODEL, NOT AN ALIAS (2026-09-11).
//
// This rode `gemini-flash-latest`, and that alias has already cost this project
// a day: 91ff645 found it had rotated the CHAT path onto a model with ZERO
// provisioned quota — 1.87K 429s in a day, 100% of requests, with no other
// error class present. An alias is a value Google may change without telling
// us, and this one is on the analysis of EVERY render on the live pipeline.
//
// Verified before moving, with a real generateContent on the PRODUCTION
// credential via /api/internal/gemini-diag — not a models.list entry, because
// listing and calling are different questions and the chat outage was a 429 on
// a model that listed perfectly well:
//   gemini-3.6-flash        200
//   gemini-flash-latest     200   (works today; the risk is that it moves)
//   gemini-2.0-flash        404
//   gemini-2.5-flash-latest 404
//   gemini-2.0-flash-001    404
// Two of the three models this file's own history has used now 404. That is the
// rotation rate the alias exposes us to.
//
// Same value the chat path is pinned to, so one model carries both surfaces and
// a future move is one decision rather than two. tests/analyze-stamp.test.js
// pins this, so a silent change is impossible in either direction.
//
// ── CORRECTION, SAME DAY ────────────────────────────────────────────────────
// The commit that made this change said it protected "every render's analysis
// on the live pipeline". THAT WAS WRONG, and I should have checked reachability
// before asserting it rather than after.
//
// This module has exactly two entry points and both are dead in production:
//   triggerPreAnalysis  <- POST /api/upload, the legacy upload route the app
//                          abandoned when uploads moved to direct-S3
//   processEditJob      <- /api/cron/process-jobs, which is not among the two
//                          crons render.yaml defines
// The evidence is in the data, not the reasoning: video_analysis_cache has 485
// rows, ZERO in the last 30 days, and its newest row is 2026-07-12 — 61 days
// cold, which is when uploads moved. Nothing has written it since.
//
// The analysis that actually runs happens in the Modal worker, on CONCRETE
// models (gemini-3.7-flash and friends) — the worker rides no alias at all, so
// the exposure this pin was urgent about did not exist on the live path.
//
// The pin STAYS, because it is correct and costs nothing: if this path is ever
// revived, it revives on a concrete model instead of one Google can rotate. But
// it was not the emergency it was described as, and the record should say so.
const GEMINI_MODEL = 'gemini-3.6-flash';
const GEMINI_GENERATE_URL = `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent`;
const GEMINI_GENERATE_TIMEOUT_MS = 180_000;

// ── Producer identity stamp ──────────────────────────────────────────────
// Every analysis_data blob this module mints carries a positive-ID stamp
// (producer + version + model + measured_at) at the single mint point
// (parseAnalysisResponse). Downstream consumers — especially the worker's
// intake, which today discriminates echoes by signature-matching a frozen
// corpus — get positive identification instead of a fragile shape guess.
// One field, cheap, permanent. Bump ANALYZER_VERSION on any schema/prompt
// change so a blob's provenance is always unambiguous.
const ANALYZER_PRODUCER = 'promptly.app-server.analyze-video';
const ANALYZER_VERSION = '2026-07-11';
...
async function uploadToGemini(buffer, mimeType, displayName) {
  const initResponse = await axios.post(
    ...
    {
      headers: {
        // AQ keys must use the header, not ?key=. Do not send both.
...
async function uploadVideoToGemini(localPath, onProgress) {
  if (!GEMINI_API_KEY) {
    throw new Error('GEMINI_API_KEY is not set');
  }
  if (!localPath || !fs.existsSync(localPath)) {
    throw new Error(`Local video path not found: ${localPath}`);
  }
  ...
  return { fileResource, mimeType };
}

async function analyzeWithGeminiFile(geminiUpload, onProgress) {
  if (!geminiUpload?.fileResource || !geminiUpload?.mimeType) {
    throw new Error('analyzeWithGeminiFile requires { fileResource, mimeType }');
  }
  let activeFile = geminiUpload.fileResource;
  if (activeFile.state !== 'ACTIVE') {
    if (onProgress) onProgress({ step: 'analyzing', detail: 'Waiting for video processing...' });
    activeFile = await waitForFileActive(activeFile.name, onProgress);
  }
  console.log('[pipeline] Gemini file active, starting generateContent');
  ...
  let rawResponse = await promptGemini(activeFile.uri, geminiUpload.mimeType);
...
function parseAnalysisResponse(text) {
  ...
  const rawOverlays = rawFrameLayout?.existing_overlays || {};
  const existingOverlays = {
    has_burned_captions: Boolean(rawOverlays.has_burned_captions),
    has_text_graphics: Boolean(rawOverlays.has_text_graphics),
    overlay_locations: typeof rawOverlays.overlay_locations === 'string'
      ? rawOverlays.overlay_locations
      : 'none detected',
  };
  // PASSTHROUGH (W3 detector): the deterministic burned-text detector merges
  // NUMERIC bands + a real measurement into existing_overlays. This choke point
  // rebuilds existing_overlays, so without preserving them a re-parse of an
  // enriched blob would silently STRIP text_bands / caption_zone_coverage /
  // detector. Carry them through when present.
  if (Array.isArray(rawOverlays.text_bands)) existingOverlays.text_bands = rawOverlays.text_bands;
  if (typeof rawOverlays.caption_zone_coverage === 'number') existingOverlays.caption_zone_coverage = rawOverlays.caption_zone_coverage;
  if (rawOverlays.detector && typeof rawOverlays.detector === 'object') existingOverlays.detector = rawOverlays.detector;
  const frameLayout = {
    subject_position: typeof rawFrameLayout.subject_position === 'string'
      ? rawFrameLayout.subject_position
      : 'unknown',
    existing_overlays: existingOverlays,
  ...
  return {
    ...
    // Positive-ID stamp — see ANALYZER_PRODUCER note. Lets any consumer identify
    // which producer/version/model minted this blob and when it was measured,
    // rather than inferring it from field-shape.
    producer: {
      name: ANALYZER_PRODUCER,
...
async function analyzeVideo(videoUrl, onProgress) {
  ...
  const fileResource = await uploadToGemini(buffer, mimeType, fileName);
  buffer = null;
  return analyzeWithGeminiFile({ fileResource, mimeType }, onProgress);
}

module.exports = {
  analyzeVideo, uploadVideoToGemini, analyzeWithGeminiFile,
  // exported for the stamp unit test + downstream provenance checks
