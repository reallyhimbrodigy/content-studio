...
async function processVideoJob({ videoUrl, vibeInput, jobId, userId, onProgress }) {
  ...
  try {
    ...
    // Start b-roll prep as soon as transcript is ready (parallel with Gemini).
    brollTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), `promptly-broll-${jobId}-`));
    const brollAssetsPromise = transcriptPromise.then(async (transcriptData) => {
      const words = Array.isArray(transcriptData?.words) ? transcriptData.words : [];
      const keywordCandidates = extractBrollKeywords(words, 8);
      if (!keywordCandidates.length) return [];

      console.log(`[broll] Keyword candidates: ${keywordCandidates.map((k) => k.keyword).join(', ')}`);
      const selected = keywordCandidates.slice(0, 3);
      const assets = await Promise.all(selected.map(async (candidate, idx) => {
        const clipInfo = await fetchBrollClip(candidate.keyword);
        if (!clipInfo) return null;
        const localPath = path.join(brollTmpDir, `broll_${idx}.mp4`);
      ...
      console.log(`[broll] Download pipeline failed: ${err.message}`);
      return [];
    });

    // Stage 1 + 2: Run Gemini, Deepgram, source download, and scdet in parallel.
    const [analysis, transcript, downloadedPath, visualCuts, brollAssets] = await Promise.all([
      analysisPromise,
      transcriptPromise,
      downloadPromise,
      scdetPromise,
    ...
    if (tightenResult.removedSeconds >= 1.0 && Array.isArray(tightenResult.segments) && tightenResult.segments.length > 1) {
      const tightenCutPoints = [{ time: 0, quality: 1, why: 'Video start' }];
      for (const seg of tightenResult.segments) {
        tightenCutPoints.push({ time: round3(seg.end), quality: 0.95, why: 'tightened segment end' });
      }
    ...
    if (Array.isArray(brollAssets) && brollAssets.length > 0) {
      analysis.broll_candidates = brollAssets.map((a) => ({
        keyword: a.keyword,
        timestamp: a.timestamp,
      }));
      console.log(`[broll] Ready assets: ${brollAssets.map((a) => a.keyword).join(', ')}`);
    }

    const editPlan = await generateEdit(analysis, transcript, vibeInput, onProgress);
    editPlan.analysis_data = analysis;
    editPlan.broll_assets = Array.isArray(brollAssets) ? brollAssets : [];
