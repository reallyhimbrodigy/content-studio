...
async function processVideoJob({ videoUrl, vibeInput, jobId, userId, onProgress }) {
  ...
  try {
    ...
    const [analysis, transcript, downloadedPath, visualCuts] = await Promise.all([
    ...
    if (userId) {
      const { data: profile } = await supabaseAdmin
        .from('profiles')
        .select('subscription_status, stripe_subscription_status, subscription_plan, plan, tier, is_pro, isPro, pro, paid')
        .eq('id', userId)
        .maybeSingle();

      const status = String(profile?.subscription_status || profile?.stripe_subscription_status || '').toLowerCase().trim();
      const plan = String(profile?.subscription_plan || profile?.plan || profile?.tier || '').toLowerCase().trim();
      const isProFlag = Boolean(profile?.is_pro || profile?.isPro || profile?.pro || profile?.paid);
      const statusPro = status === 'active' || status === 'trialing';
      const planPro = plan === 'pro' || plan === 'teams' || plan === 'paid' || plan === 'premium';
      addWatermark = !(isProFlag || (statusPro && (planPro || !plan)));
    }
