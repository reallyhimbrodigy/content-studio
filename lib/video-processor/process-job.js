...
async function processEditJob(job) {
  ...
  console.log(`\n${'='.repeat(80)}`);
  console.log('STARTING VIDEO JOB PROCESSING (DB PIPELINE)');
  console.log('='.repeat(80));
  console.log('Job ID:', job.id);
  console.log('Video URL:', job.video_url);
  console.log('Vibe Input:', job.vibe_input);
  console.log('='.repeat(80));
  ...
  try {
    ...
    console.log('[processEditJob] Edit recipe details', {
      cutCount: editRecipe?.cuts?.length || 0,
      transitionCount: editRecipe?.transitions?.length || 0,
      captionCount: editRecipe?.captions?.length || 0,
      animationCount: editRecipe?.animations?.length || 0,
      sfxCount: editRecipe?.sfxPlacements?.length || 0,
      duration: editRecipe?.metadata?.duration || clipAnalysis?.duration || null,
    });
    if (!Array.isArray(editRecipe?.cuts) || editRecipe.cuts.length === 0) {
      throw new Error('processEditJob: edit recipe generation failed - no cuts produced');
    }
    ...
    const trackClipCounts = (timeline?.timeline?.tracks || []).map((track) => track?.clips?.length || 0);
    const totalClips = trackClipCounts.reduce((sum, count) => sum + count, 0);
    const videoClips = timeline?.timeline?.tracks?.[0]?.clips || [];
    console.log('[processEditJob] Timeline details', {
      trackCount: timeline?.timeline?.tracks?.length || 0,
      trackClipCounts,
      totalClips,
      preview: videoClips.slice(0, 5).map((clip, index) => ({
        index,
    ...
    if (totalClips === 0 || videoClips.length === 0) {
      throw new Error('processEditJob: timeline building failed - no clips created');
    }
...
async function processVideoJob(input) {
  ...
  console.log('Total duration:', editRecipe?.metadata?.duration || clipAnalysis?.duration || 'unknown');
  console.log('Has transitions:', Array.isArray(editRecipe?.transitions) && editRecipe.transitions.length > 0);
  console.log('Has effects/animations:', Array.isArray(editRecipe?.animations) && editRecipe.animations.length > 0);
  console.log('Full Edit Recipe:');
  console.log(JSON.stringify(editRecipe, null, 2));
  console.log('='.repeat(80));

  if (!Array.isArray(editRecipe?.cuts) || editRecipe.cuts.length === 0) {
    console.error('❌ CRITICAL ERROR: Edit recipe has NO CUTS');
    throw new Error('processVideoJob: edit recipe generation failed - no cuts produced');
  }
  ...
  console.log(`\n${'='.repeat(80)}`);
  console.log('SHOTSTACK TIMELINE BUILT');
  console.log('='.repeat(80));
  ...
  const totalClips = (timeline?.timeline?.tracks || []).reduce(
    (sum, track) => sum + (track?.clips?.length || 0),
    0,
  );
  if (totalClips === 0 || firstTrack.length === 0) {
    console.error('❌ CRITICAL ERROR: Timeline has NO VIDEO CLIPS');
    throw new Error('processVideoJob: timeline building failed - no clips created');
  }
  console.log(`✓ Timeline has ${totalClips} total clips across ${timeline?.timeline?.tracks?.length || 0} tracks`);
