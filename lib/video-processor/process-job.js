const { uploadVideoToGemini, analyzeWithGeminiFile } = require('./analyze-video');
...
const { triggerPreAnalysis, inFlightAnalysis } = require('./pre-analyze');
  ...
  try {
    ...
    try {
      ...
      if (cacheRow?.analysis) {
        cachedAnalysis = cacheRow.analysis;
        console.log('[pipeline] Gemini cache HIT — skipping analysis');
      } else if (inFlightAnalysis.has(videoUrl)) {
        console.log('[pipeline] Gemini analysis in flight — awaiting existing promise');
        cachedAnalysis = await inFlightAnalysis.get(videoUrl);
        if (cachedAnalysis) {
          console.log('[pipeline] Gemini in-flight analysis resolved successfully');
        } else {
          console.log('[pipeline] Gemini in-flight analysis failed — running fresh');
        }
      } else {
        console.log('[pipeline] Gemini cache MISS — running analysis');
      }
    } catch (err) {
      console.warn('[pipeline] Cache lookup failed, running analysis:', err.message);
    }
    ...
    const geminiFileUploadPromise = cachedAnalysis
      ? Promise.resolve(null)
      : (async () => {
        console.log('[pipeline] Starting Gemini file upload...');
        const localPath = await downloadPromise;
        const fileHandle = await uploadVideoToGemini(localPath, onProgress);
        console.log('[pipeline] Gemini file upload complete, file handle ready');
        return fileHandle;
      })();
    ...
    const analysisPromise = cachedAnalysis
      ...
      : (async () => {
        ...
        try {
          const geminiFileHandle = await geminiFileUploadPromise;
          const analysisResult = await withTimeout(
            analyzeWithGeminiFile(geminiFileHandle, onProgress),
            240_000,
            'Gemini analysis'
          );
          console.log(
    ...
    const transcriptPromise = (async () => {
      ...
      try {
        ...
        const transcriptResultRaw = await withTimeout(
          transcribeIfNeeded(audioPath, null, onProgress),
          240_000,
          'Transcription'
        );
        const transcriptResult = transcriptResultRaw || { text: '', words: [] };
        const speechFromDeepgram = buildSpeechFromDeepgram(transcriptResult, 0);
        transcriptResult.sentence_boundaries = speechFromDeepgram?.speech?.sentence_boundaries || [];
        console.log(
    ...
    // Start keyframe encode as soon as scdet + transcription are ready so it overlaps
    // the rest of the parallel wave and the tighten/Claude stages.
    let keyframeFinishedAt = 0;
    const keyframePromise = Promise.all([normalizedSourcePromise, transcriptPromise])
      .then(([normalizedPath, transcriptForKeyframes]) => {
        const speechTimestamps = (Array.isArray(transcriptForKeyframes?.sentence_boundaries)
          ? transcriptForKeyframes.sentence_boundaries
          : [])
          .map((b) => toNum(b?.time, NaN))
          .filter((t) => Number.isFinite(t) && t >= 0);
        const allTimestamps = [...new Set([...speechTimestamps])]
          .sort((a, b) => a - b);
        console.log(`[pipeline] Starting keyframe encode from Deepgram timestamps only (${allTimestamps.length} points)`);
        return createKeyframedSource(normalizedPath, allTimestamps, path.dirname(normalizedPath));
      })
      .then((keyframedPath) => {
        keyframeFinishedAt = Date.now();
    ...
    let claudeFinishedAt = 0;
    const claudePromise = withTimeout(
      generateEdit(analysis, transcript, vibeInput, onProgress, expandedVibe, sceneFrames),
      240_000,
      'Claude edit recipe generation'
    ).then((result) => {
      claudeFinishedAt = Date.now();
      return result;
    });
    const [editPlan, keyframedPath] = await Promise.all([claudePromise, keyframePromise]);
    const waitedAfterClaudeMs = claudeFinishedAt > 0 && keyframeFinishedAt > 0
      ? Math.max(0, keyframeFinishedAt - claudeFinishedAt)
      : 0;
    console.log(`[pipeline] Keyframe encode complete, waited ${waitedAfterClaudeMs}ms after Claude finished`);
    console.log('[pipeline] Claude edit recipe complete');
    gcHint('edit generation');

    // Pre-split source clips from the edit plan
    const preSplitClipFiles = await (async () => {
    ...
    supabaseAdmin
      ...
      .delete()
      .eq('video_url', videoUrl)
      .then(() => console.log('[pipeline] Cleared pre-analysis cache'))
      .catch(err => console.warn('[pipeline] Cache cleanup failed:', err.message));

    return {
      // Grant, not a permanent link — see lib/deliverable-url.js. renderVideo
      // returns whatever the render produced (often the bare public url), and a
      // stored bare url is what makes the render prefix unrestrictable.
      rendered_video_url: await require('../deliverable-url')
        .toDeliverableUrl(finalVideoUrl, { label: 'process-job' }),
