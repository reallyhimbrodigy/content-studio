...
async function processVideoJob({ videoUrl, vibeInput, jobId, userId, onProgress }) {
  ...
  try {
    ...
    // Build a superset of likely cut boundaries and start keyframe encode now,
    // so it runs in parallel with the Claude edit recipe call.
    const keyframeTimestampSet = new Set();
    const addKeyframeTs = (value) => {
      const t = round3(toNum(value, NaN));
      if (!Number.isFinite(t)) return;
      if (t < 0) return;
      keyframeTimestampSet.add(t);
    };
    addKeyframeTs(0);
    addKeyframeTs(analysis.duration);
    for (const t of Array.isArray(visualCuts) ? visualCuts : []) addKeyframeTs(t);
    for (const b of Array.isArray(analysis?.speech?.sentence_boundaries) ? analysis.speech.sentence_boundaries : []) {
      addKeyframeTs(b?.time);
    }
    for (const seg of Array.isArray(tightenResult?.segments) ? tightenResult.segments : []) {
      addKeyframeTs(seg?.start);
      addKeyframeTs(seg?.end);
    }
    const keyframeTimestamps = [...keyframeTimestampSet]
      .filter((t) => Number.isFinite(t))
      .sort((a, b) => a - b);
    console.log(`[pipeline] Starting keyframe encode in parallel with Claude (${keyframeTimestamps.length} candidate timestamps)`);
    const keyframePromise = createKeyframedSource(
      preDownloadedPath,
      keyframeTimestamps,
