...
function probeDurationSync(filePath) {
  try {
    const value = execSync(
      `ffprobe -v error -show_entries format=duration -of csv=p=0 "${String(filePath || '').replace(/"/g, '\\"')}"`
    ).toString().trim();
  ...
  try {
    const downloadPromise = (async () => {
      ...
      try {
        ...
        console.error('[pipeline] FAILED at source pre-download:', err.message, err.stack);
        throw err;
      }
    })();
    ...
    console.log('[pipeline] Persisting analysis_data to Supabase...');
    const { error: analysisPersistError } = await supabaseAdmin
      .from('video_jobs')
      .update({ analysis_data: analysis })
      .eq('id', jobId);
    if (analysisPersistError) {
      throw new Error(`Failed to persist analysis_data: ${analysisPersistError.message}`);
    }
    console.log('[pipeline] analysis_data persisted');
    ...
    const editPlan = await withTimeout(
      generateEdit(analysis, transcript, vibeInput, onProgress, expandedVibe),
      240_000,
    ...
    if (Array.isArray(editPlan.cuts) && Array.isArray(preSplitClipFiles) && preSplitClipFiles.length > 0) {
      ...
      for (let i = 0; i < editPlan.cuts.length; i++) {
        ...
        const actual = probeDurationSync(preSplitClipFiles[i]);
        const actualSafe = Number.isFinite(actual) ? actual : nominal;
        const delta = actualSafe - nominal;
        console.log(
          `[pipeline]   Clip ${i}: nominal=${nominal.toFixed(3)}s, actual=${actualSafe.toFixed(3)}s, `
