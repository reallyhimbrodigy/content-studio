...
function detectSceneCuts(videoPath, threshold = 8) {
  // Run FFmpeg scdet filter — outputs scene change scores per frame.
  // threshold: lower = more sensitive (default 8 catches jump cuts and scene changes)
  const cmd = `ffmpeg -i "${videoPath}" -vf "scdet=threshold=${threshold}" -f null - 2>&1`;
...
async function processVideoJob({ videoUrl, vibeInput, jobId, onProgress }) {
  ...
  try {
    ...
    // Download source once in parallel so FFmpeg/scdet can reuse it.
    const [analysis, transcript, downloadedPath] = await Promise.all([
    ...
    // Detect visual cuts using FFmpeg (frame-accurate)
    const visualCuts = detectSceneCuts(preDownloadedPath);
