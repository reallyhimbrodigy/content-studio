...
async function normalizeSourceVideo(sourcePath) {
  ...
  if (sourceInfo.hasAudio) {
    ...
    normalizeArgs.splice(2, 0, '-f', 'lavfi', '-i', 'anullsrc=r=48000:cl=mono');
    normalizeArgs.push('-map', '1:a:0', '-shortest');
  }
...
async function processVideoJob({ videoUrl, vibeInput, jobId, userId, onProgress }) {
  ...
  let keyframedSourcePromise = null;
  ...
  try {
    ...
    const scdetPromise = (async () => {
      ...
      try {
        const localPath = await normalizedSourcePromise;
    ...
    sceneFramesPromise = (async () => {
      const [localPath, cuts] = await Promise.all([normalizedSourcePromise, scdetPromise]);
    ...
    const keyframeTimestamps = [
      ...visualCuts,
      ...((speech?.sentence_boundaries || []).map((b) => Number(b?.time || 0))),
    ];
    keyframedSourcePromise = createKeyframedSource(
      preDownloadedPath,
      keyframeTimestamps,
      path.dirname(preDownloadedPath)
    );
    console.log(`[pipeline] Started keyframe encode in background (${keyframeTimestamps.length} candidate timestamps)`);
    ...
    const preSplitPromise = (async () => {
      const keyframedPath = keyframedSourcePromise
        ? await keyframedSourcePromise
        : await createKeyframedSource(
          preDownloadedPath,
          (Array.isArray(editPlan.cuts) ? editPlan.cuts : []).map((cut) => Number(cut?.source_start || 0)),
          path.dirname(preDownloadedPath)
        );
