...
async function processVideoJob({ videoUrl, vibeInput, jobId, userId, onProgress }) {
  ...
  try {
    ...
    const keyframePromise = Promise.all([normalizedSourcePromise, scdetPromise, transcriptPromise])
      .then(([normalizedPath, visualCutsForKeyframes, transcriptForKeyframes]) => {
        const scdetTimestamps = Array.isArray(visualCutsForKeyframes) ? visualCutsForKeyframes : [];
        const directSpeechBoundaries = Array.isArray(transcriptForKeyframes?.sentence_boundaries)
          ? transcriptForKeyframes.sentence_boundaries
          : null;
        const deepgramSpeechBoundaries = directSpeechBoundaries || buildSpeechFromDeepgram(transcriptForKeyframes, 0).speech.sentence_boundaries;
        const speechTimestamps = deepgramSpeechBoundaries
          .map((b) => toNum(b?.time, NaN))
          .filter((t) => Number.isFinite(t) && t >= 0);
        const allTimestamps = [...new Set([...scdetTimestamps, ...speechTimestamps])]
          .sort((a, b) => a - b);
        console.log(`[pipeline] Starting keyframe encode in parallel with analysis (${allTimestamps.length} timestamps)`);
