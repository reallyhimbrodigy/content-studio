...
async function renderMultiClip(sourcePath, cuts, colorGrade, strategy, sourceRes, outputRes, editPlan, outputPath, transcript, speechSegments, tmpDir, jobId, addWatermark = false, hasBurnedCaptionsOverride = null, preSplitClipFiles = null) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (clipBrollOverlays.length > 0) {
      const baseLabel = `${videoLabelForOverlays}_overlay_base`;
      videoFilters.push(`[${videoLabelForOverlays}]null[${baseLabel}]`);
...
async function callModalRender({ clipUrls, sfxUrls, brollUrls, watermarkUrl, fontUrl, captionsUrl, ffmpegArgs, uploadUrl, jobId }) {
  ...
  console.log(
    `[modal] Payload: clips=${payload.input.clip_urls?.length || 0}, sfx=${payload.input.sfx_urls?.length || 0}, broll=${payload.input.broll_urls?.length || 0}, watermark=${!!payload.input.watermark_url}, font=${!!payload.input.font_url}, captions=${!!payload.input.captions_url}, ffmpeg_args_length=${payload.input.ffmpeg_args?.length || 0}`
  );
...
async function runMainRenderOnModal({ ffmpegArgs, clipFiles, sfxFiles, brollFiles, watermarkPath, captionsPath, needsFont, outputPath, jobId }) {
  if (!supabaseAdmin) {
  ...
  const brollUrls = brollUploads.map(({ i, brollPath, publicUrl }) => {
    ...
    console.log(`[modal] Uploaded broll_${i}: ${publicUrl?.slice(-40)}`);
    return publicUrl;
  });
