...
async function renderMultiClip(sourcePath, cuts, colorGrade, strategy, sourceRes, outputRes, editPlan, outputPath, transcript, speechSegments, tmpDir, addWatermark = false, hasBurnedCaptionsOverride = null) {
  ...
  if (brollEntries.length > 0) {
    for (let i = 0; i < brollEntries.length; i++) {
      ...
      postFilters.push(`[${b.inputIndex}:v]setpts=PTS-STARTPTS,trim=0:${b.duration.toFixed(3)},scale=${w}:${h}:force_original_aspect_ratio=decrease,pad=${w}:${h}:(ow-iw)/2:(oh-ih)/2,format=rgba,fade=t=in:st=0:d=0.2:alpha=1,fade=t=out:st=${fadeOutStart.toFixed(3)}:d=0.2:alpha=1${brollLabel}`);
      postFilters.push(`${videoOut}${brollLabel}overlay=0:0:shortest=1:eof_action=pass:enable='between(t,${b.start.toFixed(3)},${(b.start + b.duration).toFixed(3)})'${outLabel}`);
      videoOut = outLabel;
