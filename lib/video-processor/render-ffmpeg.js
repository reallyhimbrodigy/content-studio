...
async function renderMultiClip(sourcePath, cuts, colorGrade, strategy, sourceRes, outputRes, editPlan, outputPath, transcript, speechSegments, tmpDir, jobId, addWatermark = false, hasBurnedCaptionsOverride = null, preSplitClipFiles = null) {
  ...
  await logClipProbeSummaries(clipFiles, 'pre-broll');
  ...
  await compositeBrollIntoClipFiles(clipFiles, cuts, plannedBroll, brollAssets, outputRes);
  await logClipProbeSummaries(clipFiles, 'post-broll');
  const inputArgs = [];
  ...
  for (let i = 0; i < n; i++) {
    ...
    const vignetteFilter = getVignetteFilter(editPlan.vignette || 'none');
    const vChain = ['settb=AVTB', 'fps=30/1'];
...
async function callRunPodRender({ clipUrls, sfxUrls, watermarkUrl, fontUrl, captionsUrl, ffmpegArgs, uploadUrl, jobId }) {
  ...
  console.log(
    `[runpod] Payload: clips=${payload.input.clip_urls?.length || 0}, sfx=${payload.input.sfx_urls?.length || 0}, watermark=${!!payload.input.watermark_url}, font=${!!payload.input.font_url}, captions=${!!payload.input.captions_url}, ffmpeg_args_length=${payload.input.ffmpeg_args?.length || 0}`
  );
...
async function runMainRenderOnRunPod({ ffmpegArgs, clipFiles, sfxFiles, watermarkPath, captionsPath, outputPath, jobId }) {
  if (process.env.FORCE_LOCAL_RENDER === 'true') {
    console.log('[ffmpeg] FORCE_LOCAL_RENDER=true, rendering locally instead of RunPod');
    console.log(`[ffmpeg] LOCAL FFMPEG CMD: ffmpeg ${ffmpegArgs.map(shellEscapeArg).join(' ')}`);
  ...
  if (captionsPath && fs.existsSync(captionsPath)) {
    const uploaded = await uploadTempFile(captionsPath, jobId, 'captions.ass', 'text/plain');
    captionsUrl = uploaded.publicUrl;
  ...
  if (captionsPath && captionsUrl) {
    ...
    runpodFfmpegArgs = runpodFfmpegArgs.replace(new RegExp(escapedCaptionsPath, 'g'), '{CAPTIONS_PATH}');
    console.log('[runpod] CAPTIONS_PATH placeholder in args:', runpodFfmpegArgs.includes('{CAPTIONS_PATH}'));
  }
