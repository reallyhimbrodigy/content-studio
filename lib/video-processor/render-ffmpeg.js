...
function buildAnimatedDrawtextFilter(videoInLabel, overlay, text, start, end, idx, outputRes) {
  ...
  const boxBorderW = style === 'title' ? 16 : style === 'cta' ? 14 : 14;
  return `drawtext=fontfile='${fontFile}':text='${safeText}':fontsize=${fontSize}:fontcolor=white:x='${xExpr}':y='${yExpr}':alpha='${alphaExpr}':borderw=${borderW}:bordercolor=${borderColor}:box=1:boxcolor=black@0.5:boxborderw=${boxBorderW}:enable='between(t\\,${start.toFixed(3)}\\,${endTime.toFixed(3)})'`;
}
