...
const { registerPendingRunpodJob } = require('./runpod-webhook');
...
async function callRunPodRender({ clipUrls, sfxUrls, brollUrls, watermarkUrl, fontUrl, captionsUrl, ffmpegArgs, uploadUrl, jobId }) {
  ...
  const payload = {
    webhook: 'https://usepromptly.app/api/runpod-webhook',
    input: {
      clip_urls: clipUrls,
      sfx_urls: sfxUrls || [],
  ...
  console.log('[runpod] Submitting payload:', JSON.stringify(payload, null, 2));
  ...
  console.log(`[runpod] Webhook registered for job ${runpodJobId}`);

  const output = await registerPendingRunpodJob(runpodJobId, {
    timeoutMs: 120_000,
  ...
  if (output?.error) {
    throw new Error(`RunPod render error: ${output.error}`);
  }
...
async function runMainRenderOnRunPod({ ffmpegArgs, clipFiles, sfxFiles, brollFiles, watermarkPath, captionsPath, needsFont, outputPath, jobId }) {
  if (!supabaseAdmin) {
