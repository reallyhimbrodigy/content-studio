...
const MODAL_BASE_URL = 'https://api.modal.ai/v2';
...
async function renderVideo(editPlan, videoUrl, jobId, onProgress, transcript, preDownloadedPath, speechSegments = [], addWatermark = false, hasBurnedCaptions = false, preSplitClipFiles = null) {
  ...
  try {
    ...
    usedModal = Boolean(renderResult?.usedModal);
    ...
    if (usedModal) {
      try {
        await cleanupModalTempFiles(jobId);
        cleanedModalTemp = true;
      } catch (e) {
        console.warn(`[modal] Temp cleanup warning: ${e.message}`);
      }
    }
...
async function renderSingleClip(sourcePath, cut, colorGrade, strategy, sourceRes, outputRes, editPlan, outputPath, hasBurnedCaptionsOverride = null) {
  ...
  const speedSegments = normalizeSpeedSegments(cut?.speed_segments, sourceDuration, speed);
  const hasSegmentedSpeed = Array.isArray(cut?.speed_segments) && cut.speed_segments.length > 0;
  const effectiveDuration = getEffectiveDurationFromSpeedSegments(speedSegments);
  ...
  const cutZoomFilter = (!hasBurnedCaptions && shouldUseCutZoom)
    ? getCutZoomFilter(cut.source_start, cut.source_end, editPlan.analysis_data?.speech?.sentence_boundaries || [], outputWidth, outputHeight, hasSegmentedSpeed ? speedSegments : speed)
    : null;
  ...
  if (hasSegmentedSpeed) {
    const filterParts = [];
    const vBaseChain = ['settb=AVTB', 'fps=30'];
    if (aspectFilter) vBaseChain.push(aspectFilter);
    filterParts.push(`[0:v]${vBaseChain.join(',')}[vbase]`);
    const vSegLabels = [];
    for (let s = 0; s < speedSegments.length; s++) {
      const seg = speedSegments[s];
      const label = `vseg${s}`;
      vSegLabels.push(`[${label}]`);
      filterParts.push(`[vbase]trim=start=${seg.start.toFixed(3)}:end=${seg.end.toFixed(3)},setpts=${(1 / seg.speed).toFixed(4)}*(PTS-STARTPTS),fps=30[${label}]`);
    }
    if (speedSegments.length > 1) {
      filterParts.push(`${vSegLabels.join('')}concat=n=${speedSegments.length}:v=1:a=0[vspeed]`);
    } else {
      filterParts.push(`${vSegLabels[0]}null[vspeed]`);
    }
    const vPost = [];
    if (zoomFilter) vPost.push(zoomFilter);
    vPost.push('format=yuv420p', vfFilters);
    if (vignetteFilter) vPost.push(vignetteFilter);
    if (editPlan.outro && editPlan.outro !== 'none') {
      ...
      vPost.push(`fade=t=out:st=${fadeStart.toFixed(3)}:d=${fadeDuration}:color=${fadeColor}`);
    }
    filterParts.push(`[vspeed]${vPost.join(',')}[vout]`);

    filterParts.push('[0:a]asetpts=PTS-STARTPTS,afftdn=nr=10:nf=-25[abase]');
    const aSegLabels = [];
    for (let s = 0; s < speedSegments.length; s++) {
      const seg = speedSegments[s];
      const label = `aseg${s}`;
      aSegLabels.push(`[${label}]`);
      filterParts.push(`[abase]atrim=start=${seg.start.toFixed(3)}:end=${seg.end.toFixed(3)},asetpts=PTS-STARTPTS,${getAtempoFilter(seg.speed)}[${label}]`);
    }
    if (speedSegments.length > 1) {
      filterParts.push(`${aSegLabels.join('')}concat=n=${speedSegments.length}:v=0:a=1[aspeed]`);
    } else {
      filterParts.push(`${aSegLabels[0]}anull[aspeed]`);
    }
    if (editPlan.outro && editPlan.outro !== 'none') {
      const fadeDuration = 1.0;
      const fadeStart = Math.max(0, effectiveDuration - fadeDuration);
      filterParts.push(`[aspeed]afade=t=out:st=${fadeStart.toFixed(3)}:d=${fadeDuration}[aout]`);
    } else {
      filterParts.push('[aspeed]anull[aout]');
    }
    args.push(
    ...
    if (speedFilters.video) videoChain.push(speedFilters.video, 'fps=30');
  ...
  args.push(...encodeArgs(), outputPath);

  console.log(`[ffmpeg] Single clip: ${start.toFixed(1)}s -> ${cut.source_end.toFixed(1)}s (${effectiveDuration.toFixed(1)}s @ ${speed}x)`);
  await runFFmpeg(args);
}

async function createKeyframedSource(sourcePath, keyframeTimestamps, tmpDir) {
  const cutTimestamps = Array.isArray(keyframeTimestamps) ? keyframeTimestamps : [];
  const normalized = cutTimestamps
    .map((ts) => roundMs(Number(ts || 0)))
    .filter((ts) => Number.isFinite(ts) && ts > 0);
  const uniqueKeyframes = [...new Set(normalized)].sort((a, b) => a - b);
...
async function preSplitSourceClips(keyframedPath, cuts, tmpDir) {
  const clipFiles = [];
  const splitStartMs = Date.now();

  // Step 3: Extract clips from keyframed source in parallel
  console.log(`[ffmpeg] Starting parallel extraction for ${cuts.length} clip(s) from keyframed source`);
  console.log('[ffmpeg] Stream copy extraction (no re-encode)');
  const extractionPromises = cuts.map((cut, i) => {
    ...
    if (clipDuration <= 0) return Promise.resolve(null);
    ...
    return runFFmpeg(extractArgs).then(() => clipPath);
  });
  const extractedClipFiles = await Promise.all(extractionPromises);
  clipFiles.push(...extractedClipFiles.filter(Boolean));
...
async function renderMultiClip(sourcePath, cuts, colorGrade, strategy, sourceRes, outputRes, editPlan, outputPath, transcript, speechSegments, tmpDir, jobId, addWatermark = false, hasBurnedCaptionsOverride = null, preSplitClipFiles = null) {
  ...
  const n = cuts.length;
  const runPodConfigured = isModalConfigured();
  const clipFiles = (Array.isArray(preSplitClipFiles) && preSplitClipFiles.length > 0)
    ? preSplitClipFiles
    : await (async () => {
      const keyframeTimestamps = cuts.map((cut) => Number(cut?.source_start || 0));
      const keyframedPath = await createKeyframedSource(sourcePath, keyframeTimestamps, tmpDir);
      return preSplitSourceClips(keyframedPath, cuts, tmpDir);
    })();
  ...
  for (let i = 0; i < n; i++) {
    ...
    speedSegmentsByClip.push(speedSegments);
    console.log(
  ...
  for (let i = 0; i < cuts.length - 1; i++) {
    const explicitSound = cuts[i]?.transition_sound;
    const styleHint = cuts[i]?.sfx_style || explicitSound || defaultTransitionSfxStyle(cuts[i]?.transition_out);
    const soundPath = await resolveSfxPath(styleHint, soundsDir, tmpDir, variantCounters);
    ...
    const vol = getTransitionVolume(normalizeSfxStyle(styleHint), transitionTime, speechSegmentsForVolume);
    const label = `[snd${i}]`;
  ...
  const clipRangesForSfx = getOutputClipRanges(cuts, effectiveDurations);
  const textOverlaysForSfx = Array.isArray(editPlan.text_overlays) ? editPlan.text_overlays : [];
  for (let i = 0; i < textOverlaysForSfx.length; i++) {
    const overlay = textOverlaysForSfx[i] || {};
    const clipIndex = Number(overlay.appear_at_clip || 0) - 1;
    if (clipIndex < 0 || clipIndex >= clipRangesForSfx.length) continue;
    const styleHint = overlay.sfx_style || defaultTextSfxStyle(overlay.style);
    if (normalizeSfxStyle(styleHint) === 'none') continue;
    const soundPath = await resolveSfxPath(styleHint, soundsDir, tmpDir, variantCounters);
    if (!soundPath) continue;
    const ts = Math.max(0, Number(clipRangesForSfx[clipIndex]?.start || 0) + 0.02);
    const offsetMs = Math.round(ts * 1000);
    const vol = getTextSfxVolume(normalizeSfxStyle(styleHint), ts, speechSegmentsForVolume);
    const label = `[txtsnd${i}]`;
    const soundInputIndex = extraInputIndex;
    enhancementInputArgs.push('-i', soundPath);
    enhancementFilters.push(`[${soundInputIndex}:a]volume=${vol.toFixed(3)},adelay=${offsetMs}|${offsetMs}${label}`);
    sfxAudioLabels.push(label);
  ...
  for (let i = 0; i < n; i++) {
    ...
    const speedSegments = speedSegmentsByClip[i] || normalizeSpeedSegments(cuts[i]?.speed_segments, sourceDurations[i], speed);
    const hasSegmentedSpeed = Array.isArray(cuts[i]?.speed_segments) && cuts[i].speed_segments.length > 0;
    ...
    const cutZoomFilter = (!hasBurnedCaptions && shouldUseCutZoom)
      ? getCutZoomFilter(cuts[i].source_start, cuts[i].source_end, editPlan.analysis_data?.speech?.sentence_boundaries || [], outputWidth, outputHeight, hasSegmentedSpeed ? speedSegments : speed)
      : null;
    const zoomMax = hasBurnedCaptions ? 1.06 : 1.10;
    if (hasBurnedCaptions && effectiveZoom !== 'none') {
    ...
    const vignetteFilter = getVignetteFilter(editPlan.vignette || 'none');
    const vChainPost = [];
    if (zoomFilter) vChainPost.push(zoomFilter);
    vChainPost.push('format=yuv420p', colorFilterStr);
    if (vignetteFilter) vChainPost.push(vignetteFilter);
    if (i === n - 1 && editPlan.outro && editPlan.outro !== 'none') {
      const fadeDuration = 1.0;
      const fadeStart = Math.max(0, effectiveDurations[i] - fadeDuration);
      const fadeColor = editPlan.outro === 'fade_white' ? 'white' : 'black';
      vChainPost.push(`fade=t=out:st=${fadeStart.toFixed(3)}:d=${fadeDuration}:color=${fadeColor}`);
    }

    const vBaseChain = ['settb=AVTB', 'fps=30'];
    if (aspectFilter) vBaseChain.push(aspectFilter);
    let videoLabelForOverlays = `v${i}`;
    if (hasSegmentedSpeed) {
      const vBaseLabel = `v${i}_base`;
      videoFilters.push(`[${i}:v]${vBaseChain.join(',')}[${vBaseLabel}]`);
      const segLabels = [];
      for (let s = 0; s < speedSegments.length; s++) {
        const seg = speedSegments[s];
        const segLabel = `v${i}_seg${s}`;
        segLabels.push(`[${segLabel}]`);
        videoFilters.push(
          `[${vBaseLabel}]trim=start=${Number(seg.start).toFixed(3)}:end=${Number(seg.end).toFixed(3)},setpts=${(1 / seg.speed).toFixed(4)}*(PTS-STARTPTS),fps=30[${segLabel}]`
        );
      }
      const vSpeedLabel = `v${i}_speed`;
      if (speedSegments.length > 1) {
        videoFilters.push(`${segLabels.join('')}concat=n=${speedSegments.length}:v=1:a=0[${vSpeedLabel}]`);
      } else {
        videoFilters.push(`${segLabels[0]}null[${vSpeedLabel}]`);
      }
      const outLabel = `v${i}`;
      videoFilters.push(`[${vSpeedLabel}]${vChainPost.join(',')}[${outLabel}]`);
      videoLabelForOverlays = outLabel;
    } else {
      const vChain = [...vBaseChain];
      if (speedFilters.video) {
        vChain.push(speedFilters.video);
        vChain.push('fps=30');
      }
      vChain.push(...vChainPost);
      videoFilters.push(`[${i}:v]${vChain.join(',')}[v${i}]`);
      videoLabelForOverlays = `v${i}`;
    }
    const aChainPost = [];
    if (i === n - 1 && editPlan.outro && editPlan.outro !== 'none') {
      const fadeDuration = 1.0;
      const fadeStart = Math.max(0, effectiveDurations[i] - fadeDuration);
      aChainPost.push(`afade=t=out:st=${fadeStart.toFixed(3)}:d=${fadeDuration}`);
    }
    if (hasSegmentedSpeed) {
      const aBaseLabel = `a${i}_base`;
      audioFilters.push(`[${i}:a]asetpts=PTS-STARTPTS,afftdn=nr=10:nf=-25[${aBaseLabel}]`);
      const segLabels = [];
      for (let s = 0; s < speedSegments.length; s++) {
        const seg = speedSegments[s];
        const segLabel = `a${i}_seg${s}`;
        segLabels.push(`[${segLabel}]`);
        audioFilters.push(
          `[${aBaseLabel}]atrim=start=${Number(seg.start).toFixed(3)}:end=${Number(seg.end).toFixed(3)},asetpts=PTS-STARTPTS,${getAtempoFilter(seg.speed)}[${segLabel}]`
        );
      }
      const aSpeedLabel = `a${i}_speed`;
      if (speedSegments.length > 1) {
        audioFilters.push(`${segLabels.join('')}concat=n=${speedSegments.length}:v=0:a=1[${aSpeedLabel}]`);
      } else {
        audioFilters.push(`${segLabels[0]}anull[${aSpeedLabel}]`);
      }
      if (aChainPost.length > 0) {
        audioFilters.push(`[${aSpeedLabel}]${aChainPost.join(',')}[a${i}]`);
      } else {
        audioFilters.push(`[${aSpeedLabel}]anull[a${i}]`);
      }
    } else {
      const aChain = [
  ...
  for (let i = 1; i < n; i++) {
    ...
    const outVideoRawLabel = `${outVideoLabel}_raw`;
    ...
    if (isHardCut) {
      ...
      transitionFilters.push(
        `[${timelineVideoLabel}][v${i}]concat=n=2:v=1:a=0[${outVideoRawLabel}]`
      );
      transitionFilters.push(`[${outVideoRawLabel}]fps=30[${outVideoLabel}]`);
      transitionFilters.push(
      ...
      transitionFilters.push(
        `[${timelineVideoLabel}][v${i}]xfade=transition=${transition}:duration=${transitionDuration.toFixed(3)}:offset=${offset.toFixed(3)}[${outVideoRawLabel}]`
      );
      transitionFilters.push(`[${outVideoRawLabel}]fps=30[${outVideoLabel}]`);
      transitionFilters.push(
  ...
  let audioOut = `[${timelineAudioLabel}]`;
  ...
  postFilters.push(
    `${audioOut}highpass=f=80,lowpass=f=12000,equalizer=f=2800:t=q:w=1.5:g=3,equalizer=f=200:t=q:w=0.8:g=1.5,acompressor=threshold=-20dB:ratio=3:attack=5:release=50:makeup=2,dynaudnorm=f=150:g=15:p=0.95:m=10,alimiter=limit=0.95:attack=1:release=10[final_audio]`
  );
  ...
  try {
    if (runPodConfigured) {
      console.log('[ffmpeg] Sending main render to Modal GPU...');
      const sfxFiles = inputDebug
        .filter((entry) => entry.type === 'sfx' && entry.source)
        .map((entry) => entry.source);
      const needsFont = (editPlan.caption_style && editPlan.caption_style !== 'none')
        || (Array.isArray(editPlan.text_overlays) && editPlan.text_overlays.length > 0);
      await runMainRenderOnModal({
        ffmpegArgs: args,
        clipFiles,
        sfxFiles,
      ...
      console.log('[ffmpeg] No Modal config, rendering locally...');
      await runFFmpeg(args);
    }
  } finally {
 ...
 *
 * Key decisions:
 * - CRF 22: Visually near-transparent, professional quality.
 * - fast preset: Best size/speed balance without blowing past ~100s render budget.
...
function getSpeedFilters(speed) {
  ...
  return {
    video: videoFilter,
    audio: getAtempoFilter(safeSpeed),
  };
}
...
function normalizeSpeedSegments(speedSegments, clipDuration, fallbackSpeed = 1.0) {
  const duration = Math.max(0, Number(clipDuration || 0));
  const safeFallback = Math.max(0.25, Math.min(4.0, Number(fallbackSpeed) || 1.0));
  if (!Array.isArray(speedSegments) || speedSegments.length === 0 || duration <= 0) {
    return [{ start: 0, end: duration, speed: safeFallback }];
  }

  const sanitized = speedSegments
    .map((seg) => ({
  ...
  for (const seg of sanitized) {
    if (seg.start > cursor + 0.0001) {
      normalized.push({ start: cursor, end: seg.start, speed: 1.0 });
    }
    const start = Math.max(cursor, seg.start);
    const end = Math.max(start, seg.end);
    if (end > start) normalized.push({ start, end, speed: seg.speed });
    cursor = Math.max(cursor, end);
  }
  if (cursor < duration - 0.0001) {
    normalized.push({ start: cursor, end: duration, speed: 1.0 });
  }
  return normalized.map((seg) => ({
    start: roundMs(seg.start),
    end: roundMs(seg.end),
    speed: Math.max(0.25, Math.min(4.0, Number(seg.speed) || 1.0)),
  }));
}

function getEffectiveDurationFromSpeedSegments(speedSegments) {
  if (!Array.isArray(speedSegments) || speedSegments.length === 0) return 0;
  return roundMs(
    speedSegments.reduce((sum, seg) => {
      const segDur = Math.max(0, Number(seg.end || 0) - Number(seg.start || 0));
      const segSpeed = Math.max(0.25, Math.min(4.0, Number(seg.speed) || 1.0));
      return sum + (segDur / segSpeed);
    }, 0)
  );
}

function mapLocalTimeThroughSpeedSegments(localTime, speedSegments) {
  const t = Math.max(0, Number(localTime || 0));
  if (!Array.isArray(speedSegments) || speedSegments.length === 0) return t;
  let out = 0;
  for (const seg of speedSegments) {
    const start = Number(seg.start || 0);
    const end = Number(seg.end || start);
    const speed = Math.max(0.25, Math.min(4.0, Number(seg.speed) || 1.0));
    if (t <= start) break;
    const covered = Math.min(t, end) - start;
    if (covered > 0) out += covered / speed;
    if (t <= end) break;
  }
  return roundMs(out);
}
...
function getCutZoomFilter(clipStart, clipEnd, sentenceBoundaries, width, height, speedOrSegments = 1.0) {
  const segments = buildCutZoomSegments(clipStart, clipEnd, sentenceBoundaries);
  if (!segments || segments.length < 2) return null;
  const hasSpeedSegments = Array.isArray(speedOrSegments) && speedOrSegments.length > 0;
  const speed = Number(speedOrSegments) || 1.0;
  const boundaries = segments.slice(1).map((s) => {
    const local = s.start - clipStart;
    return hasSpeedSegments
      ? mapLocalTimeThroughSpeedSegments(local, speedOrSegments)
      : (local / speed);
  });
...
function getTransitionTimes(cuts, effectiveDurations = null) {
  ...
  const firstSourceDur = (cuts[0].source_end || 0) - (cuts[0].source_start || 0);
  let running = Number.isFinite(firstDuration)
    ? roundMs(firstDuration)
    : getEffectiveDurationFromSpeedSegments(normalizeSpeedSegments(cuts[0]?.speed_segments, firstSourceDur, cuts[0].speed || 1.0));
  for (let i = 0; i < cuts.length - 1; i++) {
    ...
    const nextSourceDur = (cuts[i + 1].source_end || 0) - (cuts[i + 1].source_start || 0);
    const nextDuration = Number.isFinite(nextDurationOverride)
      ? roundMs(nextDurationOverride)
      : getEffectiveDurationFromSpeedSegments(normalizeSpeedSegments(cuts[i + 1]?.speed_segments, nextSourceDur, cuts[i + 1].speed || 1.0));
...
function getOutputClipRanges(cuts, effectiveDurations = null) {
  ...
  for (let i = 0; i < cuts.length; i++) {
    ...
    const sourceDur = (cut.source_end || 0) - (cut.source_start || 0);
    const duration = Number.isFinite(durationOverride)
      ? durationOverride
      : getEffectiveDurationFromSpeedSegments(normalizeSpeedSegments(cut?.speed_segments, sourceDur, cut.speed || 1.0));
...
function mapSourceTimeToOutput(sourceTime, cuts, effectiveDurations = null) {
  ...
  for (let i = 0; i < cuts.length; i++) {
    ...
    const sourceDur = cEnd - cStart;
    const speed = Number(cut.speed || 1.0) || 1.0;
    const speedSegments = normalizeSpeedSegments(cut?.speed_segments, sourceDur, speed);
    const outDurOverride = Number(effectiveDurations?.[i]);
    const outDur = Number.isFinite(outDurOverride)
      ? outDurOverride
      : getEffectiveDurationFromSpeedSegments(speedSegments);
    if (target >= cStart && target <= cEnd) {
      return roundMs(cursor + mapLocalTimeThroughSpeedSegments(target - cStart, speedSegments));
    }
    const overlap = i < cuts.length - 1 ? getTransitionDuration(cut.transition_out) : 0;
...
function projectWordsToOutput(transcript, cuts, effectiveDurations = null) {
  ...
  for (let i = 0; i < cuts.length; i++) {
    ...
    for (const w of words) {
      ...
      const speedSegments = normalizeSpeedSegments(cut?.speed_segments, cEnd - cStart, speed);
      projected.push({
        start: roundMs(outputCursor + mapLocalTimeThroughSpeedSegments(overlapStart - cStart, speedSegments)),
        end: roundMs(outputCursor + mapLocalTimeThroughSpeedSegments(overlapEnd - cStart, speedSegments)),
        word: w.punctuated_word || w.word || '',
      });
    }
    const cutDurOverride = Number(effectiveDurations?.[i]);
    const cutDur = Number.isFinite(cutDurOverride)
      ? cutDurOverride
      : getEffectiveDurationFromSpeedSegments(normalizeSpeedSegments(cut?.speed_segments, cEnd - cStart, cut.speed || 1.0));
    const overlap = i < cuts.length - 1 ? getTransitionDuration(cut.transition_out) : 0;
...
const SFX_LIBRARY_FILES = {
  shutter: ['shutter.mp3'],
  swoosh: ['swoosh.mp3'],
  thud: ['thud.mp3'],
  pop: ['pop.mp3'],
  typing: ['typing.mp3'],
  reverb_hit: ['reverb_hit.mp3'],
  ding: ['ding.mp3'],
  ching: ['ching.mp3'],
};

function getSfxLibraryUrls() {
  const base = String(process.env.SFX_LIBRARY_BASE_URL || '').trim().replace(/\/+$/, '');
  if (!base) return null;
  const map = {};
  for (const [style, files] of Object.entries(SFX_LIBRARY_FILES)) {
    map[style] = files.map((file) => `${base}/${file}`);
  }
  return map;
}

async function downloadSfxAsset(url, destPath) {
...
function normalizeSfxStyle(style) {
...
function defaultTransitionSfxStyle(transition) {
  const t = String(transition || 'none').toLowerCase();
  if (t === 'none') return 'none';
  if (t.includes('wipe') || t.includes('smooth')) return 'swoosh';
  if (t.includes('zoom')) return 'swoosh';
  if (t.includes('fade') || t.includes('dissolve')) return 'swoosh';
  return 'swoosh';
}

function defaultTextSfxStyle(overlayStyle) {
  const s = String(overlayStyle || '').toLowerCase();
  if (s === 'cta') return 'ching';
  if (s === 'title') return 'thud';
  if (s === 'callout') return 'pop';
  return 'pop';
}

function getTransitionVolume(soundName, transitionTimestamp, speechSegments) {
  const baseVol = soundBaseVolumes[soundName] || 0.60;
  ...
  const finalVol = duringSpeech ? baseVol * 0.75 : baseVol;
  console.log(
...
function getTextSfxVolume(soundName, timestamp, speechSegments) {
  const base = {
    pop: 0.64,
    ding: 0.60,
    ching: 0.66,
  ...
  const duringSpeech = Array.isArray(speechSegments) && speechSegments.some((seg) =>
    timestamp >= Number(seg.start || 0) && timestamp <= Number(seg.end || 0)
  );
  return duringSpeech ? base * 0.8 : base;
}

async function resolveSfxPath(style, soundsDir, tmpDir, variantCounters) {
  const normalized = normalizeSfxStyle(style);
  if (normalized === 'none') return null;

  const library = getSfxLibraryUrls();
  const remoteVariants = library?.[normalized] || [];
  if (remoteVariants.length > 0) {
    const idx = variantCounters[normalized] || 0;
    variantCounters[normalized] = idx + 1;
    const url = remoteVariants[idx % remoteVariants.length];
    const ext = path.extname(url.split('?')[0] || '.mp3') || '.mp3';
    const destPath = path.join(tmpDir, `sfx_lib_${normalized}_${idx}${ext}`);
    try {
      await downloadSfxAsset(url, destPath);
      return destPath;
    } catch (e) {
      console.warn(`[sfx] Failed to download ${url}: ${e.message}`);
    }
  }

  return getSoundPath(normalized, soundsDir);
}
...
function isModalConfigured() {
  return Boolean(process.env.MODAL_API_KEY && process.env.MODAL_ENDPOINT_ID);
}

async function uploadTempFile(localPath, jobId, filename, contentType = 'application/octet-stream') {
  if (!supabaseAdmin) {
    throw new Error('Supabase admin client not configured for Modal temp upload');
  }
...
function transformMainRenderArgsForModal(args, replacements) {
  const out = [];
...
async function callModalRender({ clipUrls, sfxUrls, watermarkUrl, fontUrl, captionsUrl, ffmpegArgs, uploadUrl, jobId }) {
  const MODAL_API_KEY = process.env.MODAL_API_KEY;
  const MODAL_ENDPOINT_ID = process.env.MODAL_ENDPOINT_ID;
  if (!MODAL_API_KEY || !MODAL_ENDPOINT_ID) {
    throw new Error('Modal credentials are not configured');
  }

  const payload = {
    webhook: 'https://usepromptly.app/api/modal-webhook',
    input: {
      clip_urls: clipUrls,
      sfx_urls: sfxUrls || [],
  ...
  console.log(`[modal] Submitting to endpoint: ${MODAL_ENDPOINT_ID}`);
  console.log(
    `[modal] Payload: clips=${payload.input.clip_urls?.length || 0}, sfx=${payload.input.sfx_urls?.length || 0}, watermark=${!!payload.input.watermark_url}, font=${!!payload.input.font_url}, captions=${!!payload.input.captions_url}, ffmpeg_args_length=${payload.input.ffmpeg_args?.length || 0}`
  );
  console.log('[modal] Submitting payload:', JSON.stringify(payload, null, 2));
  console.log('[modal] font_url in payload:', payload.input.font_url || 'NOT SET');
  console.log('[modal] captions_url in payload:', payload.input.captions_url || 'NOT SET');
  console.log(`[modal] Submitting render job for ${jobId}...`);
  const runResponse = await fetch(`${MODAL_BASE_URL}/${MODAL_ENDPOINT_ID}/run`, {
    method: 'POST',
    headers: {
  ...
  if (!runResponse.ok || !runData?.id) {
    throw new Error(`Modal submit failed (${runResponse.status}): ${JSON.stringify(runData)}`);
  }

  const modalJobId = runData.id;
  console.log(`[modal] Job submitted: ${modalJobId}`);
  console.log(`[modal] Webhook registered for job ${modalJobId}`);

  const output = await registerPendingModalJob(modalJobId, {
    timeoutMs: 120_000,
    onTimeoutCheck: async () => {
      const statusResponse = await fetch(`${MODAL_BASE_URL}/${MODAL_ENDPOINT_ID}/status/${modalJobId}`, {
        headers: { Authorization: `Bearer ${MODAL_API_KEY}` },
      });
      const statusData = await statusResponse.json().catch(() => ({}));
      if (statusData?.output?.error) {
        console.error(`[modal] Worker error: ${statusData.output.error}`);
      }
      return statusData;
    },
  });
  if (output?.error) {
    throw new Error(`Modal render error: ${output.error}`);
  }
  console.log(`[modal] Render completed for ${jobId}`);
  return output;
}

async function cleanupModalTempFiles(jobId) {
  ...
  if (listError) {
    throw new Error(`Modal temp list failed: ${listError.message}`);
  }
  ...
  if (removeError) {
    throw new Error(`Modal temp cleanup failed: ${removeError.message}`);
  }
  console.log(`[modal] Cleaned up ${paths.length} temp files`);
}

async function runMainRenderOnModal({ ffmpegArgs, clipFiles, sfxFiles, watermarkPath, captionsPath, needsFont, outputPath, jobId }) {
  if (!supabaseAdmin) {
    throw new Error('Supabase admin client is required for Modal render uploads');
  }
  ...
  const fontUploadPromise = (needsFont && fs.existsSync(fontPath))
  ...
  const [clipUploads, sfxUploads, watermarkUploaded, fontUploaded, captionsUploaded] = await Promise.all([
    clipUploadsPromise,
    sfxUploadsPromise,
  ...
  if (captionsUploaded?.publicUrl) {
    ...
    console.log('[modal] Uploaded captions.ass:', captionsUrl ? 'success' : 'FAILED');
  }
  ...
  if (signedError) {
    throw new Error(`Modal signed upload URL failed: ${signedError.message}`);
  }
  const uploadUrl = normalizeSignedUploadUrl(signedData?.signedUrl);
  if (!uploadUrl) {
    throw new Error('Modal signed upload URL missing from Supabase response');
  }
  console.log('[modal] Created signed upload URL for output');

  replacementMap[outputPath] = '{OUTPUT}';
  const runPodArgs = transformMainRenderArgsForModal(ffmpegArgs, replacementMap);
  ...
  let modalFfmpegArgs = ['ffmpeg', ...runPodArgs];
  console.log(`[modal] Sending ffmpeg_args as array with ${modalFfmpegArgs.length} elements`);
  modalFfmpegArgs = modalFfmpegArgs.map((arg) =>
    String(arg)
  ...
  if (captionsPath && captionsUrl) {
    ...
    modalFfmpegArgs = modalFfmpegArgs.map((arg) => String(arg).replace(new RegExp(escapedCaptionsPath, 'g'), '{CAPTIONS_PATH}'));
    console.log('[modal] CAPTIONS_PATH placeholder in args:', modalFfmpegArgs.some((arg) => String(arg).includes('{CAPTIONS_PATH}')));
  }
  console.log('[modal] Payload font_url:', fontUrl || 'NOT SET');
  console.log('[modal] FONT_PATH placeholder in args:', modalFfmpegArgs.some((arg) => String(arg).includes('{FONT_PATH}')));
  console.log('[modal] FULL FFMPEG CMD:', modalFfmpegArgs);

  await callModalRender({
    clipUrls,
    sfxUrls,
    watermarkUrl,
    fontUrl,
    captionsUrl,
    ffmpegArgs: modalFfmpegArgs,
    uploadUrl,
    jobId,
  });
  ...
  if (!outputPublicUrl) {
    throw new Error('Modal output public URL unavailable');
  }

  const response = await fetch(outputPublicUrl);
  if (!response.ok) {
    throw new Error(`Modal output download failed: HTTP ${response.status}`);
  }
  const buffer = Buffer.from(await response.arrayBuffer());
  fs.writeFileSync(outputPath, buffer);
  console.log(`[modal] Downloaded rendered video: ${(buffer.length / 1024 / 1024).toFixed(1)}MB`);
}
...
module.exports = { renderVideo, preSplitSourceClips, createKeyframedSource };
