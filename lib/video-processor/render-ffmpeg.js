...
function buildColorFilters(colorGrade, strategy = '') {
  ...
  console.log(`[ffmpeg] Color filter chain: ${filters.join(',')}`);
  return filters;
}
...
function findNearestSilence(wordTimeline, targetTime) {
  ...
  for (let i = 0; i < wordTimeline.length - 1; i++) {
    ...
    // Bias toward the middle-to-end of the silence gap so the crossfade
    // doesn't pull in audio from the next word
  ...
  for (const word of wordTimeline) {
    ...
    if (distance < bestDistance) {
      ...
      if (wordIndex < wordTimeline.length - 1) {
        ...
        if (gap >= 0.03) {
          bestDistance = distance;
          bestTime = word.end + 0.08; // 80ms after word ends — accounts for crossfade bleed
        }
      }
    }
  }
