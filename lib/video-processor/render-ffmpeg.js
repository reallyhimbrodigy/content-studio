 ...
 */
async function renderMultiClip(sourcePath, cuts, colorGrade, strategy, sourceRes, outputRes, editPlan, outputPath, transcript, speechSegments, tmpDir) {
  ...
  for (let i = 0; i < cuts.length - 1; i++) {
    ...
    const label = `[snd${i}]`;
    enhancementFilters.push(`[${extraInputIndex}:a]adelay=${offsetMs}|${offsetMs},volume=0.8${label}`);
...
function generateSubtitleFile(transcript, captionStyle, cuts, outputRes, outputPath, captionPosition, aspectRatio) {
  const styles = {
...
Style: Default,${style.fontname},${style.fontsize},&H00FFFFFF,&H000000FF,&H00000000,&H80000000,${style.bold},0,0,0,100,100,0,0,1,2,1,${style.alignment},20,20,${marginV},1
