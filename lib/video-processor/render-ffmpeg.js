 ...
 * Uses iterative pairwise xfade: merge clip0+clip1, then result+clip2, etc.
 */
async function concatWithXfade(clipPaths, cuts, outputPath) {
  ...
  for (let i = 1; i < clipPaths.length; i++) {
    ...
    const mergedPath = isLastPair ? outputPath : path.join(path.dirname(outputPath), `merged_${i}.mp4`);

    if (transition === 'clean_cut') {
      // Simple concat for this pair
      const concatFile = path.join(path.dirname(outputPath), `concat_${i}.txt`);
      fs.writeFileSync(concatFile, `file '${currentPath}'\nfile '${nextPath}'`);
      await runFFmpeg([
      ...
      const filterComplex = [
        ...
        `[v0][v1]xfade=transition=${xfadeType}:duration=${TRANSITION_DURATION}:offset=${offset.toFixed(3)},format=yuv420p[vout]`,
        `[0:a][1:a]acrossfade=d=${TRANSITION_DURATION}:c1=tri:c2=tri[aout]`
      ].join(';');
      ...
      currentDuration = currentDuration + nextDuration - TRANSITION_DURATION;
    }
