...
function shellEscapePath(pathValue) {
  return String(pathValue || '').replace(/"/g, '\\"');
}

function probeClipFull(clipPath) {
  try {
    const raw = execSync(
      `ffprobe -v quiet -print_format json -show_format -show_streams "${shellEscapePath(clipPath)}"`,
      { timeout: 15000 }
    ).toString();
    ...
    return {
      ...
      v_codec: vs?.codec_name,
      width: parseInt(vs?.width || 0, 10),
      height: parseInt(vs?.height || 0, 10),
      fps: vs?.r_frame_rate,
      pix_fmt: vs?.pix_fmt,
      a_codec: as?.codec_name,
      has_audio: Boolean(as),
...
async function compositeBrollIntoClipFiles(clipFiles, cuts, plannedBroll, brollAssets, outputRes, actualDurations = []) {
  ...
  for (const [clipIndex, overlays] of grouped.entries()) {
    ...
    const prev = Number.isFinite(actualDurations[clipIndex]) ? actualDurations[clipIndex] : 0;
    const reprobedRaw = parseFloat(
      execSync(`ffprobe -v error -show_entries format=duration -of csv=p=0 "${shellEscapePath(clipPath)}"`)
        .toString().trim()
    );
    const reprobed = Number.isFinite(reprobedRaw) ? reprobedRaw : prev;
    console.log(
      `[ffmpeg] Clip ${clipIndex} re-probed after b-roll: ${reprobed.toFixed(3)}s `
      + `(was ${prev.toFixed(3)}s, delta=${(reprobed - prev).toFixed(3)}s)`
    );
    actualDurations[clipIndex] = reprobed;
...
async function renderMultiClip(sourcePath, cuts, colorGrade, strategy, sourceRes, outputRes, editPlan, outputPath, transcript, speechSegments, tmpDir, jobId, addWatermark = false, hasBurnedCaptionsOverride = null, preSplitClipFiles = null) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const nominal = roundMs((cuts[i]?.source_end || 0) - (cuts[i]?.source_start || 0));
    actualClipDurations[i] = roundMs(Number.isFinite(probedDuration) ? probedDuration : nominal);
  }
  await compositeBrollIntoClipFiles(clipFiles, cuts, plannedBroll, brollAssets, outputRes, actualClipDurations);

  console.log('[ffmpeg] === FINAL CLIP DIAGNOSTICS (sent to RunPod) ===');
  for (let i = 0; i < n; i++) {
    const info = probeClipFull(clipFiles[i]);
    console.log(`[ffmpeg] Clip ${i}: ${JSON.stringify(info)}`);
  }
  console.log('[ffmpeg] === END CLIP DIAGNOSTICS ===');
  ...
  if (missingAudio.length > 0) {
    console.warn(`[ffmpeg] WARNING: Clips missing audio: ${missingAudio.join(', ')}. Adding silent audio tracks.`);
    for (const idx of missingAudio) {
      const clipPath = clipFiles[idx];
      const tempPath = clipPath.replace(/\.mp4$/i, '_with_audio.mp4');
      await runFFmpeg([
        '-y',
        '-i', clipPath,
        '-f', 'lavfi',
      ...
      console.log(`[ffmpeg] Added silent audio to clip ${idx}`);
      const reprobed = await probeDuration(clipPath);
      if (Number.isFinite(reprobed)) {
        actualClipDurations[idx] = roundMs(reprobed);
      }
    }
  }
  ...
  for (let i = 0; i < n; i++) {
    ...
    const sourceDuration = roundMs(
      Number.isFinite(actualClipDurations[i]) ? actualClipDurations[i] : nominalDuration
    );
    const speed = Number.isFinite(cut.speed) ? cut.speed : 1.0;
    const effectiveDuration = roundMs(sourceDuration / speed);
    inputArgs.push(
    ...
    actualClipDurations[i] = sourceDuration;
    console.log(
  ...
  console.log('[pipeline] === EDIT PLAN vs ACTUAL ===');
  for (let i = 0; i < cuts.length; i++) {
    const cut = cuts[i];
    const nominal = Number(cut?.source_end || 0) - Number(cut?.source_start || 0);
    const actual = Number(actualClipDurations[i] || 0);
    const delta = actual - nominal;
    console.log(
      `[pipeline]   Clip ${i}: nominal=${nominal.toFixed(3)}s, actual=${actual.toFixed(3)}s, `
      + `delta=${delta >= 0 ? '+' : ''}${delta.toFixed(3)}s, transition_out=${cut?.transition_out || 'none'}`
    );
  }
  console.log('[pipeline] === END EDIT PLAN ===');
  ...
  // Use a consistent xfade/acrossfade pipeline for all junctions.
  // Hard cuts use an effectively-instant fade (0.001s) to avoid concat timestamp resets.
  let timelineVideoLabel = 'v0';
  let timelineAudioLabel = 'a0';
  let runningDuration = effectiveDurations[0];
  const transitionFilters = [];

  console.log('[ffmpeg] === TRANSITION CHAIN ===');
  for (let i = 0; i < cuts.length - 1; i++) {
    const transType = String(cuts[i]?.transition_out || 'none');
    const transDur = getTransitionDuration(transType, TRANSITION_DURATION);
    console.log(
      `[ffmpeg]   ${i} → ${i + 1}: transition="${transType}" `
      + `→ xfade_type="${isHardCutTransition(transType) ? 'fade' : transType}" duration=${transDur.toFixed(3)}s`
    );
  }
  console.log('[ffmpeg] === END TRANSITIONS ===');

  console.log('[ffmpeg] === XFADE OFFSET CALCULATION ===');
  console.log(
    `[ffmpeg]   Clip 0: actual_dur=${effectiveDurations[0].toFixed(3)}s, running=${runningDuration.toFixed(3)}s`
  );
  ...
  for (let i = 1; i < n; i++) {
    ...
    const xfadeTransition = isHardCutTransition(transition) ? 'fade' : transition;
    const offset = Math.max(0, runningDuration - transitionDuration);
    transitionFilters.push(
      `[${timelineVideoLabel}][v${i}]xfade=transition=${xfadeTransition}:duration=${transitionDuration.toFixed(3)}:offset=${offset.toFixed(3)}[${outVideoLabel}]`
    );
    transitionFilters.push(
      `[${timelineAudioLabel}][a${i}]acrossfade=d=${transitionDuration.toFixed(3)}:c1=tri:c2=tri[${outAudioLabel}]`
    );
    console.log(
      `[ffmpeg]   xfade ${i}: running(${runningDuration.toFixed(3)}) - trans(${transitionDuration.toFixed(3)}) `
      + `= offset(${offset.toFixed(3)}), then + clip${i}_dur(${effectiveDurations[i].toFixed(3)}) `
      + `= next_running(${(offset + effectiveDurations[i]).toFixed(3)})`
    );
  ...
  console.log(`[ffmpeg]   Expected output duration: ${runningDuration.toFixed(3)}s`);
  console.log('[ffmpeg] === END XFADE CALCULATION ===');
  ...
  console.log('[ffmpeg] === END FILTER_COMPLEX ===');
...
function getTransitionDuration(transition, standardDuration = 0.3) {
  return isHardCutTransition(transition) ? 0.001 : standardDuration;
}
...
async function callRunPodRender({ clipUrls, sfxUrls, watermarkUrl, fontUrl, ffmpegArgs, uploadUrl, jobId }) {
  ...
  console.log(`[runpod] Response status: ${runResponse.status}`);
  console.log(`[runpod] Response output: ${JSON.stringify(runData?.output || {}).substring(0, 1000)}`);
  if (runData?.error) {
    console.error(`[runpod] Error: ${runData.error}`);
  }
  ...
  while (Date.now() - startMs < maxWaitMs) {
    ...
    if (status === 'COMPLETED') {
      console.log(`[runpod] Response status: ${statusResponse.status}`);
      console.log(`[runpod] Response output: ${JSON.stringify(statusData?.output || {}).substring(0, 1000)}`);
      if (statusData?.error) {
        console.error(`[runpod] Error: ${statusData.error}`);
      }
...
async function runMainRenderOnRunPod({ ffmpegArgs, clipFiles, sfxFiles, watermarkPath, outputPath, jobId }) {
  if (!supabaseAdmin) {
  ...
  for (let i = 0; i < clipFiles.length; i++) {
    ...
    const token = `{CLIP_${i}}`;
    const uploaded = await uploadTempFile(clipPath, jobId, `clip_${i}.mp4`, 'video/mp4');
    replacementMap[clipPath] = token;
    clipUrls.push(uploaded.publicUrl);
    console.log(`[runpod] Uploaded clip_${i}: ${uploaded.publicUrl?.slice(-40)}`);
  }

  const uniqueSfx = [...new Set((sfxFiles || []).filter(Boolean))];
  const sfxUrls = [];
  for (let i = 0; i < uniqueSfx.length; i++) {
    const sfxPath = uniqueSfx[i];
    const token = `{SFX_${i}}`;
    const uploaded = await uploadTempFile(sfxPath, jobId, `sfx_${i}.mp3`, 'audio/mpeg');
    replacementMap[sfxPath] = token;
    sfxUrls.push(uploaded.publicUrl);
    console.log(`[runpod] Uploaded sfx_${i}: ${uploaded.publicUrl?.slice(-40)}`);
  }

  let watermarkUrl = null;
  if (watermarkPath && fs.existsSync(watermarkPath)) {
    const uploaded = await uploadTempFile(watermarkPath, jobId, 'watermark.png', 'image/png');
    watermarkUrl = uploaded.publicUrl;
  ...
  if (fs.existsSync(fontPath)) {
    const uploaded = await uploadTempFile(fontPath, jobId, 'Montserrat-Black.ttf');
    fontUrl = uploaded.publicUrl;
  ...
  const { data: signedData, error: signedError } = await supabaseAdmin.storage
    .from('videos')
    .createSignedUploadUrl(outputRemotePath);
  if (signedError) {
  ...
  if (yIndex >= 0) {
    runPodArgs.splice(yIndex + 1, 0, '-threads', '8');
  } else {
    runPodArgs.unshift('-threads', '8');
  }
  let runpodFfmpegArgs = `ffmpeg ${runPodArgs.map(shellEscapeArg).join(' ')}`;
  runpodFfmpegArgs = runpodFfmpegArgs.replaceAll('/opt/render/project/src/src/assets/fonts/Montserrat-Black.ttf', '{FONT_PATH}');
  runpodFfmpegArgs = runpodFfmpegArgs.replaceAll('/opt/render/project/src/assets/fonts/Montserrat-Black.ttf', '{FONT_PATH}');
  console.log('[runpod] Payload font_url:', fontUrl || 'NOT SET');
  console.log('[runpod] FONT_PATH placeholder in args:', runpodFfmpegArgs.includes('{FONT_PATH}'));
