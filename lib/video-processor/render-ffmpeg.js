 ...
 */
async function renderVideo(editPlan, videoUrl, jobId, onProgress, transcript, preDownloadedPath, speechSegments = [], addWatermark = false) {
 ...
 */
async function renderMultiClip(sourcePath, cuts, colorGrade, strategy, sourceRes, outputRes, editPlan, outputPath, transcript, speechSegments, tmpDir, addWatermark = false) {
  ...
  if (addWatermark) {
    const watermarkFilter = `${videoOut}drawtext=text='Made with Promptly':fontsize=28:fontcolor=white@0.65:borderw=2:bordercolor=black@0.5:x=30:y=120[video_watermarked]`;
    postFilters.push(watermarkFilter);
