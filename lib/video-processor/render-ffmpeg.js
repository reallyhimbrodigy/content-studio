...
async function renderSingleClip(sourcePath, cut, colorGrade, strategy, sourceRes, outputRes, editPlan, outputPath, hasBurnedCaptionsOverride = null) {
  ...
  const cutZoomFilter = (!hasBurnedCaptions && shouldUseCutZoom)
    ...
    : null;
  const zoomFilter = cutZoomFilter || getZoomFilter(effectiveZoom, effectiveDuration, outputWidth, outputHeight);
...
async function renderMultiClip(sourcePath, cuts, colorGrade, strategy, sourceRes, outputRes, editPlan, outputPath, transcript, speechSegments, tmpDir, addWatermark = false, hasBurnedCaptionsOverride = null) {
  ...
  if (bgMusic !== 'none') {
    const musicPath = path.join(__dirname, '..', '..', 'src', 'assets', 'music', `${bgMusic}.mp3`);
    if (fs.existsSync(musicPath)) {
      enhancementInputArgs.push('-stream_loop', '-1', '-i', musicPath);
      enhancementFilters.push(`[${extraInputIndex}:a]volume=1[bgm]`);
      bgMusicLabel = '[bgm]';
      extraInputIndex++;
    } else {
  ...
  if (bgMusicLabel) {
    if (editPlan.audio_ducking) {
      postFilters.push(`${bgMusicLabel}volume=0.15[bgm_pre]`);
      postFilters.push(`${audioOut}asplit=2[audio_for_mix][audio_for_sc]`);
      postFilters.push(`[bgm_pre][audio_for_sc]sidechaincompress=threshold=0.02:ratio=6:attack=200:release=1000[bgm_ducked]`);
      postFilters.push(`[audio_for_mix][bgm_ducked]amix=inputs=2:duration=first:dropout_transition=2[audio_mixed]`);
      audioOut = '[audio_mixed]';
    } else {
      postFilters.push(`${bgMusicLabel}volume=0.15[bgm_pre]`);
      postFilters.push(`${audioOut}[bgm_pre]amix=inputs=2:duration=first:dropout_transition=2[audio_mixed]`);
      audioOut = '[audio_mixed]';
    }
  }
...
function buildAnimatedDrawtextFilter(videoInLabel, overlay, text, start, end, idx, outputRes) {
  ...
  const fontSize = style === 'title' ? 72 : style === 'cta' ? 56 : 48;
  ...
  if (style === 'title') {
    const xExpr = `if(lt(t-${start.toFixed(3)}\\,${animIn})\\,-tw+((tw+30)*(t-${start.toFixed(3)})/${animIn})\\,30)`;
    const alphaExpr = `if(lt(t\\,${(endTime - animOut).toFixed(3)})\\,1\\,if(lt(t\\,${endTime.toFixed(3)})\\,(${endTime.toFixed(3)}-t)/${animOut}\\,0))`;
    return `drawtext=text='${safeText}':fontsize=${fontSize}:fontcolor=white:x='${xExpr}':y='${yExpr}':alpha='${alphaExpr}':borderw=2:bordercolor=black@0.6:enable='between(t\\,${start.toFixed(3)}\\,${endTime.toFixed(3)})'`;
  }

  const xExpr = position === 'top' ? '30' : '((W-tw)/2)';
