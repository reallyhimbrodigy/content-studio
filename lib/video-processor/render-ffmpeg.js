 ...
 */
async function renderVideo(editPlan, videoUrl, jobId, onProgress, transcript, preDownloadedPath, speechSegments = []) {
  ...
  try {
    ...
    if (cuts.length === 1) {
      await renderSingleClip(sourcePath, cuts[0], colorGrade, editPlan.strategy || '', sourceRes, outputRes, editPlan, outputPath);
    } else {
      await renderMultiClip(sourcePath, cuts, colorGrade, editPlan.strategy || '', sourceRes, outputRes, editPlan, outputPath);
    }

    const enhancedOutputPath = path.join(tmpDir, 'output-enhanced.mp4');
    const finalOutputPath = await applyPostEnhancements(
      outputPath,
      enhancedOutputPath,
      editPlan,
      transcript,
 ...
 */
async function renderSingleClip(sourcePath, cut, colorGrade, strategy, sourceRes, outputRes, editPlan, outputPath) {
  ...
  const zoomFilter = getZoomFilter(cut.zoom || 'none', effectiveDuration, outputRes.width || sourceRes.width || 1080, outputRes.height || sourceRes.height || 1920);
 ...
 */
async function renderMultiClip(sourcePath, cuts, colorGrade, strategy, sourceRes, outputRes, editPlan, outputPath) {
  ...
  // Step 1: Normalize each input and apply aspect/speed/zoom/color
  for (let i = 0; i < n; i++) {
    const aspectFilter = getAspectRatioFilter(editPlan.aspect_ratio || 'original', sourceRes.width, sourceRes.height);
    ...
    const aChain = ['asetpts=PTS-STARTPTS', 'afftdn=nr=10:nf=-25'];
  ...
  for (let i = 1; i < n; i++) {
    ...
    if (transition !== 'clean_cut') {
      ...
      transitionFilters.push(
        `[${prevVideoLabel}][v${i}]xfade=transition=${transition}:duration=${TRANSITION_DURATION}:offset=${offset.toFixed(3)}[${outVideoLabel}]`
      );
      transitionFilters.push(
        `[${prevAudioLabel}][a${i}]acrossfade=d=${TRANSITION_DURATION}:c1=tri:c2=tri[${outAudioLabel}]`
      );
      ...
      transitionFilters.push(
        `[${prevVideoLabel}][${prevAudioLabel}][v${i}][a${i}]concat=n=2:v=1:a=1[${concatVideoLabel}][${outAudioLabel}]`
      );
  ...
  const filterComplex = [...videoFilters, ...audioFilters, ...transitionFilters].join(';');
...
function getZoomFilter(zoom, clipDuration, width, height) {
  ...
  switch (zoom) {
    case 'slow_in':
      return `zoompan=z='min(zoom+0.0003,1.1)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${totalFrames}:s=${width}x${height}:fps=${fps}`;
    case 'slow_out':
      return `zoompan=z='if(eq(on,1),1.1,max(zoom-0.0003,1.0))':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${totalFrames}:s=${width}x${height}:fps=${fps}`;
    case 'punch_in':
      return `zoompan=z='if(lt(on,10),min(zoom+0.015,1.15),1.15)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${totalFrames}:s=${width}x${height}:fps=${fps}`;
    case 'punch_out':
      return `zoompan=z='if(lt(on,10),max(zoom-0.015,1.0),1.0)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${totalFrames}:s=${width}x${height}:fps=${fps}`;
    default:
      return null;
  }
}
...
function generateSubtitleFile(transcript, captionStyle, cuts, sourceRes, outputPath) {
  const styles = {
...
Style: Default,${style.fontname},${style.fontsize},&H00FFFFFF,&H000000FF,&H00000000,&H80000000,${style.bold},0,0,0,100,100,0,0,1,2,1,${style.alignment},20,20,${style.marginV},1
...
async function applyPostEnhancements(basePath, outputPath, editPlan, transcript, speechSegments, cuts, sourceRes, tmpDir) {
  const captionStyle = editPlan.caption_style || 'none';
  const bgMusic = editPlan.background_music || 'none';
  const hasSfx = cuts.some((c) => c.transition_sound && c.transition_sound !== 'none');
  const hasTextOverlays = Array.isArray(editPlan.text_overlays) && editPlan.text_overlays.length > 0;
  const needsCaptions = captionStyle !== 'none';
  const needsMusic = bgMusic !== 'none';
  const needsEnhancements = needsCaptions || needsMusic || hasSfx || hasTextOverlays;
  ...
  if (needsMusic) {
    ...
    if (fs.existsSync(musicPath)) {
      ...
      filterParts.push(`[${inputIndex}:a]volume=0.30[bgm]`);
      bgMusicLabel = '[bgm]';
      inputIndex++;
    } else {
  ...
  if (hasSfx) {
    ...
    for (let i = 0; i < cuts.length - 1; i++) {
      ...
      const soundPath = path.join(process.cwd(), 'assets', 'sounds', `${sound}.mp3`);
      if (!fs.existsSync(soundPath)) {
        console.log(`[ffmpeg] Transition sound missing, skipping: ${soundPath}`);
        continue;
      }
      args.push('-i', soundPath);
      const offsetMs = Math.max(0, Math.round(transitionTimes[i] * 1000));
      const label = `[snd${i}]`;
      filterParts.push(`[${inputIndex}:a]adelay=${offsetMs}|${offsetMs},volume=0.8${label}`);
  ...
  filterParts.push(`[0:v]null[video_base]`);
  if (needsCaptions) {
    const assPath = path.join(tmpDir, 'captions.ass');
    const generated = generateSubtitleFile(transcript, captionStyle, cuts, sourceRes, assPath);
    if (generated) {
      const escaped = escapeFilterPath(generated);
      filterParts.push(`${videoOut}subtitles='${escaped}'[video_captioned]`);
  ...
  if (hasTextOverlays) {
    ...
    const generatedOverlay = generateTextOverlays(editPlan.text_overlays, cuts, sourceRes, overlayAssPath);
    if (generatedOverlay) {
      const escaped = escapeFilterPath(generatedOverlay);
      filterParts.push(`${videoOut}subtitles='${escaped}'[video_overlayed]`);
  ...
  let audioBase = '[main_audio]';
  filterParts.push(`[0:a]volume=1${audioBase}`);

  if (bgMusicLabel) {
    if (editPlan.audio_ducking) {
      try {
        filterParts.push(`${bgMusicLabel}${audioBase}sidechaincompress=threshold=0.02:ratio=6:attack=200:release=1000[ducked_music]`);
        filterParts.push(`${audioBase}[ducked_music]amix=inputs=2:duration=first:dropout_transition=2[audio_mixed]`);
        audioBase = '[audio_mixed]';
      } catch (e) {
        const enableExpr = buildDuckingVolumeEnable(speechSegments, 0.2);
        if (enableExpr) {
          filterParts.push(`${bgMusicLabel}volume=0.10:enable='${enableExpr}'[bgm_ducked]`);
          filterParts.push(`${audioBase}[bgm_ducked]amix=inputs=2:duration=first:dropout_transition=2[audio_mixed]`);
          audioBase = '[audio_mixed]';
        } else {
          filterParts.push(`${audioBase}${bgMusicLabel}amix=inputs=2:duration=first:dropout_transition=2[audio_mixed]`);
          audioBase = '[audio_mixed]';
        }
      }
    } else {
  ...
  if (sfxAudioLabels.length > 0) {
    const inputs = `${audioBase}${sfxAudioLabels.join('')}`;
    filterParts.push(`${inputs}amix=inputs=${sfxAudioLabels.length + 1}:duration=first:dropout_transition=2[audio_sfx_mixed]`);
    audioBase = '[audio_sfx_mixed]';
  }

  // Always apply loudness normalization to final audio
  // Target -14 LUFS (standard for social media platforms like YouTube, Instagram, TikTok)
  const loudnormFilter = 'loudnorm=I=-14:LRA=11:TP=-1.5';
  filterParts.push(`${audioBase}${loudnormFilter}[final_audio]`);
  let audioOut = '[final_audio]';

  if (filterParts.length > 0) {
    args.push('-filter_complex', filterParts.join(';'));
  }

  args.push('-map', videoOut, '-map', audioOut, ...encodeArgs(), outputPath);

  console.log(`[ffmpeg] Applying post enhancements (captions/music/sfx=${needsEnhancements ? 'on' : 'off'}, loudnorm=on)...`);
  await runFFmpeg(args);
  return outputPath;
}
