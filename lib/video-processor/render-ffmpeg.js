...
function buildAnimatedDrawtextFilter(videoInLabel, overlay, text, start, end, idx, outputRes) {
  ...
  return `drawtext=fontfile='${fontFile}':text='${safeText}':fontsize=${fontSize}:fontcolor=white:x='${xExpr}':y='${yExpr}':alpha='${alphaExpr}':borderw=${borderW}:bordercolor=${borderColor}:enable='between(t\\,${start.toFixed(3)}\\,${endTime.toFixed(3)})'`;
}
