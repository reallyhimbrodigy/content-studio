...
async function uploadToSupabase(filePath, jobId) {
  ...
  console.log(`[ffmpeg] Uploading ${sizeMB}MB to Supabase...`);

  const fileBuffer = fs.readFileSync(filePath);
  const fileName = `rendered/${jobId}-${Date.now()}.mp4`;

  // Wrap upload in a timeout — Supabase client has no built-in timeout for storage
  ...
  const timeoutPromise = new Promise((_, reject) =>
    setTimeout(() => reject(new Error(`Upload timed out after 120s (file: ${sizeMB}MB)`)), 120_000)
  );
  ...
  console.log(`[ffmpeg] Upload complete: ${fileName}`);
