...
async function renderMultiClip(sourcePath, cuts, colorGrade, strategy, sourceRes, outputRes, editPlan, outputPath, transcript, speechSegments, tmpDir, addWatermark = false, hasBurnedCaptionsOverride = null) {
  ...
  if (addWatermark) {
    // Sora-style watermark: small dark pill + white text, bottom-right, always visible
    // Positioned at h-340 (just above TikTok/Reels UI zone so it's visible during playback)
    postFilters.push(`${videoOut}drawbox=x=(w-178):y=(h-340):w=160:h=30:color=black@0.35:t=fill[video_wm_bg]`);
    postFilters.push(`[video_wm_bg]drawtext=text='promptly':fontsize=22:fontcolor=white@0.7:x=(w-168):y=(h-336):borderw=0[video_watermarked]`);
