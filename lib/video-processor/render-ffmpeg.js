...
async function renderSingleClip(sourcePath, cut, colorGrade, strategy, sourceRes, outputPath) {
  ...
  const duration = roundMs(cut.source_end - cut.source_start);
  ...
  console.log(`[ffmpeg] Single clip: ${start.toFixed(1)}s -> ${cut.source_end.toFixed(1)}s (${duration.toFixed(1)}s)`);
...
async function renderMultiClip(sourcePath, cuts, colorGrade, strategy, sourceRes, outputPath) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    console.log(`[ffmpeg] Input ${i}: ${start.toFixed(1)}s -> ${cut.source_end.toFixed(1)}s (${duration.toFixed(1)}s)`);
  }
  ...
  // - clean_cut (and all non-dissolve transitions) are true hard cuts via concat
  // - dissolve uses crossfade (xfade/acrossfade)
  ...
  for (let i = 1; i < n; i++) {
    ...
    if (transition === 'dissolve') {
      ...
      transitionFilters.push(
        `[${prevVideoLabel}][v${i}]xfade=transition=dissolve:duration=${TRANSITION_DURATION}:offset=${offset.toFixed(3)}[${outVideoLabel}]`
      );
      transitionFilters.push(
      ...
      const concatVideoLabel = (i === n - 1) ? outVideoLabel : `${outVideoLabel}_raw`;
      transitionFilters.push(
