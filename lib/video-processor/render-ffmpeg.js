...
async function runMainRenderOnRunPod({ ffmpegArgs, clipFiles, sfxFiles, watermarkPath, outputPath, jobId }) {
  ...
  let fontUrl = null;
  if (fs.existsSync(OVERLAY_FONT_LOCAL_PATH)) {
    const uploaded = await uploadTempFile(OVERLAY_FONT_LOCAL_PATH, jobId, 'Montserrat-Black.ttf', 'font/ttf');
    fontUrl = uploaded.publicUrl;
    console.log('[runpod] Uploaded Montserrat-Black.ttf');
  }
