...
async function renderMultiClip(sourcePath, cuts, colorGrade, strategy, sourceRes, outputRes, editPlan, outputPath, transcript, speechSegments, tmpDir, jobId, addWatermark = false, hasBurnedCaptionsOverride = null, preSplitClipFiles = null) {
  try {
    ...
    console.log(`[ffmpeg] Render FFmpeg version: ${ffmpegVersion}`);
    const filters = execSync('ffmpeg -filters 2>&1 | grep -E "dynaudnorm|afftdn|xfade|drawtext"').toString();
    console.log(`[ffmpeg] Render filters: ${filters}`);
  } catch (e) {}
  ...
  // - none/clean_cut uses true hard concat (no blend)
  ...
  for (let i = 1; i < n; i++) {
    ...
    if (!isHardCutTransition(transition)) {
  ...
  if (addWatermark) {
    ...
    if (fs.existsSync(watermarkPath)) {
      ...
      postFilters.push(`${videoOut}[wm_img]overlay=x=(W-w-20):y=(H-340):shortest=1:eof_action=pass[video_watermarked]`);
...
function getTransitionTimes(cuts) {
  ...
  for (let i = 0; i < cuts.length - 1; i++) {
    ...
    const overlap = isHardCutTransition(transition) ? 0 : 0.3;
...
function getOutputClipRanges(cuts) {
  ...
  for (let i = 0; i < cuts.length; i++) {
    ...
    const overlap = i < cuts.length - 1 && !isHardCutTransition(cut.transition_out) ? 0.3 : 0;
...
async function runMainRenderOnRunPod({ ffmpegArgs, clipFiles, sfxFiles, watermarkPath, outputPath, jobId }) {
  ...
  let fontUrl = null;
  const fontUploadPath = fs.existsSync(OVERLAY_FONT_LOCAL_PATH)
    ? OVERLAY_FONT_LOCAL_PATH
    : (fs.existsSync(OVERLAY_FONT_PATH) ? OVERLAY_FONT_PATH : null);
  if (fontUploadPath) {
    const uploaded = await uploadTempFile(fontUploadPath, jobId, 'Montserrat-Black.ttf', 'font/ttf');
    fontUrl = uploaded.publicUrl;
    console.log(`[runpod] Uploaded Montserrat-Black.ttf from ${fontUploadPath}`);
  } else {
    console.warn(`[runpod] Font file missing at both paths: ${OVERLAY_FONT_LOCAL_PATH} | ${OVERLAY_FONT_PATH}`);
  }
  ...
  let runPodArgsString = `ffmpeg ${runPodArgs.map(shellEscapeArg).join(' ')}`;
  runPodArgsString = runPodArgsString.replace(
    /\/opt\/render\/project\/src\/src\/assets\/fonts\/Montserrat-Black\.ttf/g,
    '{FONT_PATH}'
  );
