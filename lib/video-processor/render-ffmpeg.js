...
async function renderMultiClip(sourcePath, cuts, colorGrade, strategy, sourceRes, outputRes, editPlan, outputPath, transcript, speechSegments, tmpDir) {
  ...
  const videoFilters = [];
  const audioFilters = [];
  const videoSplitLabels = cuts.map((_, i) => `[vin${i}]`).join('');
  const audioSplitLabels = cuts.map((_, i) => `[ain${i}]`).join('');
  const splitFilters = [
    `[0:v]split=${n}${videoSplitLabels}`,
    `[0:a]asplit=${n}${audioSplitLabels}`,
  ];

  // Step 1: Split source once, then trim each clip and apply aspect/speed/zoom/color
  for (let i = 0; i < n; i++) {
    const clipStart = roundMs(cuts[i].source_start);
    const clipEnd = roundMs(cuts[i].source_end);
    const aspectFilter = getAspectRatioFilter(editPlan.aspect_ratio || 'original', sourceRes.width, sourceRes.height);
    const speed = Number.isFinite(cuts[i]?.speed) ? cuts[i].speed : 1.0;
    const speedFilters = getSpeedFilters(speed);
    const setptsExpr = speed === 1.0 ? 'setpts=PTS-STARTPTS' : `setpts=(PTS-STARTPTS)/${speed}`;
    ...
    const vignetteFilter = getVignetteFilter(editPlan.vignette || 'none');
    const vChain = [
      `trim=start=${clipStart}:end=${clipEnd}`,
      setptsExpr,
      'settb=AVTB',
    ];
    ...
    videoFilters.push(`[vin${i}]${vChain.join(',')}[v${i}]`);

    const aChain = [
      `atrim=start=${clipStart}:end=${clipEnd}`,
    ...
    audioFilters.push(`[ain${i}]${aChain.join(',')}[a${i}]`);
  }
