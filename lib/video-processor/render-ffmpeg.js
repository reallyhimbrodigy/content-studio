...
async function callRunPodRender({ clipUrls, sfxUrls, watermarkUrl, fontUrl, ffmpegArgs, uploadUrl, jobId }) {
  ...
  console.log(
    `[runpod] Payload: clips=${payload.input.clip_urls?.length || 0}, sfx=${payload.input.sfx_urls?.length || 0}, watermark=${!!payload.input.watermark_url}, font=${!!payload.input.font_url}, ffmpeg_args_length=${payload.input.ffmpeg_args?.length || 0}`
  );
