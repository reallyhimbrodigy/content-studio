...
function transformMainRenderArgsForRunPod(args, replacements) {
  ...
  if (!out.includes('-hwaccel')) {
    const yIndex = out.indexOf('-y');
    if (yIndex >= 0) {
      out.splice(yIndex + 1, 0, '-hwaccel', 'cuda');
    } else {
      out.unshift('-hwaccel', 'cuda');
    }
  }
  return out;
}

async function callRunPodRender({ clipUrls, sfxUrls, watermarkUrl, ffmpegArgs, uploadUrl, jobId }) {
...
async function runMainRenderOnRunPod({ ffmpegArgs, clipFiles, sfxFiles, watermarkPath, outputPath, jobId }) {
  ...
  const runPodArgsString = `ffmpeg ${runPodArgs.map(shellEscapeArg).join(' ')}`;
