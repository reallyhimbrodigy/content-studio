 ...
 * - ultrafast preset: Render speed >> file size on shared CPU.
...
function buildVideoFilterChain(colorGrade, strategy, sourceRes) {
  ...
  // 1080p cap — only scale down, never up
  if (sourceRes && sourceRes.height > 1080) {
    filters.push('scale=-2:1080');
    console.log(`[ffmpeg] Scaling down from ${sourceRes.height}p -> 1080p`);
  }
