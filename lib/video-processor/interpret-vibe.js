...
async function interpretVibe(vibeInput, clipAnalysis, contentType = null) {
  ...
  const contentGuidelineMap = {
    'music-video': [
      'MUSIC VIDEO EDITING:',
      '- Beat-sync every cut to the music.',
      '- Favor rhythm over dialogue.',
      '- Keep captions minimal unless explicitly requested.',
      '- Use energetic transitions and motion emphasis.',
    ],
    automotive: [
      'AUTOMOTIVE EDITING:',
      '- Emphasize acceleration, impact, and motion peaks.',
      '- Use speed-ramp-friendly pacing and stronger motion accents.',
      '- Prioritize visual momentum and engine-driven energy.',
    ],
    cinematic: [
      'CINEMATIC EDITING:',
      '- Preserve composition with longer shot duration.',
      '- Favor smooth transitions and restrained SFX density.',
      '- Keep grading cinematic and avoid hyperactive cuts.',
    ],
    'action-sports': [
      'ACTION SPORTS EDITING:',
      '- Cut on tricks, impacts, and peak action moments.',
      '- Mix rapid cuts with occasional highlight slowdowns.',
      '- Maintain high-energy timing and momentum.',
    ],
    'talking-head': [
      'TALKING HEAD EDITING:',
      '- Cut on speech boundaries and remove dead air.',
      '- Keep captions readable and speech-aligned.',
      '- Keep music under dialogue.',
    ],
    gaming: [
      'GAMING EDITING:',
      '- Cut around high-intensity gameplay moments.',
      '- Preserve gameplay readability while maintaining pace.',
      '- Sync emphasis moments to action beats.',
    ],
    'travel-lifestyle': [
      'TRAVEL / LIFESTYLE EDITING:',
      '- Balance scenic breathing room with momentum.',
      '- Keep transitions clean and emotionally coherent.',
    ],
    'dance-performance': [
      'DANCE PERFORMANCE EDITING:',
      '- Let choreography flow; avoid disruptive over-cutting.',
      '- Sync transitions to music accents.',
    ],
    'product-showcase': [
      'PRODUCT SHOWCASE EDITING:',
      '- Keep product clarity high with clean, readable pacing.',
      '- Use tasteful transitions and minimal distraction.',
    ],
    'comedy-skit': [
      'COMEDY SKIT EDITING:',
      '- Prioritize comedic timing and reaction beats.',
      '- Cut tightly around punchlines and rhythm shifts.',
    ],
  };

  const selectedGuidelines = contentGuidelineMap[normalizedContentType.primaryType] || contentGuidelineMap['talking-head'];
  const contentContext = [
    'CONTENT TYPE DETECTED:',
    `- Primary type: ${normalizedContentType.primaryType}`,
    `- Confidence: ${Math.round((Number(normalizedContentType.confidence) || 0) * 100)}%`,
    `- Characteristics: ${Array.isArray(normalizedContentType.characteristics) ? normalizedContentType.characteristics.join(', ') : 'n/a'}`,
    `- Editing approach: ${normalizedContentType.editingApproach || 'n/a'}`,
    `- Recommended pacing: ${normalizedContentType.recommendedPacing || 'medium'}`,
    `- Music emphasis: ${normalizedContentType.musicEmphasis ? 'Yes' : 'No'}`,
    `- Captions needed: ${normalizedContentType.captionsNeeded ? 'Yes' : 'No'}`,
    '',
    'CONTENT-SPECIFIC GUIDELINES:',
    ...selectedGuidelines,
    '',
    'Apply these guidelines while interpreting the user vibe request.',
  ].join('\n');
  ...
  const fullPrompt = [
    ...
    prompt,
    '',
    `Generate editing parameters optimized for ${normalizedContentType.primaryType} content.`,
    'Adapt the output to match BOTH the detected content type and the user vibe request.',
  ].join('\n');
