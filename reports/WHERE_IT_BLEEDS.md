...
**JUDGE, generated 2026-08-15T20:57:02.301Z by `scripts/bleeds.js`.** Job window 24h; funnel + fulfillment windows stated per section. Every line [MEASURED].

## 1. Failures — 40 users / 46 jobs (24h)

| class | users | jobs | share of failing users |
|---|---:|---:|---:|
| PLATFORM_TIMEOUT | 26 | 31 | 65.0% |
| UPLOAD_NEVER_STARTED | 13 | 13 | 32.5% |
| DISPATCH_UNREACHABLE | 2 | 2 | 5.0% |

## 2. FAILED-JOB SECONDS — 46.5% of all job-lifetime seconds [7-DAY WINDOW]

**289 failed jobs / 250 users** over **7 days**, p50 lifetime **601s**, **41/day**. Total **179,322s** of user time spent on jobs that never delivered.

| quantity | jobs | seconds | share |
|---|---:|---:|---:|
| reached a worker (**Modal-billable**) | 55 | 47505 | 26.5% |
| never reached one (**$0 Modal, pure user wait**) | 234 | 131817 | 73.5% |

**USER-time and MODAL-time are different quantities and must not be blended.** A job with no `worker_started_at` and no `modal_call_id` never reached a container: it costs the user their whole wait and costs us **$0**. Here only **26.5%** of failed seconds were Modal-billable (~$0.49/day, **1.9%** of orchestration) — the rest is pure user loss at zero spend.
...
## 2b. Latency — n=96 completed (24h)

p50 **134s** (law 90) · p90 604s · p99 **1172s** (law 180) · max 1172s

| envelope class | n | users | p50 | p90 | max |
|---|---:|---:|---:|---:|---:|
| `C envelope LOST + repair` | 5 | 4 | **904s** | 1172s | 1172s |
| `B envelope LOST` | 29 | 29 | **379s** | 760s | 904s |
| `A envelope FULL` | 62 | 61 | **93s** | 193s | 547s |

Worst/best class p50 spread: **9.7x** — the pooled number above hides it.

**ENVELOPE LOSS: 35.4% of completions (34/96), 33 users.** Regression BORN 2026-08-11T23Z after 8 clean days at 0.0% (08-04..08-11). The pooled p50 above sits between classes and describes NO actual user.
...
|---|---:|---:|---|---:|---:|
| **180–240s** | 7 | 20.6% | `durable_poll` 7/7 | 110s | 7/7 |
| **870–930s** | 4 | 11.8% | `repair` 3/4 | 601s | 4/4 |
...
|---|---:|---:|---:|---:|
| **QUEUE** (create→worker pickup) | 12.3s | 406.6s | 2215.2s | 2215.2s |
| **WORK** (pickup→complete) *envelope-FULL only* | 81.6s | 173.6s | 332.8s | 332.8s |

Queue is **9%** of e2e at p50; **36.5%** of jobs wait >30s before any work begins.

**Queue and envelope loss are NEAR-THRESHOLD, not merely correlated.** Of jobs queuing <30s, **96.7%** kept their envelope (2 of 61 lost it); of jobs queuing ≥30s, **91.4%** lost it. **95.2%** of envelope-FULL jobs queued under 30s. The relation is a step at ~15–30s, so "correlates with" understates it — below the knee loss is near-absent, above it near-certain.
...
On the 900s wall [870,920] — count: **4** of 96

## 3. Route mix (24h)

`moodreel` 35 · `none` 34 · `minimal_speech_uncut` 18 · `minimal` 6 · `hype` 3

Premium share: **39.6%** (38/96).

## 4. Delivery layer — since the column landed 2026-08-11T19:50:15Z (n=142 terminal)

`repair` 36 · `callback` 56 · `NULL` 15 · `durable_poll` 25 · `reconciler` 9 · `fallback_timer` 1
...
wall_viewed **1107** → started **93** (8.4%) → paid **2** (2.2% of starters)
purchase_failed n=172, self-cancelled at the sheet **170** (98.8%) — the leak is the OFFER, not the funnel.
...
- `callback` delivery stamp: **WIRED** — completion_delivery=callback rows = 56 on real traffic.
