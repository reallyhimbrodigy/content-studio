...
**JUDGE, generated 2026-08-21T18:35:17.978Z by `scripts/bleeds.js`.** Job window 24h; funnel + fulfillment windows stated per section. Every line [MEASURED].
...
# 🔴 FAILURE RATE — **14.5%** of jobs failed in the last 6h (9/62)
...
|---|---:|---:|---:|
| 2026-08-21T07Z | 14 | 2 | 14% |
| 2026-08-21T08Z | 8 | 1 | 13% |
| 2026-08-21T09Z | 6 | 2 | 33% |
| 2026-08-21T10Z | 7 | 1 | 14% |
| 2026-08-21T11Z | 15 | 3 | 20% |
| 2026-08-21T12Z | 4 | 1 | 25% |
| 2026-08-21T13Z | 7 | 0 | 0% |
| 2026-08-21T14Z | 7 | 0 | 0% |
| 2026-08-21T15Z | 12 | 2 | 17% |
| 2026-08-21T16Z | 12 | 4 | 33% |
| 2026-08-21T17Z | 15 | 2 | 13% |
| 2026-08-21T18Z | 9 | 1 | 11% |

_12h total: 19/118 = 16.1%. Retires itself when the 6h rate goes under 10%._

---

# ⏱️ SPEED — target cohort (20–30s source) p50 **131s** vs the **60–90s** hard target — **MISS by 41s**

| cohort | n | e2e p50 | e2e p90 | vs 90s |
|---|---:|---:|---:|---|
| **20–30s source (the target)** | 119 | **131s** | 374s | +41s |
...
| all sources (context only) | 1031 | 124s | 426s | +34s |
...
|---|---:|---:|
| `render` | 74.9s | 74.6% |
| `normalize_transcribe_upload` | 21.6s | 21.5% |
| `edit_plan` | 17.8s | 17.7% |
| `upload_export` | 5.0s | 5.0% |
| `hls` | 2.2s | 2.2% |
| **worker wall total** | **100s** | 100% |

**Render split — pre-registered read, locked at `30a1e62` BEFORE the data:**

`PENDING` — **37/37** render spans are ~100% unaccounted, so the sub-timers are NOT deployed. `lane/judge-timers@feb48f8` is filed with TRUTH (reassigned out of JUDGE's lane). **No render build is decided until this reads.**
...
_**e2e 131s − worker wall 100s = ~31s outside the worker** (queue + delivery). Even at zero queue the worker alone sits at the TOP of the 60–90s window, so the target cannot be met by trimming overhead — the worker wall itself has to come down._
...
# 💬 CHAT EVENTS/DAY — **279** today, **5** of the last 11 days DARK (chat 0 with renders > 0)

| day | chat | render (control) | |
|---|---:|---:|---|
| 2026-08-11 | **0** | 20 | **DARK** |
...
## 1. Failures — 25 users / 25 jobs (24h)

| class | users | jobs | share of failing users |
|---|---:|---:|---:|
| UPLOAD_NEVER_STARTED | 16 | 16 | 64.0% |
| INTEGRITY_TRIP | 4 | 4 | 16.0% |
| WORKER_DIED | 3 | 3 | 12.0% |
| CLIP_TOO_SHORT | 2 | 2 | 8.0% |

## 2. FAILED-JOB SECONDS — 2.3% of all job-lifetime seconds [7-DAY WINDOW]

**468 failed jobs / 252 users** over **7 days**, p50 lifetime **601s**, **67/day**. Total **211,755s** of user time spent on jobs that never delivered.

| quantity | jobs | seconds | share |
|---|---:|---:|---:|
| reached a worker (**Modal-billable**) | 281 | 99401 | 46.9% |
| never reached one (**$0 Modal, pure user wait**) | 187 | 112354 | 53.1% |

**USER-time and MODAL-time are different quantities and must not be blended.** A job with no `worker_started_at` and no `modal_call_id` never reached a container: it costs the user their whole wait and costs us **$0**. Here only **46.9%** of failed seconds were Modal-billable (~$0.49/day, **1.9%** of orchestration) — the rest is pure user loss at zero spend.
...
## 2b. Latency — n=168 completed (24h)

p50 **137s** (law 90) · p90 409s · p99 **734s** (law 180) · max 879s

| envelope class | n | users | p50 | p90 | max |
|---|---:|---:|---:|---:|---:|
| `A envelope FULL` | 168 | 166 | **137s** | 409s | 879s |

**ENVELOPE LOSS: 0.0% of completions (0/168), 0 users.** Regression BORN 2026-08-11T23Z after 8 clean days at 0.0% (08-04..08-11). The pooled p50 above sits between classes and describes NO actual user.
...
|---|---:|---:|---:|---:|
| **QUEUE** (create→worker pickup) | 10.9s | 17.9s | 276.4s | 499.2s |
| **WORK** (pickup→complete) *envelope-FULL only* | 122.3s | 366.6s | 690.3s | 726.1s |

Queue is **8%** of e2e at p50; **6.0%** of jobs wait >30s before any work begins.

**Queue and envelope loss are NEAR-THRESHOLD, not merely correlated.** Of jobs queuing <30s, **100.0%** kept their envelope (0 of 158 lost it); of jobs queuing ≥30s, **0.0%** lost it. **94.0%** of envelope-FULL jobs queued under 30s. The relation is a step at ~15–30s, so "correlates with" understates it — below the knee loss is near-absent, above it near-certain.
...
On the 900s wall [870,920] — count: **1** of 168

## 3. Route mix (24h)

`none` 74 · `moodreel` 66 · `minimal_speech_uncut` 22 · `hype` 4 · `minimal` 2

Premium share: **41.7%** (70/168).

## 4. Delivery layer — since the column landed 2026-08-11T19:50:15Z (n=193 terminal)

`callback` 166 · `reconciler` 9 · `NULL` 16 · `orphan_callback` 1 · `durable_poll` 1
...
wall_viewed **946** → started **94** (9.9%) → paid **7** (7.4% of starters)
purchase_failed n=173, self-cancelled at the sheet **170** (98.3%) — the leak is the OFFER, not the funnel.
...
|---|---:|---:|---:|
| motion_graphics | 168 | 11 | **93.5%** |
| generated scenes | 74 | 0 | **100.0%** |
| brand copy | **0 — key never appears** | — | _absent, not declined_ |
| transitions | 168 | 10 | **94.0%** |
| outro | 168 | 168 | **0.0%** |
...
- Lumen scene vocabulary: **WIRED** — completions carrying scene telemetry = 74 on real traffic.
- `callback` delivery stamp: **WIRED** — completion_delivery=callback rows = 166 on real traffic.
...
| tracked+named, measured here | 273,921/mo · 32 names | $0 — **and all of it is already in Supabase** |
| **autocapture — no name at all** | **591,079/mo (68%)** | $0 today · **59% of the free tier** |
