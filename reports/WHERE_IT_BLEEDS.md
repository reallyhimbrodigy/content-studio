...
**JUDGE, generated 2026-08-15T01:48:33.989Z by `scripts/bleeds.js`.** Job window 24h; funnel + fulfillment windows stated per section. Every line [MEASURED].
...
## 2. Latency — n=147 completed (24h)

p50 **151s** (law 90) · p90 679s · p99 **1172s** (law 180) · max 1205s

| envelope class | n | users | p50 | p90 | max |
|---|---:|---:|---:|---:|---:|
| `C envelope LOST + repair` | 6 | 5 | **904s** | 1172s | 1172s |
| `B envelope LOST` | 61 | 50 | **308s** | 760s | 1205s |
| `A envelope FULL` | 80 | 80 | **60s** | 158s | 541s |

Worst/best class p50 spread: **15.1x** — the pooled number above hides it.

**ENVELOPE LOSS: 45.6% of completions (67/147), 55 users.** Regression BORN 2026-08-11T23Z after 8 clean days at 0.0% (08-04..08-11). The pooled p50 above sits between classes and describes NO actual user.
...
On the 900s wall [870,920] — count: **5** of 147

## 3. Route mix (24h)

`none` 67 · `minimal` 48 · `minimal_speech_uncut` 30 · `moodreel` 2

Premium share: **1.4%** (2/147).

## 4. Delivery layer — since the column landed 2026-08-11T19:50:15Z (n=184 terminal)

`reconciler` 138 · `repair` 6 · `NULL` 35 · `callback` 4 · `durable_poll` 1
...
### Break-even — MODAL COST as the sensitivity axis

Modal cost moves this answer ~2x more than scene count does, so it is the axis, not a footnote. Renders/month at break-even on $31.50 net ($45 less Apple 30%):
...
_Read the ROW first: pinning the Modal figure narrows the answer more than any scene decision. Across the plausible Modal range a 4-scene edit breaks even somewhere between **27 and 39 renders/month** — the scene axis moves it far less than the Modal axis does._
