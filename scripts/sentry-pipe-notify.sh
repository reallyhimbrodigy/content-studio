...
# REPO IS DERIVED, NOT HARDCODED. It read "/Users/zaclibman/content-studio"
# — one machine, and that checkout is release/256, so the pager would have
# run the health check against a tree that is not what is deployed and
# reported its verdict as production's. Same class as a fixture outside
# the tree: the path resolves, the answer describes the wrong thing.
