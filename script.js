...
const createCard = (post) => {
  ...
  function buildEntry(entry, idx) {
    ...
    const hookEl = hook ? createDetailRow('Hook', hook, 'calendar-card__hook') : null;
    ...
    const variantsBtn = makeBtn('Generate platform variants');
    variantsBtn.addEventListener('click', async (e) => {
      e.preventDefault();
      await handleGenerateVariants(entry, variantsBtn);
    });
    actionsEl.appendChild(variantsBtn);
    ...
    const entryNodes = [ideaEl, typeEl];
    if (hookEl) entryNodes.push(hookEl);
    entryNodes.push(captionRow, details);
    entryEl.append(...entryNodes);
    return entryEl;
  }
};
...
async function handleGenerateVariants(entry, triggerEl) {
  const button = triggerEl || null;
  const originalLabel = button ? button.textContent : '';
  try {
    if (button) {
    ...
    const currentUser = await getCurrentUser();
    const resolvedUserId = currentUser && typeof currentUser === 'object'
      ? currentUser.id || currentUser.email || currentUser.user?.id
      : currentUser;
    ...
    if (!resp.ok || !Array.isArray(body.variants) || !body.variants[0]?.variants) {
      throw new Error(body.error || 'Variant generation failed');
    }
    const variants = body.variants[0].variants;
    const targetDay = entry.day || payloadPost.day;
    if (Array.isArray(currentCalendar)) {
      currentCalendar = currentCalendar.map((p) =>
        Number(p.day) === Number(targetDay) ? { ...p, variants } : p
      );
      renderCards(currentCalendar);
    }
  } catch (err) {
    console.error('[Variants] generate error', err);
    alert('Could not generate platform variants right now.');
  } finally {
    if (button) {
      button.disabled = false;
      button.textContent = originalLabel || 'Generate platform variants';
    }
  }
}
...
async function ensurePlatformVariantsForCurrentCalendar() { return; }
